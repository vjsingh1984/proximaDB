...
- Technical documentation for vector ID generation fix in link:docs/technical_reference.adoc#vector-id-generation-fix[Technical Reference]

=== Fixed
- DirectVectorService now properly assigns generated IDs to vectors when client doesn't provide them
- Vector ID propagation through storage pipeline ensures all vectors are retrievable by ID
- Vector ID preservation in gRPC search results - fixed field name mismatch ("vector_id" vs "id")
- Eliminated redundant JSON/Avro serialization in gRPC responses with direct proto conversion
- Fixed hanging test in unified_sstable_reader with double-check locking pattern

== [1.0.0] - 2025-01-22

=== Major Architecture Improvements

==== WAL Refactoring - Clean Separation ✅
- *Performance*: 14,939 vectors/second (1.87x improvement)
- *Latency*: 0.07ms per vector (44% reduction)
...
- *Recovery*: 10x faster with parallel processing

==== Bloom Filter Consolidation ✅
- *Design*: Unified polymorphic architecture
- *Code Reduction*: 100% deduplication achieved
- *Strategies*: 5 implementations via strategy pattern
  * ByteAligned (SSTable optimized)
  * BitPacked (Memory efficient)
  * Simple (Small datasets)
  * Composite (Metadata aware)
  * NoOp (Disabled)
- *API*: Single consistent interface across all engines

==== LSM Engine Enhancements ✅
- *Phase 1*: Atomic compaction operations
  * ACID guarantees for all operations
  * Staging-based atomic swaps
  * Automatic rollback on failures
- *Phase 2*: Metadata bloom filters
  * Block-level statistics
  * 95% reduction in scanned blocks
...
- UnifiedSstableReader (70% code reduction)
- UnifiedParquetReader
- Shared atomic operations via UnifiedAtomicCoordinator

==== Direct Vector Service
- Direct memtable access
- 40-60% latency reduction
- Engine-specific routing
- Collection-aware operations
- No hardcoded engine defaults

=== Performance Achievements
- WAL Write: 8,000 → 14,939 vectors/sec
- LSM Query: 2.6x performance improvement
- Code Reduction: 15,000 → 4,500 lines (70%)
- Memory Usage: 40% reduction
- Recovery Time: 10x faster

=== Fixed
- Bloom filter test expectations (7 bits → 4 bits for 0.1 FPR)
- ~400 compilation warnings (unused imports, variables)
- Collection config syntax errors in tests
- Manifest test failures with overwrite option
- Engine routing to require explicit selection
- Python SDK compatibility (89/173 → 173/173 tests passing)
