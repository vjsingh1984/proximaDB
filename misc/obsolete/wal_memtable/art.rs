...
impl CollectionArt {
    ...
    pub async fn atomic_mark_for_flush(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<Vec<WalEntry>> {
        ...
        tracing::info!("🔒 ARTMemTable: Marked {} entries for flush (collection: {}, flush_id: {})", 
                      entries.len(), collection_id, flush_id);
        
        // TODO: In a production system, we would mark these entries as "flush-pending"
        // ART implementation provides excellent space efficiency during marking
    ...
    pub async fn complete_flush_removal(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<usize> {
        ...
        // Remove the entries from the ART structure
        for key in keys_to_remove {
            if let Some(entry) = self.entries.remove(&key) {
        ...
        tracing::info!("🗑️ ARTMemTable: Permanently removed {} entries (collection: {}, flush_id: {})", 
    ...
    pub async fn abort_flush_restore(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<()> {
        ...
        // so there's nothing to restore. ART implementation maintains prefix compression during restore.
        
        tracing::info!("↩️ ARTMemTable: Restored entries to active state (collection: {}, flush_id: {})", 
...
impl MemTableStrategy for ArtMemTable {
    ...
    async fn get_all_entries(&self, collection_id: &CollectionId) -> Result<Vec<WalEntry>> {
        ...
        Ok(collections
            .get(collection_id)
            .map(|art| art.get_all_entries().into_iter().cloned().collect())
            .unwrap_or_default())
    }
    ...
    async fn get_collection_stats(&self, collection_id: &CollectionId) -> Result<super::CollectionStats> {
        ...
        Ok(collections
            .get(collection_id)
            .map(|art| super::CollectionStats {
                entry_count: art.total_entries,
                memory_usage_bytes: art.memory_size,
            })
            .unwrap_or_default())
    }
