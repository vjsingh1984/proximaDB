...
impl CollectionSkipList {
        ...
        for key in keys_to_remove {
            if let Some(entry) = self.entries.remove(&key) {
                self.memory_size = self
                    .memory_size
                    .saturating_sub(Self::estimate_entry_size(entry.value()));
    ...
    async fn compact_mvcc(&mut self, keep_versions: usize) -> u64 {
        ...
        let mut vector_index = self.vector_index.write().await;
        for (_vector_id, sequences) in vector_index.iter_mut() {
    ...
    pub async fn atomic_mark_for_flush(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<Vec<WalEntry>> {
        ...
        for entry in self.entries.iter() {
            if &entry.value().collection_id == collection_id {
                entries.push(entry.value().clone());
            }
        }
        
        tracing::info!("🔒 SkipListMemTable: Marked {} entries for flush (collection: {}, flush_id: {})", 
                      entries.len(), collection_id, flush_id);
        
        // TODO: In a production system, we would mark these entries as "flush-pending"
        // SkipList implementation provides ordered iteration for consistent flush order
    ...
    pub async fn complete_flush_removal(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<usize> {
        ...
        // Remove the entries from both the skip list and vector index
        for key in keys_to_remove {
            if let Some(entry) = self.entries.remove(&key) {
        ...
        tracing::info!("🗑️ SkipListMemTable: Permanently removed {} entries (collection: {}, flush_id: {})", 
    ...
    pub async fn abort_flush_restore(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<()> {
        ...
        // so there's nothing to restore. SkipList implementation maintains ordering during restore.
        
        tracing::info!("↩️ SkipListMemTable: Restored entries to active state (collection: {}, flush_id: {})", 
...
impl MemTableStrategy for SkipListMemTable {
    ...
    async fn collections_needing_flush(&self) -> Result<Vec<CollectionId>> {
        ...
        for (collection_id, skiplist) in collections.iter() {
            ...
            if skiplist.needs_flush(effective_config.memory_flush_size_bytes) {
    ...
    // =============================================================================
    // SPECIALIZED ATOMIC FLUSH IMPLEMENTATIONS - SkipList concurrent optimizations  
    // =============================================================================

    /// SkipList-specialized atomic marking with lock-free concurrent access
    /// Overrides default implementation for maximum concurrency during disk writes
    async fn atomic_mark_for_flush(
        &self,
        collection_id: &CollectionId,
        flush_id: &str,
    ) -> Result<Vec<WalEntry>> {
        tracing::debug!("🔀 SkipListMemTable: Starting CONCURRENT atomic flush marking for collection {} (flush_id: {})", 
                       collection_id, flush_id);
        
        let collections = self.collections.read().await;
        
        if let Some(skiplist) = collections.get(collection_id) {
            // OPTIMIZATION: SkipList allows concurrent reads during marking
            // This is critical for Always sync mode where disk writes are slow
            let marked_entries: Vec<WalEntry> = skiplist.entries
                .iter()
                .map(|entry| entry.value().clone())
                .collect();
            
            tracing::info!("🔀 SkipListMemTable: CONCURRENT atomic mark completed - {} entries marked with lock-free access", 
                          marked_entries.len());
            
            Ok(marked_entries)
        } else {
            tracing::debug!("📭 SkipListMemTable: No collection found for {}", collection_id);
            Ok(vec![])
        }
    }

    /// SkipList-specialized atomic removal optimized for concurrent access
    async fn complete_flush_removal(
        &self,
        collection_id: &CollectionId,
        flush_id: &str,
    ) -> Result<usize> {
        tracing::debug!("🔀 SkipListMemTable: Starting CONCURRENT flush completion for collection {} (flush_id: {})", 
                       collection_id, flush_id);
        
        let mut collections = self.collections.write().await;
        let entries_removed = if let Some(skiplist) = collections.get(collection_id) {
            skiplist.total_entries as usize
        } else {
            0
        };
        
        // OPTIMIZATION: SkipList provides efficient concurrent-safe removal
        collections.remove(collection_id);
        ...
        tracing::info!("🔀 SkipListMemTable: CONCURRENT flush completion - {} entries removed with minimal lock contention", 
                      entries_removed);
        
        Ok(entries_removed)
    }

    /// SkipList-specialized abort optimized for concurrent access patterns
    async fn abort_flush_restore(
        &self,
        collection_id: &CollectionId,
        flush_id: &str,
    ) -> Result<()> {
        tracing::debug!("🔀 SkipListMemTable: CONCURRENT flush abort for collection {} (flush_id: {})", 
                       collection_id, flush_id);
        
        // OPTIMIZATION: SkipList supports lock-free restore operations
        // Critical for Always sync mode where aborts may happen during slow disk writes
        
        tracing::info!("🔀 SkipListMemTable: CONCURRENT flush abort completed - lock-free restore ideal for Always sync mode");
