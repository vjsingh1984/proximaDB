...
impl CollectionBTree {
    ...
    fn insert_entry(&mut self, mut entry: WalEntry) -> u64 {
        ...
        tracing::debug!("📏 MEMORY: Collection {} - entry #{} added, size: {} bytes (was: {}, now: {})", 
                       self.collection_id, self.sequence_counter, entry_size, old_memory_size, self.memory_size);
    ...
    fn clear_up_to(&mut self, sequence: u64) {
        // FIXED MEMORY ISOLATION BUG: Process removed entries first to update vector_index correctly
        
        // Calculate memory freed and update vector index for entries being removed
        let memory_freed: usize = self
            .entries
            .range(..=sequence)
            .map(|(_, entry)| {
                // Update vector index for removed entries
                if let WalOperation::Insert { vector_id, .. }
                | WalOperation::Update { vector_id, .. }
                | WalOperation::Delete { vector_id, .. } = &entry.operation
                {
                ...
                Self::estimate_entry_size(entry)
            })
            .sum();
    ...
    pub async fn atomic_mark_for_flush(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<Vec<WalEntry>> {
        ...
        tracing::info!("🔒 BTreeMemTable: Marked {} entries for flush (collection: {}, flush_id: {})", 
                      entries.len(), collection_id, flush_id);
        
        // TODO: In a production system, we would mark these entries as "flush-pending"
        // to prevent them from being modified during the flush operation
        // For now, we return the entries immediately
    ...
    pub async fn complete_flush_removal(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<usize> {
        ...
        // Remove the entries from both the main index and vector index
        for key in keys_to_remove {
            if let Some(entry) = self.entries.remove(&key) {
        ...
        tracing::info!("🗑️ BTreeMemTable: Permanently removed {} entries (collection: {}, flush_id: {})", 
    ...
    pub async fn abort_flush_restore(&mut self, collection_id: &CollectionId, flush_id: &str) -> Result<()> {
        ...
        // so there's nothing to restore. In a production system, we would clear the
        // "flush-pending" marks here.
        
        tracing::info!("↩️ BTreeMemTable: Restored entries to active state (collection: {}, flush_id: {})", 
    ...
    fn estimate_entry_size(entry: &WalEntry) -> usize {
        ...
        let operation_size = match &entry.operation {
            WalOperation::Insert { record, .. } | WalOperation::Update { record, .. } => {
                ...
                let metadata_bytes = record.metadata.len() * 32; // Estimate 32 bytes per metadata entry
                
                // 🔍 DETAILED SIZE TRACKING
                tracing::trace!("📏 SIZE_CALC: Vector {} dims = {} bytes, metadata {} entries = {} bytes", 
                               record.vector.len(), vector_bytes, record.metadata.len(), metadata_bytes);
        ...
        let total_size = base_size + operation_size + 24; // B+ tree node overhead (minimal)
        
        // 🔍 SIZE BREAKDOWN
        tracing::trace!("📏 SIZE_BREAKDOWN: base={}, operation={}, overhead=24, total={}", 
                       base_size, operation_size, total_size);
...
impl MemTableStrategy for BTreeMemTable {
    ...
    async fn collections_needing_flush(&self) -> Result<Vec<CollectionId>> {
        ...
        tracing::debug!("🔍 FLUSH_CHECK: Checking {} collections for flush thresholds", collections.len());
        ...
        for (collection_id, btree) in collections.iter() {
            ...
            let threshold = effective_config.memory_flush_size_bytes;
            let current_size = btree.memory_size;
            let needs_flush = btree.needs_flush(threshold);

            tracing::debug!("🔍 FLUSH_CHECK: Collection {} - size: {} bytes, threshold: {} bytes, needs_flush: {}", 
                           collection_id, current_size, threshold, needs_flush);

            if needs_flush {
                tracing::info!("🚨 FLUSH_TRIGGER: Collection {} NEEDS FLUSH - {} bytes >= {} bytes threshold", 
                              collection_id, current_size, threshold);
                result.push(collection_id.clone());
            } else {
                tracing::debug!("✅ FLUSH_CHECK: Collection {} below threshold - {} bytes < {} bytes", 
                               collection_id, current_size, threshold);
            }
        }

        if result.is_empty() {
            tracing::debug!("🔍 FLUSH_CHECK: No collections need flushing");
        } else {
            tracing::info!("🚨 FLUSH_TRIGGER: {} collections need flushing: {:?}", result.len(), result);
        }
    ...
    async fn get_collection_stats(&self, collection_id: &CollectionId) -> Result<super::CollectionStats> {
        ...
        Ok(collections
            .get(collection_id)
            .map(|btree| super::CollectionStats {
                entry_count: btree.total_entries,
                memory_usage_bytes: btree.memory_size,
            })
            .unwrap_or_default())
    }
