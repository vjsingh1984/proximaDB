...
def main():
    ...
    try:
        ...
        insert_result = grpc_client.insert_vectors(
            collection_id="debug_collection", vectors=vectors
        )
        print(
            f"✅ gRPC Insert result: Success={insert_result.success}, Vector IDs={insert_result.vector_ids}"
        )
        ...
        for i in range(3):
            ...
            try:
                result = grpc_client.get_vector(
                    collection_id="debug_collection", vector_id=vector_id
                )
        ...
        with httpx.Client() as client:
            response = client.get(
                "http://localhost:5678/debug/vectors/debug_collection"
            )
        ...
        with httpx.Client() as client:
            response = client.post(
                "http://localhost:5678/internal/flush/debug_collection"
            )
            if response.status_code == 200:
