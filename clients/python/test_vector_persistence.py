...
if response.status_code == 200:
    ...
    if "results" in results:
        ...
        for i, result in enumerate(results["results"][:5]):
            print(
                f"     - Result {i+1}: id={result.get('id')}, distance={result.get('distance')}, metadata={result.get('metadata')}"
            )
    else:
...
batch_search_data = {"searches": [{"vector": [0.1] * 128, "top_k": 5}]}
response = requests.post(
    f"{base_url}/api/v1/batch_search/{collection_name}", json=batch_search_data
)
...
list_data = {"limit": 10, "offset": 0}
response = requests.post(
    f"{base_url}/api/v1/vectors/{collection_name}/list", json=list_data
)
print(f"      Status: {response.status_code}")
if response.status_code == 200:
    ...
    for col in collections["collections"]:
        ...
        if vector_count > 0:
            ...
            search_response = requests.post(
                f"{base_url}/api/v1/search/{col_name}", json=search_data
            )
            if search_response.status_code == 200:
                results = search_response.json()
                print(
                    f"     - Search returned results: {len(results.get('results', []))} items"
                )
...
try:
    with open("server_recovery.log", "r") as f:
        ...
        for line in lines:
            if "WAL" in line and (
                "recovery" in line or "found" in line or "vectors" in line
            ):
                print(f"   - {line.strip()}")
except:
