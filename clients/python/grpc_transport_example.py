...
async def example_usage():
    ...
    try:
        ...
        for collection in collections:
            print(f"   - {collection['name']} ({collection['stats']['vector_count']} vectors)")
...
async def performance_comparison_example():
    ...
    try:
        ...
        if result['success']:
            print(f"⚡ Inserted {len(large_vectors)} vectors in {end_time - start_time:.3f}s")
            print(f"   Throughput: {len(large_vectors) / (end_time - start_time):.1f} vectors/sec")
            print(f"   Processing time: {result['metrics']['processing_time_us'] / 1000:.1f}ms")
