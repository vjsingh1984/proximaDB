...
def test_distance_metrics():
    ...
    for metric_enum, metric_name, expected_value in test_metrics:
        ...
        try:
            ...
            print(f"   Creating with distance_metric={metric_enum.name} (value={metric_enum.value})...")
            collection = grpc_client.create_collection(collection_name, config)
            
            # Check what was created
            print(f"   ✅ Created collection: {collection.id}")
            print(f"   📋 Config returned: distance_metric={collection.config.distance_metric.name} (value={collection.config.distance_metric.value})")
            
            # Verify with get_collection
            retrieved = grpc_client.get_collection(collection_name)
            print(f"   🔍 Retrieved config: distance_metric={retrieved.config.distance_metric.name} (value={retrieved.config.distance_metric.value})")
            
            if retrieved.config.distance_metric != metric_enum:
                print(f"   ❌ MISMATCH: Expected {metric_enum.name}, got {retrieved.config.distance_metric.name}")
            else:
