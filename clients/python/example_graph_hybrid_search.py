...
async def main():
    ...
    try:
        ...
        print(
            f"✅ Created company node: {company_result.get('id', 'company_proximadb')}"
        )
        ...
        print(
            f"✅ Created employment edge: {employment1.get('edge_type', 'WORKS_FOR')}"
        )
        ...
        print(
            f"✅ Created employment edge: {employment2.get('edge_type', 'WORKS_FOR')}"
        )
        ...
        print(
            f"✅ Created colleague edge: {colleague_edge.get('edge_type', 'COLLEAGUE')}"
        )
        ...
        print(
            f"✅ Node query found {len(node_query_result.get('nodes', []))} engineering employees"
        )
        ...
        hybrid_result = grpc_client.hybrid_search(
            ...
            vector_filters={"department": "Engineering", "active": True},
            limit=20,
        )
        ...
        for strategy in strategies:
            ...
            print(
                f"✅ Strategy {strategy}: {len(strategy_result.get('vector_results', []))} vector + {len(strategy_result.get('nodes', []))} graph results"
            )
        ...
        print(
            f"✅ Advanced vector search found {len(advanced_result.get('results', []))} results"
        )
        print(f"   - Execution time: {advanced_result.get('execution_time_ms', 0)} ms")

        # Show detailed results
        for i, result in enumerate(
            advanced_result.get("results", [])[:3]
        ):  # Show first 3
            print(f"   Result {i+1}:")
        ...
        print(
            f"✅ High precision search: {len(precision_result.get('results', []))} results (threshold=0.95)"
        )
        ...
        print(
            f"✅ Fast search with hints: {len(fast_result.get('results', []))} results (timeout=1s)"
        )
        ...
        rest_result = rest_client.advanced_vector_search(
            collection_id="test_collection", vector=test_vector, top_k=5
        )
