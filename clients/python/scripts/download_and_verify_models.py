...
class ModelDownloader:
    ...
    def download_and_verify(
        ...
        try:
            ...
            # Get model info
            print(
                f"✓ Model config: {config.model.name}, dimension={config.model.dimension}"
            )
            ...
            if actual_dim != config.model.dimension:
                raise ValueError(
                    f"Dimension mismatch: expected {config.model.dimension}, got {actual_dim}"
                )
            ...
            if config.normalize:
                ...
                print(
                    f"✓ Average L2 norm: {avg_norm:.4f} (should be ~1.0 if normalized)"
                )
    ...
    def print_summary(self):
        ...
        for r in self.results:
            ...
            download_str = (
                f"{r['download_time']:.2f}" if r["download_time"] > 0 else "N/A"
            )
            embed_str = (
                f"{r['embedding_time']:.2f}" if r["embedding_time"] > 0 else "N/A"
            )

            print(
                f"│ {status} {model_short:<37}│ {dim_str:<8} │ {download_str:<10} │ {embed_str:<10} │"
            )
...
def main():
    ...
    for model_name, desc in [
        ...
        downloader.download_and_verify(
            BGEProvider,
            ProviderConfig(model=BGE_MODELS[model_name], batch_size=32, normalize=True),
            desc,
            test_texts,
        )
    ...
    for model_name, desc in [
        ...
        downloader.download_and_verify(
            E5Provider,
            ProviderConfig(model=E5_MODELS[model_name], batch_size=32, normalize=True),
            desc,
            test_texts,
        )
