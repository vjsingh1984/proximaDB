...
def main():
    ...
    try:
        ...
        search_result = client.search_vectors(
            collection_id=collection_name, vector=query_vector, top_k=2
        )
        ...
        if vector:
            print(
                f"✅ Retrieved vector: {vector.id} with {len(vector.vector)} dimensions"
            )
        else:
        ...
        try:
            ...
            print(
                f"✅ SQL query executed, returned {len(sql_result.get('rows', []))} rows"
            )
        ...
        for col in collections:
            print(
                f"   - {col.name} ({col.dimension}D, {col.distance_metric.value}, {col.storage_engine.value})"
            )
