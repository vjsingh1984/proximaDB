...
class MonitoredProximaDBClient:
    ...
    def _register_custom_metrics(self):
        ...
        # Business metrics
        self.telemetry.metrics_collector.register_metric(
            Metric(
    ...
    def insert_with_validation(
        self, collection_name: str, vectors: List[VectorRecord]
    ) -> Dict[str, Any]:
        ...
        with SpanContext("validate_vectors") as span:
            ...
            for vector in vectors:
                ...
                self.telemetry.metrics_collector.observe_histogram(
                    "vector_quality_score", score, {"collection": collection_name}
                )
    ...
    def search_with_monitoring(
        ...
        with SpanContext("search_operation") as span:
            ...
            if filters:
                ...
                self.telemetry.metrics_collector.observe_histogram(
                    "query_complexity", len(filters), {"collection": collection_name}
                )
                span.set_tag("filter_count", len(filters))

            # Perform search
            try:
                results = self.client.search(collection_name, query_vector, top_k=top_k)
        ...
        if "proximadb_request_duration" in metrics:
            ...
            for key, histogram in durations.items():
                ...
                dashboard["performance_metrics"][operation] = {
                    ...
                    "p95_ms": (
                        histogram.get("p95", 0) * 1000 if "p95" in histogram else None
                    ),
                }
...
def simulate_production_workload(
    client: MonitoredProximaDBClient, collection_name: str, duration_seconds: int = 60
):
        ...
        try:
            if operation_type == "insert":
                    ...
                    vectors.append(
                        VectorRecord(
                            ...
                            metadata={
                                ...
                                "quality": (
                                    "low" if np.linalg.norm(vector) < 0.1 else "normal"
                                ),
                            },
                        )
                    )
                ...
                result = client.search_with_monitoring(
                    collection_name, query, top_k=random.randint(5, 20), filters=filters
                )
                logger.info(
                    f"Search completed in {result['duration_ms']:.2f}ms, "
                ...
                client.search_with_monitoring(collection_name, vector.vector, top_k=5)
...
def main():
    ...
    base_client = ProximaDBClient(
        ClientConfig(url="http://localhost:5678", timeout=30.0)
    )
    ...
    try:
        ...
        with open("demo_metrics.jsonl", "r") as f:
            for i, line in enumerate(f):
                ...
                print(
                    f"   {metric['timestamp']}: {metric['metric']} = {metric['value']}"
                )
        ...
        if "proximadb_request_duration" in metrics:
            ...
            if durations:
                ...
                print(
                    f"   - SLO Status: {'✅ PASS' if p99 < 0.1 else '❌ FAIL'} (<100ms)"
                )
