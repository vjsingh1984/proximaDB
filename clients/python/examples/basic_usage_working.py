...
def main():
    ...
    try:
        ...
        for q_idx, query_text in enumerate(test_queries):
            ...
            for i, result in enumerate(results_list):
                ...
                result_id = getattr(result, 'id', result.get('id', 'N/A') if hasattr(result, 'get') else 'N/A')
                score = getattr(result, 'score', getattr(result, 'similarity_score', 0.0))
                result_metadata = getattr(result, 'metadata', result.get('metadata', {}) if hasattr(result, 'get') else {})
                
                # Find original document for context
                original_doc = next((doc for doc in sample_documents if doc['id'] == result_id), None)
        ...
        results_list = getattr(results, 'results', results if isinstance(results, list) else [])
    ...
    print("🔍 All results shown are genuine BERT semantic similarity - no artificial boosting!")
