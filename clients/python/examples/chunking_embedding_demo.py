...
def demo_separation_of_concerns():
    ...
    chunker = TextChunker(
        ChunkingConfig(
            strategy=ChunkingStrategy.PARAGRAPH, chunk_size=200, chunk_overlap=20
        )
    )
...
def demo_different_strategies():
    ...
    for strategy in strategies:
        ...
        chunker = TextChunker(
            ChunkingConfig(strategy=strategy, chunk_size=150, chunk_overlap=20)
        )
...
def demo_complete_workflow():
    ...
    records = chunk_and_embed_text(
        ...
        chunking_config=ChunkingConfig(
            strategy=ChunkingStrategy.PARAGRAPH, chunk_size=200
        ),
    ...
    results = client.search(
        collection_id=collection_name, vector=query_embedding.tolist(), top_k=3
    )
