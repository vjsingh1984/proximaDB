...
async def generate_large_dataset(
    ...
    for i in range(num_vectors):
        ...
        yield VectorRecord(id=f"vec_{i:08d}", vector=vector.tolist(), metadata=metadata)
...
async def demo_basic_streaming(client: ProximaDBClient, collection_name: str) -> None:
    ...
    stream = VectorStream(
        ...
        progress_callback=lambda m: print(
            f"   Progress: {m.processed_items:,}/{m.total_items:,} "
...
async def demo_file_uploads(client: ProximaDBClient, collection_name: str) -> None:
    ...
    jsonl_metrics = await uploader.upload_jsonl(
        ...
        batch_size=1000,
        progress_callback=lambda m: print(
            f"   JSONL Progress: {m.progress_percentage:.1f}%"
        ),
    )
    ...
    csv_metrics = await uploader.upload_csv(
        ...
        batch_size=1000,
        progress_callback=lambda m: print(
            f"   CSV Progress: {m.progress_percentage:.1f}%"
        ),
    )
    ...
    numpy_metrics = await uploader.upload_numpy(
        ...
        batch_size=1000,
        progress_callback=lambda m: print(
            f"   NumPy Progress: {m.progress_percentage:.1f}%"
        ),
    )
...
async def demo_memory_efficient_streaming(
    client: ProximaDBClient, collection_name: str
) -> None:
...
async def demo_error_recovery(client: ProximaDBClient, collection_name: str) -> None:
    ...
    stream = VectorStream(
        ...
        retry_on_failure=True,
        max_retries=3,
        error_callback=lambda e, batch: print(
            f"   ⚠️  Error in batch: {type(e).__name__} - {str(e)[:50]}"
        ),
    )
...
async def demo_parallel_streams(client: ProximaDBClient, collection_name: str) -> None:
    ...
    for source in sources:
        ...
        task = stream.insert_stream(
            source_generator(source["name"], source["vectors"], source["dimension"]),
    ...
    for i, (source, metrics) in enumerate(zip(sources, results)):
        print(
            f"   - {source['name']}: {metrics.processed_items:,} vectors, "
...
async def main():
    ...
    try:
        ...
        config = CollectionConfig(
            ...
            metadata={"description": "Collection for streaming upload demo"},
        )
        ...
        print(
            f"   - Storage size: {collection.storage_size_bytes / 1024 / 1024:.2f} MB"
        )
