...
def get_bert_model(model_name: str = "all-MiniLM-L6-v2") -> SentenceTransformer:
    ...
    if _bert_model is None:
        ...
        logger.info(
            f"BERT model loaded - dimension: {_bert_model.get_sentence_embedding_dimension()}"
        )
    return _bert_model
...
def generate_sample_documents(num_docs: int = 10) -> List[Dict[str, any]]:
    ...
    selected_texts = (
        sample_texts[:num_docs]
        if num_docs <= len(sample_texts)
        else sample_texts * ((num_docs // len(sample_texts)) + 1)
    )
        ...
        doc = {
            ...
            "metadata": {
                "category": (
                    ...
                    if any(
                        term in text.lower()
                        for term in ["machine learning", "neural", "deep learning"]
                    )
                    else "data_science"
                ),
...
def generate_sample_products(num_products: int = 10) -> List[Dict[str, any]]:
    ...
    selected_descriptions = (
        product_descriptions[:num_products]
        if num_products <= len(product_descriptions)
        else product_descriptions * ((num_products // len(product_descriptions)) + 1)
    )
    ...
    products = []
    for i, (description, embedding) in enumerate(
        zip(selected_descriptions, embeddings)
    ):
        product = {
            "id": f"prod_{i:03d}",
...
def generate_query_embedding(
    query_text: str, model_name: str = "all-MiniLM-L6-v2"
) -> List[float]:
