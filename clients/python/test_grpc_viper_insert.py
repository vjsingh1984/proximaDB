#!/usr/bin/env python3
"""
gRPC + VIPER Insert Performance Test with Persistence
...
# Test configuration
COLLECTION_NAME = "grpc_viper_persist_test"
DIMENSION = 128
...
def generate_test_vectors(num_vectors, dimension):
    ...
    np.random.seed(44)  # Different seed for gRPC VIPER
    vectors = []
...
def main():
    print("🚀 gRPC + VIPER Insert Performance Test")
    ...
    # Create collection
    print("\n📦 Creating VIPER collection...")
    config = CollectionConfig(
        name=COLLECTION_NAME,
        dimension=DIMENSION,
        distance_metric=DistanceMetric.COSINE,
        storage_engine=StorageEngine.VIPER,
        description="gRPC + VIPER persistence test"
    )
    ...
    # Wait for VIPER flush
    print("\n⏳ Waiting for VIPER flush...")
    time.sleep(5)
    ...
    with open("grpc_viper_insert_results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print("\n📊 Results saved to grpc_viper_insert_results.json")
    ...
    # Save baseline
    with open("grpc_viper_baseline.json", "w") as f:
        json.dump({
