#!/usr/bin/env python3
"""
REST + LSM Insert Performance Test with Persistence
...
# Test configuration
COLLECTION_NAME = "rest_lsm_persist_test"
DIMENSION = 128
NUM_VECTORS = 50000
...
def generate_test_vectors(num_vectors, dimension):
    ...
    np.random.seed(43)  # Different seed for LSM
    vectors = []
...
def main():
    print("🚀 REST + LSM Insert Performance Test")
    ...
    config = CollectionConfig(
        ...
        description="REST + LSM persistence test"
    )
    ...
    with open("rest_lsm_insert_results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print("\n📊 Results saved to rest_lsm_insert_results.json")
    ...
    # Save baseline
    with open("rest_lsm_baseline.json", "w") as f:
        json.dump({
