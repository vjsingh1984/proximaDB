...
def test_sql_request_creation():
    ...
    try:
        ...
        request1 = types_pb2.ExecuteSqlRequest(query=query1, parameters=proto_params1)
        logger.info(
            f"✅ Simple query request: {len(request1.query)} chars, {len(request1.parameters)} params"
        )
        ...
        request2 = types_pb2.ExecuteSqlRequest(query=query2, parameters=proto_params2)
        logger.info(
            f"✅ Parameterized query request: {len(request2.query)} chars, {len(request2.parameters)} params"
        )
    ...
    try:
        ...
        row1_fields = [
            types_pb2.SqlRowField(
                key="id", value=types_pb2.SqlValue(string_value="vec_1")
            ),
            types_pb2.SqlRowField(
                key="category", value=types_pb2.SqlValue(string_value="electronics")
            ),
            types_pb2.SqlRowField(
                key="price", value=types_pb2.SqlValue(number_value=299.99)
            ),
        ]
        ...
        row2_fields = [
            types_pb2.SqlRowField(
                key="id", value=types_pb2.SqlValue(string_value="vec_2")
            ),
            types_pb2.SqlRowField(
                key="category", value=types_pb2.SqlValue(string_value="books")
            ),
            types_pb2.SqlRowField(
                key="price", value=types_pb2.SqlValue(number_value=19.95)
            ),
        ]
        ...
        logger.info(
            f"✅ Rows scanned: {result['rows_scanned']}, returned: {result['rows_returned']}"
        )
...
def main():
    ...
    if success:
        ...
        logger.info(
            "  - SQL value type conversion (strings, numbers, booleans, arrays, objects)"
        )
