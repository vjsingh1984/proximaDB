...
class TestSDKUpsertOperations:
    ...
    def test_basic_upsert_rest(self, rest_client, test_collection_rest):
        ...
        assert result.success == 1  # BatchResult.success is the count of successful operations
        
        # Verify insertion
        retrieved = rest_client.get_vector(collection_name, vector_id, include_metadata=True)
        assert retrieved is not None
        assert retrieved['id'] == vector_id
        print(f"DEBUG: Retrieved metadata: {retrieved.get('metadata', {})}")  # Debug print
        ...
        updated_retrieved = rest_client.get_vector(collection_name, vector_id, include_metadata=True)
    ...
    def test_basic_upsert_grpc(self, grpc_client, test_collection_grpc):
        ...
        # Verify insertion
        retrieved = grpc_client.get_vector(collection_name, vector_id, include_metadata=True)
        assert retrieved is not None
        assert retrieved['id'] == vector_id
        ...
        # Verify update
        updated_retrieved = grpc_client.get_vector(collection_name, vector_id, include_metadata=True)
        assert updated_retrieved is not None
        assert updated_retrieved['id'] == vector_id
    ...
    def test_batch_upsert_rest(self, rest_client, test_collection_rest):
        ...
        assert result.success == 10  # BatchResult.success is the count of successful operations
        
        # Update batch (upsert)
        updated_vectors = []
        for i in range(10):
            updated_vectors.append(VectorRecord(
                id=f"batch_rest_{i}",
                vector=[0.0, float(i), 0.0, 0.0],
                metadata={"batch": "rest", "index": i, "type": "updated"}
            ))
        
        result = rest_client.insert_vectors(collection_name, records=updated_vectors, upsert=True)
        assert result.success
        
        # Verify some vectors were processed
        retrieved = rest_client.get_vector(collection_name, "batch_rest_5", include_metadata=True)
        assert retrieved is not None
    ...
    def test_batch_upsert_grpc(self, grpc_client, test_collection_grpc):
        ...
        result = grpc_client.insert_vectors(collection_name, records=updated_vectors, upsert=True)
        assert result.success
        
        # Verify some vectors were processed
        retrieved = grpc_client.get_vector(collection_name, "batch_grpc_5", include_metadata=True)
        assert retrieved is not None
    ...
    def test_concurrent_upserts(self, rest_client, test_collection_rest):
        ...
        assert success_count >= 2, f"Expected at least 2 successful workers, got {success_count}"
    ...
    def test_basic_performance_check(self, rest_client, grpc_client, test_collection_rest, test_collection_grpc):
        ...
        rest_result = rest_client.insert_vectors(
            test_collection_rest.config.name, 
            records=test_vectors
        )
        ...
        grpc_result = grpc_client.insert_vectors(
            test_collection_grpc.config.name,
            records=test_vectors
        )
