...
def _load_module(name: str, file_name: str, parent_module: types.ModuleType):
    ...
    spec = importlib.util.spec_from_file_location(
        name, str(src_path / "proximadb_sdk" / "chunking_strategies" / file_name)
    )
...
def _load_root_module(name: str, file_name: str, parent_module: types.ModuleType):
    ...
    # Use proximadb_sdk (actual package location) instead of proximadb
    spec = importlib.util.spec_from_file_location(
        name, str(src_path / "proximadb_sdk" / file_name)
    )
    module = importlib.util.module_from_spec(spec)
    sys.modules[name] = module
    spec.loader.exec_module(module)
...
def _setup_modules():
    ...
    mock_module_patterns = [
        "proximadb.chunking_strategies.",
        "proximadb.document_processor",
        "proximadb.document_pipeline",
    ]
    ...
    if "proximadb.chunking_strategies" not in sys.modules:
        ...
        chunking_strategies.__path__ = [
            str(src_path / "proximadb_sdk" / "chunking_strategies")
        ]
    ...
    _load_module(
        "proximadb.chunking_strategies.sliding_window",
        "sliding_window.py",
        chunking_strategies,
    )
    _load_module(
        "proximadb.chunking_strategies.sentence", "sentence.py", chunking_strategies
    )
    _load_module(
        "proximadb.chunking_strategies.paragraph", "paragraph.py", chunking_strategies
    )
    _load_module(
        "proximadb.chunking_strategies.semantic", "semantic.py", chunking_strategies
    )
    _load_module(
        "proximadb.chunking_strategies.recursive", "recursive.py", chunking_strategies
    )
    _load_module(
        "proximadb.chunking_strategies.fixed_size", "fixed_size.py", chunking_strategies
    )
    ...
    _load_module(
        "proximadb.chunking_strategies.document_parsers",
        "document_parsers.py",
        chunking_strategies,
    )
    ...
    _load_module(
        "proximadb.chunking_strategies.factory", "factory.py", chunking_strategies
    )
    ...
    _load_root_module(
        "proximadb.document_processor", "document_processor.py", proximadb
    )
