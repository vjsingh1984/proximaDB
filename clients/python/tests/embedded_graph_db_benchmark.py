...
def generate_vector_graph_data(config: GraphConfig, dimension: int = 128) -> Tuple[List[Dict], List[Dict], np.ndarray]:
...
def benchmark_proximadb_graph(config: GraphConfig, temp_dir: str) -> Dict[str, BenchmarkResult]:
...
def benchmark_networkx(config: GraphConfig, temp_dir: str) -> Dict[str, BenchmarkResult]:
    ...
    try:
        ...
        for label in labels_to_query * 10:
            start = time.perf_counter()
            matching = [n for n, d in G.nodes(data=True) if label in d.get("labels", [])]
            query_times.append((time.perf_counter() - start) * 1000)
...
def render_comparison_table(
    ...
    operations = [
        "insert_nodes", "insert_edges", "node_lookup", "query_by_labels",
        "1hop_outgoing", "1hop_incoming", "2hop_traversal", "graph_stats", "delete_node"
    ]

    print(f"\n{'='*90}")
    print(f"GRAPH BENCHMARK RESULTS: {config.name.upper()} ({config.num_nodes:,} nodes, {config.num_edges:,} edges)")
    print(f"{'='*90}")
    ...
    if RICH_AVAILABLE:
        ...
        for op in operations:
            ...
            pdb_time = f"{pdb_result.time_ms:.3f}" if pdb_result and not pdb_result.error else "N/A"
            nx_time = f"{nx_result.time_ms:.3f}" if nx_result and not nx_result.error else "N/A"

            if pdb_result and nx_result and not pdb_result.error and not nx_result.error:
                speedup = nx_result.time_ms / pdb_result.time_ms
                speedup_str = f"{speedup:.2f}x" if speedup >= 1 else f"1/{1/speedup:.2f}x"
            else:
                speedup_str = "N/A"

            pdb_p95 = f"{pdb_result.p95_ms:.3f}" if pdb_result and pdb_result.p95_ms else "-"
            nx_p95 = f"{nx_result.p95_ms:.3f}" if nx_result and nx_result.p95_ms else "-"

            table.add_row(op, pdb_time, nx_time, speedup_str, pdb_p95, nx_p95)

        console.print(table)
    else:
        print(f"{'Operation':<20} {'ProximaDB (ms)':<16} {'NetworkX (ms)':<16} {'Speedup':<12}")
        print("-" * 64)
        for op in operations:
            pdb_result = proximadb_results.get(op)
            nx_result = networkx_results.get(op)
...
def render_throughput_summary(all_results: Dict[str, Dict[str, Dict[str, BenchmarkResult]]]) -> None:
    ...
    ops_with_throughput = ["insert_nodes", "insert_edges", "node_lookup", "1hop_outgoing", "delete_node"]
    ...
    if RICH_AVAILABLE:
        ...
        for op in ops_with_throughput:
            ...
            for cfg in configs:
                ...
                pdb_tput = f"{pdb_result.throughput:,.0f}" if pdb_result and pdb_result.throughput else "N/A"
                nx_tput = f"{nx_result.throughput:,.0f}" if nx_result and nx_result.throughput else "N/A"
...
def write_markdown_report(all_results: Dict[str, Dict[str, Dict[str, BenchmarkResult]]]) -> None:
    ...
    lines.append("")
    lines.append("Comparing ProximaDB (ORION engine) with NetworkX (reference implementation)")
    lines.append("")
...
def run_benchmark(configs: List[GraphConfig] = None):
    ...
    for config in configs:
        print(f"\n{'='*90}")
        print(f"BENCHMARK: {config.name.upper()} ({config.num_nodes:,} nodes, {config.num_edges:,} edges)")
        print(f"{'='*90}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if "error" in proximadb_results:
                ...
                print(f"    Insert nodes: {proximadb_results['insert_nodes'].time_ms:.2f}ms")
                print(f"    Node lookup: {proximadb_results['node_lookup'].time_ms:.3f}ms (p95: {proximadb_results['node_lookup'].p95_ms:.3f}ms)")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if "error" in networkx_results:
                ...
                print(f"    Insert nodes: {networkx_results['insert_nodes'].time_ms:.2f}ms")
                print(f"    Node lookup: {networkx_results['node_lookup'].time_ms:.3f}ms (p95: {networkx_results['node_lookup'].p95_ms:.3f}ms)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--size", choices=["small", "medium", "large", "all"], default="small",
