...
class TestIntelligentRouterIntegration:
    ...
    def test_router_with_mock_clients(self, router):
        ...
        search_protocol, search_client = router.route_operation(OperationType.SINGLE_SEARCH)
        insert_protocol, insert_client = router.route_operation(OperationType.BULK_INSERT, data_size=5000)
        health_protocol, health_client = router.route_operation(OperationType.HEALTH_CHECK)
    ...
    def test_fallback_behavior(self, router):
        ...
        fallback_protocols = []
        for _ in range(5):
            protocol, _ = router.route_operation(OperationType.BULK_INSERT, data_size=10000)
    ...
    def test_operation_specific_routing(self, router):
        ...
        for operation_type, data_size, expected_protocol in test_cases:
            ...
            assert protocol == expected_protocol, f"Operation {operation_type.value} should prefer {expected_protocol.value}, got {protocol.value}"
    ...
    def test_hybrid_routing_integration(self, router):
        ...
        bulk_protocol, _ = router.route_operation(OperationType.BULK_INSERT, data_size=10000)
        ...
        fallback_protocol, _ = router.route_operation(OperationType.BULK_INSERT, data_size=10000)
            ...
            try:
                for i in range(100):
                    ...
                    for operation_type, data_size in operations:
                        ...
                        success = (i + hash(operation_type.value)) % 10 != 0  # 90% success rate
...
class TestBackwardCompatibilityIntegration:
    ...
    def test_create_operation_router_integration(self):
        ...
        try:
            ...
            for operation in [OperationType.SINGLE_SEARCH, OperationType.BULK_INSERT, OperationType.HEALTH_CHECK]:
...
class TestIntelligentRouterPerformanceIntegration:
    ...
    def test_routing_overhead(self):
        ...
        try:
            ...
            print(f"Average routing time: {(routing_time * 1000) / (1000 * len(operations)):.3f}ms per operation")
            
            # Should be very fast - routing should add minimal overhead
            assert operations_per_second > 10000  # At least 10k ops/sec
            assert (routing_time / (1000 * len(operations))) < 0.001  # Less than 1ms per operation
    ...
    def test_adaptive_routing_convergence(self):
        ...
        try:
            ...
            rest_percentage = (final_choices.count(Protocol.REST) / len(final_choices)) * 100
...
class TestIntelligentRouterRealServerIntegration(BaseProximaDBTest):
    ...
    def test_router_with_real_server(self):
        ...
        try:
            ...
            bulk_protocol, bulk_client = router.route_operation(OperationType.BULK_INSERT, data_size=10000)
            assert bulk_protocol == Protocol.GRPC
