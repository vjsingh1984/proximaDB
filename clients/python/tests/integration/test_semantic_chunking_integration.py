...
@pytest.mark.skip(
    reason="Tests require unimplemented features: enable_topic_detection, enable_content_analysis, enable_caching"
)
...
This guide covered the basics of using ProximaDB for vector operations. For more advanced topics, see the detailed API reference and examples in the repository.
    ...
    def test_semantic_vs_basic_chunking_quality(self, sample_technical_document):
        ...
        assert (
            semantic_result["semantic_coherence"]
            >= sliding_result["semantic_coherence"]
        )
        ...
        code_preserved = any(
            ...
            if "code" in chunk.text.lower()
        )
        headers_preserved = any(
            chunk.metadata.get("section_header") for chunk in semantic_chunks
        )

        # Should preserve important structures
        assert (
            code_preserved or headers_preserved
        )  # At least one type should be preserved
...
French cuisine emphasizes technique and high-quality ingredients. Sauces are fundamental to French cooking.
        ...
        ml_chunks = [
            ...
            for c in chunks
            if any(
                term in c.text.lower() for term in ["machine", "neural", "algorithm"]
            )
        ]
        cooking_chunks = [
            c
            for c in chunks
            if any(term in c.text.lower() for term in ["cooking", "recipe", "pasta"])
        ]
...
Each encounter taught her something valuable about life.
        ...
        chunker = create_enhanced_semantic_chunker(
            enable_content_analysis=True, enable_topic_detection=True
        )
        ...
        for content_type, text in test_contents.items():
            ...
            coherence_scores = [
                chunk.metadata.get("coherence_score", 0) for chunk in chunks
            ]
            ...
            results[content_type] = {
                ...
                "has_patterns": any(
                    chunk.metadata.get("semantic_patterns", []) for chunk in chunks
                ),
            }
...
This is a comprehensive document designed to test caching performance.
        """
            + "\n\n## Section {}\nThis is section {} with detailed content about various topics. "
            * 50
        )
        ...
        chunks1 = chunker_no_cache.chunk_semantically(
            large_document, "perf_test", {}, config
        )
        ...
        start_time = time.time()
        chunks2 = chunker_with_cache.chunk_semantically(
            large_document, "perf_test", {}, config
        )
        ...
        start_time = time.time()
        chunks3 = chunker_with_cache.chunk_semantically(
            large_document, "perf_test", {}, config
        )
    ...
    def test_chunk_quality_metrics(self, sample_technical_document):
        ...
        basic_config = ChunkingConfig(
            strategy=ChunkingStrategy.SLIDING_WINDOW, chunk_size=600, min_chunk_size=100
        )
        ...
        enhanced_config = ChunkingConfig(
            strategy=ChunkingStrategy.SEMANTIC, chunk_size=600, min_chunk_size=100
        )
        enhanced_chunker = TextChunker(enhanced_config)
        enhanced_chunks = enhanced_chunker.chunk_text(
            sample_technical_document, "enhanced"
        )
        ...
        assert (
            enhanced_metrics["structure_preservation"]
            >= basic_metrics["structure_preservation"]
        )

        # Enhanced chunks should preserve more semantic structure
        enhanced_with_headers = sum(
            1 for c in enhanced_chunks if c.metadata.get("section_header")
        )
        basic_with_headers = sum(
            1
            for c in basic_chunks
            if c.metadata.get("section_header", "").startswith("#")
        )
...
@pytest.mark.skip(
    reason="Tests require unimplemented features: enable_topic_detection, enable_caching"
)
class TestSemanticChunkingPerformance:
    ...
    def test_large_document_performance(self):
        ...
        chunker = create_enhanced_semantic_chunker(
            enable_topic_detection=True, enable_caching=True
        )
    ...
    def test_concurrent_chunking_performance(self):
        ...
        avg_duration = sum(result[2] for result in concurrent_results) / len(
            concurrent_results
        )
    ...
    def _generate_large_document(self, sections: int, content_per_section: int) -> str:
        ...
        for i in range(sections):
            ...
            if i % 3 == 0:
                ...
                # Analytical content
                content += (
                    "Our analysis shows significant improvements in performance. "
                )
                ...
                content += (
                    "Each component plays a crucial role in the overall architecture. "
                )
                content += (
                    "The integration between components ensures seamless operation.\n\n"
                )
