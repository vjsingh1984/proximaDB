...
class TestAllCollectionConfigurations:
    ...
    def _test_config(self, client, protocol: str, config: CollectionConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            result["success"] = True
            result["actual_config"] = collection.config.model_dump() if collection.config else None
            ...
            if collection.config:
                mismatches = []
                if config.distance_metric and collection.config.distance_metric != config.distance_metric:
                    mismatches.append(f"distance_metric: requested={config.distance_metric.value}, actual={collection.config.distance_metric.value}")
                if config.storage_engine and collection.config.storage_engine != config.storage_engine:
                    mismatches.append(f"storage_engine: requested={config.storage_engine.value}, actual={collection.config.storage_engine.value}")
                if config.primary_indexing_algorithm and collection.config.primary_indexing_algorithm != config.primary_indexing_algorithm:
                    mismatches.append(f"indexing_algorithm: requested={config.primary_indexing_algorithm.value}, actual={collection.config.primary_indexing_algorithm.value}")
    ...
    def test_all_quantization_types(self, rest_client, grpc_client):
        ...
        quant_configs = [
            ...
            ("scalar_16bit", QuantizationConfig(
                enabled=True,
                type=QuantizationType.SCALAR,
                bits_per_vector=16,
                accuracy_threshold=0.98
            )),
            ("product_4bit", QuantizationConfig(
                enabled=True,
            ...
            ("product_8bit", QuantizationConfig(
                enabled=True,
                type=QuantizationType.PRODUCT,
                num_subvectors=16,
    ...
    def test_filterable_columns(self, rest_client, grpc_client):
        ...
        column_configs = [
            ...
            ("indexed", [
                FilterableColumn(name="user_id", data_type=FilterableDataType.STRING, indexed=True),
                FilterableColumn(name="timestamp", data_type=FilterableDataType.INTEGER, indexed=True),
            ]),
            ("arrays", [
                FilterableColumn(name="tags", data_type=FilterableDataType.ARRAY_STRING),
                FilterableColumn(name="scores", data_type=FilterableDataType.ARRAY_FLOAT),
            ]),
        ]
    ...
    def _get_config_description(self, config: Dict) -> str:
        ...
        if "primary_indexing_algorithm" in config and config["primary_indexing_algorithm"]:
