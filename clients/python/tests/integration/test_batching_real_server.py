...
class TestRealServerBatching(BaseProximaDBTest):
    ...
    def test_batch_vector_insertion_helper(self):
        ...
        assert (
            "batch_vec_0000" in result_ids
        ), f"Expected batch_vec_0000 in results, got: {result_ids}"
    ...
    def test_concurrent_batch_insertions(self):
        ...
        # Verify with search
        results = self.rest_client.search(collection_name, [0.1] * 384, top_k=10)
...
class TestBatchMetrics(BaseProximaDBTest):
    ...
    def test_metrics_creation(self):
        ...
        config = BatchConfig(max_batch_size=20, strategy=BatchStrategy.SIZE_BASED)
...
class TestBatchHelpers(BaseProximaDBTest):
    ...
    def test_batch_insert_vectors_helper(self):
        ...
        search_results = self.rest_client.search(
            collection_name, vectors[0].vector, top_k=5
        )
        ...
        result_ids = [r.id for r in search_results]
        assert (
            "helper_vec_0000" in result_ids
        ), f"Expected helper_vec_0000 in results, got: {result_ids}"
...
class TestGRPCBatching(BaseProximaDBTest):
    ...
    def test_grpc_batch_insertion(self):
        ...
        assert (
            "grpc_batch_0000" in result_ids
        ), f"Expected grpc_batch_0000 in results, got: {result_ids}"
    ...
    def test_cross_protocol_batching(self):
        ...
        rest_search = self.rest_client.search(
            rest_collection, vectors[0].vector, top_k=5
        )

        grpc_search = self.grpc_client.search(
            grpc_collection, vectors[0].vector, top_k=5
        )

        assert len(rest_search) == 5
        # Handle different gRPC response format
        grpc_results_data = (
            grpc_search.results if hasattr(grpc_search, "results") else grpc_search
        )
