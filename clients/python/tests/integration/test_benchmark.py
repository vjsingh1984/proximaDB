...
class TestComprehensiveBenchmark:
    ...
    def find_proximadb_root() -> str:
        ...
        for _ in range(10):  # Max 10 levels up
            if all(os.path.exists(os.path.join(current_dir, f)) for f in ["Cargo.toml", "README.adoc", "docs"]):
                return current_dir
    ...
    def documentation_text(self):
        ...
        doc_candidates = [
            os.path.join(root_dir, "docs", "appendices", "appendix_testing_development.adoc"),
        ...
        for doc_file in doc_candidates:
            if os.path.exists(doc_file):
                with open(doc_file, 'r', encoding='utf-8') as f:
                    full_text = f.read()
                logger.info(f"Loaded {len(full_text)} characters from {os.path.relpath(doc_file, os.getcwd())}")
                return full_text
        ...
        return [
            {
                ...
                "expected_terms": ["vector", "database", "architecture", "storage", "engine"],
            ...
            },
            {
                ...
                "expected_terms": ["chunk", "strategy", "semantic", "sliding", "window"],
    ...
    def benchmark_combination(
        ...
        collection_name = f"bench_{chunking_strategy.value}_{embedding_provider.model_name.replace('/', '_')}_{storage_engine.value}_{protocol}"[:50]
        ...
        try:
            ...
            for query_info in search_queries:
                ...
                search_results = client.search(
                    ...
                    vector=query_embedding.tolist() if hasattr(query_embedding, 'tolist') else query_embedding,  # Changed from query_vector to vector
                    top_k=10
                )
            ...
            if protocol == "rest":
                ...
                for query_info in search_queries:
                    # Build SQL query
                    query_embedding = embedding_provider.embed_text(query_info["embedding_text"])
                    vector_str = str(query_embedding.tolist() if hasattr(query_embedding, 'tolist') else query_embedding)
    ...
    def test_comprehensive_benchmark(
        ...
        # Run benchmarks
        results = []
        total_combinations = len(chunking_strategies) * len(embedding_providers) * len(storage_engines) * len(protocols)
    ...
    def generate_summary_report(self, results: List[BenchmarkResult]):
        ...
        sorted_by_accuracy = sorted(successful_results, key=lambda x: x.search_accuracy, reverse=True)[:5]
        for i, r in enumerate(sorted_by_accuracy, 1):
            print(f"{i}. {r.chunking_strategy} + {r.embedding_provider} + {r.storage_engine} + {r.protocol}: {r.search_accuracy:.2%}")
        ...
        for i, r in enumerate(sorted_by_speed, 1):
            print(f"{i}. {r.chunking_strategy} + {r.embedding_provider} + {r.storage_engine} + {r.protocol}: {r.total_time:.2f}s")
        ...
        rest_results = [r for r in successful_results if r.protocol == "rest" and r.sql_accuracy > 0]
        if rest_results:
            ...
            print(f"  SQL/Vector Ratio: {np.mean(sql_accuracies) / np.mean(vector_accuracies):.2f}x")
        
        # 8. Optimal combinations
        print("\n🏆 RECOMMENDED CONFIGURATIONS:")
        print("\n  For Accuracy:")
        best_accuracy = sorted_by_accuracy[0]
        print(f"    {best_accuracy.chunking_strategy} + {best_accuracy.embedding_provider} + {best_accuracy.storage_engine} + {best_accuracy.protocol}")
        
        print("\n  For Speed:")
        best_speed = sorted_by_speed[0]
        print(f"    {best_speed.chunking_strategy} + {best_speed.embedding_provider} + {best_speed.storage_engine} + {best_speed.protocol}")
        ...
        print(f"  Avg chunks per document: {np.mean([r.num_chunks for r in successful_results]):.0f}")
