...
class BERTEmbeddingService:
    ...
    def __init__(
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            ...
            self.model = None
            print(
                f"⚠️ Using simulated {self.dimension}D embeddings (install sentence-transformers for real BERT)"
            )
    ...
    def embed_texts(
        ...
        for i in range(0, total, batch_size):
            ...
            if show_progress:
                ...
                print(
                    f"\r🔄 Embedding progress: {progress:.1f}% ({i + len(batch)}/{total})",
                    end="",
                    flush=True,
                )
...
def create_sample_corpus(size_mb: float = 10.0) -> List[Dict[str, str]]:
    ...
    while total_size < target_size:
        ...
        if doc_id % 1000 == 0:
            print(
                f"Generated {doc_id} documents, {total_size / (1024*1024):.1f}MB",
                end="\r",
            )
