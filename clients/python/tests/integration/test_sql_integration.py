...
class TestSqlIntegration:
    ...
    def sql_test_collection(self, client):
        ...
        config = CollectionConfig(
            ...
            filterable_columns=[
                ...
                FilterableColumn(
                    name="brand", data_type=FilterableDataType.STRING, indexed=True
                ),
                FilterableColumn(
                    name="price",
                ...
                FilterableColumn(
                    name="created_at", data_type=FilterableDataType.STRING
                ),
            ],
        )
    ...
    def test_basic_similarity_search(self, client, sql_test_collection):
        ...
        electronics_count = sum(
            1
            for row in actual_result["rows"][:5]
            if row.get("metadata.category") == "electronics"
            or (
                isinstance(row.get("metadata"), dict)
                and row["metadata"].get("category") == "electronics"
            )
        )
    ...
    def test_pagination_with_offset(self, client, sql_test_collection):
        ...
        assert [row["id"] for row in page1["rows"][5:]] == [
            row["id"] for row in overlap["rows"][:5]
        ]
    ...
    def test_large_result_set(self, client, sql_test_collection):
        ...
        assert (
            result["row_count"] <= 5
        ), f"Expected at most 5 results, got {result['row_count']}"
        assert (
            len(result["rows"]) <= 5
        ), f"Expected at most 5 rows, got {len(result['rows'])}"
        ...
        categories = {
            row.get("metadata.category") or row.get("metadata", {}).get("category")
