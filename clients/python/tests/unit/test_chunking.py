...
"""
    ...
    def test_semantic_chunking_with_numbered_sections(self):
        ...
        config = ChunkingConfig(
            strategy=ChunkingStrategy.SEMANTIC, chunk_size=100, max_chunk_size=150
        )
        chunker = TextChunker(config)
...
"""
    ...
    def test_semantic_chunking_no_sections_fallback(self):
        ...
        config = ChunkingConfig(
            strategy=ChunkingStrategy.SEMANTIC, chunk_size=80, max_chunk_size=100
        )
        chunker = TextChunker(config)
...
"""
    ...
    @pytest.mark.skip(
        reason="Semantic chunking keeps paragraphs together - this is expected behavior"
    )
...
This is the final paragraph with conclusion."""
        ...
        for chunk in chunks:
            ...
            assert (
                len(chunk.text) <= config.chunk_size
                or len(chunk.text) >= config.min_chunk_size
            )
...
class TestFixedSizeChunking:
    ...
    def test_fixed_size_chunking_basic(self):
        ...
        config = ChunkingConfig(
            strategy=ChunkingStrategy.FIXED_SIZE, chunk_size=20, min_chunk_size=5
        )
        chunker = TextChunker(config)
...
"""
        ...
        assert (
            len(chunks) > paragraph_count
        )  # More chunks than paragraphs indicates subdivision

    def test_paragraph_chunking_small_paragraphs_skipped(self):
        """Test that small paragraphs are skipped"""
        config = ChunkingConfig(
            strategy=ChunkingStrategy.PARAGRAPH, min_chunk_size=20  # Higher minimum
        )
        chunker = TextChunker(config)
...
class TestCreateChunkerFactory:
    ...
    def test_create_chunker_with_kwargs(self):
        ...
        chunker = create_chunker(
            "sliding_window", chunk_size=256, chunk_overlap=64, preserve_sentences=False
        )
...
class TestEdgeCases:
    ...
    def test_chunk_single_character(self):
        ...
        config = ChunkingConfig(chunk_size=1, min_chunk_size=1)
        chunker = TextChunker(config)
