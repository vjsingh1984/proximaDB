...
"""
    ...
    def test_sentence_strategy(self, sample_text):
        ...
        config = ChunkingConfig(
            strategy=ChunkingStrategy.SENTENCE, chunk_size=300, min_chunk_size=100
        )
    ...
    def test_paragraph_strategy(self, sample_text):
        ...
        config = ChunkingConfig(
            strategy=ChunkingStrategy.PARAGRAPH, chunk_size=500, min_chunk_size=100
        )
    ...
    def test_recursive_strategy(self, sample_text):
        ...
        for chunk in chunks:
            ...
            assert chunk.metadata["strategy_used"] in [
                "paragraph",
                "sentence",
                "sliding_window",
            ]
...
class TestTextChunker:
    ...
    def test_chunker_initialization(self):
        ...
        # Custom config
        config = ChunkingConfig(strategy=ChunkingStrategy.SEMANTIC, chunk_size=1000)
...
"""
        ...
        for strategy in ChunkingStrategy:
            ...
            assert all(
                c.metadata["chunking_strategy"] == strategy.value for c in chunks
            )
    ...
    def test_add_context_to_chunks(self):
        ...
        chunker = TextChunker(
            ChunkingConfig(chunk_size=100, chunk_overlap=0, min_chunk_size=30)
        )

        # Long text with clear chunks (each sentence is ~50+ chars)
        text = (
            "This is the first substantial chunk of text with enough content to be meaningful. "
...
@pytest.mark.skip(
    reason="Tests require embedding provider initialization and server setup - actually integration tests"
)
class TestVectorRecordCreation(BaseProximaDBTest):
    ...
    def test_chunk_and_embed_integration(self):
        ...
        records = chunk_and_embed_text(
            ...
            chunking_config=ChunkingConfig(
                strategy=ChunkingStrategy.SENTENCE, chunk_size=200
            ),
    ...
    def test_store_chunks_in_proximadb(self):
        ...
        records = chunk_and_embed_text(
            ...
            embedding_provider=provider,
            chunking_config=ChunkingConfig(
                strategy=ChunkingStrategy.SEMANTIC, chunk_size=300
            ),
...
class TestSeparationOfConcerns:
    ...
    def test_chunking_has_no_embedding_logic(self):
        ...
        embedding_keywords = [
            ...
            "BERT",
        ]  # Only check actual imports/modules
        ...
        for strategy_file in strategies_dir.glob("*.py"):
            ...
            for line in import_lines:
                for keyword in embedding_keywords:
                    if keyword in line:
                        pytest.fail(
                            f"Found embedding import in {strategy_file.name}: {line.strip()}"
                        )
    ...
    def test_strategies_are_pluggable(self):
        ...
        text = (
            "This is the first sentence in a paragraph with substantial content. "
        ...
        for strategy in strategies:
            chunker = TextChunker(
                ChunkingConfig(strategy=strategy, chunk_size=80, min_chunk_size=20)
            )
        ...
        assert (
            len(set(chunk_counts)) > 1
        ), f"All strategies produced same number of chunks: {chunk_counts}"
