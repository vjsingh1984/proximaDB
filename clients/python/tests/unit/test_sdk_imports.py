...
class TestSDKImports:
    ...
    def test_compression_config_import(self):
        ...
        from proximadb import CompressionConfig
    ...
    def test_module_reload_stability(self):
        ...
        initial_client = proximadb.ProximaDBClient
        initial_compression = proximadb.CompressionConfig
        
        # Reload module
        importlib.reload(proximadb)
        
        # Check references are still valid
        assert proximadb.ProximaDBClient is not None
        assert proximadb.CompressionConfig is not None
    ...
    def test_import_error_messages(self):
        ...
        with pytest.raises(ImportError) as exc_info:
            from proximadb import NonExistentClass
    ...
    def test_submodule_imports(self):
        ...
        submodules = [
            'proximadb.config',
            'proximadb.models',
            'proximadb.exceptions',
            'proximadb.chunking',
            'proximadb.filters',
            'proximadb.unified_client'
        ]
    ...
    def test_public_api_completeness(self):
        ...
        for api in all_apis:
            assert hasattr(proximadb, api), f"Missing public API: {api}"
            # Also check it's in __all__
            assert api in proximadb.__all__, f"API {api} not in __all__"
