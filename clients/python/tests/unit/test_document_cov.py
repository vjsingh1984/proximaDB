...
def test_insert_uses_given_id():
    # DocumentRepository.insert is local-only (no server round-trip), so the
    # caller-supplied id wins — there is no server-echoed id.
    repo = DocumentRepository(make_client())
    doc = repo.insert("coll1", {"a": 1}, id="given")
    assert doc.id == "given"
    assert repo._documents["coll1"]["given"] is doc
