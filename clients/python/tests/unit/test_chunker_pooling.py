...
class TestChunkerPool:
    ...
    def config(self):
        ...
        return ChunkingConfig(
            strategy=ChunkingStrategy.SLIDING_WINDOW, chunk_size=512, chunk_overlap=128
        )
...
class TestPooledChunkerContext:
    ...
    @pytest.fixture
    def config(self):
        return ChunkingConfig(strategy=ChunkingStrategy.SLIDING_WINDOW, chunk_size=256)
...
class TestChunkingWithPooling:
    ...
    def test_recursive_chunking_uses_pooling(self):
        ...
        config = ChunkingConfig(
            strategy=ChunkingStrategy.RECURSIVE, chunk_size=1000, max_chunk_size=3000
        )
    ...
    def test_semantic_chunking_uses_pooling(self):
        ...
        text_with_headers = (
            """
# Section 1
This is a very long section that exceeds the maximum chunk size limit. """
            + ("Content goes here. " * 100)
            + """

# Section 2  
Another long section that also exceeds the maximum chunk size. """
            + ("More content here. " * 100)
        )

        config = ChunkingConfig(
            strategy=ChunkingStrategy.SEMANTIC, chunk_size=500, max_chunk_size=1000
        )
...
class TestPoolingPerformance:
    ...
    def test_concurrent_chunking_performance(self):
        ...
        config = ChunkingConfig(
            strategy=ChunkingStrategy.SLIDING_WINDOW, chunk_size=200
        )
