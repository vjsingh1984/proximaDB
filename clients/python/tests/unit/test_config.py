...
class TestRetryConfig:
    ...
    def test_retry_config_custom(self):
        ...
        config = RetryConfig(
            max_retries=5, backoff_factor=3.0, max_backoff=120.0, retry_on_timeout=False
        )
...
class TestConnectionConfig:
    ...
    def test_connection_config_custom(self):
        ...
        config = ConnectionConfig(pool_size=20, pool_maxsize=200, read_timeout=60.0)
...
class TestCompressionConfig:
    ...
    def test_compression_config_custom(self):
        ...
        config = CompressionConfig(
            enabled=False, algorithm="deflate", threshold_bytes=2048, level=6
        )
        assert config.enabled is False
...
class TestClientConfig:
    ...
    def test_client_config_from_dict(self):
        ...
        config_dict = {
            ...
            "connection": {"pool_size": 20, "read_timeout": 60.0},
        }
...
class TestConfigFunctions:
    ...
    def test_client_config_methods(self):
        ...
        assert (
            config.get_protocol_url(Protocol.REST) == "https://api.proximadb.com:5678"
        )
        ...
        config_multi = ClientConfig(
            url="https://api.proximadb.com", port_mode=PortMode.MULTI
        )
