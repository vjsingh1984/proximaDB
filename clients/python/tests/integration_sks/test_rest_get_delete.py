...
def test_rest_get_and_delete_batch_sks_or_legacy():
    ...
    try:
        ...
        # Insert 2 vectors
        vectors = np.array(
            [[0.2, 0.1, 0.0, 0.3], [0.25, 0.05, 0.02, 0.29]], dtype=np.float32
        )
        ids = ["gid1", "gid2"]
        ...
        got = client.get_vector(
            coll, "gid1", include_vector=False, include_metadata=True
        )
        assert isinstance(got, dict)
        assert (
            got.get("id") == "gid1"
            or got.get("vector_id") == "gid1"
            or got.get("entity_id") == "gid1"
        )
