...
class TestSSTEngine:
    ...
    def test_sst_basic_insert_search(self, sst_db, test_vectors, query_vectors):
        ...
        sst_db.create_collection(
            "sst_basic_test",
            dimension=test_vectors["dimension"],
            engine="sst"
        )
        ...
        results = sst_db.search(
            "sst_basic_test",
            query=query_vectors["queries_list"][0],
            top_k=10
        )
    ...
    def test_sst_self_match(self, sst_db, test_vectors):
        ...
        results = sst_db.search(
            "sst_selfmatch",
            query=test_vectors["vectors_list"][0],
            top_k=5
        )

        assert len(results) >= 1, "Should get at least 1 result"
        assert results[0].id == "vec_0", f"First result should be vec_0, got {results[0].id}"
        assert results[0].score > 0.99, f"Self-match score should be ~1.0, got {results[0].score}"
...
class TestCrossEngineConsistency:
    ...
    def test_engine_returns_results(self, multi_engine_dbs, test_vectors, query_vectors, engine):
    ...
    def test_engine_self_match_works(self, multi_engine_dbs, test_vectors, engine):
        ...
        assert "vec_0" in top_ids, f"Engine {engine}: vec_0 should be in top-3, got {top_ids}"
...
class TestPerformanceSanity:
    ...
    def test_search_latency_reasonable(self, perf_db, test_vectors, query_vectors):
        ...
        assert avg_latency < 100, f"Average latency should be < 100ms, got {avg_latency:.1f}ms"
