...
class TestGRPCAPICompleteness:
    ...
    def test_collection_endpoints(self, grpc_client):
        ...
        try:
            ...
            if hasattr(collections, "__await__"):
                ...
                print(
                    f"✅ ListCollections - List collections: WORKING ({len(collections)} collections)"
                )
        except Exception as e:
    ...
    def test_search_endpoints(self, grpc_client, test_collection):
        ...
        try:
            ...
            if results and len(results) > 0:
                print(
                    f"✅ SearchVectors - Basic search: WORKING ({len(results)} results)"
                )
                ...
                print(
                    f"✅ SearchVectors - Score range: VALID ({[f'{r.score:.3f}' for r in results[:3]]})"
                )
        ...
        try:
            ...
            large_k_results = grpc_client.search(
                test_collection, query_vector, top_k=100
            )
    ...
    def test_advanced_endpoints(self, grpc_client, test_collection):
        ...
        try:
            if hasattr(grpc_client, "get_vector_history"):
                history = grpc_client.get_vector_history(
                    test_collection, "search_vector_0"
                )
    ...
    def test_streaming_endpoints(self, grpc_client):
        ...
        try:
            if hasattr(grpc_client, "stream_insert"):
                print(
                    "⚠️ StreamInsert - IMPLEMENTED BUT NOT TESTED (requires streaming)"
                )
            else:
        ...
        try:
            if hasattr(grpc_client, "stream_search"):
                print(
                    "⚠️ StreamSearch - IMPLEMENTED BUT NOT TESTED (requires streaming)"
                )
            else:
    ...
    def test_error_handling(self, grpc_client):
        ...
        try:
            ...
            try:
                ...
                print(
                    "❌ InsertVector(wrong dimension) - Should return INVALID_ARGUMENT"
                )
            except ProximaDBError as e:
                if "dimension" in str(e).lower() or "INVALID_ARGUMENT" in str(e):
                    print(
                        "✅ InsertVector(wrong dimension) - Proper INVALID_ARGUMENT handling"
                    )
                else:
    ...
    def test_async_issues_diagnosis(self, grpc_client):
        ...
        if async_methods:
            ...
            print(
                "🔧 Suggestion: Update SDK to handle async methods or provide sync wrappers"
            )
    ...
    def test_endpoint_summary(self, grpc_client):
        ...
        async_issues = ["ListCollections (list) - Returns coroutine, needs async fix"]
        ...
        total_tested = (
            len(working_endpoints) + len(async_issues) + len(missing_endpoints)
        )
