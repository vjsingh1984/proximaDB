...
# Note: pytest options are defined in the main conftest.py to avoid conflicts
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        help="REST endpoint URL for ProximaDB server",
    )
    parser.addoption(
        "--grpc-endpoint",
        action="store",
        default="localhost:5679",
        help="gRPC endpoint URL for ProximaDB server",
    )
...
def generate_test_vectors(count: int, dimension: int) -> list:
    """Generate test vectors using sentence-transformers for realism"""
