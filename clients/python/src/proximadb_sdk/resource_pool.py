...
class ResourceFactory(ABC, Generic[T]):
    ...
    def health_check(self, resource: T) -> ResourceHealth:
        ...
        return ResourceHealth.HEALTHY if self.validate(resource) else ResourceHealth.UNHEALTHY
    ...
    def release(self, resource: T, destroy: bool = False):
        ...
        with self._lock:
            ...
            if self.metrics and resource_id in self._usage_times:
                ...
                self.metrics.avg_usage_time_ms = (
                    sum(self._usage_time_history) / len(self._usage_time_history)
                )
