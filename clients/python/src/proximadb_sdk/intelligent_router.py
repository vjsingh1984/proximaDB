...
class ProtocolMetrics:
    ...
    def get_score(self, strategy: RoutingStrategy) -> float:
        ...
        if (
            ...
            and time.time() < self.circuit_breaker_half_open_time
        ):
            return 0.0
        ...
        if strategy == RoutingStrategy.PERFORMANCE_BASED:
            ...
            perf_score = (
                max(0.0, min(1.0, 100.0 / self.get_avg_latency()))
                if self.get_avg_latency() != float("inf")
                else 0.5
            )
            ...
            return (
                ...
                in [ProtocolHealth.HEALTHY, ProtocolHealth.DEGRADED]
                else 0.0
            )
...
class IntelligentRouter:
    ...
    def __init__(
        self, config: RoutingConfig = None, client_config: ClientConfig = None
    ):
        ...
        with self._routing_lock:
            ...
            if preferred_protocol and self._is_protocol_healthy(preferred_protocol):
                try:
                    ...
                    logger.warning(
                        f"Failed to use preferred protocol {preferred_protocol}: {e}"
                    )

            # Select protocol based on strategy
            selected_protocol = self._select_protocol(
                operation, data_size, required_features
            )
            ...
            while client is None and attempts < self.config.max_fallback_attempts:
                try:
                    ...
                    self._metrics[selected_protocol].update_failure(
                        "client_creation_failed"
                    )

                    # Try fallback protocol
                    if self.config.enable_fallback:
                        selected_protocol = self._get_fallback_protocol(
                            selected_protocol
                        )
    ...
    def _select_protocol(
        ...
        if self.config.strategy in [
                ...
                if rule.matches(operation, data_size):
                    if self._is_protocol_healthy(rule.preferred_protocol):
                        ...
                        if self.config.strategy == RoutingStrategy.HYBRID:
                            ...
                            fallback_protocol = self._get_fallback_protocol(
                                rule.preferred_protocol
                            )
                            if fallback_protocol:
                                fallback_metrics = self._metrics[fallback_protocol]
                                # Choose based on performance if both are healthy
                                if (
                                    self._is_protocol_healthy(fallback_protocol)
                                    and fallback_metrics.get_score(
                                        RoutingStrategy.PERFORMANCE_BASED
                                    )
                                    > preferred_metrics.get_score(
                                        RoutingStrategy.PERFORMANCE_BASED
                                    )
                                    * 1.2
                                ):
            ...
            if self._current_protocol and self._is_protocol_healthy(
            ...
            if self.config.enable_load_balancing:
                ...
                if window_elapsed > self.config.load_balance_window_seconds:
                    ...
                    for protocol in scores:
                        usage_ratio = (
                            self._load_balance_counters[protocol] / total_requests
                        )
    ...
    def _is_protocol_healthy(self, protocol: Protocol) -> bool:
        ...
        return (
            metrics.health_status in [ProtocolHealth.HEALTHY, ProtocolHealth.DEGRADED]
    ...
    def _record_operation_start(self, operation: OperationType, protocol: Protocol):
        ...
        self._operation_history[key].append(
            {"start_time": time.time(), "completed": False}
        )
    ...
    def _update_learned_preferences(self):
        ...
        if (
            current_time - self._last_learning_update
            < self.config.learning_update_interval
        ):
            return
        ...
        for operation in OperationType:
            ...
            for protocol in Protocol:
                ...
                success_rate = sum(
                    1 for h in completed if h.get("success", False)
                ) / len(completed)
                avg_latency = statistics.mean(
                    h.get("latency_ms", 1000) for h in completed
                )
    ...
    def _start_monitoring(self):
        ...
        self._monitoring_thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="IntelligentRouter-Monitor"
        )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self._metrics_lock:
            return {
                ...
                "current_selection": (
                    self._current_protocol.value if self._current_protocol else None
                ),
            }
