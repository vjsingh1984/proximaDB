...
class ProximaDBAuth:
    ...
    def __init__(
        ...
        if self.config.client_cert_path and self.config.client_key_path:
            self.session.cert = (self.config.client_cert_path, self.config.client_key_path)
    ...
    def authenticate(self) -> AuthResult:
        ...
        if not self.config.enabled:
            ...
            return AuthResult(
                ...
                permissions=list(Permission)  # Give all permissions when auth is disabled
            )
    ...
    def _authenticate_jwt(self) -> AuthResult:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise AuthenticationError(f"JWT validation failed: {response.status_code}")
            ...
            logger.warning(f"JWT validation request failed, using offline validation: {e}")
    ...
    def _authenticate_oauth2(self) -> AuthResult:
        ...
        try:
            ...
            if response.status_code != 200:
                raise AuthenticationError(f"OAuth2 validation failed: {response.status_code}")
    ...
    def _authenticate_client_cert(self) -> AuthResult:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/auth/cert/validate",
                timeout=10
            )
            
            if response.status_code != 200:
                raise AuthenticationError(f"Client certificate validation failed: {response.status_code}")
    ...
    def _should_refresh_token(self) -> bool:
        ...
        return datetime.now(timezone.utc) >= (self.auth_result.token_expires_at - threshold)
        ...
        if self.auth_result and self.auth_result.access_token:
            try:
                self.session.post(
                    ...
                    headers={"Authorization": f"Bearer {self.auth_result.access_token}"},
                    timeout=10
                )
    ...
    def get_user_info(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "expires_at": self.auth_result.token_expires_at.isoformat() if self.auth_result.token_expires_at else None,
        }
