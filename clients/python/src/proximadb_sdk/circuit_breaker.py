...
class CircuitBreaker:
    ...
    async def _change_state(self, new_state: CircuitState):
        ...
        self._logger.info(f"Circuit breaker {self.name} state changed: {old_state} -> {new_state}")
    ...
    async def _clean_old_metrics(self):
        ...
        self._recent_calls = [
            (timestamp, success) for timestamp, success in self._recent_calls
    ...
    async def _calculate_delay(self, attempt: int) -> float:
        ...
        if self.config.strategy == RetryStrategy.FIXED_DELAY:
            ...
            delay = (self.config.initial_delay_ms * 
                    (self.config.backoff_multiplier ** attempt)) / 1000.0
        ...
        if self.config.jitter_enabled and self.config.strategy != RetryStrategy.JITTERED:
            jitter = random.uniform(0, self.config.jitter_max_ms / 1000.0)
            delay += jitter
...
class ResilientClient:
    ...
    def _calculate_health_score(self, metrics: CircuitBreakerMetrics) -> float:
        ...
        if (metrics.last_failure_time and 
            current_time - metrics.last_failure_time < 60):  # Last minute
            base_score *= 0.8
...
async def create_resilient_client(name: str,
                                 circuit_config: Optional[CircuitBreakerConfig] = None,
                                 retry_config: Optional[RetryConfig] = None) -> ResilientClient:
