...
class AsyncBatchProcessor(BatchProcessor):
    ...
    def _update_metrics(self, batch_size: int, latency_ms: float):
        ...
        if self.metrics.total_batches > 0:
            self.metrics.avg_batch_size = self.metrics.batched_requests / self.metrics.total_batches
            self.metrics.avg_latency_ms = self.metrics.total_latency_ms / self.metrics.total_batches
    ...
    def _collect_batch(self, batch_key: str) -> List[BatchRequest]:
        ...
        with self._queue_locks[batch_key]:
            ...
            while queue and len(batch) < self.config.max_batch_size:
                # Check time limit
                if batch and (time.time() - start_time) * 1000 >= self.config.max_wait_time_ms:
                    break
...
def create_vector_batcher(client, collection_id: str, max_batch_size: int = 100) -> 'VectorBatcher':
    """Create a vector-specific batcher"""
    return VectorBatcher(client, collection_id, max_batch_size)


def batch_insert_vectors(client, collection_id: str, vectors: List['VectorRecord'], batch_size: int = 100) -> List[Dict]:
