...
class ParagraphStrategy(ChunkingStrategyInterface):
    ...
    def _is_list_paragraph(self, text: str) -> bool:
        ...
        list_lines = sum(1 for line in lines if 
    ...
    def chunk(
        ...
        for para_text, para_start in paragraphs:
            ...
            if para_length > self.config.max_chunk_size:
                ...
                if current_chunk_paras:
                    ...
                    if len(chunk_text) >= self.config.min_chunk_size:
                        chunks.append(self._create_chunk(
                            chunk_text, current_chunk_start, chunk_index,
                            source_id, base_metadata, len(current_chunk_paras)
                        ))
                        chunk_index += 1
                    current_chunk_paras = []
                    current_chunk_length = 0
                ...
                sub_chunks = self._split_large_paragraph(para_text, self.config.chunk_size)
                for sub_chunk in sub_chunks:
                    chunks.append(self._create_chunk(
                        sub_chunk, para_start, chunk_index,
                        source_id, base_metadata, 1, is_list
                    ))
                    chunk_index += 1
            ...
            if current_chunk_paras and current_chunk_length + separator_length + para_length > self.config.chunk_size:
    ...
    def _create_chunk(
        ...
        chunk_metadata = {
            **base_metadata,
            ...
            "is_list": is_list,
            "first_line": text.split('\n')[0][:50] + "..." if len(text.split('\n')[0]) > 50 else text.split('\n')[0],
        }
