...
class SemanticStrategy(ChunkingStrategyInterface):
    ...
    def _compile_patterns(self):
        ...
        self.transition_patterns = [
            re.compile(r'^(however|moreover|furthermore|additionally|consequently|therefore|thus)', re.IGNORECASE | re.MULTILINE),
            re.compile(r'^(in conclusion|in summary|to summarize|finally)', re.IGNORECASE | re.MULTILINE),
            re.compile(r'^(first|second|third|next|then|lastly)', re.IGNORECASE | re.MULTILINE),
        ]
        
        # Section breaks
        self.section_break_pattern = re.compile(r'^[\-\*_]{3,}$', re.MULTILINE)
    
    def _identify_sections(self, text: str) -> List[Tuple[str, int, int, Dict[str, Any]]]:
    ...
    def _identify_topic_sections(self, text: str) -> List[Tuple[str, int, int, Dict[str, Any]]]:
            ...
            if section_text:
                sections.append((
                    ...
                    boundaries[i + 1],
                    {
                        ...
                        'has_header': False,
                        'boundary_type': 'topic_transition' if boundaries[i] in transitions else 'section_break'
                    }
                ))
        
        return sections if sections else [(text, 0, len(text), {'section_type': 'single', 'has_header': False})]
    ...
    def _restore_special_blocks(self, text: str, preserved_blocks: List[Dict[str, Any]]) -> str:
    ...
    def chunk(
        ...
        for section_text, start_pos, end_pos, section_metadata in sections:
            ...
            if preserved_blocks:
                section_text = self._restore_special_blocks(section_text, preserved_blocks)
            ...
            if len(section_text) <= self.config.chunk_size:
                ...
                sub_chunks = self._split_large_section(
                    section_text, start_pos, source_id,
                    chunk_index, base_metadata, section_metadata
                )
    ...
    def _split_large_section(
        ...
        for para in paragraphs:
            ...
            if current_chunk_text and len(current_chunk_text) + len(separator) + len(para) > self.config.chunk_size:
                # Create chunk
        ...
        if current_chunk_text and (len(current_chunk_text) >= self.config.min_chunk_size or not chunks):
            chunk_metadata = {
                **base_metadata,
                **section_metadata,
                "chunk_type": "semantic_split",
