...
from .models import VectorRecord, SearchResult, Collection, DistanceMetric, StorageEngine
...
class ProximaDBClientV1:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.protocol == "grpc":
            ...
            self.collection_stub = collection_pb2_grpc.CollectionServiceStub(self.channel)
            self.sql_stub = sql_pb2_grpc.SqlServiceStub(self.channel)
            self.graph_stub = graph_pb2_grpc.GraphServiceStub(self.channel)
        
        logger.info(f"ProximaDB client initialized with protocol: {self.protocol}, url: {url}")
    ...
    def create_collection(
        ...
        **kwargs
        ...
        if self.protocol == "grpc":
            return self._create_collection_grpc(name, dimension, distance_metric, storage_engine, **kwargs)
        else:
            return self._create_collection_rest(name, dimension, distance_metric, storage_engine, **kwargs)
    
    def _create_collection_grpc(self, name: str, dimension: int, distance_metric: str, storage_engine: str, **kwargs):
    ...
    def _create_collection_rest(self, name: str, dimension: int, distance_metric: str, storage_engine: str, **kwargs):
    ...
    def _list_collections_grpc(self) -> List[Collection]:
        ...
        try:
            response = self.collection_stub.ListCollections(request, timeout=self.timeout)
            return [
                Collection(
                    id=col.id,
                    name=col.name,
    ...
    def _insert_vectors_grpc(self, collection_id: str, vectors: List[VectorRecord]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "metrics": {
                    "total_processed": response.metrics.total_processed if response.metrics else 0,
                    "successful_count": response.metrics.successful_count if response.metrics else 0,
                    "failed_count": response.metrics.failed_count if response.metrics else 0,
                } if response.metrics else {}
            }
        except grpc.RpcError as e:
            raise ProximaDBError(f"gRPC error: {e.details()}")
    
    def _insert_vectors_rest(self, collection_id: str, vectors: List[VectorRecord]) -> Dict[str, Any]:
    ...
    def _get_vector_grpc(self, collection_id: str, vector_id: str) -> Optional[VectorRecord]:
        """Get vector via gRPC"""
    ...
    def _get_vector_rest(self, collection_id: str, vector_id: str) -> Optional[VectorRecord]:
        """Get vector via REST"""
    ...
    # SQL operations
    def execute_sql(self, query: str, parameters: Optional[List[Any]] = None) -> Dict[str, Any]:
        """Execute SQL query"""
        if self.protocol == "grpc":
    ...
    def _execute_sql_grpc(self, query: str, parameters: Optional[List[Any]] = None) -> Dict[str, Any]:
        ...
        request = types_pb2.ExecuteSqlRequest(
            query=query,
    ...
    def _execute_sql_rest(self, query: str, parameters: Optional[List[Any]] = None) -> Dict[str, Any]:
    ...
    def _convert_to_sql_value(self, value: Any) -> types_pb2.SqlValue:
        ...
        if isinstance(value, str):
            ...
            object_fields = {
                key: self._convert_to_sql_value(val) 
    ...
    def create_node(self, node_id: str, labels: List[str], properties: Optional[Dict[str, Any]] = None,
    ...
    def _create_node_grpc(self, node_id: str, labels: List[str], properties: Optional[Dict[str, Any]] = None,
                         embedding: Optional[List[float]] = None) -> Dict[str, Any]:
    ...
    def _create_node_rest(self, node_id: str, labels: List[str], properties: Optional[Dict[str, Any]] = None,
                         embedding: Optional[List[float]] = None) -> Dict[str, Any]:
    ...
    def create_edge(self, edge_id: str, from_node_id: str, to_node_id: str, edge_type: str,
                   properties: Optional[Dict[str, Any]] = None, weight: Optional[float] = None) -> Dict[str, Any]:
        """Create a graph edge"""
        if self.protocol == "grpc":
            return self._create_edge_grpc(edge_id, from_node_id, to_node_id, edge_type, properties, weight)
        else:
            return self._create_edge_rest(edge_id, from_node_id, to_node_id, edge_type, properties, weight)
    
    def _create_edge_grpc(self, edge_id: str, from_node_id: str, to_node_id: str, edge_type: str,
                         properties: Optional[Dict[str, Any]] = None, weight: Optional[float] = None) -> Dict[str, Any]:
    ...
    def _create_edge_rest(self, edge_id: str, from_node_id: str, to_node_id: str, edge_type: str,
                         properties: Optional[Dict[str, Any]] = None, weight: Optional[float] = None) -> Dict[str, Any]:
    ...
    def traverse_graph(self, start_node_id: str, max_depth: int = 3, 
                      edge_types: Optional[List[str]] = None, node_labels: Optional[List[str]] = None,
                      algorithm: str = "BFS", limit: Optional[int] = None) -> Dict[str, Any]:
        """Traverse graph from a starting node"""
        if self.protocol == "grpc":
            return self._traverse_graph_grpc(start_node_id, max_depth, edge_types, node_labels, algorithm, limit)
        else:
            return self._traverse_graph_rest(start_node_id, max_depth, edge_types, node_labels, algorithm, limit)
    
    def _traverse_graph_grpc(self, start_node_id: str, max_depth: int = 3,
                            edge_types: Optional[List[str]] = None, node_labels: Optional[List[str]] = None,
                            algorithm: str = "BFS", limit: Optional[int] = None) -> Dict[str, Any]:
    ...
    def _traverse_graph_rest(self, start_node_id: str, max_depth: int = 3,
                            edge_types: Optional[List[str]] = None, node_labels: Optional[List[str]] = None,
                            algorithm: str = "BFS", limit: Optional[int] = None) -> Dict[str, Any]:
    ...
    def query_nodes(self, labels: Optional[List[str]] = None, properties: Optional[Dict[str, Any]] = None,
    ...
    def _query_nodes_grpc(self, labels: Optional[List[str]] = None, properties: Optional[Dict[str, Any]] = None,
                         limit: Optional[int] = None, offset: Optional[int] = None) -> Dict[str, Any]:
        """Query nodes via gRPC"""
        filters = []
        if properties:
            for key, value in properties.items():
                filters.append(graph_pb2.PropertyFilter(
                    key=key,
                    operator=graph_pb2.PROPERTY_FILTER_OPERATOR_EQUALS,
                    value=self._convert_to_property_value(value)
                ))
        
        request = graph_pb2.NodeQuery(
            labels=labels or [],
            filters=filters
        )
    ...
    def _query_nodes_rest(self, labels: Optional[List[str]] = None, properties: Optional[Dict[str, Any]] = None,
                         limit: Optional[int] = None, offset: Optional[int] = None) -> Dict[str, Any]:
    ...
    def hybrid_search(self, collection_id: str, vector: List[float], top_k: int = 10,
                     start_node_id: Optional[str] = None, max_depth: int = 2,
        ...
        if self.protocol == "grpc":
            return self._hybrid_search_grpc(collection_id, vector, top_k, start_node_id, max_depth,
                                          combination_strategy, edge_types, vector_filters, limit)
        else:
            return self._hybrid_search_rest(collection_id, vector, top_k, start_node_id, max_depth,
                                          combination_strategy, edge_types, vector_filters, limit)
    
    def _hybrid_search_grpc(self, collection_id: str, vector: List[float], top_k: int = 10,
                           start_node_id: Optional[str] = None, max_depth: int = 2,
                           combination_strategy: str = "VECTOR_THEN_GRAPH",
                           edge_types: Optional[List[str]] = None,
                           vector_filters: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            return {
                ...
                "edges": [self._convert_edge_from_proto(edge) for edge in response.edges],
                "paths": [self._convert_path_from_proto(path) for path in response.paths],
                "vector_results": [self._convert_search_result_from_proto(result) for result in response.vector_results],
                "stats": {
    ...
    def _hybrid_search_rest(self, collection_id: str, vector: List[float], top_k: int = 10,
                           start_node_id: Optional[str] = None, max_depth: int = 2,
                           combination_strategy: str = "VECTOR_THEN_GRAPH",
                           edge_types: Optional[List[str]] = None,
                           vector_filters: Optional[Dict[str, Any]] = None,
    ...
    def advanced_vector_search(self, collection_id: str, vector: List[float], top_k: int = 10,
                              filters: Optional[Dict[str, Any]] = None,
                              include_vector: bool = False, include_metadata: bool = True,
                              accuracy_threshold: Optional[float] = None,
                              search_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Enhanced vector search with advanced parameters"""
        if self.protocol == "grpc":
            return self._advanced_vector_search_grpc(collection_id, vector, top_k, filters,
                                                   include_vector, include_metadata, accuracy_threshold, search_params)
        else:
            return self._advanced_vector_search_rest(collection_id, vector, top_k, filters,
                                                   include_vector, include_metadata, accuracy_threshold, search_params)
    
    def _advanced_vector_search_grpc(self, collection_id: str, vector: List[float], top_k: int = 10,
                                   filters: Optional[Dict[str, Any]] = None,
                                   include_vector: bool = False, include_metadata: bool = True,
                                   accuracy_threshold: Optional[float] = None,
                                   search_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _advanced_vector_search_rest(self, collection_id: str, vector: List[float], top_k: int = 10,
                                   filters: Optional[Dict[str, Any]] = None,
                                   include_vector: bool = False, include_metadata: bool = True,
                                   accuracy_threshold: Optional[float] = None,
                                   search_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _convert_search_result_from_proto(self, result) -> Dict[str, Any]:
        ...
        return {
            ...
            "score": result.score,
            "vector": list(result.vector) if result.vector else None,
            "metadata": {k: self._convert_from_sql_value(v) for k, v in result.metadata.items()},
