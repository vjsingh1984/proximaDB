...
class OpenAICompatibleProvider(EmbeddingProvider):
    ...
    def _initialize(self) -> None:
        ...
        try:
            ...
            self.api_base = self.config.extra_params.get("api_base") or \
                          os.getenv("OPENAI_API_BASE", "http://localhost:11434/v1")
            self.api_key = self.config.extra_params.get("api_key") or \
            ...
            logger.info(f"Initialized OpenAI-compatible provider with model: {self.config.model_name} "
                       f"at {self.api_base}")
    ...
    def _test_connection(self):
        ...
        if response.status_code != 200:
            raise RuntimeError(f"API test failed: {response.status_code} - {response.text}")
