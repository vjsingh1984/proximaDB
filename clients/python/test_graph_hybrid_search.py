...
def test_hybrid_search():
    ...
    try:
        ...
        vector_search_request = vector_types_pb2.VectorSearchRequest(
            collection_id=collection_id, queries=[search_query], top_k=10
        )
        ...
        print(
            f"✅ Hybrid search gRPC request: strategy={hybrid_request.combination_strategy}"
        )
    ...
    try:
        ...
        for client in [rest_client, grpc_client]:
            ...
            assert hasattr(
                client,
                (
                    "_advanced_vector_search_grpc"
                    if client.protocol == "grpc"
                    else "_advanced_vector_search_rest"
                ),
            )
            print(
                f"✅ Advanced vector search methods exist on {client.protocol} client"
            )
        ...
        rest_payload = {
            ...
            "filters": {"category": "electronics", "price": 100.0, "active": True},
        ...
        include_fields = vector_types_pb2.IncludeFields(
            vector=True, metadata=True, score=True
        )
        print(
            f"✅ Include fields: vector={include_fields.vector}, metadata={include_fields.metadata}"
        )
        ...
        print(
            f"✅ Search params: accuracy_threshold={search_params.accuracy_threshold}"
        )
        ...
        print(
            f"✅ Advanced vector search request: {request.collection_id}, top_k={request.top_k}"
        )
        ...
        for filter_name, filter_value in filter_test_cases:
            ...
            print(
                f"✅ Filter conversion: {filter_name} -> {type(converted_back).__name__}"
            )
...
def test_graph_traversal_algorithms():
    ...
    try:
        ...
        print(
            f"✅ Traversal request: start={request.start_node_id}, depth={request.max_depth}"
        )
        print(
            f"✅ Edge types: {len(request.edge_types)}, node labels: {len(request.node_labels)}"
        )
        ...
        node_query = graph_pb2.NodeQuery(
            labels=["Person", "Employee"], filters=[prop_filter], limit=50, offset=0
        )
        print(
            f"✅ Node query: {len(node_query.labels)} labels, {len(node_query.filters)} filters"
        )
...
def test_comprehensive_functionality():
    ...
    try:
        ...
        graph_methods = ["create_node", "create_edge", "traverse_graph", "query_nodes"]

        hybrid_methods = ["hybrid_search"]

        advanced_vector_methods = ["advanced_vector_search"]

        helper_methods = [
            "_convert_to_property_value",
            "_convert_from_property_value",
            "_convert_node_from_proto",
            "_convert_edge_from_proto",
            "_convert_search_result_from_proto",
        ]

        all_methods = (
            graph_methods + hybrid_methods + advanced_vector_methods + helper_methods
        )
        ...
        proto_modules = ["graph_pb2", "graph_pb2_grpc", "vector_types_pb2", "types_pb2"]

        from proximadb.v1 import graph_pb2, graph_pb2_grpc, vector_types_pb2, types_pb2
...
def main():
    ...
    if success:
        ...
        print("\n  🔀 HYBRID SEARCH:")
        print(
            "    - hybrid_search(collection, vector, start_node, combination_strategy)"
        )
        print(
            "    - Combination strategies: VECTOR_THEN_GRAPH, GRAPH_THEN_VECTOR, BALANCED"
        )
        print("    - Full integration of vector similarity and graph traversal")
        print("\n  🎯 ADVANCED VECTOR SEARCH:")
        print(
            "    - advanced_vector_search(collection, vector, filters, accuracy_threshold)"
        )
