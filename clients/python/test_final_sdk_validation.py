...
def test_proto_v1_message_generation():
    ...
    try:
        ...
        get_collection_req = collection_types_pb2.GetCollectionRequest(
            collection_id="test_collection"
        )
        logger.info(
            "✅ GetCollectionRequest proto: %s", get_collection_req.collection_id
        )
        ...
        sql_request = types_pb2.ExecuteSqlRequest(
            query="SELECT * FROM test_collection", parameters=[]
        )
...
def test_enum_mappings():
    ...
    try:
        ...
        if sdk_metric.value.upper() in proto_metrics:
            logger.info(
                f"✅ DistanceMetric mapping: {sdk_metric.value} -> {sdk_metric.value.upper()}"
            )
        else:
        ...
        if sdk_engine.value.upper() in proto_engines:
            logger.info(
                f"✅ StorageEngine mapping: {sdk_engine.value} -> {sdk_engine.value.upper()}"
            )
        else:
        ...
        if sdk_algo.value.upper() in proto_algos:
            logger.info(
                f"✅ IndexingAlgorithm mapping: {sdk_algo.value} -> {sdk_algo.value.upper()}"
            )
        else:
...
def test_full_workflow_simulation():
    ...
    try:
        ...
        proto_vectors = [
            vector_types_pb2.VectorRecord(id=vec.id, vector=vec.vector)
            for vec in vectors
        ]

        batch_request = vector_types_pb2.VectorBatchRequest(
            collection_id="test_collection", vectors=proto_vectors
        )
