...
def main():
    ...
    try:
        ...
        collection = grpc_client.create_collection(
            name="trace_test", dimension=4, distance_metric="cosine"
        )
        ...
        vector = VectorRecord(
            id="v1", vector=[1.0, 2.0, 3.0, 4.0], metadata={"test": True}
        )

        result = grpc_client.insert_vectors(
            collection_id="trace_test", vectors=[vector]  # Using NAME
        )
        ...
        try:
            vec = grpc_client.get_vector(
                collection_id="trace_test", vector_id="v1"  # Using NAME
            )
        ...
        try:
            vec = grpc_client.get_vector(
                collection_id=collection.id, vector_id="v1"  # Using ID
            )
        ...
        with httpx.Client() as client:
            response = client.get(
                f"http://localhost:5678/debug/vectors/{collection.id}"
            )
            if response.status_code == 200:
                data = response.json()
                print(f"   Unflushed vectors: {data.get('unflushed_vector_count', 0)}")
