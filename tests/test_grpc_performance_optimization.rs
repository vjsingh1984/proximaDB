...
use tonic::{transport::{Server, Channel, ClientTlsConfig}, Request, Response, Status, Code};
...
use proximadb::storage::{StorageEngine, UnifiedStorageEngine, CollectionConfig, StorageLayoutStrategy};
...
impl GrpcPerformanceTestHarness {
    ...
    pub async fn initialize_client_pool(&mut self, pool_size: usize, server_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn create_performance_collection(&self, collection_id: &str, dimension: u32) -> Result<(), Box<dyn std::error::Error>> {
        ...
        config.insert("filterable_metadata_fields".to_string(), serde_json::json!(["category", "source", "priority"]));
        
        let config_struct = prost_types::Struct {
            fields: config.into_iter().map(|(k, v)| {
                (k, Self::json_to_protobuf_value(v))
            }).collect(),
        };
        ...
        if !response.get_ref().success {
            return Err(format!("Failed to create collection: {}", response.get_ref().message).into());
        }
    ...
    pub fn generate_test_vectors(&self, count: usize, dimension: usize) -> Vec<InsertVectorData> {
        (0..count).map(|i| {
            ...
            metadata.insert("source".to_string(), serde_json::json!(format!("source_{}", i % 5)));
    ...
    fn json_to_protobuf_value(value: serde_json::Value) -> prost_types::Value {
        ...
        match value {
            ...
            _ => Value { kind: Some(Kind::StringValue(value.to_string())) },
        }
    }
}
...
async fn test_grpc_batch_insert_performance() {
    ...
    harness.initialize_client_pool(10, &server_url).await.unwrap();

    let collection_id = "batch_perf_test";
    harness.create_performance_collection(collection_id, 384).await.unwrap();
    ...
    for batch_size in batch_sizes {
        ...
        let request = Request::new(BatchInsertRequest {
            collection_identifier: Some(batch_insert_request::CollectionIdentifier::CollectionName(collection_id.to_string())),
        ...
        println!("✅ Batch size {}: {} vectors in {:?} ({:.2} ops/sec)", 
    ...
    assert!(results.iter().any(|r| r.throughput_ops_per_sec > 100.0), 
...
async fn test_grpc_concurrent_client_performance() {
    ...
    harness.initialize_client_pool(config.concurrent_clients, &server_url).await.unwrap();
    
    let collection_id = "concurrent_perf_test";
    harness.create_performance_collection(collection_id, config.vector_dimension as u32).await.unwrap();

    println!("🚀 Starting concurrent client test with {} clients", config.concurrent_clients);
    ...
    for client_id in 0..config.concurrent_clients {
        ...
        let task = tokio::spawn(async move {
            ...
            for op_id in 0..operations {
                ...
                let vectors: Vec<InsertVectorData> = (0..batch_size).map(|i| {
                    let vector: Vec<f32> = (0..dimension)
                ...
                let request = Request::new(BatchInsertRequest {
                    collection_identifier: Some(batch_insert_request::CollectionIdentifier::CollectionName(collection_name.clone())),
                    vectors,
                });
    ...
    for result in results {
        ...
        println!("📈 Client {}: {} operations, {} errors", 
                client_id, operation_times.len(), errors);
    }
    ...
    let p95_latency = all_latencies.get(p95_index).unwrap_or(&Duration::from_millis(0));
    let p99_latency = all_latencies.get(p99_index).unwrap_or(&Duration::from_millis(0));
    ...
    assert!(total_throughput > 50.0, "Should achieve >50 ops/sec with concurrent clients");
    assert!(error_rate < 0.05, "Error rate should be <5%");
    assert!(p95_latency.as_millis() < 1000, "P95 latency should be <1000ms");
...
async fn test_grpc_search_performance() {
    let mut harness = GrpcPerformanceTestHarness::new().await.unwrap();
    let server_url = harness.start_server().await.unwrap();
    harness.initialize_client_pool(5, &server_url).await.unwrap();

    let collection_id = "search_perf_test";
    let dimension = 384;
    harness.create_performance_collection(collection_id, dimension).await.unwrap();
    ...
    for (k, description) in search_scenarios {
        ...
        let request = Request::new(SearchRequest {
            collection_identifier: Some(search_request::CollectionIdentifier::CollectionName(collection_id.to_string())),
            vector: query_vector,
            k: k as u32,
...
async fn test_grpc_memory_efficiency() {
    let mut harness = GrpcPerformanceTestHarness::new().await.unwrap();
    let server_url = harness.start_server().await.unwrap();
    harness.initialize_client_pool(3, &server_url).await.unwrap();

    let collection_id = "memory_test";
    harness.create_performance_collection(collection_id, 128).await.unwrap();
...
async fn test_grpc_error_resilience() {
    let mut harness = GrpcPerformanceTestHarness::new().await.unwrap();
    let server_url = harness.start_server().await.unwrap();
    harness.initialize_client_pool(2, &server_url).await.unwrap();
    ...
    let invalid_request = Request::new(SearchRequest {
        collection_identifier: Some(search_request::CollectionIdentifier::CollectionName("nonexistent".to_string())),
    ...
    assert!(error_response.is_err(), "Should fail for nonexistent collection");
    ...
    let malformed_request = Request::new(BatchInsertRequest {
        collection_identifier: Some(batch_insert_request::CollectionIdentifier::CollectionName("test".to_string())),
...
async fn test_grpc_comprehensive_benchmark() {
    ...
    harness.initialize_client_pool(config.connection_pool_size, &server_url).await.unwrap();
    ...
    while Instant::now() < end_time {
        ...
        for i in 0..config.concurrent_clients.min(5) { // Limit concurrent tasks
            let server_url = server_url.clone();
            let collection_name = collection_id.to_string();
            let batch_size = config.batch_size;
            let dimension = config.vector_dimension;
            ...
            let task = tokio::spawn(async move {
                ...
                let vectors: Vec<InsertVectorData> = (0..batch_size).map(|j| {
                    let vector: Vec<f32> = (0..dimension)
                        .map(|_| rand::random::<f32>() * 2.0 - 1.0)
                        .collect();
                ...
                let request = Request::new(BatchInsertRequest {
                    collection_identifier: Some(batch_insert_request::CollectionIdentifier::CollectionName(collection_name)),
                    vectors,
                });
    ...
    assert!(actual_qps > 100.0, "gRPC should achieve >100 QPS in benchmark");
