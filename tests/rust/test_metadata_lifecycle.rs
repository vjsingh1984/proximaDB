...
mod metadata_lifecycle_tests {
    ...
    async fn test_metadata_as_is_storage() -> Result<()> {
        ...
        metadata.insert(
            "author".to_string(),
            serde_json::Value::String("Dr. Smith".to_string()),
        );
        metadata.insert(
            "year".to_string(),
            serde_json::Value::Number(serde_json::Number::from(2024)),
        );

        // Additional metadata fields (unlimited)
        metadata.insert(
            "title".to_string(),
            serde_json::Value::String("Advanced Vector Research".to_string()),
        );
        metadata.insert(
            "abstract".to_string(),
            serde_json::Value::String("This paper explores...".to_string()),
        );
        metadata.insert(
            "keywords".to_string(),
        ...
        metadata.insert(
            ...
            serde_json::Value::Number(serde_json::Number::from(1337)),
        );
        metadata.insert(
        ...
        metadata.insert(
            ...
            serde_json::Value::String("sha256:abc123def456...".to_string()),
        );
        ...
        println!(
            "✅ Metadata as-is storage test successful (fields: {})",
            metadata.len()
        );
        Ok(())
    }
    ...
    async fn test_metadata_transformation_logic() -> Result<()> {
        ...
        let original_metadata = {
            ...
            meta.insert(
                "abstract".to_string(),
                serde_json::Value::String("Abstract text...".to_string()),
            );
            meta.insert(
                "keywords".to_string(),
                serde_json::Value::Array(vec![
    ...
    async fn test_metadata_lifecycle_integration() -> Result<()> {
        ...
        let insert_metadata = {
            ...
            meta.insert(
                "title".to_string(),
                serde_json::Value::String("Vector Database Research".to_string()),
            );
            meta.insert(
                "abstract".to_string(),
                serde_json::Value::String("This research...".to_string()),
            );
            meta.insert(
                "keywords".to_string(),
                serde_json::Value::Array(vec![
        ...
        let (_, insert_measurement) = measure_performance!("Insert phase (as-is storage)", 1, {
        ...
        let (transformed_meta, flush_measurement) =
            measure_performance!("Flush phase (transformation)", 1, {
        ...
        let (_search_results, search_measurement) =
            measure_performance!("Search phase (optimized)", 1, {
