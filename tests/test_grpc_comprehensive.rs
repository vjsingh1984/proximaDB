...
async fn test_collection_management_comprehensive() {
    ...
    create_test_collection(&service, collection_id, 128).await.unwrap();
    ...
    let list_names_response = service.list_collection_names(list_names_request).await.unwrap();
    
    assert_eq!(list_names_response.get_ref().collection_names.len(), 1);
    assert_eq!(list_names_response.get_ref().collection_names[0], collection_name);
    ...
    let get_id_by_name_response = service.get_collection_id_by_name(get_id_by_name_request).await.unwrap();
    assert_eq!(get_id_by_name_response.get_ref().collection_id, collection_id);
    ...
    let get_name_by_id_response = service.get_collection_name_by_id(get_name_by_id_request).await.unwrap();
    assert_eq!(get_name_by_id_response.get_ref().collection_name, collection_name);
...
async fn test_vector_operations_comprehensive() {
    ...
    create_test_collection(&service, collection_id, 4).await.unwrap();
    
    // Test vector insertion
    let vector_id = insert_test_vector(&service, collection_id, client_id, test_vector.clone()).await.unwrap();
    ...
    let get_by_client_id_request = Request::new(GetVectorByClientIdRequest {
        collection_identifier: Some(get_vector_by_client_id_request::CollectionIdentifier::CollectionId(
            collection_id.to_string()
        )),
        client_id: client_id.to_string(),
    });
    
    let get_by_client_id_response = service.get_vector_by_client_id(get_by_client_id_request).await.unwrap();
...
async fn test_collection_deletion() {
    ...
    // Create collection
    create_test_collection(&service, collection_id, 64).await.unwrap();
...
async fn test_edge_cases() {
    ...
    create_test_collection(&service, collection_id, 1536).await.unwrap(); // OpenAI embedding size
    
    // Test with large vector
    let large_vector: Vec<f32> = (0..1536).map(|i| (i as f32) * 0.001).collect();
    let vector_id = insert_test_vector(&service, collection_id, "large_vector", large_vector.clone()).await.unwrap();
    ...
    let zero_vector_id = insert_test_vector(&service, collection_id, "zero_vector", zero_vector.clone()).await.unwrap();
...
async fn test_multiple_collections() {
    ...
    let list_ids_request = Request::new(ListCollectionIdsRequest {});
    let list_ids_response = service.list_collection_ids(list_ids_request).await.unwrap();
    assert_eq!(list_ids_response.get_ref().collection_ids.len(), collections.len());
    ...
    for (id, dim) in &collections {
        ...
        insert_test_vector(&service, id, &format!("vector_{}", id), test_vector).await.unwrap();
    }
