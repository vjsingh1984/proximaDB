...
mod tests {
    ...
    fn test_sql_in_operator_parsing() {
        let sql =
            "SELECT * FROM products WHERE metadata->>'brand' IN ('Apple', 'Samsung', 'Google')";
        let (parsed, plan) = parse_and_plan_sql(sql).unwrap();
                ...
                for (i, expr) in exprs.iter().enumerate() {
                    match expr {
                        FilterExpression::Comparison {
                            ...
                            assert_eq!(
                                value,
                                &serde_json::Value::String(expected_brands[i].to_string())
                            );
                        }
    ...
    fn test_sql_comparison_operators() {
        ...
        for (condition, expected_op) in test_cases {
            ...
            match filter.expression {
                ...
                _ => panic!(
                    "Expected comparison expression for condition: {}",
                    condition
                ),
            }
        }
    ...
    fn test_sql_like_operator_error() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("LIKE operator not yet implemented")
        );
    }
    ...
    fn test_sql_empty_in_clause_error() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("IN clause cannot be empty")
        );
    }
}
