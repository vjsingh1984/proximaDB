...
mod tests {
    ...
    use proximadb::query::sql_engine::{
        QueryPlanner, SqlExecutor, SqlParser,
        parser::{ComparisonOp, Condition, ParsedQuery, Value as SqlValue, WhereClause},
    ...
    impl SqlOperatorTestFixture {
        ...
        pub async fn new() -> Result<Self> {
            ...
            let viper_engine = Arc::new(
                proximadb::storage::engines::viper::ViperEngine::from_core_config(
            ...
            let sst_engine = Arc::new(
                proximadb::storage::engines::sst::SstStorage::new(
                    sst_config,
                    filesystem.clone(),
                    distance_compute,
                )
        ...
        pub async fn insert_test_vectors(&self) -> Result<()> {
            ...
            self.vector_service
                .insert_vectors_direct(&self.collection_id, Arc::new(vectors))
                .await?;

            // Force flush to ensure data is written to storage files
            self.vector_service
                .force_flush_collection(&self.collection_id)
                .await?;
    ...
    async fn test_sql_complex_logical_operators() {
        ...
        assert_eq!(
            count, 3,
            "Should find 3 items (Apple electronics OR Nike clothing)"
        );
    ...
    async fn test_sql_between_operator() {
        ...
        assert_eq!(
            count, 2,
            "Should find 2 items with price between 50 and 700"
        );
        assert!(ids.contains(&"vec_2".to_string())); // 599.99
        assert!(ids.contains(&"vec_3".to_string())); // 79.99
        ...
        assert_eq!(
            count, 4,
            "Should find 4 items with rating between 4.2 and 4.8"
        );
        assert!(ids.contains(&"vec_1".to_string())); // 4.8
    ...
    async fn test_sql_multiple_vector_scenarios() {
        ...
        for (i, query_vector) in query_vectors.iter().enumerate() {
            ...
            assert_eq!(
                count, 2,
                "Should find 2 electronics items for query vector {}",
                i
            );
            ...
            match i {
                0 => assert!(
                    ids[0] == "vec_1",
                    "vec_1 should be closest to first query vector"
                ),
                1 => assert!(
                    ids.contains(&"vec_2".to_string()),
                    "Should contain vec_2 for middle query"
                ),
                2 => assert!(
                    ids[0] == "vec_5",
                    "vec_5 should be closest to last query vector"
                ),
                _ => {}
            }
        }
    ...
    async fn test_sql_edge_cases() {
        ...
        assert_eq!(
            count, 0,
            "Should find 0 items with conflicting price conditions"
        );
