...
fn test_zstd_compression_effectiveness() {
    ...
    for (name, vector) in test_cases {
        ...
        if name.contains("sparse") {
            assert!(compression_ratio < 0.6, "Expected good compression for {} but got {:.3}", name, compression_ratio);
        } else if name == "sequential" {
            assert!(compression_ratio < 0.95, "Expected some compression for {} but got {:.3}", name, compression_ratio);
        }
    }
}
...
fn test_adaptive_compression() {
    ...
    assert!(config.compression_algorithm == CompressionAlgorithm::None || 
...
fn test_memory_pool_serialization() {
    ...
    let serialized = pool.serialize_vector_batch_pooled(&vectors, &config).unwrap();
    assert!(!serialized.is_empty());
    
    // Test pooled deserialization
    let deserialized = pool.deserialize_vector_batch_pooled(&serialized, &config).unwrap();
...
fn test_corrupted_data_handling() {
    ...
    assert!(result.unwrap_err().to_string().contains("too short for header"));
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Unknown serialization format"));
...
fn test_edge_cases() {
    ...
    match config.serialize_vector(&empty) {
        Ok(serialized) => {
            match config.deserialize_vector(&serialized) {
                Ok(deserialized) => {
...
mod streaming_compression_tests {
    ...
    fn test_streaming_compression_basic() {
        ...
        rt.block_on(async {
            ...
            let results = compressor.compress_stream(vectors.clone(), vector_config).await.unwrap();
    ...
    fn test_streaming_adaptive_buffer() {
        ...
        rt.block_on(async {
            ...
            for _ in 0..3 {
                let vectors = (0..50).map(|i| {
                    generate_test_vector(128, 0.5, "random")
                }).collect();
...
mod fixed_length_tests {
    ...
    use proximadb::core::serialization::fixed_length::{
        FixedVector, FixedLengthSerializer, Dim128, Dim512, Dim1024
    };
