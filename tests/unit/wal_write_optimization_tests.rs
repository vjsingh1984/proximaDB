...
use crate::storage::persistence::wal::{WalConfig, OptimizedWalWriter, OptimizedWalWriterConfig};
...
fn create_test_wal_config() -> WalConfig {
    WalConfig {
        multi_disk: crate::storage::persistence::wal::config::MultiDiskConfig {
            data_directories: vec!["file:///tmp/test_wal".to_string()],
            distribution_strategy: crate::storage::persistence::wal::config::DiskDistributionStrategy::RoundRobin,
...
mod batching_tests {
    ...
    async fn test_write_batching_by_size() -> Result<()> {
        // Setup
        let config = OptimizedWalWriterConfig {
            batch_size: 10,
        ...
        let writer = OptimizedWalWriter::new(config, wal_config, assignment_service).await?;
...
mod caching_tests {
    ...
    async fn test_assignment_caching() -> Result<()> {
        let config = OptimizedWalWriterConfig::default();
        let wal_config = create_test_wal_config();
        let assignment_service = Arc::new(RoundRobinAssignmentService::new());
