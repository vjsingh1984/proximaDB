...
fn test_optimized_schema_creation() {
    ...
    debug!(
        "✅ Optimized schema created with {} fields",
        schema.fields().len()
    );
}
...
fn test_binary_array_vector_serialization() {
    ...
    let schema = writer.create_optimized_schema().unwrap();
    let batch = writer
        .records_to_optimized_batch(&records, &schema)
        .unwrap();
    ...
    for (i, original_record) in records.iter().enumerate() {
        let extracted_vector = writer
            .extract_vector_from_binary_array(binary_array, i)
            .unwrap();

        // Verify vector values match exactly
        assert_eq!(extracted_vector.len(), original_record.vector.len());
        for (j, (&original, &extracted)) in original_record
            .vector
            .iter()
            .zip(extracted_vector.iter())
            .enumerate()
        {
            assert!(
                (original - extracted).abs() < f32::EPSILON,
                "Vector mismatch at record {} index {}: {} != {}",
                i,
                j,
                original,
                extracted
    ...
    debug!(
        "✅ BinaryArray serialization test passed with {:.3} compression ratio",
...
fn test_list_array_fallback_mode() {
    ...
    for (i, original_record) in records.iter().enumerate() {
        ...
        for (j, &original_value) in original_record.vector.iter().enumerate() {
            ...
            assert!(
                (original_value - extracted_value).abs() < f32::EPSILON,
...
fn test_compression_effectiveness() {
    ...
    let sparse_batch = sparse_writer
        .records_to_optimized_batch(&sparse_records, &schema)
        .unwrap();
    ...
    let dense_batch = dense_writer
        .records_to_optimized_batch(&dense_records, &schema)
        .unwrap();
    let dense_stats = dense_writer.get_optimization_stats(&dense_batch);

    debug!("📊 Compression Comparison:");
    debug!(
        "   Sparse vectors: {:.3} ratio",
        sparse_stats.compression_ratio
    );
    debug!(
        "   Dense vectors: {:.3} ratio",
        dense_stats.compression_ratio
    );

    // Sparse vectors should compress significantly better
    assert!(
        sparse_stats.compression_ratio < dense_stats.compression_ratio,
        "Sparse vectors should compress better than dense vectors"
    );
    assert!(
        sparse_stats.compression_ratio < 0.7,
...
fn test_metadata_serialization() {
    ...
    let schema = writer.create_optimized_schema().unwrap();
    let batch = writer
        .records_to_optimized_batch(&[record], &schema)
        .unwrap();
    ...
    if let Some(obj) = parsed.as_object() {
        ...
        if let Some(category_val) = obj.get("category").and_then(|v| v.as_str()) {
            assert!(category_val.len() > 0, "Category should have a value");
        }
        if let Some(confidence_val) = obj.get("confidence").and_then(|v| v.as_f64()) {
            assert!(confidence_val >= 0.0 && confidence_val <= 1.0, "Confidence should be valid");
        }
        if let Some(active_val) = obj.get("active").and_then(|v| v.as_bool()) {
...
fn test_performance_benchmark() {
    ...
    let records: Vec<VectorRecord> = (0..record_count)
        .map(|i| {
    ...
    let start = Instant::now();
    writer
        .write_batch_to_parquet(&mut parquet_writer, &batch)
        .unwrap();
    ...
    debug!(
        "   Batch creation: {:?} ({:.1} records/sec)",
        batch_time,
        record_count as f64 / batch_time.as_secs_f64()
    );
    debug!(
        "   Parquet write: {:?} ({:.1} records/sec)",
        write_time,
        record_count as f64 / write_time.as_secs_f64()
    );
    ...
    assert!(
        ...
        "Batch creation should be < 1 second"
    );
    assert!(
        write_time.as_millis() < 2000,
        "Parquet write should be < 2 seconds"
    );
...
fn test_empty_and_edge_cases() {
    ...
    let schema = writer.create_optimized_schema().unwrap();
    let batch = writer
        .records_to_optimized_batch(&[record_with_empty_vector], &schema)
        .unwrap();
...
fn test_dimension_consistency() {
    ...
    assert!(
        error_msg.contains("dimension mismatch") || error_msg.contains("dimension"),
    ...
    assert!(
        result.is_err(),
        "BinaryArray mode should also reject mixed dimensions"
    );
    ...
    assert!(
        error_msg.contains("dimension mismatch")
            ...
            || error_msg.contains("RecordBatch")
            || error_msg.contains("schema"),
        "BinaryArray error should be related to dimension/schema mismatch, got: {}",
        error_msg
    );
...
fn test_adaptive_vector_compression() {
    ...
    for (name, dimension, sparsity) in test_cases {
        ...
        debug!(
            "📈 Adaptive compression for {}: dimension={}, sparsity={:.2}, ratio={:.3}",
        ...
        if sparsity > 0.8 {
            assert!(
                ...
                "High sparsity should achieve good compression for {}",
                name
            );
        }
    }
