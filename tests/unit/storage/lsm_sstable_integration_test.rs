/// Integration test for LSM SSTable writer and reader
...
use proximadb::storage::engines::lsm::sstable_writer::SstableWriter;
use proximadb::storage::engines::lsm::readers::unified_sstable_reader::{
...
async fn test_sstable_write_read_integration() -> Result<()> {
    ...
    for (i, vec) in vectors.iter().enumerate() {
        let mut lsm_record = LsmRecord::from_vector_record(vec.clone(), "test_collection");
