//! Unit tests for VIPER clustering models
//!
//! Tests the ML clustering model functionality including:
//! - Model manager initialization and configuration
...
async fn test_model_training() {
    ...
    let collection_id = "training_test";
...
async fn test_clustering_statistics() {
    ...
    // Generate test vectors
    let vectors = generate_test_vectors(3000, 64);
    ...
    assert_eq!(stats.total_vectors, 3000);
    ...
    assert_eq!(total_clustered, 3000, "All vectors should be clustered");
}
...
#[test]
async fn test_training_priority() {
    // Test training priority ordering
    let priorities = vec![
    ...
    assert_eq!(TrainingPriority::Low as i32, 1);
    assert_eq!(TrainingPriority::Normal as i32, 2);
    assert_eq!(TrainingPriority::High as i32, 3);
    assert_eq!(TrainingPriority::Critical as i32, 4);
}
