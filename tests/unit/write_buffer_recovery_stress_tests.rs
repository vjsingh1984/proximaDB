        ...
        .map(|i| VectorRecord {
            ...
            metadata: vec![
                ...
                proximadb::proto::proximadb::MetadataItem {
                    ...
                    value: Some(proximadb::proto::proximadb::metadata_item::Value::StringValue(chrono::Utc::now().timestamp().to_string())),
                },
            ],
    ...
    async fn stress_test_concurrent_recovery() -> Result<()> {
        ...
        for (idx, temp_dir) in temp_dirs.iter().enumerate() {
            ...
            tasks.spawn(async move {
                create_test_wal_files(&wal_dir, &collection_id, 3).await
            });
        }
        ...
        assert_eq!(all_files.len(), 15, "Should have created 15 WAL files (5 collections * 3 files each)");
