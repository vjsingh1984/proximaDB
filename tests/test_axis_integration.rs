...
use proximadb::index::axis::{
    AxisIndexManager, AxisConfig, IndexStrategy, IndexType, 
    CollectionCharacteristics, MigrationDecision,
};
...
async fn test_axis_manager_creation() {
    ...
    assert!(axis_manager.is_ok(), "AXIS manager should be created successfully");
}
...
async fn test_collection_analysis_and_optimization() {
    ...
    let result = axis_manager.analyze_and_optimize(&collection_id.to_string()).await;
...
async fn test_mvcc_filtering_during_query() {
    ...
    use proximadb::index::axis::manager::{HybridQuery, VectorQuery, MetadataFilter, FilterOperator};
...
async fn test_migration_decision_making() {
    ...
    let result = axis_manager.analyze_and_optimize(&collection_id.to_string()).await;
    assert!(result.is_ok(), "Migration analysis should complete without errors");
}
...
async fn test_performance_metrics_collection() {
    ...
    assert!(metrics.total_vectors_indexed > 0, "Should track vector insertions");
    assert!(metrics.total_collections_managed > 0, "Should track collections");
}
...
async fn test_vector_deletion_with_soft_deletes() {
    ...
    for i in 0..50 {
        let result = axis_manager.delete(&collection_id.to_string(), Uuid::new_v4()).await;
...
async fn test_concurrent_operations() {
    ...
    for handle in handles {
        ...
        assert!(result.is_ok(), "Concurrent insertion task should complete successfully");
    }
    
    // Verify system stability after concurrent operations
    let metrics = axis_manager.get_metrics().await;
    assert!(metrics.total_vectors_indexed > 0, "Should have processed vectors from concurrent tasks");
}
...
async fn test_system_recovery_and_consistency() {
    ...
    for coll_id in 0..3 {
        ...
        assert!(result.is_ok(), "Recovery analysis should succeed for all collections");
    }
    
    // Verify system metrics remain consistent
    let metrics = axis_manager.get_metrics().await;
    assert_eq!(metrics.total_collections_managed, 3, "Should track all collections");
}