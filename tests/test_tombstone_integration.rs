...
async fn test_tombstone_functionality() {
    ...
    let deletion_result = storage_engine
        .soft_delete(&collection_id, &vector_id)
        .await
        .unwrap();
    assert!(
        deletion_result,
        "Deletion should return true for existing vector"
    );
    ...
    assert!(
        !deletion_result,
        "Deletion should return false for non-existing vector"
    );
...
async fn test_tombstone_persistence_after_flush() {
    ...
    for i in 0..5 {
        ...
        assert!(
            result.is_none(),
            "Deleted vector {} should not exist after flush",
            i
        );
    }
    ...
    for i in 5..10 {
        ...
        assert!(
            result.is_some(),
            "Non-deleted vector {} should still exist after flush",
            i
        );
    }
