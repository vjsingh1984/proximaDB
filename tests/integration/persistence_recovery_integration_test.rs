...
mod advanced_recovery_tests {
    ...
    // TODO: Update this test to use the correct ProximaDB graph API once the
    // high-level wrapper methods match the ORION engine API patterns.
    // The test currently uses non-existent methods like `create_graph_collection`
    // and types like `proximadb::graph::types::Node` which don't exist.
    // For now, graph durability is tested via:
    //   - tests/graph_persistence_service_test.rs (direct ORION engine test)
    //   - tests/wal_path_correctness_test.rs (WAL write verification)
    //
    // #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
