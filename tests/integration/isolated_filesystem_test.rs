...
async fn test_isolated_filesystem_basic_operations() -> Result<()> {
    ...
    // Write file
    filesystem.write(&test_file, test_content.as_bytes(), None).await?;
...
async fn test_isolated_filesystem_directory_operations() -> Result<()> {
    ...
    let nested_dir = format!("{}/subdir/nested/{}", env.temp_dir.path().display(), env.collection_id());
    ...
    filesystem.write(&nested_file, content.as_bytes(), None).await?;
...
async fn test_isolated_filesystem_concurrent_operations() -> Result<()> {
    ...
    for i in 0..concurrent_operations {
        ...
        let handle = tokio::spawn(async move {
            ...
            let content = format!("Concurrent content {} for collection: {}", i, collection_id_clone);
            
            // Write file
            filesystem.write(&file_path, content.as_bytes(), None).await?;
    ...
    assert_eq!(successful_operations, concurrent_operations,
        "All {} concurrent operations should succeed", concurrent_operations);
    
    // Verify all files exist using a fresh filesystem instance
    let final_filesystem = factory.get_filesystem(&storage_url)?;
    let final_files = final_filesystem.list(&format!("{}", env.temp_dir.path().display())).await?;
    let concurrent_files: Vec<_> = final_files.iter()
...
async fn test_isolated_filesystem_error_handling() -> Result<()> {
    ...
    // This should succeed
    filesystem.write(&valid_file, content.as_bytes(), None).await?;
...
async fn test_isolated_filesystem_large_file_operations() -> Result<()> {
    ...
    let chunk_data = format!("Collection {} chunk data: ", env.collection_id())
        .repeat(chunk_size / 50); // Repeat to fill chunk
...
async fn test_isolated_filesystem_multi_collection_isolation() -> Result<()> {
    ...
    for (i, env) in environments.iter().enumerate() {
        ...
        filesystem.write(&test_file, content.as_bytes(), None).await?;
    }
    ...
    for (i, env) in environments.iter().enumerate() {
        ...
        assert!(content_str.contains(env.collection_id()),
            "Content should contain collection ID for environment {}", i);
        assert!(content_str.contains(&format!("env {}", i)),
            "Content should contain environment number for environment {}", i);
        ...
        assert!(storage_url.contains(&own_dir),
