...
use proximadb::core::{VectorRecord, LsmConfig};
...
fn create_test_lsm_config(base_path: &str) -> LsmConfig {
    LsmConfig {
        memtable_size_mb: 1,
...
async fn test_lsm_search_with_flush() {
    ...
    let collection_id = "test_lsm_collection";
    // WAL writer creates nested structure: base/collection_id/wal/collection_id/logs/
    let wal_path = format!("{}/{}/wal/{}/logs", base_path, collection_id, collection_id);
    ...
    // Manually assign the collection to avoid assignment service issues
    // DirectVectorService uses its own WAL system, LSM is purely SSTable-based
    // IMPORTANT: Assignment service will append /{collection_id}/data to the base URL
    ...
    let lsm_engine = Arc::new(
        LsmTree::new(collection_id.to_string(), lsm_config.clone(), filesystem.clone())
            .await
            .unwrap()
    );
    ...
    println!("Phase 1: Inserting vectors to WAL + memtable");
    ...
    println!("Inserted {} vectors", result.entries_written);
    ...
    println!("Found {} unflushed vectors", unflushed_proto_vectors.len());
    ...
    println!("Converted {} vectors to core format", all_core_vectors.len());
    ...
    match lsm_engine.do_flush(&flush_params).await {
        Ok(result) => {
            println!("Direct LSM flush result: success={}, entries_flushed={}, files_created={}", 
                result.success, result.entries_flushed, result.files_created);
        }
        Err(e) => {
            println!("Direct LSM flush error: {:?}", e);
        }
    }
    ...
    println!("LSM data directory: {}", lsm_data_dir);
    ...
    println!("Collection storage URL from assignment: {}", storage_url);
    ...
    if let Ok(entries) = std::fs::read_dir(actual_storage_path) {
        let sst_files: Vec<_> = entries
            ...
            .collect();
        println!("Found {} SSTable files in {}", sst_files.len(), actual_storage_path);
        ...
        println!("Could not read directory: {}", actual_storage_path);
    }
    ...
    complex_filters.insert("category".to_string(), serde_json::Value::String("B".to_string()));
    complex_filters.insert("type".to_string(), serde_json::Value::String("secondary".to_string()));
    ...
    println!("✅ All LSM SSTable search tests passed!");
}
...
async fn test_lsm_compaction_and_search() {
    ...
    // Create directory structure for the collection
    let collection_id = "test_compaction";
    ...
    // Manually assign the collection
    let assignment_service = get_assignment_service();
    assignment_service.assign_collection(
        collection_id,
        &[proximadb::core::config::StorageLocation {
    ...
    println!("Creating multiple SSTables for compaction test");
    
    for batch_num in 0..4 {
        let mut batch = Vec::new();
        for i in 0..25 {
            let vector = VectorRecord {
                id: Some(format!("batch{}_vec{}", batch_num, i)),
                vector: vec![batch_num as f32, i as f32, 0.0],
                metadata: vec![
                    MetadataItem {
                        key: "batch".to_string(),
                        value: batch_num.to_string(),
                    }
                ],
                ..Default::default()
            };
    ...
    println!("Found {} vectors after compaction", results.len());
    assert!(results.len() >= 10, "Should find vectors after compaction");
    ...
    println!("✅ LSM compaction test passed!");
}
...
async fn test_lsm_bloom_filter_efficiency() {
    ...
    println!("Testing bloom filter efficiency with metadata");
        ...
        let vector = VectorRecord {
            ...
            metadata: vec![
                MetadataItem {
                    ...
                    value: "RARE_CATEGORY_XYZ".to_string(),
                },
                MetadataItem {
                    key: "type".to_string(),
                    value: format!("rare_type_{}", i),
                }
            ],
            ..Default::default()
        };
    ...
    // Force flush to create SSTable with bloom filters
    tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
    
    // Search for non-existent metadata (bloom filter should help skip blocks)
    let mut filters = HashMap::new();
    filters.insert("category".to_string(), serde_json::Value::String("NON_EXISTENT_CATEGORY".to_string()));
    ...
    println!("Search for non-existent metadata took {:?}", duration);
    assert!(results.is_empty(), "Should not find results for non-existent category");
    
    // Search for rare metadata
    filters.insert("category".to_string(), serde_json::Value::String("RARE_CATEGORY_XYZ".to_string()));
    ...
    println!("✅ LSM bloom filter test passed!");
}