...
async fn create_test_setup() -> (
    ...
    let raptor_engine = Arc::new(RaptorEngine::new(
...
async fn test_raptor_engine_creation_and_insertion() {
    ...
    let flush_params = proximadb::storage::traits::FlushParameters {
        ...
        force: true,
        synchronous: true,
        collection_config: Some(collection_service.get_collection_proto("raptor_test_collection").await.unwrap().unwrap()),
    };
    ...
    let vector = raptor_engine.vector_by_id("raptor_test_collection", "vec_10").await.unwrap();
