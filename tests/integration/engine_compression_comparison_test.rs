...
async fn test_engine_compression(
    ...
    match engine_type {
        "SST" => {
            ...
            let uncompressed_size = get_directory_size(
                env_uncompressed.get_sst_data_directory().as_path()
            ).await;
            ...
            let compressed_size = get_directory_size(
                env_compressed.get_sst_data_directory().as_path()
            ).await;
        ...
        "VIPER" => {
            ...
            let uncompressed_size = get_directory_size(
                env_uncompressed.get_viper_data_directory().as_path()
            ).await;
            ...
            let compressed_size = get_directory_size(
                env_compressed.get_viper_data_directory().as_path()
            ).await;
...
async fn test_engine_compression_comparison() -> Result<()> {
    ...
    for (data_type, vectors) in &test_configs {
        for engine in &["SST", "VIPER"] {
            for (algo, level) in &algorithms {
                info!("Testing {} engine with {} data using {}", engine, data_type, algo);
                
                match test_engine_compression(
                    engine,
                    data_type,
                    vectors.clone(),
                    algo,
    ...
    let sst_dense: Vec<_> = results.iter()
        .filter(|r| r.0 == "SST" && r.1 == "DENSE")
        .collect();
    let viper_dense: Vec<_> = results.iter()
    ...
    if !sst_dense.is_empty() && !viper_dense.is_empty() {
        ...
        info!("  SST best: {} with ratio {:.3} ({}% reduction)", 
            sst_best.2, sst_best.5, ((1.0 - sst_best.5) * 100.0) as i32);
        info!("  VIPER best: {} with ratio {:.3} ({}% reduction)",
            viper_best.2, viper_best.5, ((1.0 - viper_best.5) * 100.0) as i32);
    }
    
    // SST vs VIPER comparison for sparse data
    let sst_sparse: Vec<_> = results.iter()
        .filter(|r| r.0 == "SST" && r.1 == "SPARSE")
        .collect();
    let viper_sparse: Vec<_> = results.iter()
    ...
    if !sst_sparse.is_empty() && !viper_sparse.is_empty() {
        ...
        if viper_best.5 < sst_best.5 {
            info!("  🏆 VIPER achieves {}% better compression for sparse data",
                ((sst_best.5 / viper_best.5 - 1.0) * 100.0) as i32);
        } else {
            info!("  🏆 SST achieves {}% better compression for sparse data", 
