...
async fn test_binary_filtering_95_percent_reduction() -> Result<()> {
    ...
    for data in &quantized {
        assert!(data.filter.is_some(), "Binary sketch missing for vector {}", data.id);
    }
    ...
    println!("✅ Binary filtering achieved {:.1}% reduction", 
        binary_stage.metrics.reduction_percent);
...
async fn test_pq_distance_table_precomputation() -> Result<()> {
    ...
    let pq_stage = stages.iter()
        .find(|s| s.stage == SearchStage::PQRanking);
    ...
    if let Some(stage) = pq_stage {
        ...
        println!("✅ PQ ranking with distance table completed in {:.2}ms", time_ms);
    }
...
async fn test_progressive_search_pipeline() -> Result<()> {
    ...
    for stage in &stages {
        ...
        println!("   {:?}: {} -> {} candidates ({:.1}% reduction, {:.2}ms)",
            stage.stage,
    ...
    println!("✅ Progressive search achieved {:.1}% total reduction", total_reduction);
...
async fn test_quantization_with_various_dimensions() -> Result<()> {
    ...
    for dim in dimensions {
        ...
        println!("   ✓ Dimension {} - Compression: {:.1}%", dim, savings * 100.0);
...
async fn test_sst_integration_with_quantization() -> Result<()> {
    ...
    println!("✅ SST integration with quantization achieved {:.2}x compression",
        stats.compression_ratio);
