...
use proximadb::proto::proximadb::{StorageEngine, MetadataItem};
...
async fn test_sst_collection_with_proper_routing() -> anyhow::Result<()> {
    ...
    debug!("Created test environment with collection: {}", collection_id);
    ...
    debug!("✅ Successfully flushed {} vectors to SST, created {} files", 
    ...
    assert!(sst_file_count > 0, "Should have created at least one SST file");
    assert!(total_size > 0, "SST files should have non-zero size");
    
    debug!("✅ Found {} SST files with total size {} bytes", sst_file_count, total_size);
    ...
    let search_results = sst_engine.search_vectors_unified(
        collection_id,
        &storage_url,
        &query_vector,
        5,
    ...
    assert!(search_results[0].id.ends_with("_vec1"), 
