    ...
    let uncompressed_size = {
        ...
        match engine_type {
            "SST" => {
                ...
                let collection_uncompressed = env_uncompressed
                    .create_test_collection_with_settings(
                        StorageEngine::Sst,
                        dimension as i32,
...
async fn test_comprehensive_sparsity_compression_benchmark() -> Result<()> {
        ...
        for engine in &["SST", "VIPER"] {
            for (algo, levels) in &algorithm_configs {
                ...
                for level in levels {
                    ...
                    match benchmark_configuration(engine, vectors.clone(), *sparsity, algo, *level)
                        .await
                    {
                        Ok(result) => {
                            info!(
                                "    ✅ Ratio: {:.3}, Size: {}→{} bytes, Time: {}ms, Latency: {:.2}ms",
    ...
    for sparsity in &sparsity_levels {
        let sst_results: Vec<_> = results
            .iter()
            .filter(|r| r.engine == "SST" && r.sparsity == *sparsity)
            .collect();

        let viper_results: Vec<_> = results
            .iter()
    ...
    for sparsity in &sparsity_levels {
        ...
        if let Some(sst_best) = sst_results
            .iter()
            .min_by(|a, b| a.query_latency_ms.partial_cmp(&b.query_latency_ms).unwrap())
        {
            if let Some(viper_best) = viper_results
                .iter()
                .min_by(|a, b| a.query_latency_ms.partial_cmp(&b.query_latency_ms).unwrap())
            {
                ...
                info!(
                    "│ {:7}% │ {:6.2}ms ({:5}) │ {:6.2}ms ({:5}) │ {:8} │",
                    sparsity,
                    sst_best.query_latency_ms,
                    sst_best.algorithm,
                    viper_best.query_latency_ms,
                    viper_best.algorithm,
                    winner
                );
            }
    ...
    if let (Some(ds), Some(dv)) = (dense_sst, dense_viper) {
        if ds.compression_ratio < dv.compression_ratio {
            info!(
                "│ Dense ML Embeddings    │ SST      │ {} level {} ({:.1}% save) │",
                ds.algorithm,
                ds.level,
                (1.0 - ds.compression_ratio) * 100.0
            );
        } else {
            info!(
                "│ Dense ML Embeddings    │ VIPER    │ {} level {} ({:.1}% save) │",
                dv.algorithm,
                dv.level,
                (1.0 - dv.compression_ratio) * 100.0
            );
        }
    }

    // Sparse vectors (90% sparsity)
    let sparse_sst = results
        .iter()
    ...
    if let (Some(ss), Some(sv)) = (sparse_sst, sparse_viper) {
        if ss.compression_ratio < sv.compression_ratio {
            info!(
                "│ Sparse Vectors (90%)   │ SST      │ {} level {} ({:.1}% save) │",
                ss.algorithm,
                ss.level,
                (1.0 - ss.compression_ratio) * 100.0
            );
        } else {
            info!(
                "│ Sparse Vectors (90%)   │ VIPER    │ {} level {} ({:.1}% save) │",
                sv.algorithm,
                sv.level,
                (1.0 - sv.compression_ratio) * 100.0
            );
        }
    }
    ...
    if let Some(ll) = low_latency {
        info!(
            "│ Low Latency (<1ms)     │ {:8} │ {} ({:.2}ms query)      │",
            ll.engine, ll.algorithm, ll.query_latency_ms
        );
    }

    // Maximum compression
    let max_compression = results.iter().min_by(|a, b| {
        a.compression_ratio
            .partial_cmp(&b.compression_ratio)
            .unwrap()
    });
    ...
    if let Some(mc) = max_compression {
        info!(
            "│ Max Compression        │ {:8} │ {} L{} ({:.1}% save)      │",
            mc.engine,
            mc.algorithm,
            mc.level,
            (1.0 - mc.compression_ratio) * 100.0
        );
    }
