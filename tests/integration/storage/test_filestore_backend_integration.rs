...
async fn test_filestore_backend_direct_operations() -> Result<()> {
    ...
    debug!("   📝 Collection record created: {} (UUID: {})", record.name, record.uuid);
    ...
    debug!("   📄 Files after upsert: {} (delta: +{})", after_upsert_files, after_upsert_files - initial_files);
    ...
    match retrieved {
        Some(retrieved_record) => {
            debug!("   ✅ Collection retrieved: {} (UUID: {})", retrieved_record.name, retrieved_record.uuid);
    ...
    for col in &collections {
        debug!("      - {} (UUID: {}, dimension: {})", col.name, col.uuid, col.dimension);
    }
    ...
    debug!("   📄 Files after second upsert: {} (delta: +{})", after_second_files, after_second_files - after_upsert_files);
    ...
    debug!("   📄 Files after deletion: {} (delta: {})", after_delete_files, (after_delete_files as i32) - (after_second_files as i32));
...
async fn test_collection_service_with_filestore_backend() -> Result<()> {
    ...
    debug!("   📄 Files after creation: {} (delta: +{})", after_create_files, after_create_files - initial_files);
    ...
    match retrieved {
        Some(retrieved_record) => {
            debug!("   ✅ Collection retrieved via service: {}", retrieved_record.name);
    ...
    debug!("   ⏱️  Processing time: {}μs", delete_result.processing_time_us);
    
    // Check files after deletion
    let after_delete_files = count_files_recursive(&metadata_dir)?;
    debug!("   📄 Files after deletion: {} (delta: {})", after_delete_files, (after_delete_files as i32) - (after_create_files as i32));
...
async fn test_filestore_persistence_across_restarts() -> Result<()> {
    ...
    debug!("\n🔄 Phase 3: Second restart to verify deletion persistence...");
    {
        ...
        let collections = collection_service.list_collections().await?;
        debug!("   📋 Collections after second restart: {}", collections.len());
