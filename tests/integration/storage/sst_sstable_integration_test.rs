...
async fn test_sstable_write_read_integration() -> Result<()> {
    ...
    let writer = SstableWriter::new(&sst_path, 4096, filesystem.clone()).with_bloom_config(
        BloomFilterConfig {
    ...
    debug!(
        "Bloom filter test - vec1: {}, vec4 (shouldn't exist): {}",
        might_contain_vec1, might_contain_vec4
    );

    assert!(
        might_contain_vec1,
        "Bloom filter should indicate vec1 might exist"
    );
    ...
    debug!(
        "Filtered search (category=A) returned {} results",
    ...
    assert_eq!(
        filtered_results.len(),
        2,
        "Should find 2 results with category A"
    );
    assert!(
        filtered_results.iter().all(|r| r.id != "vec2"),
        "Should not include vec2"
    );
