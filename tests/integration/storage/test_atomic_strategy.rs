...
use proximadb::storage::persistence::filesystem::{
    FileOptions, FileSystem,
    atomic_strategy::{
        AtomicRetryConfig, AtomicWriteConfig, AtomicWriteExecutor, AtomicWriteExecutorFactory,
        AtomicWriteStrategy, AutoDetectExecutor, CloudOptimizedExecutor, DirectWriteExecutor,
...
async fn test_direct_write_executor_real_io() {
    ...
    executor
        .write_atomic(&fs, test_path, test_data, None)
        .await
...
async fn test_same_mount_temp_executor_real_io() {
    ...
    if temp_path.exists() {
        ...
        assert!(
            count <= 1,
            "Temp directory should be cleaned up after successful write"
        );
    }
...
async fn test_same_mount_temp_nested_directories() {
    ...
    fs.create_dir_all("level1/level2/level3")
        .await
    ...
    let read_data = fs
        .read(test_path)
        .await
        .expect("Should be able to read nested file");
    assert_eq!(read_data, test_data);
}
...
async fn test_cloud_optimized_compression() {
    ...
    let executor_no_compress = CloudOptimizedExecutor::new(
        ...
        false,
        8,
        AtomicWriteConfig::default(),
    );
    let not_compressed = executor_no_compress
        .compress_data(&test_data)
        .await
...
async fn test_auto_detect_executor_real_io() {
    ...
    executor
        .cleanup_temp_files(&fs)
        .await
        .expect("Auto detect cleanup should succeed");
}
...
async fn test_factory_convenience_methods() {
    ...
    // Test cloud executor
    let cloud_executor =
        AtomicWriteExecutorFactory::create_cloud_executor(PathBuf::from("/tmp/cloud_test"));
    assert_eq!(cloud_executor.strategy_name(), "cloud_optimized");
}
...
async fn test_large_file_atomic_write() {
    ...
    executor
        .write_atomic(&fs, test_path, &large_data, None)
        .await
    ...
    let read_data = fs
        .read(test_path)
        .await
        .expect("Should be able to read large file");
...
async fn test_error_handling_invalid_path() {
    ...
    let result = executor
        .write_atomic(&fs, invalid_path, test_data, None)
        .await;
...
async fn test_temp_path_generation() {
    ...
    let temp_path1 = executor
        .generate_temp_path(test_path)
        .expect("Should generate temp path");
    let temp_path2 = executor
        .generate_temp_path(test_path)
        .expect("Should generate temp path");
...
async fn test_file_options_integration() {
    ...
    executor
        .write_atomic(&fs, test_path, test_data, Some(options))
        .await
...
async fn test_concurrent_atomic_writes() {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            executor
                .write_atomic(&*fs_clone, &test_path, &test_data, None)
                .await
                .expect("Concurrent atomic write should succeed");

            // Verify the write
            let read_data = fs_clone
                .read(&test_path)
                .await
