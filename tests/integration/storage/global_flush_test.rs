...
fn create_sized_vector_records(
    collection_id: &str,
    count: usize,
...
async fn test_global_shrink_factor_behavior() -> Result<()> {
    ...
    for shrink_factor in shrink_factors {
        ...
        debug!(
            "Shrink factor {}: target_size={} bytes, reduction_needed={} bytes",
...
async fn test_global_flush_config_integration() -> Result<()> {
    ...
    for (collection_threshold, global_threshold, shrink_factor) in configs {
        ...
        assert_eq!(
            effective_config.memory_flush_size_bytes,
            collection_threshold
        );

        debug!(
            "Config test passed: collection_threshold={} bytes, global_threshold={} bytes, shrink_factor={}",
...
async fn test_global_flush_background_manager_integration() -> Result<()> {
    ...
    for (collection_id, memory_size, should_trigger) in test_scenarios {
        let triggered = manager
            .trigger_flush_if_needed(&collection_id.to_string(), memory_size)
            .await?;
        assert_eq!(
...
async fn test_global_flush_performance_metrics() -> Result<()> {
    ...
    assert!(
        populate_duration.as_millis() < 1000,
        "Population took too long: {}ms",
        populate_duration.as_millis()
    );
    assert!(
        flush_selection_duration.as_micros() < 10000,
        "Flush selection took too long: {}μs",
        flush_selection_duration.as_micros()
    );
    assert!(
        memory_calc_duration.as_micros() < 1000,
        "Memory calculation took too long: {}μs",
        memory_calc_duration.as_micros()
    );
