...
async fn test_url_validation_for_different_providers() -> Result<()> {
    ...
    for url in valid_urls {
        ...
        assert!(
            result.is_ok(),
            "URL validation failed for valid URL: {}",
            url
        );
        debug!("✅ Valid URL: {}", url);
    }
    ...
    for url in invalid_urls {
        ...
        assert!(
            result.is_err(),
            "URL validation should have failed for invalid URL: {}",
            url
        );
...
async fn test_wal_batch_strategy_url_routing() -> Result<()> {
    ...
    strategy
        .initialize(&config, filesystem_factory.clone())
        .await?;
    ...
    for cloud_url in invalid_urls {
        ...
        match health_result {
            Ok(is_healthy) => {
                ...
                debug!(
                    "✅ Invalid URL correctly marked as unhealthy: {}",
                    cloud_url
                );
            }
            Err(e) => {
...
async fn test_cloud_url_construction() -> Result<()> {
    ...
    for base_url in base_urls {
        ...
        assert!(
            ...
            "Constructed URL should be valid: {}",
            full_url
        );
...
async fn test_edge_cases_in_url_parsing() -> Result<()> {
    ...
    for (url, expected_path) in edge_cases {
        if let Ok(_) = filesystem_factory.validate_url(url) {
            ...
            assert_eq!(
                path, expected_path,
                "Path extraction failed for URL: {}",
                url
            );
