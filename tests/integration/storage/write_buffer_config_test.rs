...
use proximadb::storage::persistence::write_ahead_log::config::{
    CloudBackupConfig, CloudStorageType, CollectionWalConfig, CompressionAlgorithm,
    CompressionConfig, DiskDistributionStrategy, MemTableConfig, MemTableType, MultiDiskConfig,
    PerformanceConfig, RetentionPolicy, SyncMode, WriteBufferConfig, WriteBufferStrategyType,
};
...
mod wal_config_tests {
    ...
    fn test_cloud_backup_config() -> Result<()> {
        ...
        assert_eq!(
            backup_config.enable_compression,
            deserialized.enable_compression
        );
        assert_eq!(
            backup_config.backup_interval_secs,
            deserialized.backup_interval_secs
        );
    ...
    fn test_collection_wal_config() -> Result<()> {
        ...
        assert_eq!(
            collection_config.strategy_override,
            deserialized.strategy_override
        );
        assert_eq!(
            collection_config.flush_size_override,
            deserialized.flush_size_override
        );
    ...
    fn test_wal_config_comprehensive() -> Result<()> {
        ...
        assert_eq!(
            ...
            deserialized.memtable.memtable_type
        );
        assert_eq!(
            config.multi_disk.data_directories,
            deserialized.multi_disk.data_directories
        );
        ...
        assert_eq!(
            ...
            deserialized.enable_background_compaction
        );
        assert_eq!(
            config.collection_overrides.len(),
            deserialized.collection_overrides.len()
        );
    ...
    fn test_wal_config_defaults() {
        ...
        assert_eq!(config.strategy_type, WriteBufferStrategyType::AvroBatch);
    ...
    fn test_config_validation_properties() {
        ...
        assert!(
            config.performance.global_flush_threshold >= config.performance.memory_flush_size_bytes
        );
