...
use proximadb::storage::persistence::write_ahead_log::background_manager::{
    BackgroundMaintenanceManager, BackgroundTaskStatus,
};
use proximadb::storage::persistence::write_ahead_log::config::WALConfig;

/// Helper function to create test vector records
fn create_test_vector_records(
    collection_id: &str,
    count: usize,
...
async fn test_collection_flush_threshold_trigger() -> Result<()> {
    ...
    memtable
        .add_wal_batch("test_collection", small_batch)
        .await?;
    ...
    memtable
        .add_wal_batch("test_collection", large_batch)
        .await?;
...
async fn test_background_manager_flush_trigger() -> Result<()> {
    ...
    let should_flush = manager
        .trigger_flush_if_needed(&collection_id, small_memory_size)
        .await?;
    assert!(!should_flush);
    ...
    let should_flush = manager
        .trigger_flush_if_needed(&collection_id, large_memory_size)
        .await?;
    assert!(should_flush);
...
async fn test_collection_isolation_in_flush_decisions() -> Result<()> {
    ...
    let should_flush_1 = manager
        .trigger_flush_if_needed(&collection_1, large_memory_size)
        .await?;
    assert!(should_flush_1);
    ...
    let should_flush_2 = manager
        .trigger_flush_if_needed(&collection_2, large_memory_size)
        .await?;
...
async fn test_flush_threshold_edge_cases() -> Result<()> {
    ...
    let high_threshold_collections = memtable
        .collections_needing_flush(1024 * 1024 * 1024)
        .await?; // 1GB
