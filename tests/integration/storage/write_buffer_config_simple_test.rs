...
use proximadb::storage::persistence::write_ahead_log::config::{
    CollectionWalConfig, CompressionConfig, DiskDistributionStrategy, MemTableConfig, MemTableType,
    MultiDiskConfig, PerformanceConfig, SyncMode, WriteBufferConfig, WriteBufferStrategyType,
};
...
mod wal_config_tests {
    ...
    fn test_collection_wal_config() -> Result<()> {
        ...
        assert_eq!(
            collection_config.memory_flush_size_bytes,
            deserialized.memory_flush_size_bytes
        );
        assert_eq!(
            collection_config.disk_segment_size,
            deserialized.disk_segment_size
        );
        assert_eq!(
            collection_config.default_ttl_days,
            deserialized.default_ttl_days
        );
    ...
    fn test_collection_config_overrides() {
        ...
        let large_config = config
            ...
            .unwrap();
        assert_eq!(
            large_config.memory_flush_size_bytes,
            Some(100 * 1024 * 1024)
        );
