...
async fn create_test_setup() -> (
    ...
    let prism_engine = Arc::new(PrismEngine::new(
...
async fn test_prism_engine_creation_and_insertion() {
    ...
    let flush_params = proximadb::storage::traits::FlushParameters {
        ...
        force: true,
        synchronous: true,
        collection_config: Some(collection_service.get_collection_proto("prism_test_collection").await.unwrap().unwrap()),
    };
    ...
    let vector = prism_engine.vector_by_id("prism_test_collection", "vec_10").await.unwrap();
