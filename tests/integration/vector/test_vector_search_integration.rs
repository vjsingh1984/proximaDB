...
async fn test_vector_search_basic() {
    ...
    storage
        .create_collection("search_collection".to_string())
        .await
        .unwrap();
    ...
    let results = storage
        .search_vectors(&"search_collection".to_string(), vec![1.0, 0.0, 0.0], 3)
        .await
        .unwrap();
    ...
    assert!(
        exact_match.is_some(),
        "Should find exact match vector_x in results"
    );
    ...
    let results = storage
        .search_vectors(&"search_collection".to_string(), vec![0.0, 1.0, 0.0], 5)
        .await
        .unwrap();
    ...
    let stats = storage
        .get_index_stats(&"search_collection".to_string())
        .await
        .unwrap();
...
async fn test_vector_search_with_deletions() {
    ...
    storage
        .create_collection("delete_test".to_string())
        .await
        .unwrap();
    ...
    let results = storage
        .search_vectors(&"delete_test".to_string(), vec![5.0, 0.0, 0.0], 10)
        .await
        .unwrap();
    ...
    for i in 0..5 {
        let deleted = storage
            .soft_delete(&"delete_test".to_string(), &vector_ids[i])
            .await
            .unwrap();
        assert!(deleted);
    }
    ...
    let results = storage
        .search_vectors(&"delete_test".to_string(), vec![2.0, 0.0, 0.0], 10)
        .await
        .unwrap();
    ...
    let stats = storage
        .get_index_stats(&"delete_test".to_string())
        .await
        .unwrap()
        .unwrap();
    assert_eq!(stats["size"], serde_json::json!(5));
...
async fn test_vector_search_different_algorithms() {
    ...
    storage
        .create_collection_with_metadata(
            "brute_force_collection".to_string(),
            Some(brute_force_metadata),
        )
        .await
        .unwrap();
    ...
    // Test search
    let results = storage
        .search_vectors(&"brute_force_collection".to_string(), vec![2.0, 2.0], 3)
        .await
        .unwrap();
    ...
    storage
        .create_collection_with_metadata(
            "manhattan_collection".to_string(),
            Some(manhattan_metadata),
        )
        .await
        .unwrap();
    ...
    let results = storage
        .search_vectors(&"manhattan_collection".to_string(), vec![1.0, 0.0], 2)
        .await
        .unwrap();
...
async fn test_index_optimization() {
    ...
    storage
        .create_collection("optimization_test".to_string())
        .await
        .unwrap();
    ...
    let stats_before = storage
        .get_index_stats(&"optimization_test".to_string())
        .await
        .unwrap()
        .unwrap();
    ...
    // Optimize index
    storage
        .optimize_index(&"optimization_test".to_string())
        .await
        .unwrap();

    // Get stats after optimization (should be the same for this test)
    let stats_after = storage
        .get_index_stats(&"optimization_test".to_string())
        .await
        .unwrap()
        .unwrap();
    ...
    let results = storage
        .search_vectors(&"optimization_test".to_string(), vec![0.0, 1.0, 0.5], 10)
        .await
        .unwrap();
    assert_eq!(results.len(), 10);
