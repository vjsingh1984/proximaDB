...
async fn create_test_setup() -> (
    ...
    let nova_engine = Arc::new(NovaEngine::new(
...
async fn test_nova_engine_creation_and_insertion() {
    ...
    let flush_params = proximadb::storage::traits::FlushParameters {
        ...
        force: true,
        synchronous: true,
        collection_config: Some(collection_service.get_collection_proto("nova_test_collection").await.unwrap().unwrap()),
    };

    let flush_result = nova_engine.do_flush(&flush_params).await.unwrap();
    assert!(flush_result.success);
    assert_eq!(flush_result.entries_flushed, Some(100));

    let vector = nova_engine.vector_by_id("nova_test_collection", "vec_10").await.unwrap();
