...
async fn test_quantization_with_256kb_blocks() -> Result<()> {
    ...
    for block_size_kb in block_sizes {
        ...
        info!("\n  📈 Compression Metrics:");
        info!("    • Uncompressed size: {:.2} MB", uncompressed_size as f64 / (1024.0 * 1024.0));
...
async fn test_pq_quantization_256kb_blocks() -> Result<()> {
    ...
    info!("  • Original size: {:.2} MB", original_size as f64 / (1024.0 * 1024.0));
    ...
    info!("  • Vectors in last block: {}", num_vectors % vectors_per_block);
    ...
    info!("  • Actual compression: {:.1}x", original_size as f64 / flush_result.bytes_written as f64);
    info!("  • Files created: {}", flush_result.files_created);
