...
async fn test_lsm_compaction_and_search() {
    ...
    for batch_num in 0..4 {
        ...
        println!("Inserted batch {} with {} vectors", batch_num, batch.len());
        ...
        match lsm_engine.do_flush(&flush_params).await {
            Ok(result) => {
                println!("Batch {} LSM flush result: success={}, entries_flushed={}, files_created={}", 
                    batch_num, result.success, result.entries_flushed, result.files_created);
            }
            Err(e) => {
                println!("Batch {} LSM flush error: {:?}", batch_num, e);
            }
        }
    ...
    println!("Checking for SSTable files in: {}", storage_path);
    ...
    if let Ok(entries) = std::fs::read_dir(storage_path) {
        let sst_files: Vec<_> = entries
            ...
            .filter(|entry| entry.path().extension().map_or(false, |ext| ext == "sst"))
            .collect();
        println!("Found {} SSTable files before compaction:", sst_files.len());
        for file in &sst_files {
            println!("  - {} (size: {} bytes)", 
                file.file_name().to_string_lossy(),
                file.metadata().map(|m| m.len()).unwrap_or(0)
    ...
    println!("Waiting for potential automatic compaction...");
    tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
    ...
    if let Ok(entries) = std::fs::read_dir(&storage_path) {
        let sst_files: Vec<_> = entries
            ...
            .filter(|entry| entry.path().extension().map_or(false, |ext| ext == "sst"))
            .collect();
        println!("SSTable files after waiting: {}", sst_files.len());
    }
    ...
    if results.is_empty() {
        println!("WARNING: No vectors found! Debugging...");
        
        // Check directory state (manifest removed - using directory discovery)
        println!("  Directory-based discovery now used instead of manifest");
    } else {
        println!("First 5 results:");
        for (i, result) in results.iter().take(5).enumerate() {
            println!("  [{}] id: {}, distance: {:?}", i, result.id, result.distance);
        }
    }
...
async fn test_lsm_bloom_filter_efficiency() {
    ...
    let unflushed_proto_vectors = direct_service.debug_list_all_unflushed_vectors(collection_id).await.unwrap();
    println!("Found {} unflushed vectors in memtable", unflushed_proto_vectors.len());
    ...
    println!("Converted {} vectors to core format for flush", all_core_vectors.len());
    ...
    println!("LSM flush result: success={}, entries_flushed={}, files_created={}", 
    ...
    println!("Search without filters returned {} results", all_results.len());
    ...
    println!("Search with rare metadata filter returned {} results", results.len());
        ...
        if let Ok(entries) = std::fs::read_dir(storage_path) {
            for entry in entries.filter_map(|e| e.ok()) {
                if entry.path().extension().map_or(false, |ext| ext == "sst") {
                    ...
                    println!("SSTable file: {} ({} bytes)", entry.file_name().to_string_lossy(), file_size);
                }
            }
        }
