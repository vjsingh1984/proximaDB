...
impl RealTimeVectorIngestion {
    ...
    pub async fn start_ingestion(&mut self, duration: Duration) -> Result<StreamingMetrics, Box<dyn std::error::Error>> {
        println!("🔄 Starting real-time vector ingestion at {:.2} vectors/sec", self.ingestion_rate_per_sec);
        ...
        while Instant::now() < end_time {
            ...
            let vectors: Vec<InsertVectorData> = (0..self.batch_size).map(|i| {
            ...
            let request = Request::new(BatchInsertRequest {
                collection_identifier: Some(batch_insert_request::CollectionIdentifier::CollectionName(self.collection_name.clone())),
                vectors,
            });
...
impl ContinuousSearchQuery {
    ...
    pub async fn start_continuous_search(&mut self, duration: Duration) -> Result<StreamingMetrics, Box<dyn std::error::Error>> {
        println!("🔍 Starting continuous search queries at {:.2} queries/sec", self.query_rate_per_sec);
        ...
        let interval_duration = Duration::from_millis(
            (1000.0 / self.query_rate_per_sec) as u64
        );
        ...
        while Instant::now() < end_time {
            ...
            let request = Request::new(SearchRequest {
                collection_identifier: Some(search_request::CollectionIdentifier::CollectionName(self.collection_name.clone())),
                vector: query_vector,
                k: self.k,
        ...
        println!("   📊 Executed {} queries, found {} total results", queries_sent, total_results);
...
async fn test_grpc_realtime_vector_ingestion() {
    ...
    for (rate, batch_size, description) in ingestion_scenarios {
        println!("📊 {}: {:.1} vectors/sec in batches of {}", description, rate, batch_size);
        ...
        println!("   ✅ Ingested {} vectors at {:.2} vectors/sec", 
                metrics.messages_sent * batch_size, metrics.throughput_messages_per_sec);
        println!("   📈 Total bytes transferred: {} KB", metrics.bytes_transferred / 1024);
        println!("   ❌ Errors: {}", metrics.stream_errors);
        
        // Performance assertions for real-time ingestion
        assert!(metrics.stream_errors == 0, "Real-time ingestion should have no errors");
        assert!(metrics.throughput_messages_per_sec > rate * 0.8, 
...
async fn test_grpc_continuous_search_queries() {
    ...
    for batch_start in (0..population_size).step_by(population_batch_size) {
        ...
        let vectors: Vec<InsertVectorData> = (batch_start..batch_end).map(|i| {
        ...
        let request = Request::new(BatchInsertRequest {
            collection_identifier: Some(batch_insert_request::CollectionIdentifier::CollectionName(collection_name.to_string())),
            vectors,
        });
    ...
    for (query_rate, k, description) in search_scenarios {
        ...
        let metrics = search_query.start_continuous_search(test_duration).await.unwrap();
        
        println!("   ✅ Executed {} queries at {:.2} queries/sec", 
                metrics.messages_sent, metrics.throughput_messages_per_sec);
        println!("   ⏱️  Average latency: {:.2}ms", metrics.avg_message_latency_ms);
        println!("   ❌ Errors: {}", metrics.stream_errors);
        
        // Performance assertions for continuous search
        assert!(metrics.stream_errors == 0, "Continuous search should have no errors");
        assert!(metrics.throughput_messages_per_sec > query_rate * 0.7, 
               "Should achieve >70% of target query rate");
        assert!(metrics.avg_message_latency_ms < 200.0, 
...
async fn test_grpc_concurrent_ingestion_and_search() {
    ...
    let create_response = clients[0].clone().create_collection(create_request).await.unwrap();
    assert!(create_response.get_ref().success);

    // Add initial data for searches to find
    let initial_vectors: Vec<InsertVectorData> = (0..100).map(|i| {
        let vector: Vec<f32> = (0..dimension)
            .map(|_| rand::random::<f32>() * 2.0 - 1.0)
            .collect();
    ...
    let initial_response = clients[0].clone().batch_insert(initial_request).await.unwrap();
    ...
    println!("   Ingestion: {} vectors at {:.2} vectors/sec", 
            ingestion_metrics.messages_sent * 10, ingestion_metrics.throughput_messages_per_sec);
    println!("   Search: {} queries at {:.2} queries/sec, {:.2}ms avg latency", 
            search_metrics.messages_sent, search_metrics.throughput_messages_per_sec, 
            search_metrics.avg_message_latency_ms);
    
    // Both operations should maintain good performance concurrently
    assert!(ingestion_metrics.stream_errors == 0, "Concurrent ingestion should have no errors");
    assert!(search_metrics.stream_errors == 0, "Concurrent search should have no errors");
    assert!(ingestion_metrics.throughput_messages_per_sec > 20.0, 
           "Ingestion should maintain >20 vectors/sec under concurrent load");
    assert!(search_metrics.throughput_messages_per_sec > 10.0, 
           "Search should maintain >10 queries/sec under concurrent load");
    assert!(search_metrics.avg_message_latency_ms < 300.0, 
