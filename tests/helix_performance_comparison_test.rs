...
mod performance_comparison_tests {
    ...
    impl PerformanceMetrics {
        ...
        fn print_summary(&self) {
            ...
            println!("Flush: {:.2}ms ({:.0} vectors/sec)", 
                self.flush_time.as_millis(), 
                self.flush_throughput);
            
            if let Some(compact_time) = self.compaction_time {
                println!("Compaction: {:.2}ms ({:.2} MB/s)", 
                    compact_time.as_millis(),
                    self.compaction_throughput.unwrap_or(0.0));
            }
            
            println!("Query Latency:");
            println!("  Avg: {:.2}ms", self.avg_query_time.as_micros() as f64 / 1000.0);
            println!("  P50: {:.2}ms", self.p50_query_time.as_micros() as f64 / 1000.0);
            println!("  P95: {:.2}ms", self.p95_query_time.as_micros() as f64 / 1000.0);
            println!("  P99: {:.2}ms", self.p99_query_time.as_micros() as f64 / 1000.0);
    ...
    async fn test_scalability_comparison() {
        ...
        for &size in &sizes {
            ...
            // Test HELIX
            {
                let temp_dir = TempDir::new().unwrap();
                let config = HelixConfig::default();
                let engine = Arc::new(HelixEngine::new(
                    "test_collection".to_string(),
                    config,
        ...
        for i in 0..sizes.len() {
            ...
            println!("{:12} | {:14.2}ms | {:12.2}ms | {:.2}x",
                size,
    ...
    async fn test_concurrent_query_performance() {
        ...
        let query_vectors: Vec<Vec<f32>> = (0..50)
            .map(|i| vectors[i * 200].vector.clone())
            .collect();
        ...
        for concurrency in [1, 5, 10, 20] {
            ...
            println!("Concurrency {}: {:.2} QPS ({:.2}ms avg latency)",
                concurrency,
                qps,
