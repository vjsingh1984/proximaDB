...
mod tests {
    use super::*;
    use proximadb::core::{
        MetadataQuery, MetadataQueryEngine, MetadataQueryBuilder, FieldQuery, 
    ...
    fn test_logical_not_operator() {
        ...
        let query_fail = MetadataQuery::Not(Box::new(
            MetadataQuery::field_eq("category", json!("electronics"))
        ));
    ...
    fn test_comparison_operators() {
        ...
        let metadata = {
            ...
            meta.insert("description".to_string(), json!("High-performance gaming laptop with RGB lighting"));
            meta
        };
        
        // Numeric comparisons
        assert!(engine.evaluate(&MetadataQuery::Field(FieldQuery {
            field: "price".to_string(),
            operator: ComparisonOperator::GreaterThan,
            value: json!(50.0),
        }), &metadata).unwrap());
    ...
    fn test_array_operations() {
        ...
        let in_query = MetadataQuery::field_in("category", vec![json!("electronics"), json!("books")]);
