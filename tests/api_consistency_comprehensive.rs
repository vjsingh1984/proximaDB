...
use proximadb::proto::proximadb::{
    ...
    CollectionConfig, DistanceMetric, StorageEngine, CollectionOperation,
    MetadataItem, IncludeFields,
};
...
mod comprehensive_api_tests {
    ...
    async fn test_complete_workflow() {
        ...
        let base_url = std::env::var("REST_API_URL")
        ...
        assert!(batch_response.success, "Batch insert failed: {:?}", batch_response.error_message);
        assert_eq!(
        ...
        assert!(search_response.success, "Search failed: {:?}", search_response.error_message);
        ...
        let progressive_response: VectorOperationResponse = client
            .post(format!("{}/api/v1/progressive/search/{}", base_url, collection_id))
            .json(&search_request)
            .send()
            .await
        ...
        assert!(progressive_response.success, "Progressive search failed: {:?}", progressive_response.error_message);
        ...
        assert!(delete_response.success, "Collection deletion failed: {:?}", delete_response.error_message);
    ...
    async fn test_error_handling() {
        ...
        assert_eq!(response.status(), 400, "Expected BAD_REQUEST for empty collection ID");
        ...
        assert_eq!(response.status(), 400, "Expected BAD_REQUEST for empty queries");
        ...
        assert_eq!(response.status(), 404, "Expected NOT_FOUND for non-existent collection");
    ...
    async fn test_response_consistency() {
        ...
        assert!(health_json.get("status").is_some(), "Health response should have status field");
