...
impl OptimizedGrpcConnectionPool {
    ...
    pub async fn new(server_url: String, pool_size: usize) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for i in 0..pool_size {
            ...
            connection_metrics.insert(i, ConnectionMetrics {
    ...
    pub async fn update_metrics(&self, connection_id: usize, bytes_sent: usize, bytes_received: usize, error: bool) {
        let mut metrics = self.connection_metrics.write().await;
        if let Some(conn_metrics) = metrics.get_mut(&connection_id) {
    ...
    pub async fn get_aggregated_metrics(&self) -> NetworkOptimizationMetrics {
        ...
        let active_connections = metrics.values()
            .filter(|m| m.requests_sent > 0)
            .count();
...
async fn test_grpc_connection_pool_optimization() {
    ...
    for pool_size in pool_sizes {
        ...
        let pool = OptimizedGrpcConnectionPool::new(server_url.clone(), pool_size).await.unwrap();
            ...
            let task = tokio::spawn(async move {
                ...
                match client.batch_insert(request).await {
                    Ok(response) => {
                        ...
                        pool.update_metrics(conn_id, request_size, response_size, false).await;
                        (duration, true)
                    },
                    Err(_) => {
...
async fn test_grpc_http2_stream_optimization() {
    ...
    let create_response = client_clone.create_collection(create_request).await.unwrap();
    assert!(create_response.get_ref().success);
    ...
    println!("   Launching {} concurrent streams on single connection", concurrent_streams);
    ...
    for i in 0..concurrent_streams {
        ...
        let task = tokio::spawn(async move {
            ...
            let request = Request::new(BatchInsertRequest {
                collection_identifier: Some(batch_insert_request::CollectionIdentifier::CollectionName("stream_test".to_string())),
    ...
    let successful_streams = results.iter()
        .filter_map(|r| r.as_ref().ok())
        .filter(|(_, result, _)| result.is_ok())
        .count();
    
    let stream_latencies: Vec<Duration> = results.iter()
        .filter_map(|r| r.as_ref().ok())
    ...
    println!("   ⚡ {} streams completed in {:?}", successful_streams, total_duration);
    println!("   📊 Stream throughput: {:.2} streams/sec", stream_throughput);
    ...
    assert!(successful_streams >= concurrent_streams * 90 / 100, 
           "Should complete >90% of concurrent streams successfully");
    assert!(stream_throughput > 20.0, 
           "HTTP/2 multiplexing should enable >20 streams/sec");
    assert!(avg_stream_latency < 1000.0, 
...
async fn test_grpc_compression_optimization() {
    ...
    for (enable_compression, description) in compression_configs {
        ...
        let test_vectors: Vec<InsertVectorData> = (0..batch_size).map(|i| {
            ...
            let vector: Vec<f32> = if i % 2 == 0 {
                ...
                (0..large_vector_size).map(|_| rand::random::<f32>()).collect()
            };
        ...
        println!("     🎯 Simulated compressed size: {} KB (ratio: {:.2})", 
                compressed_size / 1024, simulated_compression_ratio);
        
        if enable_compression {
            assert!(simulated_compression_ratio < 0.8, 
...
async fn test_grpc_keepalive_optimization() {
    ...
    let endpoint = Endpoint::from_shared(server_url.to_string()).unwrap()
        .keep_alive_while_idle(true)
    ...
    println!("   📈 Expected connection reuse rate: {:.2}", connection_reuse_rate);
    
    // Keep-alive should maintain connection for reuse
    assert!(connection_reuse_rate > 0.8, 
