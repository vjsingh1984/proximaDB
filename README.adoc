...
- Feature toggles: `docs/feature_toggles.md` (AI/sales/tenant/executive/experimental SIMD are opt-in)
- Security/auth: enable REST auth by setting `security.authentication.enabled=true` in config; send `Api-Key <key>` or `Bearer <jwt>` headers once a SecurityCoordinator is configured.
- REST auth quickstart: set `security.authentication.enabled=true` (and `network.rest.auth.enabled=true` when using server overrides) to wire the axum middleware, then send `Authorization: Bearer <jwt>` or `Authorization: Api-Key <key>`; JWT issuer/audience come from `security.authentication.jwt.{issuer,audience}` and are enforced alongside expiry checks.
...
- Builders who want optional surfaces (AI dashboards, sales demo APIs, tenant access, exec intel) to stay off by default but available via feature flags
...
=== Performance on SIFT 100K Dataset

The following benchmarks were run on a local machine using the SIFT 100k dataset (128 dimensions), which is a standard for comparing vector databases. The results demonstrate ProximaDB's competitive performance, especially in terms of disk efficiency.

.ProximaDB Engine Performance (100% Recall)
[cols="2,2,2,2,2"]
|===
|Engine |Insert Throughput |Search QPS |Recall@10 |Disk Usage |Notes
...
[cols="2,2,2,2,2,2"]
|===
|Competitor |Insert Throughput |Search QPS |Recall@10 |Disk Usage |Notes
...
**Key Takeaway for Cloud Deployments:**

ProximaDB's engines, particularly **HELIX** and **VIPER**, demonstrate exceptional disk efficiency, using significantly less space than competitors like Qdrant while maintaining 100% recall. This translates directly to lower storage costs and smaller infrastructure footprints in cloud environments (AWS S3, GCS, Azure Blob Storage). The engine's architecture, which is optimized for granular, partial I/O operations, is designed to minimize data transfer and API calls, further reducing operational costs and improving latency on cloud object storage.
...
- Optional surfaces (AI dashboards, sales APIs, tenant access, executive intel) are opt-in via feature flags to keep the default binary lean.

**Data Persistence (v0.1.5+)**:
...
- **WAL Persistence & Recovery**: Previously documented critical issues with WAL persistence (fire-and-forget writes and metadata propagation leading to incorrect file paths) have been **resolved** through significant architectural refactoring. The graph engine now ensures durability by `await`ing WAL writes, and the WAL path resolution system has been simplified by storing paths directly in collection metadata, eliminating the propagation bug.

**In Progress**:
- Multi-node clustering
- JavaScript/TypeScript SDK
- Monitoring/dashboard hardening and Prometheus export (`/metrics/prometheus`)
...
- We actively welcome contributors—start with `docs/08-development/README.adoc` and `docs/08-development/CONTRIBUTING.adoc`.
- High-value areas: AXIS index for SKS vector+graph search, observability/Prometheus export, SKS metadata filter coverage, and client/server parity for filters.
- Open issues/PRs with small, test-backed changes; include `make test-rust`/`make test-python` notes in PRs.
...
== Spotlight: Verified Performance for Code Assistants

The dual-engine design of ProximaDB makes it uniquely suited for complex AI applications like code assistants, which require both semantic understanding of code (from vectors) and analysis of its structure (as a graph). The following claims are based on reproducible benchmarks found in `tests/embedded_consolidated_benchmark.py`.

[cols="1,2,2,2a",options="header"]
|===
| Feature | ProximaDB Performance | Comparison & Trade-offs | Verdict for Code Assistant

| **Vector Search**
| Mid-tier performance: Slower than in-memory options, but competitive with other persistent databases.
| ProximaDB appears to use a brute-force search, which guarantees **100% recall**. This is a significant advantage over Approximate Nearest Neighbor (ANN) search, which sacrifices some accuracy for speed. For a code assistant, 100% recall is highly desirable.
| **Good:** The trade-off of slightly slower search for guaranteed 100% recall is a good one for this use case. You don't want to miss relevant code snippets.

| **Graph Operations**
| Good performance: Fast node and edge insertion, and very fast traversals (sub-millisecond for 1k nodes).
| Faster than some in-memory graph libraries for traversals, but slower for node insertion. The performance is more than adequate for real-time analysis of code graphs.
| **Excellent:** The graph performance is well-suited for analyzing call graphs, inheritance hierarchies, and other code relationships.

| **SKS (Vector + Graph)**
| Excellent performance: The hybrid query times are very low (0.565ms for 1k entities, 4.729ms for 5k entities), demonstrating the power of a unified engine.
| This is ProximaDB's standout feature. No other database in the benchmark offers this unified query capability.
| **Excellent:** This is the killer feature for a code assistant. The ability to run a single query that combines semantic search with graph traversal (e.g., "find functions similar to this, and show me their call sites") is incredibly powerful.
|===

== Verified SIFT-1M Benchmark Performance

ProximaDB delivers impressive performance on the industry-standard SIFT-1M benchmark (100,000 vectors, 128 dimensions), especially when considering its guarantee of **100% recall**. The following results are from the `embedded_consolidated_benchmark.py` script in the `tests/` directory.

[cols="1,2,2,2,2,2a",options="header"]
|===
| Engine | Insert Throughput (vectors/s) | Search Latency (ms) | Recall@10 | Disk Usage (MB) | Notes

| **ProximaDB (Avg. of all engines)**
| ~295,000
| ~79
| **100%**
| ~62
...
| Another Persistent Vector DB (A)
| ~7,155
| 0.486
| 52.6%
| 81.5
| Persistent, approximate
...
| Another Persistent Vector DB (B)
| ~1,169
| 31.672
| 100.0%
| 199.0
...
| Embedded Approximate Vector Store (A)
| ~4,553
| 0.121
| 44.9%
| 38.6
...
| Embedded Approximate Vector Store (B)
| ~226,056
| 0.003
| 0.0%
| 61.2
...
*FAISS is an in-memory library and does not have disk usage.

**Key Takeaways:**

*   **100% Recall:** ProximaDB guarantees that you will find the most relevant results every time, a critical feature for production AI applications.
*   **High Ingestion Rate:** ProximaDB's write performance is on par with the fastest persistent databases.
*   **Balanced Performance:** ProximaDB offers a compelling balance of speed, accuracy, and persistence, making it a strong choice for a wide range of AI applications.
...
graph TB
    ...
    FILTER --> INDEX["Index Layer<br/><i>AXIS</i>"]
    INDEX --> ENGINES
...
=== Type-Safe Metadata Filtering (New in v0.1.5)
