...
[.lead]
Build semantic search, RAG stores, and relationship‑aware retrieval with:

- High‑throughput vector storage and type‑safe metadata filtering
- Columnar analytics for bulk operations and compression
- Optional graph traversal (BFS/DFS/shortest path) when relationships matter
...
**Real-World ROI**:
- **30% faster research workflows**: Find relevant papers via embeddings, then traverse citation networks for context
...
=== Working Example: Document Retrieval + Citation Traversal

Our integration test demonstrates SKS in action (see `clients/python/tests/integration/test_sks_semantic_graph_and_vector.py`):

[source,python]
----
# 1. Create vector collection for document embeddings
client.create_collection("papers", dimension=8)

# 2. Insert document embeddings (vector similarity)
vectors = [
    VectorRecord(id="doc_rag", vector=[0.90, 0.10, ...],
                 metadata={"title": "RAG Overview"}),
    VectorRecord(id="doc_transformer", vector=[0.88, 0.12, ...],
                 metadata={"title": "Transformer Architectures"}),
]
client.insert_vectors("papers", vectors)

# 3. Create graph collection for relationships
client.create_graph(graph_id="default", name="Citation Network")

# 4. Create nodes and edges (graph relationships)
client.create_node(node_id="doc_rag", labels=["Document"], ...)
client.create_edge(edge_id="e1", from_node_id="doc_rag",
                   to_node_id="doc_transformer", edge_type="CITES")

# 5. Hybrid query: Vector search + Graph traversal
# Find most similar document by semantics
results = client.search("papers", query_vector, top_k=1)
top_doc = results[0].id  # "doc_rag"

# Traverse citation network from that document
related = client.traverse_graph(start_node_id=top_doc, max_depth=2,
                                edge_types=["CITES"])
# Returns: doc_rag → doc_transformer → [related papers]
----
