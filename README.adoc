...
- **Entity store durability**: Data does NOT persist across server restarts (WAL infrastructure exists but disabled by default)
- **Graph persistence**: Collection metadata persists, but graph nodes/edges are volatile (in-memory only)
- Workaround: Re-insert data on server startup until persistence is enabled in v0.2.0+

**In Progress**:
- Multi-node clustering
- JavaScript/TypeScript SDK
- Monitoring dashboard
- Persistent entity store and graph (v0.2.0)
