...
- **Known issue**: WAL pool metadata propagation bug can direct WAL files to incorrect paths, impacting recovery until fixed (see `URGENT_FIX_INSTRUCTIONS.md` / `FINAL_FIX_NEEDED.md`); validate with `RECOVERY_TEST_FRESH.sh` after applying the fix.
