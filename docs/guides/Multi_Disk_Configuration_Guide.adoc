...
[source,toml]
----
# Local high-performance WriteBuffer with cloud storage
[storage.wal_config]
wal_urls = [
    "file:///mnt/nvme1/wal",     # Local NVMe for performance
    "file:///mnt/nvme2/wal",     # Local NVMe for performance
    "s3://backup-wal/cluster1"   # Cloud backup WriteBuffer
]
distribution_strategy = "PerformanceBased"
...
=== WriteBuffer Configuration Tuning
...
capacity_config = { 
    max_wal_size_mb = 20480,           # 20GB WriteBuffer per collection
...
disk_type = { NetworkStorage = { latency_ms = 5.0 } }
capacity_config = {
    max_wal_size_mb = 1024,            # 1GB WriteBuffer per collection
...
# Monitor WriteBuffer flush operations
...
==== WriteBuffer URL Conversion Errors
[source,bash]
----
