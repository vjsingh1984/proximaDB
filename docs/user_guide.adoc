...
**Storage-Aware Intelligence**:: Automatic WAL → VIPER → LSM engine selection based on data location

**Two-Stage Search**:: Binary quantization for candidate selection + FP32 refinement for accuracy

**Multi-Engine Architecture**:: VIPER (Parquet columnar) and LSM (row-based SSTable) storage engines
...
collection_config = {
    ...
    "storage_engine": "auto",  # Auto-select VIPER or LSM
    "quantization_config": {
