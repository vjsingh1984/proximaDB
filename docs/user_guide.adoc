...
Learn how to use ProximaDB to create collections, manage vector data, perform similarity searches, and integrate with AI frameworks like BERT, OpenAI, and Cohere.

== Overview

ProximaDB is a production-ready vector database with REST (port 5678) and gRPC (port 5679) APIs, supporting:

- **Vector Operations**: Insert, search, update, delete with metadata
- **AI Integration**: Native BERT support (384, 768, 1024 dimensions)  
- **Storage**: Multi-cloud with automatic flushing and compression
- **Performance**: 6.10x faster search with ML-driven clustering
...
----
git clone https://github.com/vjsingh1984/proximadb.git
cd proximadb && cargo run --bin proximadb-server
----
