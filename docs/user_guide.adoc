...
=== Step 1: Create Your First Proto-Native Collection
...
Collections in ProximaDB use proto-native metadata with automatic storage engine selection and quantization support.

**Using Unified Python SDK:**
[source,python]
----
from proximadb import UnifiedClient

# Unified client supports both REST and gRPC
...
**Using REST API (with Proto JSON mapping):**
[source,bash]
----
curl -X POST http://localhost:5678/collections \
  ...
  -d '{
    ...
    "description": "Proto-first document embeddings"
  }'
----

**Using gRPC (Native Proto):**
[source,bash]
----
# gRPC call with native protobuf (64MB message support)
grpcurl -plaintext -d '{
  "collection": {
    "collection_id": "ai_documents",
    "dimension": 768,
...
}' localhost:5679 proximadb.VectorService/CreateCollection
----
...
- **Handler Consolidation**: 85% code reduction eliminating REST/gRPC duplication
- **Storage-Aware Search**: Automatic engine selection with optimized performance improvement
- **Hardware Optimization**: SIMD/GPU acceleration with intelligent quantization
...
**Issue: Empty Vector IDs**
```
Vector IDs are empty or null after insertion
```
*Solution:*
- Ensure you're using the latest version of ProximaDB
- The DirectVectorService now properly generates IDs when not provided
- For existing vectors with empty IDs, see migration guide in link:technical_reference.adoc#vector-id-generation-fix[Technical Reference]
- Client-provided IDs are always preferred over auto-generated ones
...
=== SDK Consolidation Benefits

The ProximaDB Python SDK has been consolidated from 7 separate client implementations down to a single unified client, achieving:
...
client = ProximaDBClient(url="localhost", protocol=Protocol.GRPC)  # For max performance
client = ProximaDBClient(url="localhost", protocol=Protocol.REST)  # For compatibility
----
...
2. **Enhanced Connection Management**: HTTP/2, connection pooling, keepalive
3. **Better Error Handling**: Unified error types across protocols
4. **TLS/mTLS Support**: Client certificates for secure connections
5. **Storage-Aware Search**: optimized performance improvement with optimizations
