= ProximaDB Requirements Specification: Complete System Requirements
:doctype: book
:toc: left
:toclevels: 3
:sectlinks:
...
[.lead]
*ProximaDB Requirements Specification* defines comprehensive system requirements for the cloud-native vector database engineered for AI applications. This document tracks implementation status and provides detailed functional and non-functional requirements.

image::Production_Readiness_Status.png[Production Readiness Status,width=100%]

[NOTE]
====
🔗 **Related Documents**: This specification connects to link:../README.adoc[Project Overview], link:hld.adoc[High-Level Design], and link:lld.adoc[Low-Level Design]
====

=== Requirements Implementation Dashboard
...
|📊 **Vector Operations**
|✅ Core Complete, Advanced Partial
|85%
...
**Tagline**: _Proximity at Scale_ - Enterprise-grade vector database with unified memtable architecture and intelligent storage-aware search.

== 🏗️ Architecture Overview

The ProximaDB architecture is built on several key innovations documented through comprehensive UML analysis:

image::uml/images/ProximaDB_System_Overview.png[System Overview,width=100%]

=== Unified Memtable Architecture

The system uses a single `MemtableManager` interface with pluggable backends:

* **Global Partitioned Memtable**: Core data structure with Arc-based memory optimization
* **WAL Behavior Wrapper**: Handles write-ahead logging with configurable sync modes
* **LSM Behavior Wrapper**: Manages log-structured merge operations
* **VIPER Engine**: Pure WAL delegation for Parquet optimization

image::uml/images/ProximaDB_Core_Classes.png[Core Classes,width=100%]

=== Storage-Aware Polymorphic Search

The search system automatically selects optimal engines based on data location:

* **WAL Search Engine**: Zero-copy access to unflushed data
* **VIPER Search Engine**: ML-optimized clustering for Parquet files
* **LSM Search Engine**: Bloom filter optimization for SSTables
* **Multi-Tier Deduplication**: ID-based result merging with tier priorities

==== Vector Search Flow

image::uml/images/ProximaDB_Vector_Search_Sequence.png[Vector Search Sequence,width=100%]

**Performance Achievement**: 6.10x improvement through intelligent multi-tier search with 100% accuracy guarantee.
...
|**Vector Search** |✅ Complete |**Multi-tier unified search with deduplication, VectorService**
...
|Critical
|✅ IMPLEMENTED (ML clustering with K-means++, 20% growth threshold)
...
**Focus**: Storage-aware compaction, IndexConfig integration, AI inference and distributed capabilities
**Dependencies**: Phase 2 complete

- 🚧 **Arrow/Parquet Compaction**: Dynamic schema generation, version merging, expiry logic
- ✅ **IndexConfig Integration**: Per-collection index configuration with smart defaults
...
**Implementation Location**: `src/services/vector_service.rs`
**Effort**: 4-5 days
...
== Phase 3 Implementation Status Update (January 2025)

=== 🚀 Major Architectural Breakthrough: Storage-Aware Compaction

**Status**: 🔄 **IN PROGRESS - Major Components Implemented**
**Updated Phase 3 Completion**: **40% Complete** (up from 5%)

==== ✅ **COMPLETED: Arrow/Parquet Compaction Architecture**

**Implementation Location**: `src/storage/persistence/wal/background_manager.rs` → `src/storage/engines/viper/core.rs`

**Key Architectural Decisions**:

1. **🏗️ Proper Separation of Concerns**: 
   - ❌ **Before**: Background manager handled Parquet-specific schema generation
   - ✅ **After**: VIPER engine handles all storage-specific operations
   - **Rationale**: Compaction is storage-aware and should be handled by the engine that understands the data format

2. **📊 Dynamic Schema Generation**:
   - Native Parquet `array<float32>` for vectors (not binary serialization)
   - Dynamic quantized vector columns (PQ/SQ/Binary) based on collection settings
   - Per-collection filterable metadata as separate Parquet columns
   - Automatic schema evolution based on collection configuration changes

3. **🔄 Version Merging & Expiry Logic**:
   - Proper Arrow-based Parquet file reading and writing
   - Latest version per ID retention (optimistic concurrency control)
   - Expired record filtering based on TTL timestamps
   - Immutable vector support (null/empty IDs bypass version merging)

4. **⚡ Performance Optimizations**:
   - Batch processing with 1000-record batches for optimal Parquet performance
   - Schema caching to avoid repeated collection service calls
   - Sequential execution: FLUSH → COMPACTION → INDEXING

==== 🎯 **COMPLETED: IndexConfig Integration**

**Implementation Locations**: 
- `src/index/config.rs` - Configuration types and smart defaults
- `src/index/axis/manager.rs` - AXIS integration
- `src/services/collection_service.rs` - Persistent storage
- `src/network/rest/handlers.rs` - API endpoints

**Features Implemented**:
- ✅ Configurable per-collection index update modes (sync/async/hybrid)
- ✅ Algorithm-specific parameters (HNSW, IVF) with smart defaults
- ✅ Collection service integration for durable configuration storage
- ✅ REST API support with comprehensive validation
- ✅ AXIS manager integration for proper index building

==== 🔧 **COMPLETED: Protocol & Infrastructure**

**Fixed Issues**:
- ✅ Protobuf enum conflicts (`HYBRID` → `HYBRID_MODE`)
- ✅ Arrow/Parquet compilation dependencies
- ✅ Collection service integration patterns
- ✅ Sequential flush-compaction-indexing pipeline

==== 📋 **IMMEDIATE NEXT STEPS**

1. **🔗 Complete VIPER Compaction Implementation**:
   - Move Arrow/Parquet compaction logic from background manager to VIPER
   - Implement collection service dependency injection
   - Add schema generation caching in VIPER

2. **🚀 Storage Engine Interface Completion**:
   - Implement `get_vector_by_id` for both LSM and VIPER
   - Ensure VectorRecord batch handling consistency
   - Complete expired record physical deletion

3. **📊 Collection Service Integration**:
   - Replace simulation with actual collection service calls
   - Implement real-time schema generation during flush/compaction
   - Add collection configuration change detection

==== 🎯 **ARCHITECTURAL INSIGHTS**

**Design Decision**: **Storage-Aware Compaction Belongs in Storage Engines**

- **Problem**: Background manager was handling Parquet-specific schema generation
- **Solution**: Move compaction logic to VIPER where it belongs
- **Benefits**: 
  - Proper separation of concerns (orchestration vs. storage-specific logic)
  - Better schema caching opportunities (same thread for flush/compaction)
  - Direct access to collection service for real-time configuration
  - Engine-specific optimizations (Arrow/Parquet for VIPER, different for LSM)

**Future State**: Each storage engine implements its own compaction strategy:
- **VIPER**: Arrow/Parquet compaction with dynamic schema generation
- **LSM**: SSTable merging with engine-optimized serialization
- **Background Manager**: Pure orchestration layer delegating to engines
