...
the edge into one policy plane and names the deployment seam. The crate/type names
(`proximadb-edge`, `EdgeRequest`, `EdgePolicyPlane`) remain proposals, but the *E0→E1 slice has
shipped* (2026-06-21): E0 routes pgwire through the same policy stages as REST/gRPC, and the unifying
edge types `EdgePolicyContext` (one client-IP → `KouLocality` classification) + `ResultShapePolicy`
(the locality-driven shaping stage) are live in `src/metrics/consumption_metrics.rs`, consumed across
pgwire/REST/Arrow Flight for result-egress metering + lossless shaping. See §8.
====
...
| *E0 — close the consistency gap* [*SHIPPED 2026-06-20/21*] | pgwire now routed through the same
policy stages as REST/gRPC (tenant, authn/authz, per-IP rate-limit, trace); client-IP carried on the
pgwire session. | `network/multiplex`, `network/middleware`, pgwire server
| *E1 — canonical envelope + one plane* [*PARTIAL 2026-06-21*] | Shipped: `EdgePolicyContext` (one
client → `KouLocality` seam) + `ResultShapePolicy` (shaping stage), consumed by pgwire/REST/Flight
for result-egress metering + lossless ZSTD/gzip shaping. Remaining: the full `EdgeRequest` +
`EdgePolicyPlane` envelope running every stage once over all four surfaces. | `proximadb-edge` (new)
