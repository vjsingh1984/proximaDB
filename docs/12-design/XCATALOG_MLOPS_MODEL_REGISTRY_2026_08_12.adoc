...
Status: *core contract, NativeCatalog persistence, and fail-closed collection-binding resolution
implemented; public lifecycle API and MLflow REST compatibility are rollout slices*. The shape is
additive and legacy-read-safe.
...
. *Implemented*: typed contracts, validation, collection pins, stable hash, command mutations,
  optimistic concurrency, NativeCatalog persistence, SDK payload adapter, and one fail-closed
  binding-resolution query over digest/dimension/decision/runtime/name.
. Add generated REST/gRPC lifecycle handlers and regenerate SDKs from OpenAPI/proto. Handlers lower
  to catalog commands and the shared resolution query; no handwritten Python transport.
