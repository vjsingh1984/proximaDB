...
`scripts/check_workspace_boundaries.py --storage-up-edges`. Current checkout count is 273
remaining `src/storage` up-edges (`compute` 239, `index` 14, `query` 10, `services` 10), and
`scripts/check-layering.sh` enforces `--max-storage-up-edges 273` in CI. Use this count as the
