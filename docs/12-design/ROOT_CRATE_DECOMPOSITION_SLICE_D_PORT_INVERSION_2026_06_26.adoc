...
`scripts/check_workspace_boundaries.py --storage-up-edges`. Current checkout count is 271
remaining `src/storage` up-edges (`compute` 239, `index` 12, `query` 10, `services` 10), and
`scripts/check-layering.sh` enforces `--max-storage-up-edges 271` in CI. Use this count as the
...
The 275 → 273 reduction added `StorageEventLogPort` to `proximadb-storage-ports`, rewired
`storage::engines::event_log_integration` to depend on that port, and implemented the port on
`services::events::log::EventLogService`. Storage now emits storage-native event facts while the
service layer performs the AXIS `IndexEvent` translation.

The 273 → 271 reduction retyped `storage::common::compaction_orchestrator::CompactionCoordinator`
from AXIS `EventLogService` to `StorageEventLogPort`. The coordinator was already injection-based,
so this removes a storage→index type dependency without changing service discovery or compaction
readiness behavior.
...
| D2 | **Define `proximadb-storage-ports`** — the `IndexPort` + storage-facing event/metadata traits. **DONE (CollectionMetadataPort):** crate created (`crates/storage/proximadb-storage-ports`, storage tier — same-tier deps allowed) with `async fn collection(&str) -> Result<Option<Collection>>` (the *measured* surface is one method; `get_collection_stats` turned out to be on `AxisManager`, not the service); `impl CollectionMetadataPort for CollectionService`. **DONE (StorageEventLogPort):** storage publishes flush/compaction/can-compact facts through the port; `EventLogService` performs the AXIS `IndexEvent` translation; `CompactionCoordinator` consumes the port for compaction readiness checks. **Remaining:** `IndexPort` (leaks `AxisHybridQuery`/`AxisManagerQueryResult`/`AxisConfig`/`AxisVectorIndex` — per-type facade-vs-pre-extract decision; re-exporting root types into the port crate is a *cycle*, impossible). | Low–Med
