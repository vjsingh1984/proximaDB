...
python scripts/bench/build_token_budget_corpus.py \
  ...
  --overflow-policy split --deduplicate exact --max-chunks 560000
----

The BGE contract pins base and small repository revisions independently while producing one shared
...
and cached 20k-row shards cannot be reused across a text hash, revision, contract or dimension.
