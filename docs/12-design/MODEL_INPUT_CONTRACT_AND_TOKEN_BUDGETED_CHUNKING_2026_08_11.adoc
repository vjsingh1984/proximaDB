...
source-token coverage per stored chunk, making redundant-boundary amplification observable. The
corpus records both its transport-script SHA-256 and a deterministic digest of the complete loaded
SDK chunking-strategy source tree; every later stage records its tracked producer SHA-256, and
