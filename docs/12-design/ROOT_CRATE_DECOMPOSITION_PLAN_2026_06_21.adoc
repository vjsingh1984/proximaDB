...
* Extracted: `proximadb-{records,codec,block-format,storage-common,object-store,iceberg-engine,
  storage-kv,storage-transaction}`.
...
| A0 | `proximadb-storage-kv` ← `src/storage/kv/` (re-export) | ~0.1K | Low | ✅ Done
| A1 | `proximadb-storage-transaction` ← `src/storage/transaction/` (re-export) | ~2.7K | Low | ✅ Done (PR #329)
| A2 | `proximadb-storage-encryption` ← `src/storage/encryption/` | ~1.7K | Low | **Next clean leaf** — only 2 refs to `crate::storage::persistence` to resolve
| — | `proximadb-storage-formats` ← `src/storage/formats/` | ~5K | ~~Low~~ → High | 🚫 *Blocked, not a quick win.* `formats/adapters.rs` binds all six engines (`crate::storage::engines::*`, 12 refs) — engine-bound. Defer behind E. (Disproves the original "formats-first"; see #145.)
| — | `proximadb-storage-cache` ← `src/storage/cache/` (wraps `proximadb-cache`) | ~11K | ~~Low~~ → High | 🚫 *Blocked, not a quick win.* 11/27 files depend on the root-resident, 90-consumer `storage::traits::UnifiedStorageFormat` (+ root `metrics`/`observability`). Requires D (trait port-inversion) first.
| C | Persistence: WAL + disk-manager port crates | ~? | Med | Pending
...
The clean storage leaves (kv, transaction, encryption) are the independent quick wins that
prove the re-export pattern and start shrinking the binary. *Note:* the originally-named
"quick wins" (formats, cache) were disproved on closer inspection — both have hard upward
edges into root-resident subsystems and are blocked behind D/E, not Low-risk. D is the
linchpin: the ~403 up-edges mean engines cannot move until the index/compute/metadata
boundaries are ports — and the same `storage::traits::UnifiedStorageFormat` port-inversion
unblocks cache. E/F are large and must be coordinated.
...
The proof-of-pattern slices have landed: `proximadb-storage-kv` (done) and
`proximadb-storage-transaction` (PR #329) are both pure `git mv` + re-export extractions of
verified clean leaves, layering-green with no call-site churn.

A cold-subtree check disproved the originally-named first two slices: *formats* is
engine-bound (`adapters.rs` → all six engines) and *cache* depends on the root-resident
90-consumer `storage::traits::UnifiedStorageFormat` plus root `metrics`/`observability`.
Neither is a mechanical quick win; both are deferred behind the *D* port-inversion / *E*
engine extractions.

*Next clean leaf:* `proximadb-storage-encryption` ← `src/storage/encryption/` (~1.7K LOC) —
resolve its 2 references to `crate::storage::persistence`, then a pure `git mv` + re-export PR.
After the leaf inventory is exhausted, *D* (the trait/index/compute port-inversion enabler)
is the linchpin that unblocks both cache and the large engine slices.
