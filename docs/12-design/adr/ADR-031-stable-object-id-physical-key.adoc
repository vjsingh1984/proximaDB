...
[cols="1,3"]
|===
|Status|**Superseded (u64-as-identity) by ADR-0083 (2026-07-30).** The global `object_id` u64 is retired as a collection identity; the composite `CollectionIdentity {account, namespace, collection}` is now the sole canonical identity. This ADR's *path-isolation* intent (name is metadata; structural isolation) survives in ADR-0083; its *u64-as-physical-key* decision is reversed — two identity systems (u64 + composite) was an inconsistency seam (the #1325 regression). See ADR-0083 + `CATALOG_IDENTITY_RESEARCH_2026_07_30.adoc`.
|Date|2026-06-26
|Deciders|Vijaykumar Singh
|Relates to|ADR-020 (Canonical WAL Authority), ADR-025 (Deletion Vectors), ADR-010 (PAX Block Format), ADR-024 (single canonical type — the 2026-06-27 identity addendum distinguishes this catalog `object_id` from the data-record `oid` and from `pgwire_oid`), ADR-027 (DrPath / storage layout), `CODESIGN_DIMENSIONAL_ARCHITECTURE_2026_06_19.adoc` (structural isolation)
|Identity authority|**This ADR is the authority for catalog-object surrogate identity** (the immutable `u64` `object_id`). The would-be `ADR-033-catalog-object-surrogate-identity` (PR #436) was **closed** and its decisions folded here — that number duplicated the merged ADR-033 (primary-storage-format-strategy) and would hard-fail the #441 `td-adr-unique` guard. Execution plan: `CATALOG_OID_MIGRATION_2026_06_27.adoc` (TD-181).
|Supersedes|—
|===

[NOTE]
====
**Consolidation (2026-06-27).** The catalog-object identity decision lives **here**,
in ADR-031 — not in a separate ADR-033. A parallel session drafted
`ADR-033-catalog-object-surrogate-identity` (`object_oid`); it was closed because
`033` was already taken (primary-storage-format) and re-using it fails the new
`td-adr-unique` CI guard (#441). Its decisions, the salvaged execution plan
(`CATALOG_OID_MIGRATION_2026_06_27.adoc`, TD-181), and the ADR-024 identity
addendum are reconciled into this ADR. `object_id` (this ADR + the merged
`CatalogTableSchema.object_id` / `IdAllocator`) and `object_oid` (the closed
draft) name the **same** surrogate. Where the original tenets below diverge from
the consolidated model, see "== Reconciliation & amendments (2026-06-27)".
====
...
== Reconciliation & amendments (2026-06-27)

A parallel session drafted catalog-object identity as `ADR-033-catalog-object-surrogate-identity`
(`object_oid: u64`, central monotonic sequence, oid-keyed `DrPath`). That ADR is
**closed** (PR #436): `033` was already the merged primary-storage-format ADR, so
re-using it fails the #441 `td-adr-unique` guard. Its decisions are **the same
surrogate** as this ADR's `object_id` and are consolidated here; the salvaged
execution plan (`CATALOG_OID_MIGRATION_2026_06_27.adoc`, TD-181) and the ADR-024
identity addendum land alongside.

=== Scope (broadened from "collection" to all catalog objects)

The identity model applies to **every catalog object** — account, namespace,
table/collection, column, index — not only the table/collection. The
collection (table) is the **implemented** case (the merged `CatalogTableSchema.object_id`
+ `IdAllocator`, O0–O2); the others follow the same model via the migration plan.

=== Amendments to the "== Design tenet" above

Where the original tenet text (written 2026-06-26, table-scoped) diverges from the
consolidated model, the following govern:

. **One system-wide monotonic sequence, not per-type spaces.** A single central
  sequence owned by the catalog authority mints every catalog object's `object_id`,
  so an oid is globally unique across **all** types (a stale reference is
  unambiguously dangling, never silently aliased; no per-type width bookkeeping).
  This **supersedes** the original "per-type id space" wording (where a `tenant_id`
  and an `object_id` could both be `5`). Practical impact is small: only the *table*
  allocator is implemented, and it becomes one consumer of the single sequence.
  Reserve a low range (`0..RESERVED_MAX`) for pinned system objects.

. **Tenant / account are externally assigned, not catalog surrogates.** `tenant_id`
  (and account) come from the auth/control plane; only `namespace`/`table`/`column`/`index`
  are catalog-minted `u64` surrogates. This **supersedes** the original
  "`tenant_id`, `account_id` → `u32` surrogate." Tenant/account still appear in the
  path (structural isolation + locality) and as billing attributes — they are simply
  not minted by the data-plane sequence.

. **Width is uniform `u64` for catalog objects.** With tenant/account no longer
  catalog surrogates (amendment 2), the per-type `u32` tuning loses its subject;
  catalog objects are uniform `u64` (matches the single sequence). The original
  width-tuning argument is retained only as historical rationale.

. **A column has both ids, by role (no conflict).** Its **catalog identity** is a
  `u64` `object_id` (so catalog→catalog references + rename are oid-keyed); its
  **physical mapping** into Parquet/PAX stays the `i32` Iceberg field-id
  (`CatalogColumn.id`, ADR-010). Different roles, both retained.

=== Scope boundary (identity ≠ data-record handle)

Per the ADR-024 identity addendum: the **catalog-object** `object_id` (`u64`, this
ADR) is distinct from the **data-record** handle `ProximaRecord.oid: String`
(natural key or engine-minted ULID — high-rate, distributed, coordination-free;
settled), and from `pgwire_oid()` (the PostgreSQL *type* OID, wire-compat only).
This ADR changes the *collection/catalog* key, never the data-record oid.

== References

* `CATALOG_OID_MIGRATION_2026_06_27.adoc` (the phased execution plan for this ADR; TD-181).
* ADR-024 (single canonical type — 2026-06-27 identity addendum: catalog `object_id` vs record `oid` vs `pgwire_oid`).
* ADR-027 (DrPath / storage layout — the migration's Phase 3 keys the path off `object_id`).
* (Closed) `ADR-033-catalog-object-surrogate-identity` (PR #436) — decisions folded into this ADR.
