...
2026-07-21). ADR-065 extends this ADR with Region B; TD-RDSTRAT-8 extends it with a default-OFF
large-segment probe directory; neither replaces the one-object/header-first/self-describing-footer
decisions. TD-RDSTRAT-9 owns collection file/cache/compaction policy outside the segment.
...
cache #1069, and bounded-parallel reads #1094 have shipped. Point-lookup Bloom remains open.
Successor layouts/encodings retain independent mixed-read, recall, and cost gates.
...
(RaBitQ index, SQ8 rerank projection, exact f32) are physical representations of one logical
...
block-local SQ8 path, but a v2 footer requires its encoding map. An explicitly unknown value
codec/version is never reinterpreted as SQ8. An unknown transform or compressor on a canonical
stripe fails closed; an unknown optional projection may be bypassed only when a declared
canonical source can satisfy the request. Unknown optional metadata sections may be ignored.
Because this phase is unreleased, old PR1 binaries are not required to read v2; footer-v2 writes
stay default-OFF until new readers are deployed.
