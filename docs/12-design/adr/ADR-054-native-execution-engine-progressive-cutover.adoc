...
[cols="1,3"]
|===
|Status|**Accepted** (ratified 2026-07-09; proposed 2026-07-08 after multi-agent review v1→v3)
|Date|2026-07-08 (proposed) · 2026-07-09 (ratified)
|Deciders|Vijaykumar Singh (ratified 2026-07-09 — the §12 open questions are resolved with their stated recommendations; see Revision Notes v4)
|Relates to|ADR-039 (`ExecutionEngine` / `LogicalNode` seam), ADR-052 (vectorized-over-Arrow via DataFusion; PAX-native scan as the wedge), ADR-050 (cost-based routing + stats-fed planning), ADR-033 (vertical inside, standard outside), ADR-025 (deletion vectors), ADR-011 (bloom/ANN pre-filter), `DATAFUSION_BALLISTA_DISTRIBUTED_EXECUTION_2026_06_05.adoc`, `RELATIONAL_DOCUMENT_GRAPH_CONVERGENCE_2026_05_14.adoc`
...
|Tracked under|Phase 0 contracts = #755 (crate `proximadb-execution-contracts`, no separate TD). link:../../10-quality/td/TD-OLAP-10-native-filterproject-pipeline.adoc[TD-OLAP-10] (native vectorized FilterProject + pipeline executor), link:../../10-quality/td/TD-OLAP-11-native-radix-bloom-spill-hash-join.adoc[TD-OLAP-11] (native radix-partitioned, spilling, bloom-pre-filtered hash join), link:../../10-quality/td/TD-OLAP-12-native-morsel-scheduler.adoc[TD-OLAP-12] (native morsel scheduler), link:../../10-quality/td/TD-OLAP-13-fused-multimodal-operators.adoc[TD-OLAP-13] (fused ANN+filter+aggregate operator). Composes with existing TD-OLAP-1..8; TD-OLAP-9 remains separate DPP work (`iceberg-partition-spec-unblocks-dpp`), not the contracts crate.
|===
...
== Revision Notes (v4 — ratified 2026-07-09)

The decider ratified ADR-054 as **Accepted**. Ratification carries these decisions
(the §12 open questions, resolved with their stated recommendations):

[cols="1,3",options="header"]
|===
| §12 question | Ratified decision
| 1. `ExecutionOperator` granularity — one join operator or three? | **One `HashJoinOperator`** that internally owns build→probe (keeps `Pipeline` a single chain; the two-phase break is internal). Per TD-OLAP-11 §"Pipeline composition."
| 2. Native vectorized in the existing engine vs a sibling crate? | **One `ComputeBackend::Native`** with an internal hook (`native_vectorized_enabled()` / `try_vectorized`); contracts in `proximadb-execution-contracts` (already landed #755). No peer `ComputeBackend` variant.
| 3. Morsel size 2048 vs tunable? | **Const default 2048** (`MORSEL_SIZE`) with a per-operator runtime override.
| 4. Native ↔ DataFusion interop below the seam? | **No.** Any fragment-level hybrid needs a separate ADR (§7.4).
| 5. Promotion threshold — parity or strict-better? | **Parity-or-better in-memory; strict-better in the spill regime** (the gap-closing claim). Encoded in every TD's ledger gate.
| 6. Multimodal operators in this ADR or a follow-on? | **Rationale here (§6); TDs are Phase 5 (TD-OLAP-13).** Contracts must be right first.
|===

Ratification does **not** promote any native operator to default-on: it accepts the
*plan + contracts + reversal of ADR-052 invariant 2's operator prohibition*. Every
operator still ships default-off and is promoted only through the TD-OLAP-4 ledger
gate (§5.3, §8). The tracked implementation TDs carry the concrete designs:
TD-OLAP-10 (Phase 2, landed), TD-OLAP-11 (Phase 3 join, design filed), TD-OLAP-12
(Phase 4 scheduler), TD-OLAP-13 (Phase 5 multimodal).
...
. **Axis A — the binding constraint has shifted.** ClickBench Phase A (2026-07-06) measured that with accurate I/O metering the gap to DuckDB is no longer bytes-read-dominated; it is **compute + pgwire** — a ~2.2× median gap (post I/O-attribution fix; the raw pre-cache release run was ≈8.8×, dominated by a fixed open+plan/registration floor) plus specific compute pathologies (the q07 UInt16 MIN/MAX outlier). For a class of queries (heavy joins per the SF=0.01 join micro-ledger §1.3.4, multimodal co-execution), DataFusion's kernels *are* the binding constraint, and that is precisely the case ADR-052 said it would accept ("DataFusion's per-op microbenchmark ceiling vs Velox ... the ledger makes the trade explicit rather than hidden").
...
| ProximaDB median wall — pre-cache release (35/43 ok) | 230 ms (debug 250 ms) | Release did NOT collapse it (raw ≈8.8× vs DuckDB) → the median lever is a fixed ~45 ms OPEN+PLAN/registration floor (`prepare_dataframe` re-opens the object per query), NOT a CPU-kernel seam
| ProximaDB median wall — post I/O-attribution + registration fix | ~60–69 ms | **~2.2× median** vs DuckDB (the decisive post-fix number — see the decisive-finding quote below)
| DuckDB median wall (release, 35/43 ok) | 26 ms | Identical lowercase Parquet baseline
| Real `bytes_read` total (43 q, after I/O-attribution fix) | 382 MB | I/O is small; the median gap is NOT bytes-read
| `COUNT(*)` `bytes_read` (after exact-stats elision) | 0.5 MB (footer-only) | Projection pushdown + metadata-agg elision work
| q07 UInt16 MIN/MAX | reads 5 MB, takes ~10 s | **Pure compute pathology** — an isolated residual kernel gap, NOT I/O
| Per-query `head` over-count (fixed) | whole-object-size | Was a metering bug inflating `bytes_read`/billing
|===

**Decisive finding:** *"With accurate metering ... the remaining wall gap vs DuckDB (~2.2× median) is now clearly compute + pgwire, not I/O — dominated by the q07 UInt16 MIN/MAX 10 s outlier (reads only 5 MB; pure compute)."* The split this ADR relies on: the ClickBench *median* gap is mostly the fixed OPEN+PLAN/registration floor (a caching fix, not an operator-engine fix), while the *operator-compute* case for native execution rests on the isolated kernel pathologies (q07) and the TPC-H SF=0.01 join micro-ledger (§1.3.4, Q3 22× / Q14 13×). Native operators target the latter, not the registration floor.
...
* **The co-design velocity argument:** a vectorized kernel can be benchmarked in isolation (`criterion` over a `PrimitiveArray`), promoted/demoted per the ledger, and swapped without a compiler dependency. This follows the measured, incremental co-design method.
...
| **4** | Native morsel scheduler (NUMA-aware); wire native operators into the scheduler; the native engine is now a complete (if narrow) operator set. | link:../../10-quality/td/TD-OLAP-12-native-morsel-scheduler.adoc[TD-OLAP-12] | Ledger: end-to-end ClickBench + TPC-H/TPC-DS at parity-or-better
| **5** | Native vectorized primary for supported native-storage shapes; Volcano fallback inside `ComputeBackend::Native`; DataFusion remains the separate Parquet-backed OLAP route. Add fused multimodal operators (`VectorSearchOperator`, `GraphTraversalOperator`, `DocumentProjectionOperator`, `FusedAnnFilterAggregate`). | link:../../10-quality/td/TD-OLAP-13-fused-multimodal-operators.adoc[TD-OLAP-13] | Multimodal benchmark (the wedge); per-query ledger vs DataFusion
|===
...
This ADR applies the co-design method explicitly:
...
== 13. Relationship to ADR-052 (the precise change)

This ADR-054 makes exactly one substantive change to ADR-052's decision set: it **reverses invariant 2's prohibition on owning operators** — progressively, behind contracts and ledger gates — while leaving invariants 3, 4, 5 intact and refining invariant 1 only insofar as the native engine gains an internal vectorized path:
...
| 2. NativeOlap = PAX-native SCAN, not an operator engine | **Reversed (gated):** the _"not an operator engine"_ prohibition is reopened — an *operator plane* is added behind contracts, default-off, substituted progressively and promoted only where the ledger proves DataFusion is binding. The scan stays the wedge. | Prohibition reversed behind contracts + ledger gates; scan role unchanged
...
The invariant-2 reversal is justified by evidence ADR-052 did not have at acceptance (ClickBench Phase A, 2026-07-06, and the TPC-H SF=0.01 join micro-ledger §1.3.4): for a class of compute-bound queries (heavy joins, the q07 kernel pathology, multimodal co-execution) the operator plane — not bytes-scanned — is the binding constraint, exactly the condition under which ADR-052's "do NOT reimplement operators" prohibition is reopened. The reversal authorizes *building behind contracts*, not shipping: every native operator stays default-off until the SF1/SF10 ledger gate clears (§5.3, §8).
...
* Design docs: `docs/12-design/CODESIGN_DIMENSIONAL_ARCHITECTURE_2026_06_19.adoc` (co-design method and P8 vertical-inside-standard-outside); `docs/12-design/DATAFUSION_BALLISTA_DISTRIBUTED_EXECUTION_2026_06_05.adoc` (distributed blueprint); `docs/12-design/RELATIONAL_DOCUMENT_GRAPH_CONVERGENCE_2026_05_14.adoc` (multimodal operator sketches).
