...
:status: Accepted (2026-08-03; implementation and live surface evidence reconciled)
:date: 2026-08-02

== Status

Accepted and implemented. The identity carrier, total stable-id mint,
fail-closed composition rule, and live SQL/record transport matrix are pinned.
...
* TD-ABAC-11 — mint + strictness: default-tenant mint, binding-provisioning
  ...
  search/point-get, authenticated gRPC v2 unary/streaming search + point-get,
  and authenticated Arrow Flight canonical record search now have live
  row-policy evidence.
