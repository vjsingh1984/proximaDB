...
20. `SDK_MODULARIZATION_AND_SPEC_DRIVEN_GENERATION_2026_06_21.adoc` +
   ...
   Companion lexical contract:
   link:LEXICAL_QUERY_CONTRACT_AND_ENGINE_CONVERGENCE_2026_08_22.adoc[natural-text versus advanced
   syntax semantics, production BM25/Tantivy capability routing, and measured convergence gates].
...
23. `UNIFIED_RESOURCE_LEASE_LOCKING_REVIEW_2026_06_24.adoc` +
   Approved correction plan for the `feat/unified-resource-key` branch. Retains the unified
   `ResourceKey` and strategy direction, but gates DML/protocol lock exposure on foundational fixes:
   mixed-read-safe legacy lease paths, canonical path-safe resource identity, full-key renewal,
   real `TenantContext` for DML locks, enforced hierarchy, guard-owned renewal/release, and
