...
clean enough to enforce isolation/billing and to evolve. Each dimension is also a metered *TAM
surface*: the per-dimension meter (KSU/KRU/KIU/KOU/cache/tier) is the unit a commercial layer prices.
(Market-size figures and GTM specifics are commercial concerns tracked privately — see
...
cold/warm cliff is), and the *billing meter* (the per-dimension metering unit).
...
*Metered surface:* object storage is the durable-base meter (KSU + the read share of KRU); it is the
substrate of the lakehouse / cloud-database surface this dimension addresses.
...
*Billing meter:* [*DONE — C3*] `proximadb_kou_bytes_total{tenant_id, kou_locality, direction}` (the
*KOU* — Outgress Unit — meter; *distinct from KEU = embedding*) plus the per-query
`io_trace.egress_bytes` term, fed by one convergent `consumption_metrics::record_kou_bytes` at *both*
the object-store read boundaries (`direction=read`) and the result boundaries (`direction=result`),
instrumented across *pgwire, REST, and Arrow Flight*. `KouLocality`
(`local`/`cross_az`/`cross_region`/`cross_cloud`/`on_prem`/`internet`/`unknown`) is OSS *mechanism*:
read-egress locality is *derived* from compute-vs-store region (`install_placement`); result-egress
locality is *classified* from the client IP (`EdgePolicyContext`) against a terraform-emitted scope
map. Only chargeable buckets feed the cost term, so it stays inert on the free same-region path; the
per-(cloud, bucket) $ weights stay anvaiops *policy*. `ResultShapePolicy` adds locality-driven,
lossless byte-minimization (ZSTD on Flight, gzip at the gateway) for far clients. Closes the
previously-unmetered networking dimension — full multi-cloud model in
link:OUTGRESS_KOU_MULTICLOUD_COST_MODEL_2026_06_21.adoc[OUTGRESS_KOU_MULTICLOUD_COST_MODEL_2026_06_21].

*Metered surface:* networking is the outgress meter (KOU) and the cost floor of every serverless DB;
the disaggregation/RDMA research wave (GaussDB, PolarDB-MP, PVLDB 2024–2025) signals where the
AI-infra investment is heading.
...
*Metered surface:* the cache tier is the performance lever — it converts the same object-storage
surface into a higher-performance serverless tier (the warm-pool / provisioned-cache meter).
...
*Metered surface:* the compute router (KRU + KIU meters) is what lets ProximaDB address the *whole*
cloud-database / serverless-database surface rather than only the narrow *vector-DB* segment. *This
breadth is the core engineering argument for the multi-engine pivot:* co-designing compute to serve
relational + vector + graph + document over one lakehouse spans a far larger workload surface than a
single-modality vector box.

=== 2.5. Dimension 5 — Governance & Security (the enterprise margin surface)

*Physical "curve":* governance has no byte cost — its cost is *trust and compliance*. Industry
practice edition-gates BYOK/CMK, audit logs, and advanced RBAC, and treats compliance certs (SOC 2,
HIPAA BAA, FedRAMP) as hard gates. (Specific vendor commercial terms are GTM intelligence tracked
privately — see link:OSS_ENTERPRISE_BOUNDARY_2026_06_17.adoc[OSS_ENTERPRISE_BOUNDARY].)
...
*Metered surface:* governance is the tier-entitlement meter — a smaller but higher-value surface than
the compute/storage base, which is *why* it sits as the enterprise tier on top of a commoditizing
...
Cost(q) =  w_storage · bytes_stored·time          (KSU)            ← per-tenant aggregate, not per-query
         ...
         + w_egress  · bytes_moved·kou_locality_cost (KOU, Dimension 2)   [DONE — C3 meter + cost term]
...
*egress* (KOU), ingest (KIU), and compute terms in neutral units from the measured
`IoTraceSnapshot`, and applies the per-tenant tier multiplier (C5 DI seam) to the *reported* cost.
The egress and ingest terms are *inert by default* (zero on the free same-region path / for read-only
routes), so routing is unchanged until a cross-region/-cloud/internet path is involved. `w_storage·time` (KSU) stays
...
compute-ms, and cache; the per-query bytes-moved/egress quantity (KOU) and the per-query (not
per-tenant-aggregate) granularity are *now supplied* by the `IoTraceSnapshot` + `proximadb_kou_bytes_total`.
...
| C3 — Network metering | [*meter DONE*] `proximadb_kou_bytes_total{tenant, kou_locality, direction}`
counter (KOU) + per-query `io_trace.egress_bytes`; result-egress instrumented at pgwire/REST/Arrow
Flight. Remaining: same-region placement enforcement; pgwire wire-compression; egress-aware lossy caps.
...
== 6. Metered-Surface Summary (per dimension)

Each dimension carries its own metering unit — the per-dimension meter a commercial layer prices.
(Market-size figures and CAGRs are commercial concerns tracked privately — see
link:OSS_ENTERPRISE_BOUNDARY_2026_06_17.adoc[OSS_ENTERPRISE_BOUNDARY].)

[cols="1,2,2", options="header"]
|===
| Dimension | Billing unit (meter) | Surface addressed
| Object storage | KSU + read-KRU | Data lakehouse / DBaaS durable base
| Networking | KOU | Outgress floor of serverless DB; AI-infra data movement
| Local disk / cache | cache-tier meter | Performance tier on the storage surface (warm-pool)
| Compute / modality | KRU + KIU | Cloud-database / serverless-database compute surface
| Governance / security | tier entitlement | Governance / data-security / privacy surface
|===

*Engineering read:* the narrow single-modality vector box is where the brand was born but is a small
slice of the cloud-database, lakehouse, serverless, and AI-infra surfaces. Co-design across the five
dimensions — one engine, one router, one trace-driven cost model, open seams — is precisely the move
that lets ProximaDB meter *each dimension as its own surface* and address the broader workload
container rather than competing only inside the vector segment.
...
egress (KOU — Outgress Unit, distinct from KEU = embedding) is metered (shipped: read + result
directions at pgwire/REST/Flight). Governance is metered as tier entitlement.
