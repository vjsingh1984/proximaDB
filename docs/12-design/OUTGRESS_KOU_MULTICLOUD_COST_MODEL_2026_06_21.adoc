...
|===
| Stage | Move
| 1 | Region-affinity metered multi-tenant SaaS + KOU pass-through; same-region free; cloud+region-encoded endpoints
| 2 | Egress-free object storage (Cloudflare R2 / Tigris, $0 egress) *under* the data plane → read-egress structurally $0; recover via storage+ops
| 3 | PrivateLink / Private Link / PSC as a paid connectivity add-on (kills the internet hop; break-even ~80–95 GB/mo on Azure)
| 4 | BYOC — data plane in the customer's own account (customer bears storage+egress directly; pull-based outbound-only control, no inbound); strongest compliance, heaviest per-customer ops
|===

Stage→tier packaging and pricing are commercial/GTM specifics tracked privately
(see `OSS_ENTERPRISE_BOUNDARY_2026_06_17.adoc`).
...
bundled; realistic BYOC per-customer operational cost. (Commercial viability
thresholds are tracked privately — see `OSS_ENTERPRISE_BOUNDARY_2026_06_17.adoc`.)
