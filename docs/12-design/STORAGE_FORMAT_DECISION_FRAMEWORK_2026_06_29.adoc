...
== The io-trace-aware substrate

* **`CountingFileSystem`** (`crates/storage/proximadb-storage-filesystem-types/src/counting.rs`)
  — optional pass-through wrapper for process-global diagnostic counters.
  `PROXIMADB_COUNT_FS_IO` remains default OFF and does not own per-query
  attribution.
...
* **Physical-read ownership:** each leaf filesystem records its successful full
  and ranged reads directly into the active task-local trace. A full object GET
  and a ranged GET both increment `get_ops`; only the latter increments
  `range_gets`. This is independent of the diagnostic wrapper and avoids both
  missing whole-object reads and wrapper/leaf double counting.
* **Second object-store stack:** the upstream `object_store` adapter retains the
  `IoRecorder` DIP hook (`IoTraceFsRecorder`) because it does not pass through the
  leaf `FileSystem` implementations. Both stacks therefore converge on the same
  `IoTrace`, while `CountingFileSystem` remains diagnostics-only.

So: an active `IoTrace` scope receives physical GET/byte/range attribution
without an env gate. `--features io-trace` enables trace emission/observers;
`PROXIMADB_COUNT_FS_IO=1` adds only the global diagnostic counters.
...
. For each (workload, format-profile) pair → build with `--features io-trace`.
  Add `PROXIMADB_COUNT_FS_IO=1` only when process-global diagnostic totals are
  also required.
...
* **`PROXIMADB_COUNT_FS_IO`** — opts `CountingFileSystem` into process-global
  bench/diagnostic counters; it is not required for per-query physical I/O traces.
