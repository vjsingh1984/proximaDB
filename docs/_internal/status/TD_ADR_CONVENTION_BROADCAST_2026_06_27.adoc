...
. **Rule files are local-only (gitignored).** `CLAUDE.md` / `GEMINI.md` / `AGENTS.md` are **not
  committed** — each developer keeps a local copy and seeds it from `docs/agent-rules/*.seed` (see
  `docs/agent-rules/README.adoc`). They only *point* to the canonical doc above; they don't carry
  the rules. (They were briefly committed in #441; reverted to local-only the same day — a stale
  `.gitignore` rule had contradicted the committed state, and the team chose genuinely-local rule
  files with committed seeds.)
