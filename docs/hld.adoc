...
----
classDiagram
    class AvroWalManager {
        +storage_backend: WalStorageBackend
        +compression: RecoveryOptimizedCompression
        +multi_storage_config: MultiStorageConfig
        +recovery_config: RecoveryConfig
        +append_entry(entry: AvroWalEntry) Future~u64~
        +read_all_parallel() Future~Vec~AvroWalEntry~~
        +checkpoint(sequence: u64) Future~()~
        +recover_adaptive() Future~Vec~AvroWalEntry~~
    }
