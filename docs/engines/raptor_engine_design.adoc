...
graph LR
    ...
    subgraph "P² Matrix Approach"
        M["P² Matrix<br/>Upper Triangle"] --> Q["Quantized INT8"]
...
graph TB
    subgraph "RAPTOR File Structure"
        ...
        RG --> V["Vectors<br/>Compressed"]
        RG --> P2["P² Matrix<br/>Upper Triangle"]
        RG --> PK["P×K Matrix<br/>Adaptive"]
        RG --> BF["Bloom Filter<br/>ID Lookup"]
        
        FT --> K2["K² Matrix<br/>Full, Always"]
...
[source,mermaid]
----
pie title "Memory Distribution for 1M Vectors"
    "P² Matrices" : 50
    "P×K Matrices" : 55
    "Bloom Filters" : 18
    "K² Matrix" : 0.04
...
For 1M vectors (K=100 rowgroups, P=1024 vectors/rowgroup):
```
K² Matrix:        40KB (100×100×4 bytes)
P² Matrices:      50MB (100 rowgroups × 512KB)
P×K Matrices:     10-100MB (adaptive coverage)
Bloom Filters:    18MB (100 × 180KB)
Total Index:      ~80-170MB (8-17% of vector data)
```
