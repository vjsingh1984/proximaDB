...
image::raptor_phase1_cluster_selection.svg[Phase 1: Cluster-Level Selection, align="center", width=100%]

The above diagram illustrates how the K×K matrix enables intelligent cluster selection:

* **Query Analysis**: Query vector Q computes distances to all K centroids simultaneously
* **Boundary Detection**: When ratio d_i/d_j > 0.8, clusters are considered neighbors requiring exploration
* **Budget Allocation**: Search resources allocated proportionally based on centroid distances
* **Matrix Lookup**: O(1) access to pre-computed inter-centroid distances

=== Phase 2: Vector-Level Spillover (P×K)

image::raptor_phase2_vector_spillover.svg[Phase 2: Vector-Level Spillover, align="center", width=100%]

The P×K matrix reveals vectors that "spill over" their assigned cluster boundaries:

* **Spillover Detection**: P×K matrix tracks which vectors are closer to neighboring centroids
* **Threshold**: When >15% of vectors are closer to another centroid, expansion is triggered
* **Dynamic Discovery**: Uncovers clusters (like C4) not identified in Phase 1
* **Adaptive Storage**: Only stores significant deviations (>5% from mean) to save space
...
graph TD
    ...
    subgraph "Boosting-Guided Expansion"
        ...
        BCF --> CP["Cross-Cluster Penalties"]
