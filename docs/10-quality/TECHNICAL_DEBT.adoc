...
| done (flag flip = follow-up)
| Remaining (follow-up, lower priority): flip `PROXIMADB_WAREHOUSE_DRPATH` on by default after a backfill/repath migration for snapshots written under the legacy `data/{tenant}/{ns.join}/{table}` layout; populate `CatalogNamespace.tenant_id` (DR/CRR P0.5) so `build_for_pool` pool-class routing applies.
|===
