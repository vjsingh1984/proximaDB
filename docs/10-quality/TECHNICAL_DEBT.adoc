...
| P3
| MEDIUM
| Partial — Rust SDK done (TS rewrite running in parallel)
| Surfaced 2026-05-30 after reconciling the OpenAPI graph schemas (`docs/openapi/proximadb-openapi.yaml`) against the live server handler `crates/platform/proximadb-api/src/rest/v1/graph.rs`. The earlier spec additions had wrong field names / wrapping for `CreateGraphRequest` (required `name` → now `graph_id`), `CreateNodeRequest` (flat `{id, ...}` → wrapped `{node: NodeInput}`), `CreateEdgeRequest` (flat `{source, target, ...}` → wrapped `{edge: EdgeInput}` with `from_node_id` / `to_node_id` / `edge_type` / `id`), and `TraverseRequest` (had `direction` / `max_nodes` → server-true keys are `start_node_id` (not `start_node`) + `node_labels` + `limit`). **Rust SDK rewrite landed 2026-05-30** (`clients/rust/src/graph.rs`): all six builders (`GraphBuilder`, `NodeBuilder`, `EdgeBuilder`, `TraversalBuilder`, `add_nodes`, `add_edges`) now emit server-true bodies. The three previously-`#[ignore]`d contract tests (`create_graph_matches_spec`, `create_node_matches_spec`, `create_edge_matches_spec`) are unskipped; two new tests for `batchCreateNodes` / `batchCreateEdges` were added. **TypeScript SDK rewrite (`clients/nodejs-embedded/src/graph.ts`) is still open and running in parallel.**
| 1-2 weeks (TS only)
| Acceptance: (1) `clients/nodejs-embedded/src/graph.ts` `GraphBuilder.execute()` posts `{graph_id, name?, description?}`; `NodeBuilder.execute()` and `GraphHandle.addNodes()` either post per-node `{node: NodeInput}` (matching spec) or expose a separate batch builder that posts to a batch endpoint the server actually exposes; `EdgeBuilder.execute()` / `addEdges()` mirror with `{edge: EdgeInput}` using `from_node_id` / `to_node_id` / `edge_type` / `id`; `TraversalBuilder.execute()` posts `start_node_id` (not `start_node`), drops `direction` / wraps `relationships` as `edge_types`, and adds optional `node_labels`. (2) **DONE for Rust SDK at `clients/rust/src/graph.rs`** (2026-05-30). (3) Unskip the four graph contract tests (TS) and any Rust analog; assert spec-required keys present in the captured body. **Rust analogs unskipped + 2 new batch tests added 2026-05-30 — all 20 contract tests pass, zero TD-095 ignores remain.** (4) Add a per-language live-server smoke (insert node, get node, traverse) so URL+body+server actually exchange happy-path data.
|===
