...
| P5
| HIGH
| *Partial — BFS-bounded primitive exists; single-step variant + tool exposure missing*
...
| 2-3 weeks (1-2 weeks remaining)
| **Verified**: April 25, 2026. Paper's contribution is **iterative single-step navigation**, not breadth-bounded walks. Our current implementation does breadth-bounded BFS in one call — useful but doesn't directly match the paper's approach. **Status**: Implemented April 2026 at `src/graph/service_traversal_api.rs:355-410`; test at `src/graph/service.rs::test_graph_walk_tool`. **Remaining work**: (1) add a true *single-step* primitive `graph_step(current_node, edge_filter) -> {neighbors, properties}` alongside the bounded walk; (2) REST endpoints `POST /api/v1/graph/{id}/walk` and `POST /api/v1/graph/{id}/step`; (3) proto `GraphWalkRequest/StepRequest/Response` + gRPC; (4) MCP / OpenAI function-call schema; (5) edge-filter + projection support; (6) LangChain/CrewAI tool integration in `clients/python/src/proximadb_sdk/integrations/`. **Acceptance**: agent walks 1M-node graph with ≤8K tokens/step; both walk and step interfaces available. See `PROXIMADB_RECOMMENDATIONS.md` §2.2.

| TD-047
| Evolutionary Query Planning (DBPlanBench-style)
| P5
| MEDIUM
| *Partial — generic evolutionary scaffold exists; LLM-mutation operator (the paper's novelty) does NOT*
...
| 4-6 weeks (split into two sub-deliverables)
| **Verified**: April 25, 2026. **Important**: paper's contribution is **LLM-as-mutation-operator + DBPlanBench harness on DataFusion**. Our skeleton uses *random topological-order swaps* — useful, but classical genetic-scheduling, not the paper's contribution. Track these as separate sub-deliverables. **Status**: Skeleton at `src/query/unified/evolutionary.rs` (file **untracked in git** — must commit first). Wired into `src/query/unified/optimizer.rs:226-242` behind `OptimizerConfig.enable_evolutionary_optimizer` (default `false`); config at `optimizer.rs:62-69`. **Sub-deliverable A (no LLM)**: (1) commit skeleton; (2) measured wall-time fitness via `PlanExecutionCache` — the existing `cost_fn` callback in `evolutionary.rs:28-35` supports this; (3) regression benchmark `benches/query_optimizer.rs`. **Sub-deliverable B (paper's contribution)**: (4) spec under `docs/_internal/roadmap/specifications/` for LLM-mutation + DBPlanBench-style harness on top of `ProximaTableProvider`/`ProximaScanExec`; build only after Context Lake/RUBICON decisions clarify hot-path LLM dependency. **Acceptance**: A — ≥5% improvement over greedy on 4+ component joins; never worse on simple. B — DBPlanBench-style benchmark with reproducible 4×+ speedup on at least one workload. See `PROXIMADB_RECOMMENDATIONS.md` §3.1.
