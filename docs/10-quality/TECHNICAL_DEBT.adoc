...
| P4
| LOW
| Partial — ~41 warnings remain (down from 164; 75% cleared 2026-05-28)
| `cargo clippy --workspace --lib --bins -- -D warnings` is now CLEAN across all 50+ non-root workspace crates and the workspace `cargo fmt --all -- --check` exits 0. The root `proximadb` crate is now down to ~41 clippy warnings (`cargo clippy -p proximadb --lib`), from 164 at the start of the multi-day sweep. Remaining backlog: 12 collapsible_if-into-outer-match (these contain `?` operators and side-effects in the inner bodies — converting to match-arm guards is unsafe), 22 unwrap_used / expect_used / panic-in-production (need real `Result`-handling refactors at WAL / I/O / SQL-parse boundaries), 3 large_enum_variant (need Box-ing — touches every constructor + match site), 3 truly-dead-code items requiring product decisions, plus a sub-10-warning long tail.
| 4-6 hours remaining cleanup
| **Sweep across 14 batches landed 2026-05-28** (`7db1d804e` → `555ed5b2a`): all foundation + horizontal + storage + modality + query + control + platform crates clippy-clean under `-D warnings`; root-crate now at 75% reduction (164 → 41). Batch-by-batch: manual_checked_ops sweep (17 files), derivable_impls sweep (7), unnecessary_sort_by → sort_by_key (12), collapsible_if let-chain sweeps (waves 7/8/9 totaling 26 files), doc_lazy_continuation + auto-deref + useless_conversion bundle (14), clone_on_copy + singletons (8), long-tail singleton sweep (18), dead-alias deletion + needless_range_loop allows (7), dead-code helpers + remaining field-init (6). **Acceptance for closing**: `cargo clippy -p proximadb --lib -- -D warnings` exits 0 (or each remaining lint has an audited `#[allow(...)]` with an inline justification). The remaining ~41 are dominated by error-handling refactors (real product-code change, not mechanical) so a focused PR is still warranted — the "easy" sweep mass is gone.
...
| 1-2 days
| **PARTIALLY RESOLVED 2026-05-28**: `clients/python/tests/integration/test_v2_insert_search_e2e.py` landed with two test cases — (a) live insert_records → search round trip asserting top-k contains an inserted ID, (b) missing-collection contract asserting the SDK surfaces the round-2 HTTP 404 (not the old silent success-shaped response). Both tests use the new `rest_client` fixture that prefers `PROXIMADB_TEST_SERVER_URL` and falls back to the existing `embedded_db` path. **Remaining for full closure**: wiring the Python integration suite into `make release-check` (currently runs only the Rust smokes). Track as a thin follow-up in a future Make target update.
...
| 6+ months
| Surfaced 2026-05-28 (Vector Lakebase trend review). Depends on TD-077 (distributed transport) and is gated by TD-087/TD-088 (which make scale-to-zero economical first). Acceptance: (1) `Long-running` / `OnDemand` / `OfflineBatch` compute modes over the same logical store; (2) warm-pool scheduler with attach-on-request / release-on-idle; (3) metadata queryable with no compute resident; (4) cost/latency model documented. Sequence *after* R1/R2/R3.
