...
| 6-8 weeks
| Extend `VectorTopK` planning to state exact, predicate-aware ANN, prefilter-exact, or post-filter fallback. Push tenant/RLS and typed predicates into candidate generation where supported. `EXPLAIN` must disclose recall/failure risk for fallback plans. Coordinates with Phase C in the overhaul spec.
...
| HIGH
| Open
| `DocumentService.documents` (`RwLock<HashMap>`) and `DocumentService.collections` are written directly from ~20 call sites in `service.rs` (lines 257–1971). These in-memory writes are the primary durable authority for documents in the non-canonical-store path. Crash recovery depends on WAL replay into this map, but any race between the map write and WAL write is an inconsistency window. The canonical `UpsertCanonicalDocumentRecord` WAL variant exists but is opt-in (`canonical-document-store` feature flag).
| 4-6 weeks
| Gate all `self.documents.write()` calls behind the canonical-store feature flag. Default-enable the canonical WAL path and make the in-memory map a projection cache, not a durable source. Coordinates with `RELATIONAL_DOCUMENT_GRAPH_CONVERGENCE_2026_05_14.adoc` Phase 2 remaining item: extract document WAL ownership fully into shared canonical record-store recovery.
...
| 3-5 weeks
| Make `flush_wal()` write a `CanonicalOperation::Checkpoint` entry to the canonical WAL first, then allow the engine WAL as a short-lived performance buffer only. Recovery must use the canonical WAL as authority; ORION engine WAL replay becomes a best-effort fast-path that is validated against canonical records. Coordinates with `RELATIONAL_DOCUMENT_GRAPH_CONVERGENCE_2026_05_14.adoc` Phase 3 remaining item.
