...
| TD-097
| Spark connector JNI bridge — wire impl needs Java test env
| P3
| MEDIUM
| Partial — pilot landed 2026-05-31 (`8704472ba`)
| Surfaced 2026-05-30 during the connector-conformance sprint. `src/connectors/spark.rs` provides 9 `jni_*` functions (get_table_schema, plan_input_partitions, create_partition_reader, read_next_batch, close_partition_reader, create_data_writer, write_batch, commit_writer, abort_writer) that compile but return placeholder values (empty JSON, empty Vec, 0). The pure-Rust functions can't be called from a JVM as written — they lack the JNI calling convention, JNI-mangled symbol names, and `JNIEnv` plumbing. **Pilot 2026-05-31** (`8704472ba`): new satellite crate `crates/binding/proximadb-spark-jni` (cdylib, depends on the main `proximadb` rlib + `jni = "0.21"`) wraps ONE `jni_*` function (`jni_get_table_schema`) as `Java_org_proximadb_spark_NativeProximaDB_getTableSchema`. JVM test at `clients/jvm/spark-connector/src/test/java/org/proximadb/spark/NativeProximaDBTest.java` (plain `main()` smoke harness, no Gradle) loads `libproximadb_spark_jni.dylib` and verifies the round trip on JDK 19 / macOS aarch64 (PASS 3/3 checks). Module-level scaffold note in `src/connectors/spark.rs` still calls out the remaining 8 jni_* methods.
| 1-2 weeks remaining (8 jni_* wrappers + JNI-aware impls + Gradle + CI)
| Acceptance: (1) **DONE 2026-05-31** for the wire-up pattern — cdylib + Java test work end-to-end. (2) Mechanically wrap the other 8 `jni_*` in `crates/binding/proximadb-spark-jni/src/lib.rs` using the same `extern "system"` pattern as `Java_org_proximadb_spark_NativeProximaDB_getTableSchema`. (3) Each Rust `jni_*` constructs the underlying `ConnectorStorageAdapter` call and serializes the result as Arrow IPC / JSON (currently they return placeholders). (4) Migrate `clients/jvm/spark-connector` from the plain-main smoke harness to Gradle + JUnit 5; add per-method conformance assertions (schema fetch hits `getCollectionSchema` etc.). (5) Wire into CI as a new `spark-connector-jvm` job (build cdylib + run gradle test). The local `run-smoke.sh` is the bridge until Gradle lands.

| TD-098
| Trino connector — 2 of 7 Flight methods still scaffold
| P3
| MEDIUM
| Partial — 5 of 7 live as of 2026-06-02 (`f3284b9ba` + `e1f622fac` + `d40888326` + `ba124601d` + `3afe04d00`)
| Surfaced 2026-05-30. The Trino connector at `src/connectors/trino.rs` had 7 scaffold `flight_*` methods. **PARTIAL 2026-06-02**: 5 of 7 methods live, all using the same dial template: `grpc://` → `http://` normalize → `Channel::from_shared(...).connect_lazy()` + `FlightServiceClient::new(channel)` (tonic ≥0.12 removed the convenience `connect()` shorthand). `flight_list_schemas` (`f3284b9ba`) — `list_flights`, buckets `FlightInfo.flight_descriptor.path[0]` into sorted `TrinoSchema` list, empty-Vec on connect/list failure. `flight_list_tables` (`e1f622fac`) — `list_flights`, filters by `FlightDescriptor.path[0] == schema`, deduped Vec<String> of the second path segment. `flight_get_table_schema` (`d40888326`) — `get_schema(FlightDescriptor::new_path([schema, table]))` → decode IPC bytes via `ArrowSchema::try_from(&SchemaResult)` → `Option<Arc<ArrowSchema>>`, None on connect/RPC/decode failure. `flight_get_splits` (`ba124601d`) — `get_flight_info(FlightDescriptor { path=[schema, table], cmd=JSON(constraint) })` → decode each `endpoint[i].ticket.ticket` bytes via `serde_json::from_slice::<TrinoSplit>` → `Vec<TrinoSplit>`, empty-Vec on connect/RPC failure, individual bad tickets skipped silently. `TrinoPageSource::get_next_page` (`3afe04d00`) — first streaming RPC: `do_get(Ticket { ticket: JSON(split) })` → wrap response in `arrow_flight::decode::FlightRecordBatchStream` → pull next `RecordBatch` → encode each column as a single-column stream-IPC `TrinoBlock` → `TrinoPage`. Connect / RPC failures mark the source `finished` immediately; per-batch decode/encode failures stop iteration cleanly. Signature: `pub fn new(split, schema) -> Self` → `pub async fn new(endpoint, split, schema) -> Self` and `pub fn get_next_page(&mut self) -> Option<TrinoPage>` → async. Test fixture `cc47a7d3e` (extended `d40888326` + `ba124601d` + `3afe04d00`) adds in-process tonic mock at `tests/connectors_flight_contract.rs::trino_flight_pilot` (~330 LOC; reusable for the remaining 2 page-sink helpers); 10 tests cover happy-path + unreachable-endpoint fallback for the 5 live methods. Flight gate: 5 → 15 passing. The structural ticket / descriptor contract gates remain intact.
| 1-2 weeks remaining (2 page-sink helpers × per-method migration)
| Acceptance: (1) **DONE 2026-06-02** for `flight_list_schemas` + `flight_list_tables` + `flight_get_table_schema` + `flight_get_splits` + `TrinoPageSource::get_next_page`; remaining 2 are the Page-sink streaming ops: `TrinoPageSink::{append_page, finish}` (do_put streaming, server-acknowledged via `PutResult` per write). Migration shape mirrors the page-source side but on the write path. (2) **DONE 2026-06-02** — `MockFlight` fixture at `tests/connectors_flight_contract.rs::trino_flight_pilot` is reusable; `get_schema` + `get_flight_info` + `do_get` are all wired (the `do_get` mock streams `FlightData` messages built via `arrow_flight::utils::batches_to_flight_data`); extend with `do_put` (client-streaming `FlightData` + server-streaming `PutResult`) for the remaining helpers. (3) Remove the scaffold marker on the module in `src/connectors/trino.rs` once the last method migrates. (4) **DONE** — `connectors-contract-gates` / `flight` cell runs the pilot tests.

| TD-099
| v2 OpenAPI lacks a records-scan / list endpoint Hadoop needs
| P3
| LOW
| Partial — spec + handler stub + Hadoop SDK + contract gate landed 2026-06-01 (`6b79cce23`)
| Surfaced 2026-05-30 during the connector-conformance sprint. The Hadoop connector's `ProximaRecordReader::next_record()` is a sync `-> bool` API that Hadoop's InputFormat contract calls in a loop to drain records from a split. The v2 OpenAPI spec had NO endpoint that scans records — only POST `/search` with `top_k` (similarity search, wrong semantics for unindexed table scan), GET `/records/{id}` for point reads, POST `/records/batch` for inserts. **PARTIAL CLOSE 2026-06-01** (`6b79cce23`): spec now declares POST `/api/v2/collections/{collection_id}/records/scan` (operationId `scanRecords`) with `{cursor?, limit?, filter?, include_vector?, include_text?}` → `{records[], next_cursor?, scanned_count?}` (ScanRecords{Request,Response} schemas, both optional everywhere, snake_case). Server has a stub handler at `src/network/rest/v2/records.rs::scan_records` that returns an empty page (`{records: [], next_cursor: None, scanned_count: Some(0)}`); Hadoop SDK has `ProximaRecordReader::fetch_next_page()` (async) which POSTs `{cursor, limit: 1000}` and stores the continuation token. Contract gate `hadoop_fetch_next_page_posts_v2_records_scan` asserts the spec lookup + body shape + mock round trip.
| 1 week remaining (real server-side scan delegation + sync-bridge for Hadoop)
| Acceptance: (1) **DONE 2026-06-01** — spec POST `/api/v2/collections/{id}/records/scan` with `{cursor?, limit?, filter?}` → `{records, next_cursor?}`. (2) Replace the stub `scan_records` handler with `RecordScan` trait delegation: iterate the collection's storage engine, honour `cursor` as a (file_id, offset) opaque token, page at `limit`. (3) Reimplement `ProximaRecordReader::next_record()` to drive the async `fetch_next_page()` via `tokio::runtime::Handle::block_on` since Hadoop's calling convention is sync. (4) **DONE 2026-06-01** — contract gate test added at `tests/connectors_openapi_contract.rs::hadoop_fetch_next_page_posts_v2_records_scan`. Total connectors REST gate: 5 → 6 passing.
...
| LOW
| Open — target v0.2
| CONVERGENCE_AUDIT S2. `src/network/grpc/{entity,document,graph,security,hybrid_search,streaming,observability}_service.rs` carry `// DEPRECATED … removed in version 0.3.0` markers over the migrated impls in `crates/platform/proximadb-api/src/grpc/v1/`. Land in v0.2: pull the removal forward and update/remove the stale `0.3.0` markers.
| 1 week (v0.2)
| Acceptance: (1) Shims deleted in v0.2; the `0.3.0` removal markers updated/removed. (2) No remaining importers. (3) gRPC tests green against the platform-crate impls.
...
| MEDIUM
| Open — convergence
| CONVERGENCE_AUDIT S3. `VectorRecord` / `SqlValue` / `SqlObject` (proto v1) are still used internally; canonical is `ProximaRecord` / `ProximaValue` / `ProximaType`. Keep proto v1 for the wire only; migrate internal usage one handler file at a time, leaving parse→convert edge adapters in place.
| 2-3 sprints
| Acceptance: (1) Internal code paths use canonical types; proto v1 confined to wire adapters. (2) No internal `VectorRecord`/`SqlValue` construction outside edge adapters. (3) Tests green.
