...
| MEDIUM
| Open
| Surfaced 2026-05-30 during the connector-conformance sprint. `src/connectors/spark.rs` provides 7 `jni_*` functions (get_table_schema, plan_input_partitions, create_partition_reader, read_next_batch, close_partition_reader, create_data_writer, write_batch, commit_writer, abort_writer) that compile but return placeholder values (empty JSON, empty Vec, 0). They cannot be exercised by the Rust contract-gate pattern used for DuckDB / Hadoop (`tests/connectors_openapi_contract.rs`) because the JNI ABI requires a running JVM + a Spark DataSource V2 test harness on the Java side. Module-level scaffold note landed in commit `3ff11106d`.
| 1-2 weeks
| Acceptance: (1) Stand up a `clients/jvm/spark-connector` Gradle test module that loads the native library and exercises each `jni_*` via JNA / panama-foreign. (2) Each `jni_*` constructs the underlying `ConnectorStorageAdapter` call and serializes the result as Arrow IPC / JSON. (3) Per-method conformance test (Java-side junit) that asserts the JNI round trip against the OpenAPI spec where applicable (schema fetch hits `getCollectionSchema` etc.). (4) Wire into CI as a new `spark-connector-jvm` job. Until then, the Rust-side `jni_*` functions stay scaffolds and the module-level comment in `src/connectors/spark.rs` calls out the gap.
