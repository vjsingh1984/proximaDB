...
| TD-098
| Trino connector — 5 of 7 Flight methods still scaffold
| P3
| MEDIUM
| Partial — 2 of 7 live as of 2026-06-01 (`f3284b9ba` + `e1f622fac`)
| Surfaced 2026-05-30. The Trino connector at `src/connectors/trino.rs` had 7 scaffold `flight_*` methods. **PARTIAL 2026-06-01**: 2 of 7 methods live. `flight_list_schemas` (`f3284b9ba`) — async, dials `Channel::from_shared(...).connect_lazy()` + `FlightServiceClient::new(channel)` (tonic ≥0.12 removed the convenience `connect()` shorthand) → `list_flights`, buckets `FlightInfo.flight_descriptor.path[0]` into sorted `TrinoSchema` list, normalizes `grpc://` → `http://` for tonic, empty-Vec on connect/list failure. `flight_list_tables` (`e1f622fac`) — same dial pattern, filters the `list_flights` stream by `FlightDescriptor.path[0] == schema` and returns deduped Vec<String> of the second path segment. Test fixture `cc47a7d3e` adds in-process tonic mock at `tests/connectors_flight_contract.rs::trino_flight_pilot` (~140 LOC; reusable for the remaining 5 helpers); 4 tests cover happy-path bucketing + schema filter + unreachable-endpoint fallback for both methods. Flight gate: 5 → 9 passing. The structural ticket / descriptor contract gates remain intact.
| 1-2 weeks remaining (5 helpers × per-method migration)
| Acceptance: (1) **DONE 2026-06-01** for `flight_list_schemas` + `flight_list_tables`; remaining 5: `flight_get_table_schema`, `flight_get_splits`, `TrinoPageSource::get_next_page`, `TrinoPageSink::{append_page, finish}` follow the same `Channel::from_shared(...).connect_lazy()` + `FlightServiceClient::new` + dial-relevant-RPC pattern. (2) **DONE 2026-06-01** — `MockFlight` fixture at `tests/connectors_flight_contract.rs::trino_flight_pilot` is reusable; extend it with `get_schema` / `do_get` / `do_put` canned responses per method. (3) Remove the scaffold marker on the module in `src/connectors/trino.rs` once the last method migrates. (4) **DONE** — `connectors-contract-gates` / `flight` cell runs the pilot tests.
