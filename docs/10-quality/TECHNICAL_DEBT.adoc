...
| 3-4 weeks
| **Verified**: April 25, 2026. **Important rename**: earlier TD draft called this "Evolutionary Semantic Joins" — that was wrong. The paper proposes **block nested loops with batched prompts**, not evolutionary sampling. Different cost model and complexity analysis. **Status (scaffolding closed April 27, 2026)**: cosine semantic join still at `src/query/unified/executor.rs::execute_semantic_join`. Mode dispatch landed in `dispatch_semantic_join`; AST in `src/query/unified/ast.rs` now has `JoinType::Semantic { threshold, top_k, mode: SemanticJoinMode }` where `SemanticJoinMode::Cosine` is the default (backward-compat regression test pinned) and `SemanticJoinMode::LlmBlockBatch(BlockBatchConfig)` is the new paper-faithful mode. `BlockBatchConfig { batch_size_left, batch_size_right, max_calls }` with `validate()` rejecting zero-sized fields in deterministic order; conservative defaults (16/16/64) tuned for an 8K-token context model. Feature flag `llm-joins` (default off) added in `Cargo.toml`. `execute_block_batch_semantic_join` is in tree: when feature is OFF, logs a clear `warn!` and returns an empty `JoinResult` with the original left rows in `unmatched_left` so callers can fall back; when feature is ON, computes block layout (`blocks_left × blocks_right`, capped at `max_calls`), logs the planned shape, and returns empty pending the LLM-client integration. 12 inline tests cover the contract: default mode, structural equality including mode, valid defaults, three independent zero-field rejections, deterministic-order multi-zero validation, no-panic empty result with feature off, no-panic empty result on invalid config, cosine remains default dispatch (regression guard against accidentally flipping the default to LlmBlockBatch which would silently degrade every existing semantic join). Both feature-on and feature-off configurations compile and test green. **Remaining work**: (1) wire an actual LLM client to the LlmBlockBatch path -- the paper's prompt-packing format (block nested loops with row batches in one prompt asking the model to identify all matching pairs); (2) implement the paper's optimal batch-size formula given configured LLM context window; (3) adaptive variant for cases where output size is unpredictable; (4) Prometheus metric for LLM call count. **Acceptance**: ≥10× LLM-call reduction vs nested-loop baseline at equal precision@10; bounded and observable. See `PROXIMADB_RECOMMENDATIONS.md` §3.3.
...
* TD-R25..TD-R27 added May 8, 2026. Phase A/B/E foundation work completed via TDD: type system unification (`ProximaType` Arrow+pgwire), `ProximaRecord` spec §3 envelope (`proximadb-records` crate), and engine-level RLS scaffold (`rls_record_filter()` hook in `UnifiedStorageEngine`, overridden in SST + VIPER). All three items resolve the core of TD-054 as defined in `MULTIMODAL_OVERHAUL_SPEC_2026_05_08.adoc`. Remaining multi-model phases (C query algebra, D three-layer storage, F workspace skeleton) are in-progress.

_Last Updated_: May 8, 2026
