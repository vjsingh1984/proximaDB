...
| MEDIUM
| Open
| Surfaced 2026-05-30 after reconciling the OpenAPI graph schemas (`docs/openapi/proximadb-openapi.yaml`) against the live server handler `crates/platform/proximadb-api/src/rest/v1/graph.rs`. The earlier spec additions had wrong field names / wrapping for `CreateGraphRequest` (required `name` → now `graph_id`), `CreateNodeRequest` (flat `{id, ...}` → wrapped `{node: NodeInput}`), `CreateEdgeRequest` (flat `{source, target, ...}` → wrapped `{edge: EdgeInput}` with `from_node_id` / `to_node_id` / `edge_type` / `id`), and `TraverseRequest` (had `direction` / `max_nodes` → server-true keys are `start_node_id` (not `start_node`) + `node_labels` + `limit`). The Rust and TypeScript SDK graph builders (`clients/rust/src/graph.rs`, `clients/nodejs-embedded/src/graph.ts`) still emit the *pre-correction* bodies — e.g. TS `GraphBuilder.execute()` posts `{name, description}` instead of `{graph_id, name?, description?}`, and `NodeBuilder.execute()` / `addNodes()` post a batched `{graph, nodes: [...]}` envelope instead of the spec-required `{node: NodeInput}` singleton. URL + method routes are correct against the v2 spec; only the JSON body shapes are wrong. The contract gates in `clients/nodejs-embedded/tests/openapi_contract.test.ts` (4 graph ops) and `clients/rust/tests/openapi_contract.rs` (analogous Rust ops) are `it.skip` / `#[ignore]` pending the SDK body-shape rewrite to avoid red CI while the spec is the durable source of truth.
| 1-2 weeks
| Acceptance: (1) `clients/nodejs-embedded/src/graph.ts` `GraphBuilder.execute()` posts `{graph_id, name?, description?}`; `NodeBuilder.execute()` and `GraphHandle.addNodes()` either post per-node `{node: NodeInput}` (matching spec) or expose a separate batch builder that posts to a batch endpoint the server actually exposes (graph handler in `crates/platform/proximadb-api/src/rest/v1/graph.rs` does NOT have a batch endpoint today — confirm before re-shaping); `EdgeBuilder.execute()` / `addEdges()` mirror with `{edge: EdgeInput}` using `from_node_id` / `to_node_id` / `edge_type` / `id`; `TraversalBuilder.execute()` posts `start_node_id` (not `start_node`), drops `direction` / wraps `relationships` as `edge_types`, and adds optional `node_labels`. (2) Same rewrite on the Rust SDK at `clients/rust/src/graph.rs`. (3) Unskip the four graph contract tests (TS) and any Rust analog; assert spec-required keys present in the captured body. (4) Add a per-language live-server smoke (insert node, get node, traverse) so URL+body+server actually exchange happy-path data. Coordinates with the Rust agent's parallel work on the same drift.
|===
