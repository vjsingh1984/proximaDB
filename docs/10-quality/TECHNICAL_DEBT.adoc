...
| TD-092
| BM25↔vector hybrid fusion — wire the live Tantivy full-text leg into the fusion path
| P2
| MEDIUM
| Open
| **Near-term parity (un-wired primitive, not new build).** Tantivy full-text indexing is production (`src/storage/document/indexes/fulltext.rs`) and the RRF/weighted/8-strategy fusion engine is production (`src/core/search/hybrid/hybrid_search.rs`), but `src/core/search/hybrid/bm25_wrapper.rs` is a *stub* — so the BM25 leg never reaches the fusion path and "hybrid search" fuses dense vectors only. Zilliz markets "full-spectrum search across vectors, text, JSON, geospatial with hybrid retrieval and reranking" as table stakes, so this is a visible competitive-parity gap that is nearly free (integration of two finished components). Also corrects `VECTOR_LAKEBASE_ALIGNMENT_2026_05_28.adoc`, whose scorecard prematurely lists "hybrid BM25 + RRF/weighted fusion" as shipping (✅) when the BM25 leg is not wired.
| 1-2 weeks
| Surfaced 2026-05-28 (plan refinement). Acceptance: (1) `bm25_wrapper` backed by the live Tantivy index rather than a stub; (2) hybrid search fuses BM25 + dense candidates through the existing fusion engine (RRF default); (3) integration test asserting a keyword-only match surfaces via hybrid search that a pure-vector query misses. Sequence *before* Phase 8 (Parity Sprint).
...
| MEDIUM
| Open
| **Near-term parity (un-wired primitive, not new build).** The SQL `RERANK` SRF, the ONNX model cache, and 8 of 9 §4.10 rank metrics are live end-to-end (`crates/modalities/proximadb-rank-*`), but no production `CandidateProvider` / `RankServices` is constructed at server boot — so the RERANK path degrades to retrieval-only without profile dispatch. It needs a `HybridSearchBackend`-backed provider wired into `SharedServices`. Integration of finished primitives; previously deferred to "a fresh session" (see memory `project-rank-services-production-wiring-2026-05-28`).
| 1-2 weeks
| Surfaced 2026-05-28 (plan refinement). Acceptance: (1) `RankServices` constructed at startup over the hybrid backend and registered in `SharedServices`; (2) SQL `RERANK` dispatches rank profiles end-to-end across REST / gRPC / Arrow Flight (not retrieval-only); (3) the 9th metric (async loader, R-5b) closed or explicitly tracked. Sequence *before* Phase 8 (Parity Sprint). Pairs with TD-092 — both feed F1 re-embed/quality passes downstream.
