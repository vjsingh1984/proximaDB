...
| TD-098
| Trino connector — 3 of 7 Flight methods still scaffold
| P3
| MEDIUM
| Partial — 4 of 7 live as of 2026-06-01 (`f3284b9ba` + `e1f622fac` + `d40888326` + `ba124601d`)
| Surfaced 2026-05-30. The Trino connector at `src/connectors/trino.rs` had 7 scaffold `flight_*` methods. **PARTIAL 2026-06-01**: 4 of 7 methods live, all using the same dial template: `grpc://` → `http://` normalize → `Channel::from_shared(...).connect_lazy()` + `FlightServiceClient::new(channel)` (tonic ≥0.12 removed the convenience `connect()` shorthand). `flight_list_schemas` (`f3284b9ba`) — `list_flights`, buckets `FlightInfo.flight_descriptor.path[0]` into sorted `TrinoSchema` list, empty-Vec on connect/list failure. `flight_list_tables` (`e1f622fac`) — `list_flights`, filters by `FlightDescriptor.path[0] == schema`, deduped Vec<String> of the second path segment. `flight_get_table_schema` (`d40888326`) — `get_schema(FlightDescriptor::new_path([schema, table]))` → decode IPC bytes via `ArrowSchema::try_from(&SchemaResult)` → `Option<Arc<ArrowSchema>>`, None on connect/RPC/decode failure. `flight_get_splits` (`ba124601d`) — `get_flight_info(FlightDescriptor { path=[schema, table], cmd=JSON(constraint) })` → decode each `endpoint[i].ticket.ticket` bytes via `serde_json::from_slice::<TrinoSplit>` → `Vec<TrinoSplit>`, empty-Vec on connect/RPC failure, individual bad tickets skipped silently. Test fixture `cc47a7d3e` (extended `d40888326` + `ba124601d`) adds in-process tonic mock at `tests/connectors_flight_contract.rs::trino_flight_pilot` (~210 LOC; reusable for the remaining 3 page-source/sink helpers); 8 tests cover happy-path + unreachable-endpoint fallback for the 4 live methods. Flight gate: 5 → 13 passing. The structural ticket / descriptor contract gates remain intact.
| 1-2 weeks remaining (3 page-source/sink helpers × per-method migration)
| Acceptance: (1) **DONE 2026-06-01** for `flight_list_schemas` + `flight_list_tables` + `flight_get_table_schema` + `flight_get_splits`; remaining 3 are the Page-source / sink ops: `TrinoPageSource::get_next_page` (do_get streaming), `TrinoPageSink::{append_page, finish}` (do_put streaming). Migration shape differs slightly because these are streaming RPCs. (2) **DONE 2026-06-01** — `MockFlight` fixture at `tests/connectors_flight_contract.rs::trino_flight_pilot` is reusable; `get_schema` + `get_flight_info` are both wired; extend with `do_get` (page reads, streaming) / `do_put` (page writes, streaming) for the remaining helpers. (3) Remove the scaffold marker on the module in `src/connectors/trino.rs` once the last method migrates. (4) **DONE** — `connectors-contract-gates` / `flight` cell runs the pilot tests.
