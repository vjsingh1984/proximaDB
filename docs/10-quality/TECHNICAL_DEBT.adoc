...
| TD-168 | Graph topology RAM-resident adjacency index + batched frontier | HIGH | Open | D1 | PERF
| **ADR-034 / cost-spine P5 (the adversarial workload).** Graph edges are co-located canonical columns in cold PAX blocks (`EDGE_SRC/TGT/TYPE/WEIGHT`, `crates/storage/proximadb-block-format/src/record.rs:34-37`) — good for "read a node *and* its edges in one block GET", but **traversal pointer-chases through object storage**: a k-hop walk is `k` serial round-trips (1s+ floor at ~250 ms/hop) because each frontier's adjacency must be read from cold blocks before the next hop. Vectors get a separate index (AXIS); the graph gets none. Extract a small **RAM/SSD-resident adjacency index** (src→[tgt ids], IDs only — tiny vs payloads) so frontier expansion is RAM pointer-chasing, then **one** batched `read_ranges` fetches the frontier's node payloads — turning `O(k·frontier)` round-trips into `O(k)`. Largest *architectural* gap relative to the cost model. **Verify:** k-hop traversal round-trip count drops to `O(k)`; correctness vs the co-located-edge path. See ADR-034.
