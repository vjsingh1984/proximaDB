...
| TD-156 | Configurable PAX block / row-group / stripe geometry | MED | Open | D1 | PERF
| Block/row-group/stripe size are the co-design I/O-granularity tunables; the writer takes a fixed `target_block` today. Make them per-collection/per-deployment configurable (same seam as TD-155); default by storage class — small (tens of KiB) for local/embedded, large (8-16 MiB row groups) for object store (the TD-151 coalescing lever). See ADR-026.
