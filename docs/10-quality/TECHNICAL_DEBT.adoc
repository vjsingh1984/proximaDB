...
| TD-096
| Write-time Z-order spatial sort for SST block layout (block-pruning recall fix)
| P2
| HIGH
| Open
| Surfaced 2026-05-30 via `bench_warm_path_profile` shadow recall measurement (`flat_default_vs_force_exact` field, committed in `403fd1ba1`). The SST block layer assumes spatial locality — records inside a block are nearby in vector space, so the block centroid is a meaningful summary that lets centroid-based pruning skip blocks confidently. **The current implementation breaks this assumption at write time**: `sort_vectors_for_sstable_encoding` (`src/storage/engines/sst/utils.rs:43`) sorts records by `oid` lexicographically before block write, and compaction's `write_merged_records` (`src/storage/engines/sst/compactor_impl.rs:348`) preserves merge order without re-sorting. Block centroids end up as statistical mixtures of unrelated records rather than true cluster centers. Empirically measured at 100K × 128d random cosine vectors with the default Sqrt block-pruning mode: `flat_default_vs_force_exact ≈ 0.05` (5% recall vs literal brute-force scan). PCA training exists (`src/storage/engines/sst/pca_manager.rs`) and Z-order encoding exists, but neither is consulted by the write-time sort. Mitigated by `SearchMode::Exact` auto-disabling pruning (`403fd1ba1`), but `SearchMode::Approximate` still degenerates at scale — customers wanting >0.5 recall must use the AXIS index path instead of relying on flat block pruning. Bench scaling table: 10K = ~1.0 (under MIN_BLOCKS_FOR_PRUNING=100 threshold, no pruning fires); 25K = ~0.95; 100K = ~0.05.
| 1-2 weeks
| Acceptance: (1) `sort_vectors_for_sstable_encoding` (and the compaction merge path) sort records by spatial Z-order code derived from the PCA-projected vector, not `oid`. (2) The per-collection PCA model is trained on the first flush's data and reused for subsequent flushes; compaction may retrain. (3) Block centroids reflect spatial clusters — measurable via a new test that asserts within-block max-distance is bounded relative to between-block centroid distance. (4) `bench_warm_path_profile` `flat_default_vs_force_exact >= 0.90` at 100K with the default `BlockPruneMode::Sqrt` (matches the 95-97% recall the centroid-pruning algorithm achieves on properly-organized data). (5) Build-time regression < 30% (Z-order sort is O(N log N), same complexity as the existing oid sort; PCA projection is O(N × d × k) where k is the PCA component count, typically small). (6) No public API surface changes — existing `SearchMode::Exact` callers continue to bypass pruning; the fix only makes `SearchMode::Approximate` recall meaningful at scale.
|===
