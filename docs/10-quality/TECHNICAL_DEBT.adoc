...
| Gate is green (recall@10 0.932 @N=100k, PR #331; mixed-read-safe proven, PR #335), so the flip is unblocked. Staged: ship default-OFF one release (env `PROXIMADB_PAX_VECTOR_SEGMENTS`+`QUANT`), then flip the write default to PAX+RaBitQ. Leave a kill-switch. Storage-format mandate #8 (mixed-read-safe, default-off until baked, recall-gated). See ADR-026.
...
| `src/automl/` + AXIS `adaptive_engine` ARE wired + running, but audited 2026-06-26 as (a) tuning ONLY HNSW/IVF index params (`ef_search`/`nprobe`/`M`/`nlist`) from latency heuristics (p50/p99), (b) NOT consuming `io_trace`, (c) NOT tuning storage geometry (block/row-group) or quant tier. So no model can refine geometry/quant today. Extend it to consume `io_trace` (TD-158) and add geometry (TD-156 target_block) + `VectorQuant` to its tunable set; then it is the substrate for TD-157 and the future RL layer. See ADR-026 tuning hierarchy.
| TD-160 | Compile-time gate for the perf/geometry trace class only (billing stays always-on) | HIGH | Open | D1 | PERF
| ADR-027 two-class trace. Add a Cargo feature (`io-trace`, default OFF) that makes the perf/geometry `record_*` fns compile to inline no-ops when off (zero per-read cost in normal mode). CRITICAL: the gate applies ONLY to the perf/geometry class — the BILLING meters (KEU/KOU/KRU/KIU) are always-on, full-fidelity, never gated. Compact encoding: u64 counters + int enums (replace the per-query `route`/`compute_ms` `String` fields with int enums). `instrument`/`IoTrace`/snapshot stay unconditional (consumers get a zero snapshot when off).
| TD-161 | Coalesced SST/LSM-backed metering + trace writer (dual-sink) | MED | Open | D1 | PERF
| ADR-027. Both trace classes are write-heavy small-record workloads (per-query meters) — writing each to object store = the small-files problem (per-PUT fee + read amplification). Reuse the SST/LSM machinery (memtable -> threshold-flush a large PAX segment -> background compaction), the Aurora/Neon/Turso pattern. Billing = full-fidelity but BATCHED (N meters coalesced into one segment, not sampled); perf = may sample/aggregate. Dual-sink: durable per-tenant under `data/{tenant}/_metering/billing/` + `_trace/perf/` (the ProximaDB differentiator — peers meter externally only) AND external OTLP/Prometheus -> AnvaiOps (industry standard).
| TD-162 | PAX as the single internal format (collapse the storage fork) | HIGH | Open | D1 | ARCH
| ADR-027 Decision 1. Today: PAX (vectors) + Parquet (warehouse/relational, DataFusion) + ProximaBlocks (legacy) = a storage fork. Unify: SST/LSM writes PAX for ALL modalities; one record reader (`read_segment_records` -> PaxSegmentScanner); one SQL lowering (relational SQL -> PAX scan; native OLAP engine scans PAX, DataFusion interim); one I/O-trace path. Parquet demoted to the interop seam only (external/Iceberg/federated/export via the magic-detect router + RecordReader trait + Arrow-Flight IPC). Mixed-read-safe (mandate #8). Completes ADR-024 (type unified -> format unified). Phased with the native-OLAP engine (NATIVE_OLAP_ENGINE_BLUEPRINT).
