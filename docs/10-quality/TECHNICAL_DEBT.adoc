...
| 3-4 months
| *Implemented (engine), Mar 2026.* **v0.2 support tier: _Experimental in v0.2 (see SUPPORTED_SURFACE)_** -- external-table / external-catalog execution is not a v0.2 supported surface; do not market it as supported or production. _Implementation detail:_ all major catalog backends are code-complete:
|  |  |  |  |  |
|---|---|---|---|---
...
| 1 month
| *Implemented (engine), Mar 2026.* **v0.2 support tier: _Beta in v0.2 (see SUPPORTED_SURFACE)_** -- the security runtime is Beta; admin/status APIs and stronger online revocation policy remain incomplete. _Implementation detail:_ enterprise-grade mutual TLS infrastructure is code-complete:
|  |  |  |  |
|---|---|---|---
...
| 1-2 months
| *Implemented (SDK), Mar 2026.* **_Not a v0.2 supported surface (see SUPPORTED_SURFACE)_** -- framework integrations ship in the Python SDK but are not listed as a Supported v0.2 product surface; do not release-note them as production. _Implementation detail:_ 7 frameworks implemented:
|  |  |  |  |
|---|---|---|---|---
...
| P1/P4
| MEDIUM
| Open — characterization needed
| Surfaced 2026-06-14 while disproving the long-claimed "v2 INSERT→search-index flush gap". `tests/v2_post_flush_search_e2e.rs` proves *correctness* — v2 vector search (`enable_proxima_record=false`) and metadata-filtered scan (`enable_proxima_record=true`) both still return the right records after a real `ProximaDB::force_flush_collection` (WAL drain + `compact_collection`). What that test does NOT establish is whether post-flush retrieval is served by a *populated ANN index* (HNSW/IVF over the SST/PAX segments) or by a brute-force scan over flushed storage. At small N the two are indistinguishable; at scale the difference is recall + tail latency, not correctness. Open question: does the flush/compaction path register flushed vectors with the per-collection AXIS/ANN index so cold (flushed) segments retain target recall, or does recall silently degrade toward brute-force as data ages out of the WAL memtable? Scope spans the specialized engines (SST today; HELIX/NOVA), real object storage (S3/ADLS/GCS) reads, multi-flush + compaction churn, and post-restart recovery reload. This is a *performance/recall* concern and belongs to the recall-band track, not the (now-complete) hybrid metadata-filter work.
...
| P1
| HIGH (SaaS isolation)
| Partially mitigated (2026-06-15) — input validation landed; isolation still best-effort
| Surfaced 2026-06-15 by the new DrPathBuilder tenant-path guard (`scripts/check_tenant_path_guard.py`). `DmlService::materialize_table_to_parquet` (`src/services/dml/mod.rs`) builds the object prefix `data/{tenant}/{ns...}/{table}` by hand instead of routing through `DrPathBuilder::build`, and the production trigger (`DmlTableMaterializer::materialize`, the `ALTER TABLE … MATERIALIZE` path) passes `None` for `TenantContext` → everything materializes under the literal `DEFAULT_TENANT_PLACEHOLDER` ("default_tenant"). So warehouse snapshots are NOT tenant-isolated on this path. DONE (interim hardening, `e93dcf91d`): every segment (tenant_id / each namespace level / table) is now validated via the canonical `DrPathBuilder::validate_id` (no `..`/separator/whitespace/NUL injection into the key), and the default is a named, documented constant. BLOCKED on two things for the real fix: (a) the native catalog leaves `CatalogNamespace.namespace_id`/`tenant_id` = `None` (pending the P0.5 backfill migration), so `DrPathBuilder::build` would fail-closed today; (b) the DDL `TableMaterializer` trait carries no `TenantContext`, so request tenancy never reaches the writer.
| ~1 sprint (after namespace-id backfill)
| Acceptance: (1) thread a real `TenantContext` from the DDL/DML entrypoints through `TableMaterializer::materialize` into `materialize_table_to_parquet` (no silent default in multi-tenant mode). (2) Once namespace-id backfill lands, route the prefix through `DrPathBuilder::build`/`build_for_pool` and use `DrResolvedPath::root_prefix()` rather than a `format!` literal; remove the guard's `KNOWN_BYPASSES` entry. (3) Assert the materialized object's owning-namespace tenant matches the request tenant (reject cross-tenant materialize). (4) e2e proving two tenants materializing the same table name land under distinct, isolated prefixes.
|===
