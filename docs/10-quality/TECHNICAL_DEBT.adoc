...
| P3
| MEDIUM
| Partial — pilot landed 2026-06-01 (`f3284b9ba` + `cc47a7d3e`)
| Surfaced 2026-05-30. The Trino connector at `src/connectors/trino.rs` had 7 scaffold `flight_*` methods. **PARTIAL 2026-06-01**: `flight_list_schemas` is live (`f3284b9ba`) — async, dials `FlightServiceClient::connect` → `list_flights`, buckets `FlightInfo.flight_descriptor.path` into sorted `TrinoSchema` list, normalizes `grpc://` → `http://` for tonic, empty-Vec on connect/list failure. Test fixture `cc47a7d3e` adds in-process tonic mock at `tests/connectors_flight_contract.rs::trino_flight_pilot` (~140 LOC; reusable for the other 6 helpers); 2 new tests cover happy-path bucketing + unreachable-endpoint fallback. Flight gate: 5 → 7 passing. The structural ticket / descriptor contract gates remain intact.
| 1-2 weeks remaining (6 helpers × per-method migration)
| Acceptance: (1) **DONE 2026-06-01** for `flight_list_schemas`; remaining 6: `flight_list_tables`, `flight_get_table_schema`, `flight_get_splits`, `TrinoPageSource::get_next_page`, `TrinoPageSink::{append_page, finish}` follow the same `FlightServiceClient::connect` + dial-relevant-RPC pattern. (2) **DONE 2026-06-01** — `MockFlight` fixture at `tests/connectors_flight_contract.rs::trino_flight_pilot` is reusable; extend it with `get_schema` / `do_get` / `do_put` canned responses per method. (3) Remove the scaffold marker on the module in `src/connectors/trino.rs` once the last method migrates. (4) **DONE** — `connectors-contract-gates` / `flight` cell runs the pilot test.
