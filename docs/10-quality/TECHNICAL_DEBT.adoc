...
| HIGH
| Open
| L2 (NVMe) and L3 (Network) cache backends are defined as trait interfaces but not implemented (TODO stubs). `MemoryBackend` returns `StorageError::CapacityExceeded` when full with no fallback. DuckDB spills to temp directory; Polars uses streaming partitioned execution. ProximaDB OOMs on larger-than-memory analytical queries.
| 3-4 weeks
| Implement L2 NVMe backend + external merge sort for ORDER BY + hash join spill.
...
| MEDIUM
| Open
| ProximaCodec supports 7 encoding schemes (Raw, BitPacked, Delta, FOR, PatchedBase, Dictionary, RunLength) but codec is selected statically, not adaptively per segment. DuckDB samples each segment, tries multiple codecs, picks best compression ratio. Same timestamp column uses same codec whether sorted or random.
| 2-3 weeks
| 20-40% potential storage reduction for mixed workloads.
...
| MEDIUM
| Open
| `src/compute/distance_computation/engine.rs` uses unroll-4 instruction pipelining (good) but computes each vector distance individually against query. No matrix transposition for true multi-vector SIMD. For 128-dim vectors with AVX2, matrix approach could yield 2-4x improvement.
| 2 weeks
| Optimization, not critical path. Current approach is functional and uses SIMD well.
...
| HIGH
| Open
| 2PC protocol defined in `src/storage/multimodel/transaction/` but no 2PC participants are implemented for individual stores. Each modality (vector, document, graph, observability) handles transactions independently. Cross-modal queries have no consistency guarantees. Cannot claim ACID for cross-modal operations.
| 4-6 weeks
| Required for "unified database" positioning. Define per-store 2PC participants, wire into federated executor.
...
| MEDIUM
| Open
| Row group metadata does not include quantized vector bounds (min/max per quantization level). Cannot skip row groups based on approximate distance from query vector. Current: read full vectors → quantize → compute distance. Better: check quantized bounds → skip row group if estimated distance exceeds threshold.
| 2-3 weeks
| 5-20x improvement for filtered vector searches on large collections.
