...
**Status 2026-05-27 (slices 1-3 landed)**:
...
* 9 new TD-064 tests (218 AXIS tests pass).

**Remaining work**:

* Plumb `AxisManagerQueryResult.predicate_shortfall` through `services/operations/vectors` and `RichSearchResponse` so REST/gRPC handlers can populate `TraceBuilderInputs.predicate_shortfall` (currently always `None` at the handler).
* IVF inline predicate pushdown (replace oversample-and-postfilter with nprobe-time predicate evaluation) — TD-064 Phase 3.
* PreFilter / route-selection mode wiring through `ADR-011 AnnFilteringPolicy::routing_mode()` rather than the current `AnnFilteringMode::Inline` switch.
* Non-AXIS index deletion (`src/index/{hnsw,ivf,diskann,sparse_hnsw}` orphans — see `docs/12-design/NON_AXIS_INDEX_MIGRATION_AUDIT_2026_05_27.adoc`).
