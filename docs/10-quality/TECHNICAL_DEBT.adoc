...
| P5
| MEDIUM
| *Implemented — needs benchmark + corrected positioning*
...
| 3-4 weeks (1-2 weeks remaining for benchmark + SDK)
| **Verified**: April 25, 2026. Paper data: **RRF wins relevance** (nDCG@10 = 0.828, best of 6 configs); B5 is the speed/diversity option. Earlier TD framing as a strict upgrade was wrong — downgraded from HIGH to MEDIUM. **Status**: Implemented April 2026. **Where**: `src/core/search/hybrid/fusion.rs:106-253`; `src/core/search/hybrid/mod.rs:213-228`. Tests at `fusion.rs:2074-2160`. **Remaining work**: (1) benchmark RRF vs Projection on MS MARCO/BEIR — confirm relevance/speed/diversity tradeoff; (2) docs framing it as "default RRF for relevance, choose Projection for diversity or low-latency fusion"; (3) SDK opt-in only — do NOT default to Projection; (4) REST hybrid endpoint exposure. **Acceptance**: benchmark report shows tradeoff; SDK exposes `hybrid_mode="projection"`; default remains `"rrf"`. See `PROXIMADB_RECOMMENDATIONS.md` §1.2.
