...
| TD-098
| Trino connector — 4 of 7 Flight methods still scaffold
| P3
| MEDIUM
| Partial — 3 of 7 live as of 2026-06-01 (`f3284b9ba` + `e1f622fac` + `d40888326`)
| Surfaced 2026-05-30. The Trino connector at `src/connectors/trino.rs` had 7 scaffold `flight_*` methods. **PARTIAL 2026-06-01**: 3 of 7 methods live, all using the same dial template: `grpc://` → `http://` normalize → `Channel::from_shared(...).connect_lazy()` + `FlightServiceClient::new(channel)` (tonic ≥0.12 removed the convenience `connect()` shorthand). `flight_list_schemas` (`f3284b9ba`) — `list_flights`, buckets `FlightInfo.flight_descriptor.path[0]` into sorted `TrinoSchema` list, empty-Vec on connect/list failure. `flight_list_tables` (`e1f622fac`) — `list_flights`, filters by `FlightDescriptor.path[0] == schema`, deduped Vec<String> of the second path segment. `flight_get_table_schema` (`d40888326`) — `get_schema(FlightDescriptor::new_path([schema, table]))` → decode IPC bytes via `ArrowSchema::try_from(&SchemaResult)` → `Option<Arc<ArrowSchema>>`, None on connect/RPC/decode failure. Test fixture `cc47a7d3e` (extended `d40888326`) adds in-process tonic mock at `tests/connectors_flight_contract.rs::trino_flight_pilot` (~160 LOC; reusable for the remaining 4 helpers); 6 tests cover happy-path + unreachable-endpoint fallback for the 3 live methods. Flight gate: 5 → 11 passing. The structural ticket / descriptor contract gates remain intact.
| 1-2 weeks remaining (4 helpers × per-method migration)
| Acceptance: (1) **DONE 2026-06-01** for `flight_list_schemas` + `flight_list_tables` + `flight_get_table_schema`; remaining 4: `flight_get_splits`, `TrinoPageSource::get_next_page`, `TrinoPageSink::{append_page, finish}` follow the same `Channel::from_shared(...).connect_lazy()` + `FlightServiceClient::new` + dial-relevant-RPC pattern. (2) **DONE 2026-06-01** — `MockFlight` fixture at `tests/connectors_flight_contract.rs::trino_flight_pilot` is reusable; `get_schema` is wired (canned `Arc<ArrowSchema>` → IPC bytes via `SchemaAsIpc::try_into`); extend with `get_flight_info` (splits) / `do_get` (page reads) / `do_put` (page writes) per method. (3) Remove the scaffold marker on the module in `src/connectors/trino.rs` once the last method migrates. (4) **DONE** — `connectors-contract-gates` / `flight` cell runs the pilot tests.
