...
| P7
| HIGH
| Partially delivered — graph done 2026-06-21; document CRUD done; document scan/query branch ready 2026-06-23; entity still open
| The canonical `proximadb.v2.ProximaRecordService` covers record writes, search, and SQL. **Graph is delivered**: `proto/proximadb/v2/graph.proto` (`proximadb.v2.ProximaGraphService`) + `src/network/grpc/v2/graph_service.rs`, always-registered on the multiplexed port and consumed by the Python SDK. **Document is partially delivered**: `proto/proximadb/v2/document.proto` + `src/network/grpc/v2/document_service.rs` expose v2-native document CRUD; `feat/document-v2-query` adds `QueryDocuments` for value-free projection/sort/pagination scans. Value-bearing document filters, update, and aggregate stay deferred until they can use a ProximaValue-native filter rather than the lossy v1 `SqlValue` path. Entity (`entity.proto`/`entity.rs`) remains REST-v2-only; callers needing gRPC-native entity resolution must use REST or the deprecated v1 compat adapter (off by default). NOTE: the v2 services live in the main crate under `src/network/grpc/v2/` (mirroring where `ProximaRecordServiceImpl` actually lives), not `crates/platform/proximadb-api/src/grpc/v2/` as originally drafted.
| 3-4 sprints
| Deliverables: (1) [DONE for graph + document] author `proto/proximadb/v2/{graph,document}.proto` using `proximadb.v2.*` message types (no v1 message re-use); (2) [DONE for graph — `ProximaGraphServiceImpl`; partial for document — CRUD + scan/query] implement the v2 services; (3) [DONE for graph + document] register in `multi_server.rs` alongside ProximaRecordService (all three startup paths via the `canonical_*_grpc_service` helpers); (4) [DONE for graph; partial for document] add gRPC tests — graph handler-level tests cover node/edge CRUD, label query, traversal, shortest-path, conversion, and structural tenant namespacing; document tests cover neutral value conversion and the v2 sort-to-v1 backing-query adapter; (5) OpenAPI/grpc-gateway annotations for shared REST/gRPC schema. Remaining scope: EntityServiceV2 plus document filters/update/aggregate. *Acceptance:* `grpcurl` can list and call `proximadb.v2.ProximaGraphService` (DONE), `proximadb.v2.ProximaDocumentService` (CRUD DONE; query branch ready), `proximadb.v2.EntityService` without enabling `PROXIMADB_GRPC_V1_COMPAT`. *Blocks:* TD-121 (v1 cannot be removed until v2 parity exists). *Graph deferred RPCs tracked by* TD-124. *Coordinates with:* TD-120 (pg_catalog) for entity type metadata; TD-119 (Iceberg manifests) for document storage layout.
