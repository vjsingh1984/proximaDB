...
| HIGH
| Open
| Slice 2 of T3.1 (commit `1dca6ea7b`) annotated `crates/foundation/proximadb-records/src/lib.rs:848` with `#[serde(skip, default)]` to keep the V1 bincode wire format stable. The canonical WAL uses `rmp_serde::to_vec_named` (`src/services/canonical_wal.rs:225`), which is schema-tolerant by itself — but `#[serde(skip)]` drops `branch_id` from the serialization regardless of encoder. Result: every record read back via `FramedTableWalAppender::read_entries_from_path` has `branch_id: None`, so `merge_branches` cannot identify branch entries and returns `None`. The REST endpoint `POST /api/v1/collections/:collection/branches/:branch/merge` is therefore non-functional for any real-world data; the 6 unit tests in `src/graph/merge.rs` only pass because they construct `CanonicalWalEntry` via struct literals (bypassing the WAL roundtrip).
| 1-2 days
| **DISCOVERED 2026-05-27** by `tests/graph_branch_merge_integration_test.rs` (Slice 8). All 5 tests fail at `merge_branches(...).expect("seed produced mergeable branches")` because `branch_id` is lost in the WAL roundtrip. Tests are marked `#[ignore]` with a pointer to this TD pending the fix. **Two fix options:** (A) Switch the canonical WAL encoding from `ProximaRecord` to `ProximaRecordV2` (which carries `branch_id` natively without `#[serde(skip)]`); this is a WAL format change and needs an upgrade path for existing data. (B) Add `branch_id: Option<String>` directly to `CanonicalWalEntry` in `crates/storage/proximadb-storage-common/src/wal_entry.rs` — branch identity becomes a WAL-frame property rather than a record property, which is the cleaner layering (the WAL is the merge boundary, not the record). Either way the fix is its own slice with careful blast-radius review (`ProximaRecord` has 551+ construction sites in the workspace).
|===
