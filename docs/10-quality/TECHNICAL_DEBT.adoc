...
| P7
| MEDIUM
| Open (proposed 2026-06-17) — the generation-fenced commit primitive (`commit_fenced`, `e4d9b05fb`) shared with TD-117 has landed; the metadata.json/manifest-list/refs snapshot layer is still open
...
| P7
| HIGH
| Open (proposed 2026-06-18)
| The canonical `proximadb.v2.ProximaRecordService` (`crates/platform/proximadb-api/src/grpc/v2/`) covers record writes, search, and SQL. Graph (`graph.proto`/`graph.rs`), document (`document.proto`/`document.rs`), and entity (`entity.proto`/`entity.rs`) workloads are currently REST-v2-only; there are no `proximadb.v2.GraphService`, `proximadb.v2.DocumentService`, or `proximadb.v2.EntityService` gRPC endpoints. Callers that need gRPC-native graph traversal, document CRUD, or entity resolution must either use the deprecated v1 compat adapter (off by default) or fall back to REST. This becomes a blocking gap for any gRPC-first SDK once TD-121 removes v1.
| 3-4 sprints
| Deliverables: (1) author `proto/proximadb/v2/graph.proto`, `proto/proximadb/v2/document.proto`, `proto/proximadb/v2/entity.proto` using `proximadb.v2.*` message types (no v1 message re-use); (2) implement `GraphServiceV2`, `DocumentServiceV2`, `EntityServiceV2` in `crates/platform/proximadb-api/src/grpc/v2/`; (3) register these services in `multi_server.rs` alongside the existing v2 ProximaRecordService; (4) add gRPC integration tests for graph traversal (BFS/DFS), document CRUD (insert/get/update/delete/search), and entity resolution; (5) add OpenAPI/grpc-gateway annotations so the REST v2 and gRPC v2 surfaces share schema definitions. *Acceptance:* `grpcurl` can list and call `proximadb.v2.GraphService`, `proximadb.v2.DocumentService`, `proximadb.v2.EntityService` without enabling `PROXIMADB_GRPC_V1_COMPAT`. *Blocks:* TD-121 (v1 cannot be removed until v2 parity exists). *Coordinates with:* TD-120 (pg_catalog) for entity type metadata; TD-119 (Iceberg manifests) for document storage layout.
