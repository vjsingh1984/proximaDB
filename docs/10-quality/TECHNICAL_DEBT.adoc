...
| 1-2 months
| *IN PROGRESS Mar 2026*: **PostgreSQL connector 100% complete** - Full logical replication with pgoutput protocol, WAL data reception via CopyBothStream, slot management, offset tracking, LSN tracking. MySQL 90% complete (binlog decoder + connection layer complete; requires production COM_BINLOG_DUMP implementation). MongoDB 90% complete (change stream decoder + MongoDbChangeStreamer connection layer complete; requires live change stream wiring). Feature-gated behind `experimental-cdc-connectors`. Debezium still recommended for production MySQL/MongoDB.
|  |  |  |  |
|---|---|---|---
...
| **MySQL** | 🔄 90% | `src/cdc/connectors/mysql/connector.rs` | Binlog decoder complete, MySqlBinlogStreamer connection layer, load_last_position(), run_mysql_binlog_stream() async loop. TODO: Production COM_BINLOG_DUMP protocol |
| **MongoDB** | 🔄 90% | `src/cdc/connectors/mongodb/connector.rs` | Change stream decoder complete, MongoDbChangeStreamer connection layer, connect(), open_change_stream(), load_last_resume_token(). TODO: Live change stream wiring with async iteration, BSON conversion |
