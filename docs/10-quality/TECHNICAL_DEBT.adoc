...
| 3-4 weeks
| `src/observability/ingestion/adapters/otlp.rs` is stub. Need gRPC (4317) + HTTP (4318) servers. Trace storage in `src/observability/traces/` exists.

| TD-019
| Cypher query language partial
| P2
| MEDIUM
| Open
| Parser exists (`src/graph/query/parser.rs`) but incomplete engine integration. Not production-ready vs. Neo4j openCypher.
| 2-3 months
| Implement full openCypher grammar: MATCH, WHERE, RETURN, ORDER BY, SKIP/LIMIT, WITH, UNION. Integrate with ORION engine traversal.
...
| 4-6 weeks
| Implement `CrossModelTransactionCoordinator` with two-phase commit. See Task #5 for details. Coordinated writes via WAL.
|===
...
| Yes (TD-007 blocks "production-ready" claims). TD-013 resolved.

| Missing Critical Features
| ~TD-008~, TD-009, TD-010, ~TD-011~, TD-018, TD-019, TD-020
| Yes (TD-008 was table stakes, now resolved). TD-011 partially resolved (5/7 frameworks). TD-018, TD-019, TD-020 block enterprise adoption.
