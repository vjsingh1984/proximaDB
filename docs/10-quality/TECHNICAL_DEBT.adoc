...
| TD-098
| Trino connector — 6 of 7 Flight methods still scaffold
| P3
| MEDIUM
| Open — structural contract guarded
| Surfaced 2026-05-30 during the connector-conformance sprint. The Trino connector at `src/connectors/trino.rs` defines `flight_list_schemas`, `flight_list_tables`, `flight_get_table_schema`, `flight_get_splits`, plus the Page-source/sink ops (`TrinoPageSource::get_next_page`, `TrinoPageSink::append_page`, `TrinoPageSink::finish`) — all 7 return placeholder values and none construct a `FlightServiceClient` against the live `ProximaFlightService` at `src/network/arrow_ipc/service.rs`. **What's already guarded** by `tests/connectors_flight_contract.rs` (5 tests): the JSON shape of `ArrowFileTicket` (round trip) and the `FlightDescriptor` path patterns (`["relational", table_fqn]` + `["vectors", collection_id]`) the server's routing logic recognizes. So spec-level drift fails CI today. **What's missing**: actual FlightClient construction + a tonic `Server::builder()` test fixture (pattern in `tests/fp16_flight_e2e.rs`) so end-to-end RPC round trips can be asserted.
| 1-2 weeks
| Acceptance: (1) Implement each of the 7 Flight methods to construct a `FlightServiceClient::connect(config.flight_endpoint)` and dial the relevant RPC (`list_flights`, `get_schema`, `do_get`, `do_put`, `do_action`). (2) Add per-method TDD tests using an in-process tonic `Server::builder()` mount of a tiny `FlightService` impl that returns canned `FlightInfo` / `FlightData` / `Result` streams. (3) Remove the scaffold marker on the module in `src/connectors/trino.rs:582`. (4) Cell `connectors-contract-gates` / `flight` already runs these; just unskip-and-extend.
...
| LOW
| Open
| Surfaced 2026-05-30 during the connector-conformance sprint. The Hadoop connector's `ProximaRecordReader::next_record()` is a sync `-> bool` API that Hadoop's InputFormat contract calls in a loop to drain records from a split. The v2 OpenAPI spec has NO endpoint that scans records (only POST `/search` with `top_k` for similarity search, GET `/records/{id}` for point reads, POST `/records/batch` for inserts). RecordReader.next_record() was left as a sync scaffold in commit `8cbe28290` (C6 of the sprint) — the wire-call surface for it doesn't exist yet. A POST `/search` with caller-controlled `top_k` would only approximate scan via similarity ordering, which is wrong for an unindexed table scan.
| 2-3 days (spec) + 1 week (server impl)
| Acceptance: (1) Spec out POST `/api/v2/collections/{collection_id}/records/scan` in `docs/openapi/proximadb-openapi.yaml` (operationId `scanRecords`) with pagination shape `{cursor?, limit?, filter?}` → `{records, next_cursor?}`. (2) Wire the server route to expose the existing iteration path used for compaction or for VIPER cold reads. (3) Reimplement `ProximaRecordReader::next_record()` (and `initialize()`) to dial the new endpoint with a cursor held on the reader state; convert to async if Hadoop's calling convention allows, else block on a runtime handle. (4) Add a contract gate test mirroring the DuckDB / Hadoop pattern.
...
| LOW
| Open
| P3 of the agent-memory layer. A mem0 `VectorStoreBase` provider (registered in `VectorStoreFactory`) targeting ProximaDB, and an optional mem0g graph-store mapping onto the native graph engine (collapsing the Qdrant+Neo4j pair into one backend). Document agentmemory's `CLIENT_TYPE=POSTGRES` path. Conformance tests against mem0's interface. **Carry-over from TD-100**: bind pgwire parameter placeholders (`$1`) in `execute_vector_search` (vector + metadata values) — TD-100 landed literal-value WHERE pushdown only; mem0's PGVector provider parameterizes its queries.
| 1-2 weeks
| Acceptance: (1) mem0 configured with the ProximaDB provider round-trips add/search/update/delete. (2) mem0g graph ops succeed against the native graph engine, or the gap is documented. (3) Adapter conformance test in CI.
...
| LOW
| Open — deferred field
| Session scope starts as `props["session_id"]` (no record-shape change). Promote to a first-class `ProximaRecord.session_id` field ONLY if a query-path need is proven after TD-101. Tracked to avoid silently adding a record field for a property `props` already carries.
| 2-3 days (if promoted)
| Acceptance: (1) Memory queries can filter by session. (2) Decision recorded (keep in `props` vs promote) with the query-path justification.
