...
| P5
| HIGH
| Partial (emission landed; recovery + production wiring remain)
...
| 3-5 weeks (≈1/3 landed)
| **EMISSION HALF RESOLVED 2026-05-27** by commit `b36a24b17`: added `canonical_wal_appender: Option<Arc<dyn TableWalAppender>>` injection on `GraphOperationsService` (mirrors the `canonical_record_store` pattern) plus `with_canonical_wal_appender` builder. `flush_wal` now persists a `CanonicalOperation::Checkpoint(SnapshotManifest { sequence_number, timestamp_ms, collection_ids, projection_freshness })` to the canonical WAL via `FramedTableWalAppender::append_operations` before the engine WAL flush, returning `ProximaDBError::Internal` if the append fails so callers see the lost durability guarantee. Verified by 3 cycles in `tests/graph_canonical_checkpoint_test.rs` (basic emission, monotonic WAL sequences across repeated flushes, no-op when appender is None). **Remaining work to close TD-066:** (a) production wiring — thread the existing `FramedTableWalAppender` from `SharedServices` into `GraphOperationsService::new()` so production deployments get the persistence by default; (b) recovery hook — add `recover_from_canonical_wal` for ORION that reads the latest Checkpoint and scopes engine-WAL replay to entries after the checkpoint LSN; (c) engine WAL truncation by canonical LSN. Coordinates with `RELATIONAL_DOCUMENT_GRAPH_CONVERGENCE_2026_05_14.adoc` Phase 3.
