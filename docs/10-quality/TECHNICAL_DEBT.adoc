...
| TD-097
| Spark connector JNI bridge — wire layer done; product impls + Gradle remain
| P3
| MEDIUM
| Partial — wire layer + CI shipped 2026-05-31..06-01 (`8704472ba` + `a52af195d` + `32e96e35c`)
| Surfaced 2026-05-30 during the connector-conformance sprint. `src/connectors/spark.rs` provides 9 `jni_*` functions (get_table_schema, plan_input_partitions, create_partition_reader, read_next_batch, close_partition_reader, create_data_writer, write_batch, commit_writer, abort_writer) that compile but return placeholder values (empty JSON, empty Vec, 0). The pure-Rust functions cannot be called from a JVM as written — they lack the JNI calling convention, JNI-mangled symbol names, and `JNIEnv` plumbing. **WIRE LAYER + CI SHIPPED**: satellite crate `crates/binding/proximadb-spark-jni` (cdylib, depends on the main `proximadb` rlib + `jni = "0.21"`) wraps all 9 `jni_*` functions with `extern "system" fn Java_org_proximadb_spark_NativeProximaDB_*` exports (`a52af195d`, follow-up to pilot `8704472ba`). JVM smoke at `clients/jvm/spark-connector/src/test/java/org/proximadb/spark/NativeProximaDBTest.java` (plain `public static void main` harness, no Gradle) loads `libproximadb_spark_jni.dylib` and runs 11 round-trip checks against every export on JDK 19 / macOS aarch64. CI job `spark-jni-smoke` (`.github/workflows/ci.yml:569` + `32e96e35c`) is path-filtered (`needs.changes.outputs.spark_jni`), builds the cdylib, compiles the Java sources via `javac`, runs the harness in `java -cp build`; gates the `ci-success` aggregator. Remaining: Rust impls behind the wrappers still return placeholders (no `ConnectorStorageAdapter` plumbing), and the smoke harness has not migrated to Gradle + JUnit 5 (so per-method conformance assertions live in the plain `main()`).
| 1 week remaining (product-side `ConnectorStorageAdapter` plumbing + Gradle/JUnit5 migration)
| Acceptance: (1) **DONE 2026-05-31** for the wire-up pattern — cdylib + Java test work end-to-end. (2) **DONE 2026-06-01** — all 9 `jni_*` wrapped as `extern "system" fn Java_org_proximadb_spark_NativeProximaDB_*` in `crates/binding/proximadb-spark-jni/src/lib.rs` (`a52af195d`). (3) Each Rust `jni_*` constructs the underlying `ConnectorStorageAdapter` call and serializes the result as Arrow IPC / JSON (currently they still return placeholders — product work). (4) Migrate `clients/jvm/spark-connector` from the plain-main smoke harness to Gradle + JUnit 5; add per-method conformance assertions (schema fetch hits `getCollectionSchema` etc.). (5) **DONE 2026-06-01** — dedicated `spark-jni-smoke` CI job builds cdylib + runs Java harness (`32e96e35c`); local `run-smoke.sh` mirrors the same steps.
