...
| TD-149 | CI: advisory reds accumulate on develop from concurrent merges | MED | Open | D5 | RELIABILITY
| The feat→develop gate hardened (#286) to require all-green, but non-compile advisory jobs accumulate red as concurrent sessions merge PRs that skip the advisory gate. Each advisory red then blocks the next PR's all-green merge. Pattern observed 2026-06-24/25: T4.2 (rustup `rustfmt-preview` conflict — fixed #298 via toolchain component pre-install), Clippy `let_underscore_future` (#302), Python-Lint generated-stub `black` drift (multiple PRs — convention: generated `*_pb2*.py` must be `black`-formatted, CI lints `src tests`). Fix direction: either make advisory jobs blocking at merge time (prevent advisory-red PRs from landing on develop), or add a pre-merge all-green sweep to the develop gate.
