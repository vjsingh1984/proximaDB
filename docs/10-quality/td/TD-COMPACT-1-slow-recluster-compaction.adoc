...
|===
| Field | Value
| Status | **Resolved (verified at 1M, 2026-07-24).** All three findings closed: (1) the 666s was the TD-COMPACT-2 phantom-embedding-column blow-up (compaction passed `records.len()` as `embedding_count` → 105k phantom columns → 40-52 GB RSS → swap → 500-1280 s encode) — root-caused by #1173/#1175's instrumentation, fixed by #1175, VERIFIED on the 1M SIFT ratchet: **9 compactions in 19.7 s total (mean 2.2 s), peak RSS 7.4 GB (was 52 GB), 1M ingest 88 s with zero drain-pauses**; (2) the recall dip is gone — recall is **identical pre/post compaction** (setA 0.9850==0.9850, setB 0.9815==0.9815 on disjoint query sets; the original 0.999→0.986 dip does not reproduce); (3) the 0.0 MB/s throughput metric was a silent size-probe failure + a display artifact (a real ~28 MB compaction over a swap-stalled 694 s prints as 0.0 MB/s at one decimal) — fixed with robust plugin-FS→std::fs size probes that WARN instead of silently zeroing, absolute-MB logging, and event-sourced prometheus counters (`proximadb_compactions_total`, `_bytes_{read,written}_total`, `_seconds` — measured live: 9 events, 261 MB read / 261 MB written). Observation flagged to #1175: absolute recall 0.985 vs the pre-storm 0.999 baseline (pre==post, so not compaction-caused; possibly the MAX_BLOCK_ROWS=16k bound re-cutting 21k-row flush cells — layout variance to confirm). Original filing below. Measured 2026-07-23 on SIFT 1M (6 L0 files → 1 L1). The "re-cluster compaction" (RaBitQ re-encode + IVF re-cluster) took 666s — 66× slower than the expected ~10s for 1M vectors at ~1.5μs/vector. Throughput metrics reported 0.0 MB/s read/write (broken or misleading). Recall dipped 0.999→0.986 (re-clustering shifts IVF cell boundaries).
...
== Review note (2026-07-23 TD sweep)

Call-chain audit for TD-IVF-1 confirms S3's premise: the only reachable `AxisClusteringEngine::train_model` callers are the IVF `External` build path (`dual_store_ivf.rs`) and the poll-driven discovery recluster pass — the 666s re-cluster compaction re-enters the same k-means that TD-IVF-1 removes from restart. Fixing S3 (carry the persisted A0 model into the compacted segment) therefore addresses BOTH the compaction wall-clock and the recall dip (cell boundaries stay put), and it is the same mixed-read-safe persisted-model-reuse mechanism as TD-IVF-1 S1. Kept OUT of the 2026-07-23 TD-sweep PR deliberately: it is flush-path surgery that needs its own 1M-recall ratchet run before merging.
