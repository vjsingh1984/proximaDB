...
|===
| Field | Value
| Status | **Resolved (2026-07-14).** Current `push_predicates` extracts cross-side equality conjuncts from `Filter(CrossJoin)` and rewrites the join to `Inner` with a hash-capable strategy (`1a424f3c6`); residual predicates stay above the join. Later TD-REL-LOWER-6/7 cover disjunctive shared keys and N-way connectivity. Planner unit `comma_join_filter_normalizes_to_hash_join_with_residual` plus the native join e2es pin the original grind class.
