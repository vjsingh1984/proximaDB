...
| Relates | link:TD-XMODAL-7-crossmodal-udtf-subquery-stack-overflow.adoc[TD-XMODAL-7] (sibling resource-bound defect — stack), link:../../12-design/adr/ADR-043-unified-relational-expression-evaluation.adoc[ADR-043], link:../TECHNICAL_DEBT.adoc[TD-104], link:../TECHNICAL_DEBT.adoc[TD-138] (`HybridFusionEngine` retirement onto the neutral seam), link:../TECHNICAL_DEBT.adoc[TD-123] (v1-proto ratchet / v1 sunset — REST `/api/v2` + gRPC v2 are the only canonical surfaces)
...
3. **Partial production wiring** — `graph_fusion_search_with_labels` now overlays
   `PROXIMADB_FUSION_POOL_CAP` and executes through `Fuser`, so the canonical graph-fusion path can
   be bounded default-off. Residual live paths remain: `fusion_service.rs::fuse_optimized` (entity
   orchestrator) and `ResultFuser` (`proximadb-query/src/fusion.rs`) do **not** yet use
...
So: the protective mechanism exists in the canonical v2 core and is wired into one live graph
fusion path, but remains default-off and incomplete across the full cross-modal fabric. Residual
wide fan-out paths can still buffer the entire source union in RAM before the output trim — the
OOM / saturation path. Sibling of
link:TD-XMODAL-7-crossmodal-udtf-subquery-stack-overflow.adoc[TD-XMODAL-7] (heap-side vs
stack-side).
...
1. **Complete fail-loud propagation.** `PoolCapMode` and `FusionStats.pool_cap_exceeded` exist;
   remaining callers must reject or surface `pool_cap_exceeded` when configured for fail-loud mode.
2. **Wire residual v2 paths.** Thread `pool_cap` + `pool_cap_mode` into
   `fusion_service.rs::fuse_optimized` (entity-orchestrator serving path) and
   `proximadb-query::ResultFuser`, default-off. `Fuser` becomes the single enforcement point once
   the live paths migrate fully onto it (TD-138).
3. **Test.** Error-mode + an over-cap source fails loud / is bounded on each live path; normal
   queries are byte-identical with caps unset.
