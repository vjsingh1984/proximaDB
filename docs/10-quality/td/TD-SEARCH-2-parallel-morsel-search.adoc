...
|===
| Field | Value
| Status | **S1 + S2 shipped.** S2 (2026-07-25): true multi-core Stage-A rank via `spawn_blocking` morsels — `RaBitQRegion::rank_range_scored` (block-format) chunk-ranks disjoint row ranges; merging per-chunk `(row, score)` pools by score is exactly equivalent to the sequential rank (unit-proven across degrees/metrics). Adaptive degree = all cores when the scan is ALONE in flight (`INFLIGHT_SCANS` RAII counter), sequential otherwise — the measured posture: lone-query cold c=1 went 5.8→10.4 QPS / 174→96 ms mean (stable across 3 runs), while every concurrency-sharing formula tried (cores/inflight, cores/(inflight+1)) was inside the box's ±30% run-to-run variance with matched-pair regressions — so concurrency runs the baseline path, zero throughput risk. `PROXIMADB_SEARCH_MORSEL_DEGREE` forces a fixed degree (0=adaptive, 1=off, n=n). The original ≥70%-scaling-efficiency c=16 gate is REFRAMED: on the 2–3-segment post-flush-floor bed, c=16 inter-query concurrency already saturates cores; S2's deliverable is the lone/low-concurrency cold-latency floor (the interactive case the 2-segment layout ceded). S3 (SearchParams audit) RESOLVED 2026-07-25: the type-downsizing half is ALREADY DONE on develop (`top_k: Option<u16>`, `timeout_ms: Option<u32>` — the table below is stale); the remaining half (`progressive_scenario`/`optimization_hint` String→enum, 27 files; 7 `enable_*` flags → engine config, 6 files) is DECLINED as unmeasured micro-optimization — ~24 B + 2 allocs/query against measured evidence that rank CPU and GET round-trips dominate; a 27-file churn PR fails the co-design 'trace before you tune' bar. Reopen only with a profile showing SearchParams allocation on a hot path. Pressure-test design included (informed by measured io-trace data).
