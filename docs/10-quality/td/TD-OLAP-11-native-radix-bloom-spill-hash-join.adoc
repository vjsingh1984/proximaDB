...
|===
| Field | Value
| Status | **In progress — Phase 3.0 (in-memory native hash join) landed via #779 (2026-07-08); Phase 3.1 (Grace spill) pending.** Phase 3.0 shipped `HashJoinBuildOperator`/`HashJoinProbeOperator`/`JoinHashTable` in `src/query/execution/native_join_ops.rs` (blocking build + `OnceLock`-published frozen table; the `MemoryPool` exists, `SpillSink` is not yet implemented → `InsufficientMemory` falls back to Volcano). Phase 2 (`FilterProjectOperator` + `HashAggregateOperator`, the `PhysExpr`/`lower_physical`/`Pipeline` substrate this join reuses) landed earlier (#755/#758/#760/#772). This TD defines the **#1 performance lever**: the native hash join that closes the Q3/Q14 join gap (Q3 594 ms / Q14 296 ms, 99% `compute_ms` — DataFusion 54's hash join does not spill and cannot host the multimodal join semantics that are ProximaDB's wedge).
