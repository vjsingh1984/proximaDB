...
* Credential-side stamping of the gateway role beyond JWTs — **API keys:
  CLOSED 2026-08-30** (`ApiKeyInfo.roles`, `#[serde(default)]`; the
  conversion unions configured roles with `api_user`; exact-match
  `gateway`/`operator` only — a typo grants nothing; grants NO
  `UnifiedPermission`, delegation-only). **SSO/mTLS: still open** (same
  shape, unclaimed).
