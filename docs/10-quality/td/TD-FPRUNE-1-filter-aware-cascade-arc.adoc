...
* **P2 — footer-resident block stats (zero-GET pruning). IN PROGRESS.**
  ...
  ** **Remaining wiring:** (a) storage-common footer ser/de — write the payload
     in `write_block_entry` (currently hardcodes `stats_len=0`,
     `segment_layout.rs:582`) + parse it in `read_blocks` + carry it on
     `FooterBlockEntry` (drops `Copy`; only ~4 call sites, all in
     `pax_block.rs`); (b) writer — populate per-block from filterable-column
     `ColumnMeta` at footer build (`pax_block.rs:1664/2100`), `stats_kind =
     MinMax`, behind a default-OFF gate until baked; (c) Stage F
     (`pax_filtered_row_allow`) — `evaluate_block` over `FooterBlockStats`
     FIRST, fetch+decode only the survivors (pruned blocks cost zero body GET).
  ** **Evidence gate:** filtered search `io_trace` `object_gets`/`bytes_read`
     drop with selectivity (pruned blocks unfetched), recall unchanged, on both
     the synthetic partition and Wikipedia `lang` corpora.
