...
|===
| Field | Value
| Status | **Open — collection read-cost controller not implemented (reconciled 2026-07-28).** Async background compaction and L0 admission watermarks are live; #1294 made the 6/10 watermark path restart-safe by exempting mandatory WAL recovery. Those are bounded operational controls, not ADR-062's query-GET/cache/payback controller. Implement observe/recommend mode before allowing read-cost-driven compaction decisions.
| Priority | P1 for multi-segment scale economics. TD-RDSTRAT-8's trained v3/A0 path is already default ON; this TD no longer blocks that format promotion. It blocks only adaptive collection-level decisions that trade GETs/cache residency against compaction write amplification.
...
* Async compaction and the configured L0 slowdown/stop watermarks bound background
  work and live-ingest admission. #1294 ensures recovery materialization bypasses
  those live-ingest watermarks. None of these controls
...
| Bounded parallel `read_ranges` | #1094 shipped, default-OFF pending cloud evidence | Wall/tail latency by overlapping GETs; not GET billing or bytes
