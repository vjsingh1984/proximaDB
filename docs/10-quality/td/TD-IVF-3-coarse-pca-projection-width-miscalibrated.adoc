...
. **Flip preconditions** — all three must pass before the compiled default moves:
  ...
  .. **PCA conditioning at width — RESOLVED, and it caps the answer at 32.**
     This precondition was checked and **failed**. The extractor uses power
     iteration with deflation from a fixed seed vector and no
     re-orthogonalisation; past a spectrum-dependent depth it returns
     near-duplicates of components already extracted — measured onset at
     component **42** for a `0.90^j` spectrum and **59** for `0.95^j`, in both
     cases duplicating component 2, an early high-variance direction. That
     double-counts a direction in the metric that ranks coarse cells, and recall
     cannot see it because the coarse projection only selects cells. Filed as
     **TD-IVF-5** with an executing test. Consequence: `IVF_NCOMP_FLOOR_CEILING`
     ships at **32**, not the measured optimum of 64. It lifts when the extractor
     re-orthogonalises.
+
NOTE: TD-IVF-5 is also a **candidate explanation for the one anomaly in the table
above** — width 64 won at 384-d and 768-d but *lost* to 32 at 128-d. A
faster-decaying effective spectrum at 128-d would make its components past ~40
degenerate, which is the shape of that result. Testable against the persisted A0
model of the existing `n_comp=128` beds; not yet checked.
