...
. **Flip preconditions** — all three must pass before the compiled default moves:
  ...
  .. **PCA conditioning at width.** Components are extracted by 20-iteration power
     iteration with deflation, so components 32–64 may be materially worse
     conditioned than 1–14. End-to-end recall can absorb a few bad directions,
     which is precisely why the measurement study cannot see this. Assert
     unit-norm, mutual orthogonality, and non-increasing Rayleigh quotients. **A
     failure here means the answer is 32, not 64.**
