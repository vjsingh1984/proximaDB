...
|===
| Field | Value
| Status | **Narrowed (boot symptom resolved for co-design collections).** #1163 gates recovery flushes: `index_flushed_into_axis` treats a missing `collection_config` as "no AXIS" and an empty `index_configs` as skip, and the AXIS EventLog consumer acknowledges-and-skips flush events for collections without `index_configs` — so a restart with co-design (no-`index_configs`) collections performs NO k-means training and NO in-memory projection (verified 2026-07-23 by call-chain audit: the only reachable `train_model` callers are the gated IVF build path and the poll-driven discovery recluster pass; `AxisManager::new` itself is lazy — no threads, no training, no I/O at construction, so boot-time construction is free). REMAINING scope: collections that opted into AXIS via `index_configs` still re-train on restart instead of loading the persisted model (S1 below); same reuse applies to compaction (TD-COMPACT-1 S3). Note: the `Option<Arc<AxisManager>>` boot refactor sketched in TD-AXIS-2 was assessed and REJECTED — construction is lazy/cheap, so Option-izing ~50 sites across ~19 files buys zero boot time; the per-collection gates are the correct mechanism. Measured 2026-07-23: server at 100% CPU (one core) for minutes after boot, logging "🎯 Training clustering model for collection 1 with 1000000 vectors". The PAX segment already stores the IVF centroids + PCA model in the A0 region (written at flush time) — but the startup path re-trains k-means from scratch instead of loading them.
