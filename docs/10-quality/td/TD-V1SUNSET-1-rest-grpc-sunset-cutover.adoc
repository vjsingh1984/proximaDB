...
* **Live conflict found (decision needed, not fixed here):** the
  `enterprise-catalogs` feature nests its API under `/api/v1/catalogs`
  (`canonical/handlers.rs:1809`) — squarely inside the prefix this
  middleware 410s when compat is off (the default). With the feature
  enabled and compat off, the enterprise catalog surface is dead-on-arrival;
  working around it requires re-enabling the ENTIRE v1 compat surface. CI
  never builds the feature, so nothing would catch this. Resolving it
  (relocate to `/api/v2/...`, or exempt the subpath) is an enterprise-facing
