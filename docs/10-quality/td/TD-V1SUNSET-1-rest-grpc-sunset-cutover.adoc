...
|===
| Field | Value
| Status | **RESOLVED 2026-08-29 — see the final reconciliation section.** (Earlier: partially invalidated, see Reconciliation 2026-07-09.) Step 1 (REST flip) landed (#814). Step 2 (relocate deprecation-header util) landed. Step 4 (gRPC v1 api-crate delete) already complete on `develop`. **Step 3 INVALIDATED** — that dir holds LIVE `/api/v2` port-backed routers; it must NOT be deleted (a rename is TD-104). **Step 5 (remove `v1_sunset` middleware) EXECUTED (#1773)** — evidence-closed early; see Resolution.
