...
|===
| Field | Value
| Status | **In progress (slice 1 landed).** Wired into the default prometheus registry (scraped by `/metrics/prometheus` via `prometheus::gather()`), per `src/metrics/operational_metrics.rs`: (1) `proximadb_queries_total` / `proximadb_queries_failed_total` + `proximadb_search_latency_seconds` histogram at the query-facade search entry (`QueryEngineAdapter::vector_search`); (2) survivor-range cache `proximadb_survivor_cache_{hits,misses,bytes}` gauges mirrored from `TenantCache`'s internal atomics on each lookup; (3) segment-invariants cache `proximadb_segment_invariants_cache_{hits,misses}_total` counters + `_bytes` gauge at its only read boundary (a hit = 3 GETs avoided). The colliding stale struct-exporter stanzas were removed (TD-METRICS-2). REMAINING: ingest-side counters (vectors_total by real count), per-tenant labels (v2 — thread tenant_id into the search path), wiring or retiring the `/metrics/json` struct fields. Original finding below. Three metrics gaps, all proven by the SIFT1M benchmark: (1) basic operational counters read 0/stale; (2) survivor cache + segment invariants cache have ZERO prometheus exposure despite delivering 100% hit rate (0 GETs, 0.5ms hot vs 157ms cold); (3) `/metrics` endpoint is dead weight (admin UI uses `/metrics/prometheus` + `/metrics/json`).
