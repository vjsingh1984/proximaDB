...
| Status
| **Partially resolved by #1064; current through 2026-07-28.** The single-version
  footer's typed encoding-map section, the shared exact
  ...
  No later change wires the clustered vector transform into Region B or promotes
  another lossy storage representation; those are not implicit follow-ups.
...
=== As-built single-version footer with additive typed sections

Because this work was unreleased, #1064/#1069 evolved the footer in place rather
than introducing a compatibility version that would add no user value. The
footer keeps version byte `1`; the sectionless form stays byte-identical and
typed additions use an optional-section envelope:

```
[footer_version:u8 = 1]
[row_count | Region-A/B extents | SQ8 parameters | embed_dim/count]
[legacy representation summary]
...
* No segment-magic, header-layout, or footer-version bump. New
  value/transform/compression tags are not format magics.
* The reader accepts the sectionless legacy form and the typed-section form.
* The footer validates all lengths with checked arithmetic and skips unknown
  optional section tags/versions.
* A malformed recognized section is an error, not a legacy fallback.
* Section order is deterministic on write; readers do not depend on order.

Implemented section tags cover the `StripeEncodingMap` and optional auxiliary
region extent mirrors. Tags are stable and each section owns its own version.
The full schema snapshot remains TD-TBL-4/ADR-047 work.
...
* Sectionless footer: use the PR1/block-local `VectorParamBlock` path; legacy
  SQ8 remains readable.
* Encoding-map section present: validate and dispatch its declared descriptors.
...
**Design target, not current serialized state (reconciled 2026-07-28).**
`StripeEncodingDescriptor` carries source role/fidelity and vector-transform
semantics, but it does not yet serialize `built_metric`, `metric_epoch`, or the
normalization contract. The intended immutable physical contract is:
...
. If only an existing quantized projection is available, compaction may
  ...
  forbidden.
. Once the descriptor fields above ship, a collection metric change increments
  `metric_epoch`. Compatible generic
...
all input rows. The typed encoding map generalizes that fact from `has_f32_tier` to an
...
. Footer tests: sectionless footer parse; typed-section round-trip;
  unknown optional section skip; malformed/truncated section fail; explicit
...
. Segment tests: encoding sections are emitted from actual block metadata;
  sectionless footer reads through the PR1 path; gate OFF writes legacy SQ8;
  gate ON with IVF writes lossless chunks; both forms coexist in one query.
...
* typed optional footer sections for encodings and auxiliary-region references,
  plus the sectionless legacy read path;
...
* Typed encoding/auxiliary footer sections round-trip; the reader also reads
  the sectionless form.
* A sectionless footer reads legacy SQ8; a present encoding map is authoritative;
  explicit unknown data encoding never aliases to SQ8.
...
arm **does satisfy the bytes sub-gate** and its typed-footer/shared-codec foundation
...
* 2026-07-15 — filed after PR1 #1019 landed. Audit established one shared codec
  ...
  codebooks local to each PAX block, typed optional footer sections with a
  sectionless legacy interpretation, optional Bloom references, and fail-closed
  unknown encoding behavior. This initial
...
* 2026-07-17 — #1064 shipped the single-version footer encoding-map section,
  the default-OFF clustered SQ8 transform,
...
* 2026-07-28 — reconciled the document with the as-built single-version footer;
  the earlier “footer v2” proposal did not ship.
