...
== CURRENT STATE (2026-08-17) — read this before the dated sections below
...
| — | `object_id` minted on the LIVE catalog (`SystemCatalog`), durable across restart | #1607
| — | `Target::try_from_typed_triple` — ADR-0083-canonical, fail-closed on overflow | #1630
| L1/L2 | **Typed identity re-landed ⇒ relational ABAC ENFORCES live** across pgwire/gRPC/REST-SQL/Flight; the 3 `#[ignore]`s lifted (suite 7 passed, 0 ignored) | #1636
|===
...
. ~~**TD-AUTHZ-2**~~ — **RESOLVED** (#1607 + #1636). Correcting this entry's own diagnosis: it
  said the defect lay "in the wiring between `NativeCatalog` and the introspection projection."
  It did not. The live catalog (`SystemCatalog`) simply **never minted the ids** — neither
  `object_id` nor `stable_namespace_id` — so `dml/mod.rs:1466`'s two-field gate could not be
  satisfied and `introspection/lib.rs:424` rendered `None` as an empty string via
  `unwrap_or_default()`. Both halves were needed and neither subsumed the other, proven by the
  e2e failure MOVING (`:285` parse → `:786/:904/:1023` enforcement). Still latent and filed
  separately: that `unwrap_or_default()` rendering turns a missing identity into a value
...
. ~~**L2.1** `$subject.<attr>` substitution~~ — **SHIPPED.** `compile_security_filter` now takes
  `&SubjectAttributes` as a **required** parameter (not an optional second entry point: a security
  compile that cannot see the subject would silently compare against the literal string
  `"$subject.dept"` and match nothing). Placeholders resolve through
  `SubjectAttributes::load_bearing`, so a **claim-sourced** value denies rather than satisfying the
  predicate — the anti-forgery property, teeth-proven by swapping `load_bearing` for `get`: exactly
  one test fails and the other 63 still pass.
+
Two framing corrections worth keeping: this was a **missing capability, not a hole** —
`compile_security_filter` took no subject *at all*, so subject-parameterized policy could not be
expressed. And there was no legacy spelling to harvest: `rls/service.rs` carries no placeholder
syntax. `$subject.<attr>` was already the spelling documented on
`FileSystemPredicateObjectStore` (`dept == $subject.dept`), so it was reused rather than
re-invented — one spelling, not two.
+
Retiring `rls/policy.rs` + `secure_operations.rs` remains open and is tracked under L2.5.
. **L2.2** Close the `System` reads that stand in for a missing client context.
  **Re-surveyed 2026-08-20 — the original entry was wrong in three ways**, so it is restated here
  rather than patched.
+
--
**a. "Six" is an undercount by roughly 4×.** There are **22** sites whose origin string carries
`[CLIENT-PLACEHOLDER]`, across nine subsystems: `dml` (14), and one each in `legacy::search`,
`table_write_executor`, `rest::analytics`, `graph::hybrid`, `unified::executor`, `streaming`,
`embedded`, `embedded::streaming`. (Reproduce with the grep in (b) — an earlier pass of this
survey said 23 by double-counting `legacy.rs`, whose other two System reads carry the honest
`records_search (no abac enforcer)` / `(no client subject)` origins, not a placeholder.)

**b. The line numbers were stale, and citing them was the wrong index anyway.** Every one had
moved (`vectors/legacy.rs:6512` → `:816`/`:833`/`:3530`; `fusion_service.rs:775` → `:546`/`:552`).
The durable index is the **origin string**: the codebase already declares which reads are missing
their client context, so the inventory is a grep, not a hand-maintained list. Use
`grep -r 'CLIENT-PLACEHOLDER'`.

**c. These are not "bypasses" in the escape-hatch sense.** `SystemReadContext` carries no subject,
produces no cache key, and **deliberately does not implement `ClientServable`** — so a sink that
serves clients cannot be handed one without failing to compile. That guard is intact (exactly one
`impl ClientServable`, for `AuthorizedReadContext`). The gap is that these reads run *unfiltered
under a reason that is not true*, not that they can leak to a client through this seam.
--
+
**A related erosion worth fixing with them.** `SystemReadReason` exists so that "naming the reason
is what makes `SystemReadContext` auditable rather than a blanket escape hatch" — but **48 of 61**
uses are `Statistics`, including 14 DML reads that emit no statistics. The reason enum has lost its
discriminating power; only the `origin` string still carries information. Either the placeholders
get their real client context (the fix), or they need a reason that says what they are.

**Legitimately named and out of scope:** `IndexBuild` (4), `ForeignKeyResolution` (5), the
`(no abac enforcer)` / `(no client subject)` sites (3, honestly named), and 22 test-scaffolding
sites (all marked `::tests` in the origin).
. **L2.3** ~~Tenant-partition the predicate-object store~~ — **RE-RANKED, not a security gap.**
  The original entry ("honor the trait contract at `compile.rs:62-64`; today cross-tenant deref
  resolves") is accurate about the *code* and wrong about the *risk*. Cross-tenant deref does
  resolve, but predicate objects are **cluster-scope, operator-administered**: both the write
  (`PUT /api/v2/abac/predicate-objects/{id}`) and the binding that names one
  (`PUT /api/v2/abac/policy-bindings/{tenant}/{id}`) require `SystemAdmin` ∪ `ConfigureSystem`, and
  **all 14** handlers in `rest/canonical/abac_admin.rs` call the fail-closed `authorize_operator`
  first — audited, not assumed. No tenant-facing surface writes either. So only a cluster operator,
  who already administers every tenant, can create a predicate object or point a binding at one:
  there is no escalation to close.
+
The real defect was a **documentation contradiction**: the trait doc promised "structurally
tenant-scoped… a cross-tenant ref fails to resolve" while `FileSystemPredicateObjectStore`'s doc,
in the same file, said "not tenant-partitioned… resolves the same regardless of tenant." A reader
could not tell which guarantee the code kept. Both now state the operator-gated reality, and
`predicate_objects_are_cluster_scope_not_tenant_scoped` pins the arity of `get` so a tenant
dimension cannot be added without someone re-reading the scope note.
+
**Reopen only if a tenant-facing policy API is added** — then the store must be keyed by
`(TenantStableId, ObjectId)` *before* that API ships.
