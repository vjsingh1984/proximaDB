...
|===
| Field | Value
| Status | **Superseded as an implementation direction (reconciled 2026-07-28).** The measured block-centroid/VOE prune route could not satisfy recall and GET gates together. ADR-062/065 replaced it with one-object coalesced Regions A/B, and TD-RDSTRAT-8 persists the compaction IVF plan as Region A0 for query-visible cell probing. Retain this document as negative evidence and lineage; do not build its separate directory or block-prune end state.
| Severity | Historical — the problem was real, but the proposed mechanism is not the current contract.
