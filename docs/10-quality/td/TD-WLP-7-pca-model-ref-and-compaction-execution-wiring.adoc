...
|===
| Field | Value
| Status | Part 1 (execution wiring) Implemented 2026-07-15; Part 2 (VOE emission + `pca_model_ref`) still open. **Part 1:** `Compaction::run_due_compaction` (synchronous `check_compaction_needed` → `perform_compaction_enhanced`, no background workers ⇒ deterministic; atomic-swap path for ADR-046) wired into `flush_implementation` after `should_trigger_compaction` (best-effort — records `compaction_error` + `engine_metrics["compaction_ran"]`, never fails the flush). Fixed TWO stacked bugs that kept compaction dark: (a) the trigger was an unwired stub, (b) `check_compaction_needed` discovered extension `"sst"` while flush writes `.pax` (ADR-049 M1-3) so it found nothing — now `"pax"`; and threaded the per-collection `l0_threshold:` through so the executor agrees with the arming gate. TDD: `tests/sst_compaction_execution_test.rs` (armed AppendBulk flush executes compaction inline; Churn + global hard-disable never do). **Part 2 (still open, follow-up):** compaction output does not yet emit the VOE directory entry (compaction.rs write sites call `write_pax_segment_compacted` but not `emit_after_flush`), so the read-side centroid prune can't use the re-clustered segment's centroids; and `retrain_pca_model_for_compaction` persists `__model/pca_model.bin` but never attaches `pca_model_ref` to the directory. These are cleanly separable and tested on their own.
