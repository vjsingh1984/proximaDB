...
|===
| Field | Value
| Status | **In progress** (Slice 1, 2026-07-16) — filed under link:../../12-design/adr/ADR-064-query-engine-pipeline-decision-dispatch-lowering-trace.adoc[ADR-064] (Decision 2), Phase-2 of the ADR-064 rollout. (Fills the free TD-REL-LOWER-5 slot between -4 and -6.) **Approach chosen: incremental per-family** (the proven #1003→#1044 path) rather than a general dependent-join pass — an Explore sweep found the 4 declines are 3 distinct mechanisms and there is NO `DependentJoin`/`Apply`/`MarkJoin` primitive today, so a full Neumann-Kemper rewrite is a large algebra+executor change deferred (possibly indefinitely) in favour of closing each family behind the ratchet. **Slice 1 (this PR): Q2** — a correlated scalar aggregate over a *multi-table* subquery body. Extracted the FROM machinery into `lower_from` (shared by `lower_select`) and let `lower_correlated_scalar_aggregate` lower a multi-table body through it; the inner join conditions + inner-local filters fold into the inner Filter, the one correlation equi stays the group/join key. Native TPC-H 18 → 19/22, row-exact. **Next slices (own PRs):** Q11 (HAVING-aggregate + scalar subquery), Q18/Q20 (IN/EXISTS in expression position).
