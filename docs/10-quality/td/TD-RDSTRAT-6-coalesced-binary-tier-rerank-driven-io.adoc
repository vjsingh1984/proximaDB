...
|===
| Field | Value
| Status | **Core implemented; point lookup + large-segment successor open (reconciled 2026-07-17).** Layout/read path #1019; adaptive-M/cache/IopsBudget cost control #1060; lossless scalar bytes #1064; ADR-065 Region-B SQ8 hoist + range cache #1069. Per-block OID Bloom remains open. Large-segment Region-A/B selective probing moved to link:TD-RDSTRAT-8-two-level-ivf-coarse-probe.adoc[TD-RDSTRAT-8]. The historical measurements below remain provenance; current shipped order is SQ8-Morton at coalesced flush and PCA+IVF at compaction.
