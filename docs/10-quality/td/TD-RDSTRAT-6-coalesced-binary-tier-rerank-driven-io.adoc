...
|===
| Field | Value
| Status | **Core scope resolved; canonical contract current (reconciled 2026-07-28).** Layout/read path #1019; default-on route #1024; adaptive-M/cache/IopsBudget #1060; lossless scalar bytes #1064; Region-B SQ8/range cache #1069; bounded parallel reads #1094. TD-RDSTRAT-8's v3/A0 successor is also default ON (#1210/#1234), with cache/config seams closed by #1227/#1252. Per-block point-lookup OID Bloom remains the only open item from this TD. Collection file/cache/compaction policy remains link:TD-RDSTRAT-9-collection-read-cost-envelope.adoc[TD-RDSTRAT-9]. Historical measurements below remain provenance; current order is SQ8-Morton at coalesced flush and PCA+IVF/Hilbert at compaction.
| Severity | Resolved core; the remaining OID Bloom is a bounded point-lookup tail, not a blocker for vector scan/rerank serving.
| Component | `src/storage/engines/sst/segment_format.rs` (writer+reader); `crates/storage/proximadb-block-format/src/writer.rs::PaxBlockWriter` (`:134`) + `crates/storage/proximadb-storage-common/src/pax_block.rs::PaxSegmentScanner` (`:887`) + `stripe.rs::SegmentMeta`; `src/storage/engines/sst/search/mod.rs::try_pax_cascade`; `src/storage/engines/core/coalesce_strategy.rs` + `sst_query_engine.rs::{plan_selected_block_ranges (:733), ObjectRangeCoalescePolicy (:289)}`; `src/storage/engines/sst/compaction.rs`; `crates/storage/proximadb-storage-common` (per-backend `IopsBudget` — NOT the `StorageProfile` workload enum); `crates/storage/proximadb-storage-filesystem-types` (`FileSystem::read_ranges` — default is a sequential loop; the GET win is the coalesce policy's); `crates/storage/proximadb-object-store/src/lib.rs::put_if_absent` (`:247`, create-only — compaction CAS via Iceberg fresh-name+manifest-swap, ADR-020).
...
Same magic/version. PR1 initially shipped behind
`PROXIMADB_PAX_COALESCED_RABITQ=1`, default OFF; #1024 later made the canonical
coalesced route default ON with kill-switch semantics. Readers remain
mixed-read-safe. Later on-disk encodings stay independently default OFF until
their recall/cost and readability ratchets pass.
...
PR1 implemented the readable footer **core**: RaBitQ/row/dimension summary,
legacy encoding summary, block extents, and an empty typed stats slot
(`StatsKind::None`). Because the format was unreleased, #1064/#1069 evolved that
same footer version in place with typed optional sections for the per-stripe map
and auxiliary-region references. The sectionless form retains its legacy
interpretation; explicit unknown data encodings fail closed rather than
aliasing to SQ8. Full schema snapshot/evolution remains TD-TBL-4/ADR-047.
...
* 2026-07-15 — PR1 #1019 landed and measured recall@10 `0.984`, `20` range GETs/query, and
  ...
  the shipped footer core/default-OFF gate. The unreleased footer later evolved
  in place through additive typed sections rather than a second footer version.
