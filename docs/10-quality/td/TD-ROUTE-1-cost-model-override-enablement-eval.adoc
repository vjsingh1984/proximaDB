...
|===
| Field | Value
| Status | **Open** — persistence landed (#721); the pgwire override eval landed (`tests/route_cost_override_pgwire_eval.rs`) and **found a hard blocker**: flipping an `olap/parquet` *join* to Native/Volcano errors (Volcano has no join executor). Scan/aggregate flips preserve results; the *global* override stays OFF until the candidate set excludes Volcano for join-bearing plans (see §Finding).
