...
- **520+ tests passing**: Comprehensive coverage with production scenarios
...
- `distance_metric`: One of 'cosine', 'euclidean', 'dot_product', 'hamming'

===== Basic Vector Search
[source,sql]
----
SELECT id, metadata,
       VECTOR_SIMILARITY(vector, ARRAY[0.1, 0.2, 0.3, ...], 'cosine') as similarity
FROM my_collection
...
----
SELECT id, metadata, 
       VECTOR_SIMILARITY(vector, ARRAY[0.1, 0.2, ...], 'cosine') as similarity
FROM my_collection
WHERE metadata->>'category' = 'electronics'
  AND CAST(metadata->>'price' AS FLOAT) < 100.0
  AND metadata->>'status' IN ('active', 'featured')
ORDER BY similarity DESC
LIMIT 10
----
...
SELECT metadata->>'category' as category,
       COUNT(*) as count,
       AVG(VECTOR_SIMILARITY(vector, ARRAY[0.1, 0.2, ...], 'cosine')) as avg_similarity,
       MAX(VECTOR_SIMILARITY(vector, ARRAY[0.1, 0.2, ...], 'cosine')) as max_similarity
FROM my_collection
WHERE VECTOR_SIMILARITY(vector, ARRAY[0.1, 0.2, ...], 'cosine') > 0.7
