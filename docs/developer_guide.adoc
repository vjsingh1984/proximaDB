...
ProximaDB includes a powerful SQL query engine that supports vector similarity search alongside traditional SQL operations.
...
==== Python SDK SQL Example
[source,python]
----
# Using SQL through Python SDK
query = """
    SELECT id, metadata,
...
for row in results:
    print(f"Product: {row['id']}, Similarity: {row['similarity']:.4f}")
