...
async def storage_aware_search():
    ...
    print(f"SST search: {len(lsm_results)} results")
...
export PROXIMADB_WriteBuffer_STRATEGY="avro"
...
[source,toml]
----
[storage.wal_config.performance.cloud_backup]
enabled = true
backup_url = "s3://my-bucket/proximadb-wal/"
strategy = "OnFlush"
verify_integrity = true

[storage.wal_config.performance.cloud_backup.frequency]
operations_threshold = 1000
time_threshold_secs = 300
size_threshold_bytes = 104857600  # 100MB

[storage.wal_config.performance.cloud_backup.cleanup_policy]
retention_days = 7
max_backups = 100
cleanup_frequency_hours = 24

[storage.wal_config.performance.cloud_backup.retry_config]
max_retries = 3
...
config.performance.cloud_backup = Some(CloudBackupConfig {
    enabled: true,
    backup_url: "s3://my-bucket/wal/".to_string(),
...
- `test_persistence_wal.py` - Data persistence and WriteBuffer operations
...
[source,asciidoc]
----
image::diagram-name.png[Description, width=800, align=center]
...
4. Reference in AsciiDoc:
   ```asciidoc
   image::proximadb-feature-name.png[Feature Name, width=800, align=center]
