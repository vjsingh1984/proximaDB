...
6. **Object storage = flat keys + per-object access tier; the tier is the cost lever, not the
   scheme/namespace (ADR-036)** — target the Blob endpoint, flat keys, **HNS-off** on Azure
   (`ADLS Gen2 = Blob + HNS`; `adls`/`abfs`/`az`/`azure` all resolve to the *same* flat Blob backend,
   `adls`/`abfs` are aliases, NOT DFS/ABFS/HNS). The at-rest cost term is **GB-month**, moved by the
   access tier (Hot/Cool/Cold/Archive) plumbed through the canonical cloud-neutral `ObjectAccessTier`
   / `FileOptions.storage_class` (→ Azure `x-ms-access-tier` / S3 `x-amz-storage-class`), never
   per-cloud strings at call sites; default-safe.
====
...
23. **CI/CD Tiering & Security Governance:** CI runs in tiers matched to the `feat -> develop -> qa -> main` promotion flow: **feat->develop** = LIGHT (compile + fmt/clippy + layering + proto + panic-policy + security-audit + docs) **plus the Rust-correctness suites** (unit/integration tests + feature-matrix) so latent breakage (broken doctests, feature-gating gaps, test-isolation bugs) surfaces on the introducing PR — not at develop->qa, **develop->qa MEDIUM** (+ the qa-gate TPC-H/TPC-DS + ANN-recall conformance ratchets), **qa->main FULL** (+ coverage + python multi-version + docker + benchmarks + CodeQL). Protected branches gate on the `CI Success` aggregator; **`develop` requires it too** — don't expect to merge red. Heavy scans (CodeQL) run only at the qa->main boundary + weekly. **Never commit secrets/credentials** (even in tests/fixtures — secret-scanning + push-protection are on; CodeQL flags credential literals). Report vulnerabilities via `SECURITY.md` private reporting, not public issues.
...
29. **SDK REST Transport is Spec-Driven (generated, never hand-rolled) — sync AND async:** Every SDK's REST surface, in every language, MUST be generated from the canonical OpenAPI spec (`docs/openapi/proximadb-openapi.yaml`, itself `utoipa`-emitted from the axum handlers) via the per-language generator (Python `openapi-python-client` → `_generated/rest`, emitting `sync`/`sync_detailed` **and** `asyncio`/`asyncio_detailed` per endpoint), wired behind a thin hand-written ergonomic facade (pooling/retries/locality-gzip/auth). Do NOT hand-roll REST request-building or response-parsing in SDK code (raw `httpx`/`requests` against hand-written routes/payloads) — it reintroduces the server↔SDK drift the spec exists to kill. The **async** client wires the generated `asyncio` functions exactly as the **sync** client wires the generated `sync` functions (see `clients/python/src/proximadb_sdk/protocols/_rest_codegen.py`). The merge-blocking `<lang>-sdk-codegen-drift` gate enforces conformance; a spec change MUST regenerate every SDK client in the same PR. (gRPC = generated proto stubs; Arrow Flight = Arrow schema; pgwire = standard PG driver.) Ref TD-126. This mirrors Core Directive #15 in CLAUDE.md; keep them in sync.
30. **SOLID for Database Development (depend on stable seams not internal paths; test via ports not internal paths; isolate test state; split before it conflicts; short-lived branches):** The recurring PR failures — stale imports (`pv1`), fragile unit tests, merge-conflict pile-ups, drift-stale branches — are symptoms of *ad-hoc dependencies* ("duck typing" in a compiled language). Make dependencies **explicit and stable** via traits/ports so internal refactors (proto migration, god-object decomposition) don't cascade-break dependents. (a) Services depend on **stable crate paths** (`proximadb_proto::`) or **port traits** (`Arc<dyn Port>`), never internal module paths (`crate::proto::...`) that get shimmed/removed during decomposition; proto types are fine at the gRPC boundary — the issue is the path. (b) Tests exercise behavior through trait objects, not internal module paths — a refactor should never break a test, only a behavior change. (c) Every test uses a unique `tempdir` (leaked via `std::mem::forget` for file-backed stores); no shared `/tmp` paths. (d) No PR modifies a file that >2 other open PRs touch — extract shared concerns first (Open/Closed). (e) Feature branches merge within 48h or rebase daily. See `docs/12-design/SOLID_DATABASE_DEVELOPMENT_STANDARDS_2026_06_27.adoc`. This mirrors Core Directive #17 in CLAUDE.md; keep them in sync.
