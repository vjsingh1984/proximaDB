...
11. **Determinism & Test Hygiene:** Tests must be deterministic and isolated — use **`nextest`** for process isolation; **no non-daemon background threads** (an un-shutdown pool deadlocks the interpreter/runner at exit); pin temp state under `tempdir`, not shared paths. Verify the **server binary builds**, not just `--lib` (a green `--lib` skips `#[cfg(test)]` + feature-gated code). Run perf-sensitive checks on a quiet machine. The `native_volcano_stream_truncates_without_error` test was a known flake under `cargo test` (shared-process cross-test deadlock from a non-daemon thread); under **nextest's process-per-test isolation + 240s slow-timeout** it passes reliably (verified 5/5, <1ms). A 10s `tokio::time::timeout` wrapper (#403) adds defense-in-depth. The CI "Rust Tests (unit)" job's long runtime is compile-bound (`CARGO_BUILD_JOBS=1`), not a test hang.
