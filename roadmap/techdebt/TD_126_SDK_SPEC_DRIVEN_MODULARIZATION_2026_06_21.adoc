...
*Retained (intentional scope boundary).* The standalone `proximadb_sdk.embedding_interface` root
module (its own `EmbeddingProvider`/`EmbeddingConfig` + `create_embedding_provider` factory) is
NOT deleted: it is a separate public module still consumed by the chunking concern and its tests,
outside the `embedding_providers` overlay. Collapsing it would require rewriting working
cross-concern (chunking) tests and is tracked with the chunking DI-seam item below.

*Deferred (follow-up).*

* *Embedding-provider DI seam for semantic chunking* — see the chunking "Deferred" item above;
  folding `embedding_interface` into `core` belongs with that seam.
