...
impl CollectionRequestBuilder {
    pub fn from_json(json: serde_json::Value) -> Result<CollectionRequest, String> {
        ...
        let mut request = CollectionRequest {
            ...
            collection_id: json.get(key).and_then(|v| v.as_str()).map(String::from),
        ...
        if let Some(config_json) = json.get(key) {
    ...
    fn parse_collection_config(json: &serde_json::Value) -> Result<CollectionConfig, String> {
        ...
        let storage_engine = if let Some(engine_str) = json.get(key).and_then(|v| v.as_str()) {
        ...
        let primary_indexing_algorithm = if let Some(algo_str) = json.get(key).and_then(|v| v.as_str()) {
        ...
        Ok(CollectionConfig {
            ...
            primary_index: json.get(key)
                .and_then(|v| v.as_str())
                .map(String::from)
                .unwrap_or_default(),
            auto_index_selection: json.get(key)
                .and_then(|v| v.as_bool())
                .unwrap_or(false),
            description: json.get(key).and_then(|v| v.as_str()).map(String::from),
            tags: json.get(key)
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default(),
            owner: json.get(key).and_then(|v| v.as_str()).map(String::from),
...
impl VectorBatchRequestBuilder {
    pub fn from_json(json: serde_json::Value) -> Result<VectorBatchRequest, String> {
        ...
        let collection_id = json.get(key)
            .and_then(|v| v.as_str())
            .ok_or("Missing collection_id")?
            .to_string();
            
        let vectors = json.get(key)
            .and_then(|v| v.as_array())
        ...
        Ok(VectorBatchRequest {
            ...
            batch_timeout_ms: json.get(key).and_then(|v| v.as_i64()),
            request_id: json.get(key).and_then(|v| v.as_str()).map(String::from),
        })
    }
    
    fn parse_vector_record(json: &serde_json::Value) -> Result<VectorRecord, String> {
        let vector = json.get(key)
            .and_then(|v| v.as_array())
        ...
        Ok(VectorRecord {
            id: json.get(key).and_then(|v| v.as_str()).map(String::from),
            vector,
            metadata,
            timestamp: chrono::Utc::now().timestamp() as u32,
            updated_at: Some(chrono::Utc::now().timestamp() as u32),
            version: Some(1),
            expires_at: json.get(key).and_then(|v| v.as_i64()).map(|v| v as u32),
            quantized: None,
...
impl VectorSearchRequestBuilder {
    pub fn from_json(json: serde_json::Value) -> Result<VectorSearchRequest, String> {
        ...
        let queries = if let Some(queries_array) = json.get(key).and_then(|v| v.as_array()) {
            // New proto-aligned format
            queries_array.iter()
                .map(|q| Self::parse_search_query(q))
                .collect::<Result<Vec<_>, _>>()?
        } else if let Some(vector) = json.get(key).and_then(|v| v.as_array()) {
        ...
        let top_k = json.get(key)
            .and_then(|v| v.as_i64())
            .unwrap_or(10) as i32;
            
        // Parse search optimization
        let search_optimization = if let Some(opt_json) = json.get(key) {
        ...
        let include_fields = if let Some(fields_json) = json.get(key) {
            Some(crate::proto::proximadb::IncludeFields {
                vector: fields_json.get(key).and_then(|v| v.as_bool()).unwrap_or(false),
                metadata: fields_json.get(key).and_then(|v| v.as_bool()).unwrap_or(true),
                similarity: fields_json.get(key).and_then(|v| v.as_bool()).unwrap_or(true),
                // rank removed -  fields_json.get(key).and_then(|v| v.as_bool()).unwrap_or(true),
            })
        } else {
            // Fallback to old format fields
            Some(crate::proto::proximadb::IncludeFields {
                vector: json.get(key).and_then(|v| v.as_bool()).unwrap_or(false),
                metadata: json.get(key).and_then(|v| v.as_bool()).unwrap_or(true),
                similarity: true,
                // rank removed -  true,
            })
        };
    ...
    fn parse_search_optimization(json: &serde_json::Value) -> Result<SearchParams, String> {
        ...
        Ok(SearchParams {
            top_k: json.get(key).and_then(|v| v.as_u64()).map(|v| v as u32),
            filters,
            accuracy_threshold: json.get(key).and_then(|v| v.as_f64()).map(|f| f as f32),
            include_expired: Some(json.get(key).and_then(|v| v.as_bool()).unwrap_or(false)),
            timeout_ms: json.get(key).and_then(|v| v.as_u64()),
            enable_two_stage: Some(json.get(key).and_then(|v| v.as_bool()).unwrap_or(false)),
            enable_clustering_hint: Some(json.get(key).and_then(|v| v.as_bool()).unwrap_or(false)),
            enable_metadata_filtering_hint: Some(json.get(key).and_then(|v| v.as_bool()).unwrap_or(false)),
