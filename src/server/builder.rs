...
use crate::storage::builder::{StorageSystemBuilder, StorageSystem};
    ...
    fn default() -> Self {
        Self {
            ...
            fallback_algorithms: vec![
                IndexingAlgorithm::IVFPQ,
                IndexingAlgorithm::BruteForce,
            ],
...
impl ServerBuilder {
    ...
    pub fn with_hardware_acceleration(mut self, acceleration: HardwareAcceleration) -> Self {
        ...
        self.server_config.compute.enable_simd = matches!(acceleration, HardwareAcceleration::SIMD | HardwareAcceleration::Auto);
        self.server_config.compute.enable_gpu = matches!(acceleration, HardwareAcceleration::CUDA | HardwareAcceleration::ROCm | HardwareAcceleration::OpenCL);
        self
    }
    ...
    pub async fn build(self) -> Result<ProximaDBServer> {
        ...
        tracing::info!("🌐 Network: {}:{}", self.server_config.network.host, self.server_config.network.port);
        tracing::info!("⚡ Hardware acceleration: {:?}", self.server_config.compute.acceleration);
        tracing::info!("🔍 Indexing algorithm: {:?}", self.server_config.indexing.default_algorithm);
        tracing::info!("📏 Distance metric: {:?}", self.server_config.indexing.default_distance_metric);
...
impl ProximaDBServer {
    ...
    pub async fn start(&self) -> Result<()> {
        tracing::info!("🚀 Starting ProximaDB server on {}:{}", 
                      self.config.network.host, self.config.network.port);
...
impl std::fmt::Debug for ProximaDBServer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ProximaDBServer")
            .field("endpoint", &format!("{}:{}", self.config.network.host, self.config.network.port))
            .field("acceleration", &self.config.compute.acceleration)
            .field("indexing_algorithm", &self.config.indexing.default_algorithm)
            .field("distance_metric", &self.config.indexing.default_distance_metric)
...
mod tests {
    ...
    async fn test_server_builder_basic() {
        ...
        assert_eq!(builder.server_config.compute.acceleration, HardwareAcceleration::SIMD);
        assert_eq!(builder.server_config.indexing.default_algorithm, IndexingAlgorithm::HNSW);
        assert_eq!(builder.server_config.indexing.default_distance_metric, DistanceMetric::Cosine);
    }
    ...
    async fn test_layered_configuration() {
        ...
        assert_eq!(builder.server_config.compute.acceleration, HardwareAcceleration::CUDA);
        assert_eq!(builder.server_config.compute.cuda_devices, vec![0, 1]);
        
        // Storage config (inspected through storage builder)
        assert_eq!(builder.storage_builder.config().data_storage.layout_strategy, StorageLayoutStrategy::Viper);
        assert_eq!(builder.storage_builder.config().data_storage.data_urls.len(), 2);
        
        // Indexing config
        let hnsw_params = builder.server_config.indexing.algorithm_params.get("hnsw").unwrap();
