...
impl PropertyIndex {
    ...
    pub fn query_range(&self, min_value: &PropertyValue, max_value: &PropertyValue) -> Result<Vec<String>> {
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        let values_size: usize = self.btree_index.values()
...
impl LabelIndex {
    ...
    pub fn query_by_labels_and(&self, labels: &[String]) -> Result<Vec<NodeId>> {
        ...
        let mut result: HashSet<NodeId> = self.label_to_nodes
            .get(&labels[0])
            .cloned()
    ...
    fn estimate_memory_usage(&self) -> usize {
        let label_to_nodes_size: usize = self.label_to_nodes.iter()
            .map(|(label, nodes)| label.len() + nodes.len() * std::mem::size_of::<NodeId>())
            .sum();
        
        let node_to_labels_size: usize = self.node_to_labels.iter()
            .map(|(node_id, labels)| {
                node_id.len() + labels.iter().map(|l| l.len()).sum::<usize>()
            })
            .sum();
...
impl IndexManager {
    ...
    pub fn create_node_property_index(&mut self, property_key: String) -> Result<()> {
        if self.node_property_indexes.contains_key(&property_key) {
            return Err(ProximaDBError::InvalidInput(
                format!("Property index for '{}' already exists", property_key)
            ));
        }
    ...
    pub fn create_edge_property_index(&mut self, property_key: String) -> Result<()> {
        if self.edge_property_indexes.contains_key(&property_key) {
            return Err(ProximaDBError::InvalidInput(
                format!("Edge property index for '{}' already exists", property_key)
            ));
        }
    ...
    pub fn index_node(&mut self, node: &Node) -> Result<()> {
        // Add to label index
        self.node_label_index.add_node(node.id.clone(), &node.labels)?;
    ...
    pub fn index_edge(&mut self, edge: &Edge) -> Result<()> {
        ...
        self.edge_type_index.add_edge(edge.id.clone(), &edge.edge_type)?;
    ...
    pub fn remove_edge_from_indexes(&mut self, edge: &Edge) -> Result<()> {
        ...
        self.edge_type_index.remove_edge(&edge.id, &edge.edge_type)?;
