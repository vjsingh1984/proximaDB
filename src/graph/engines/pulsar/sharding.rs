...
impl ConsistentHashRing {
    ...
    pub fn get_shard(&self, node_id: &NodeId) -> u32 {
        ...
        if let Some((&_, &shard_id)) = self.ring.range(hash..).next() {
            ...
            self.ring.iter().next().map(|(_, &shard_id)| shard_id).unwrap_or(0)
        }
    }
    ...
    pub fn remove_shard(&mut self, shard_id: u32) -> Result<Vec<(u64, NodeId)>> {
        ...
        let keys_to_remove: Vec<_> = self.ring.iter()
    ...
    pub fn get_all_shards(&self) -> Vec<u32> {
        self.ring.values().cloned().collect::<std::collections::HashSet<_>>().into_iter().collect()
    }
...
mod tests {
    ...
    fn test_node_distribution() {
        ...
        assert!(unique_shards.len() > 1, "Nodes should distribute across multiple shards");
    }
    ...
    fn test_distribution_stats() {
        ...
        assert_eq!(stats.expected_virtual_nodes_per_shard, VIRTUAL_NODES_PER_SHARD);
    ...
    fn test_load_balancer() {
        ...
        balancer.update_shard_load(0, ShardLoadStats {
        ...
        // Add hot shard
        balancer.update_shard_load(1, ShardLoadStats {
