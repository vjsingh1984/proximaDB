    ...
    pub async fn manual_migrate(&self, movements: Vec<DataMovement>) -> Result<Vec<Result<()>>> {
        ...
        for movement in movements {
            let result = match movement {
                DataMovement::DemoteNode(node_id) => self.migrate_node_to_cold(&node_id).await,
                DataMovement::DemoteEdge(edge_id) => self.migrate_edge_to_cold(&edge_id).await,
    ...
    pub async fn get_tiering_recommendations(&self) -> Result<TieringRecommendations> {
        ...
        if hot_utilization > 0.9 {
            ...
            recommendations.recommendation_reason =
                "Hot tier is over 90% full. Consider migrating cold data.".to_string();

            // Get candidates for cold migration
            let cold_candidates = self.get_migration_candidates().await?;
            recommendations.should_migrate_to_cold = cold_candidates
                .into_iter()
                .take(20)
                .map(|c| c.item_id)
                .collect();
            ...
            recommendations.recommendation_reason =
                "Hot tier has available space. Consider promoting frequently accessed cold data."
                    .to_string();
...
mod tests {
    ...
    async fn create_test_setup() -> (TieringManager, Arc<OrionGraphEngine>) {
        ...
        let cold_tier = Arc::new(
            ColdStorageBackend::new(super::super::ColdStorageBackend::Json, temp_dir.path())
                .await
                .unwrap(),
        );
