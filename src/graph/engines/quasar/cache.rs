...
impl AccessPatternCache {
    ...
    pub async fn record_access(&self, item_id: &str, access_time: Instant) {
        ...
        self.update_average_access_frequency(&mut stats, &access_data).await;
    }
    ...
    fn calculate_access_frequency(&self, access_info: &AccessInfo) -> f64 {
        ...
        let time_span = access_info.last_access.duration_since(access_info.first_access);
...
mod tests {
    ...
    async fn test_multiple_accesses() {
        ...
        cache.record_access("item1", now).await;
        cache.record_access("item1", now + Duration::from_secs(1)).await;
        cache.record_access("item1", now + Duration::from_secs(2)).await;
    ...
    async fn test_lru_eviction() {
        ...
        cache.record_access("item1", now).await;
        cache.record_access("item2", now + Duration::from_secs(1)).await;
        cache.record_access("item3", now + Duration::from_secs(2)).await;
    ...
    async fn test_access_frequency_calculation() {
        ...
        cache.record_access("frequent_item", now + Duration::from_secs(30)).await;
        cache.record_access("frequent_item", now + Duration::from_secs(60)).await;
    ...
    async fn test_promotion_decision() {
        ...
        let should_promote = cache.should_promote(
            "recent_item", 
        ...
        // Old item
        cache.record_access("old_item", now - Duration::from_secs(7200)).await;
        
        let should_not_promote = cache.should_promote(
            "old_item", 
            Duration::from_secs(300)
        ).await;
    ...
    async fn test_demotion_decision() {
        ...
        let should_demote = cache.should_demote(
            "cold_item", 
    ...
    async fn test_access_summary() {
        ...
        cache.record_access("test_item", now + Duration::from_secs(10)).await;
