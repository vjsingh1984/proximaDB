...
impl XxHash64 {
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        while self.buffer.len() >= 32 {
            ...
            self.v2 = self.round(self.v2, u64::from_le_bytes(chunk[8..16].try_into().unwrap()));
            self.v3 = self.round(self.v3, u64::from_le_bytes(chunk[16..24].try_into().unwrap()));
            self.v4 = self.round(self.v4, u64::from_le_bytes(chunk[24..32].try_into().unwrap()));
    ...
    pub fn finish(&self) -> u64 {
        ...
        while remaining.len() >= 8 {
            ...
            hash = hash.rotate_left(27).wrapping_mul(Self::PRIME1).wrapping_add(Self::PRIME4);
        ...
        if remaining.len() >= 4 {
            ...
            hash = hash.rotate_left(23).wrapping_mul(Self::PRIME2).wrapping_add(Self::PRIME3);
...
mod tests {
    ...
    fn test_fnv_avalanche() {
        ...
        assert!(different_bits >= 16, "Poor avalanche effect: only {} bits differ", different_bits);
    }
    ...
    fn test_hash_string_convenience() {
        ...
        assert_eq!(XxHasher::hash_string(str_data), XxHasher::hash_bytes(bytes_data));
        assert_eq!(FnvHasher::hash_string(str_data), FnvHasher::hash_bytes(bytes_data));
    }
