...
use crate::index::{GlobalIdIndex, MetadataIndex, DenseVectorIndex, SparseVectorIndex, JoinEngine};
use super::{
    AxisConfig, AdaptiveIndexEngine, IndexMigrationEngine, PerformanceMonitor,
    IndexStrategy, IndexType, MigrationDecision,
};
...
impl AxisIndexManager {
    ...
    pub async fn query(&self, query: HybridQuery) -> Result<QueryResult> {
        ...
        let results = self.join_engine.execute_query(
            &query,
        ...
        let active_results: Vec<_> = results.into_iter()
            .filter(|result| {
    ...
    pub async fn analyze_and_optimize(&self, collection_id: &CollectionId) -> Result<()> {
        ...
        let characteristics = self.adaptive_engine.analyze_collection(collection_id).await?;
        
        // Determine if migration is beneficial
        let decision = self.adaptive_engine.should_migrate(collection_id, &characteristics).await?;
        
        match decision {
            MigrationDecision::Migrate { from, to, estimated_improvement, .. } => {
            ...
            MigrationDecision::Stay { reason } => {
                println!("AXIS: Collection {} staying with current strategy: {}", collection_id, reason);
            }
        }
    ...
    async fn start_migration(
        ...
        migrations.insert(collection_id.clone(), MigrationStatus {
            migration_id,
        ...
        tokio::spawn(async move {
            let result = migration_engine.execute_migration(&collection_id, from, to).await;
            ...
            match result {
                Ok(migration_result) => {
                    ...
                    metrics.average_migration_time_ms = 
                        (metrics.average_migration_time_ms * (metrics.total_migrations - 1) + 
                         migration_result.duration_ms) / metrics.total_migrations;
                    
                    println!("AXIS: Migration completed for collection {} in {}ms", 
                ...
                Err(e) => {
                    ...
                    eprintln!("AXIS: Migration failed for collection {}: {}", collection_id, e);
                }
            }
        });
    ...
    async fn ensure_collection_strategy(&self, collection_id: &CollectionId) -> Result<()> {
        ...
        let characteristics = self.adaptive_engine.analyze_collection(collection_id).await?;
        let strategy = self.adaptive_engine.recommend_strategy(&characteristics).await?;
    ...
    pub async fn get_migration_status(&self, collection_id: &CollectionId) -> Option<MigrationStatus> {
        let migrations = self.active_migrations.read().await;
...
pub enum VectorQuery {
    Dense { vector: Vec<f32>, similarity_threshold: f32 },
    Sparse { vector: HashMap<u32, f32>, similarity_threshold: f32 },
}
