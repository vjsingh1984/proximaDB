...
impl EventLogQueue {
    ...
    async fn persist_state(&self) -> Result<()> {
        let state = QueueState {
            ...
            file_status: self.file_status.iter().map(|e| e.value().clone()).collect(),
            processed_offsets: self
                .processed_offsets
                .iter()
...
mod tests {
    ...
    async fn test_persistence_and_recovery() {
        ...
        {
            let queue =
                EventLogQueue::new(base_url, "test_collection".to_string(), filesystem_factory)
                    .await
                    .unwrap();
