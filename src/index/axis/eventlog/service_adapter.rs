...
use super::{
    EventLogConfig, EventLogManager, ExtractionMode, FileIndexingStatus, IndexEvent,
    service_interface::*,
};
...
impl EventLogServiceAdapter {
    ...
    pub async fn new(manager: Arc<EventLogManager>, mode: ServiceMode) -> Result<Arc<Self>> {
    ...
    pub async fn embedded(
        ...
        collection_cache: Arc<
            dashmap::DashMap<String, Arc<crate::proto::proximadb_v1::Collection>>,
        >,
    ) -> Result<Arc<Self>> {
...
impl EventLogQuery for EventLogServiceAdapter {
    ...
    async fn get_extraction_hints(
        ...
        self.manager
            .get_extraction_hints(&event.collection_id, event, index_type)
            .await
    }
...
impl EventLogCommand for EventLogServiceAdapter {
    ...
    async fn mark_batch_processed(&self, updates: Vec<ProcessedUpdate>) -> Result<()> {
        for update in updates {
            self.mark_processed(&update.event_id, &update.index_name)
                .await?;
        }
        Ok(())
    }
    ...
    async fn cleanup_compacted(&self, collection_id: &str, files: Vec<String>) -> Result<()> {
        self.manager
            .cleanup_compacted_files(collection_id, files)
            .await
    }
    ...
    pub async fn create(
        ...
        let service: Arc<dyn EventLogService> = match deployment_mode.as_deref() {
            ...
            Some("distributed") => {
                ...
                let manager =
                    EventLogManager::new(config, filesystem_factory, collection_cache).await?;
