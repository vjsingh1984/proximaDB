...
impl SimpleFlushNotifier {
    ...
    pub fn notify_compaction(
        ...
        let collection_id_clone = collection_id.clone();
        tokio::spawn(async move {
            let _ = service.add_compaction_event(
                &collection_id_clone,
                output_files,
                vector_count,
                storage_engine,
        ...
        debug!("Notified AXIS about compaction for collection {}", collection_id);
    }
    
    /// Check if file can be compacted (async but fast lookup)
    pub async fn can_compact(&self, collection_id: &str, file_path: &str) -> bool {
        self.queue_service.can_compact(collection_id, file_path).await
    }
    ...
    pub async fn cleanup_compacted(
        ...
        self.queue_service.cleanup_compacted_files(collection_id, deleted_files).await
    }
...
mod tests {
    ...
    async fn create_test_notifier() -> (SimpleFlushNotifier, Arc<MetadataQueueService>, TempDir) {
        ...
        let queue_service = MetadataQueueService::new(
            config,
            filesystem_factory,
            collection_cache,
        ).await.unwrap();
    ...
    async fn test_notify_flush_never_blocks() {
        ...
        assert!(elapsed.as_millis() < 10, "Notify took {:?}, should be instant", elapsed);
