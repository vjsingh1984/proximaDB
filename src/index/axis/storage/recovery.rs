...
use crate::index::axis::storage::serialization::{
    ...
    Index, SerializationError,
};
use crate::index::axis::integration::tiering_manager::AxisTieringManager;
use crate::index::axis::integration::collection_state::{CollectionStateManager, CollectionTierState, TierLevel};
...
impl IndexRecoveryManager {
    ...
    fn recover_collection_with_retries<'a>(
        ...
        match state {
            CollectionTierState::Memory { .. } => {
                info!("Collection {} already in memory, skipping recovery", collection_id);
                return Ok(());
            }
            
            CollectionTierState::Disk { disk_location, .. } => {
                info!("Recovering collection {} from disk: {:?}", collection_id, disk_location);
                self.recover_from_disk(collection_id, &disk_location.to_string_lossy()).await?;
            }
            
            CollectionTierState::Cloud { location, .. } => {
                info!("Recovering collection {} from cloud: {}", collection_id, location);
                self.recover_from_cloud(collection_id, &location).await?;
            }
            
            CollectionTierState::Unbuilt => {
                info!("Collection {} is unbuilt, looking for checkpoint", collection_id);
    ...
    async fn recover_from_checkpoint(
            ...
            if self.filesystem.exists(&delta_path).await
                ...
                if !deltas.is_empty() {
                    ...
                    if let Some(reconstructed) = delta_manager.reconstruct_current_state()? {
                        ...
                        self.load_index_to_memory(collection_id, reconstructed, index_type).await?;
                        
                        // Store delta manager for future updates
                        self.delta_managers.insert(collection_id.to_string(), delta_manager);
        ...
        self.load_index_to_memory(collection_id, checkpoint.index_data, index_type).await?;
    ...
    fn update_recovery_status(&self, collection_id: &str, status: RecoveryStatus) {
        self.recovery_status.insert(collection_id.to_string(), status);
    }
    ...
    async fn log_recovery_stats(&self) {
        ...
        info!("  Total vectors recovered: {}", stats.total_vectors_recovered);
    ...
    pub async fn create_checkpoint(
        ...
        let checkpoint = IndexSerializer::create_checkpoint(
            index_type,
            index_data,
            collection_id,
        )?;
        ...
        let checkpoint_path = format!("axis/checkpoints/{}/{}.checkpoint", 
        ...
        info!("Created checkpoint {} for collection {}", 
            checkpoint.checkpoint_id, collection_id);
...
mod tests {
    ...
    async fn test_recovery_status() {
        ...
        match status {
            RecoveryStatus::InProgress { progress_percent, .. } => {
