...
mod tests {
    ...
    async fn test_annoy_basic_functionality() {
        ...
        let index = AxisAnnoyIndex::new(config, 8);
    ...
    async fn test_annoy_cosine_distance() {
        ...
        let index = AxisAnnoyIndex::new(config, 4);
    ...
    async fn test_annoy_with_filter() {
        ...
        let filter = |record: &VectorRecord| -> bool {
            record.metadata.iter().any(|item| item.key == "category" && item.value == "1")
        };
    ...
    async fn test_annoy_search_k_parameter() {
        ...
        let index1 = AxisAnnoyIndex::new(config.clone(), 8);
        ...
        let index2 = AxisAnnoyIndex::new(config, 8);
