...
impl CollectionStateManager {
    ...
    pub fn update_transition_progress(
        &self,
        collection_id: &str,
        progress: f32,
    ) -> Result<()> {
        if let Some(mut state) = self.states.get_mut(collection_id) {
            if let CollectionTierState::Transitioning { ref mut progress as p, .. } = *state {
                *p = progress;
                debug!("📊 Transition progress for {}: {:.1}%", collection_id, progress * 100.0);
            }
        }
        Ok(())
    }
    ...
    pub fn complete_transition(
        ...
        let (from_tier, to_tier, started_at) = if let Some(state) = self.get_state(collection_id) {
            match state {
                CollectionTierState::Transitioning { from, to, started_at, .. } => {
                    let from_tier = Self::state_to_tier(&*from);
                    (from_tier, to, started_at)
                }
                _ => return Err(anyhow!("Collection {} not in transitioning state", collection_id)),
            }
        } else {
        ...
        info!("✅ Completed transition for {} to {:?} ({} ms)", 
    ...
    pub fn record_access(&self, collection_id: &str) {
        let mut history = self.access_history
            .entry(collection_id.to_string())
        ...
        history.access_count_1h = history.recent_accesses.iter()
            .filter(|&&t| t > hour_ago)
            .count() as u64;
        
        history.access_count_24h = history.recent_accesses.iter()
            .filter(|&&t| t > day_ago)
            .count() as u64;
    ...
    pub fn get_collections_by_heat(&self) -> Vec<(String, f64)> {
        let mut collections: Vec<(String, f64)> = self.states
            .iter()
            .map(|entry| {
    ...
    fn get_state_size(state: &CollectionTierState) -> usize {
        match state {
            ...
            CollectionTierState::Cloud { compressed_bytes, .. } => *compressed_bytes,
            _ => 0,
        }
    }
}
...
mod tests {
    ...
    fn test_state_transitions() {
        ...
        manager.complete_transition(
            collection_id,
            memory_state,
            TransitionReason::UserQuery,
        ).unwrap();
    ...
    fn test_promotion_demotion_candidates() {
        ...
        manager.set_state(hot_id, CollectionTierState::Disk {
        ...
        manager.set_state(cold_id, CollectionTierState::Memory {
