...
use super::clustering::{ClusteringConfig, ClusteringAlgorithm, KMeansConfig};
...
impl ClusterManager {
    ...
    pub async fn cluster_vectors(&mut self, vectors: &[Vec<f32>]) -> Result<Vec<ClusterAssignment>> {
        if vectors.is_empty() {
            return Ok(Vec::new());
        }
    ...
    async fn run_kmeans(&mut self, vectors: &[Vec<f32>], config: &KMeansConfig) -> Result<()> {
        ...
        for iteration in 0..config.max_iterations {
            ...
            let changed = new_assignments.iter()
