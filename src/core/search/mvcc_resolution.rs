...
impl MvccResolver {
    ...
    pub fn resolve_batch(&self, records: Vec<VectorRecord>) -> Vec<VectorRecord> {
        ...
        for record in records {
            if let Some(id) = &record.id {
                if id.is_empty() || id == "null" || id == "none" || id.trim().is_empty() {
                    ...
                    id_groups.entry(id.clone()).or_insert_with(Vec::new).push(record);
                }
            } else {
                // No ID = append-only
                append_only_records.push(record);
        ...
        for (id, mut versions) in id_groups {
            ...
            if has_expired {
                debug!("MVCC: All versions of ID '{}' are deleted due to expiry", id);
            ...
            for record in versions {
                ...
                if version == expected_version {
                    ...
                    debug!("MVCC: Version gap detected for ID '{}': expected {}, found {}", 
                           id, expected_version, version);
                    break;
                } else {
                    // Version < expected_version, this is an older version, skip it
                    debug!("MVCC: Skipping older version {} for ID '{}' (expected {})", 
...
mod tests {
    ...
    fn create_record(id: Option<String>, version: Option<u32>, timestamp: u32, expires_at: Option<u32>) -> VectorRecord {
        VectorRecord {
            id,
    ...
    fn test_multiple_ids_different_versions() {
        ...
        let mut by_id: std::collections::HashMap<String, &VectorRecord> = std::collections::HashMap::new();
    ...
    fn test_expired_records_mark_entire_id_deleted() {
        ...
        for record in &resolved {
            match &record.id {
                None => append_only_count += 1,
                Some(id) => { by_id.insert(id.clone(), record); }
            }
        }
