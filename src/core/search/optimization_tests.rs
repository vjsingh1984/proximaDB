...
mod tests {
    ...
    use crate::storage::cache::specialized::{QueryCache, VectorStore, MetadataStore};
    ...
    async fn test_query_preprocessing_with_simd() {
        ...
        let result = preprocessor.preprocess(&query_vector, DistanceMetric::Cosine).await;
        ...
        // Test cache hit
        let result2 = preprocessor.preprocess(&query_vector, DistanceMetric::Cosine).await;
    ...
    async fn test_metadata_filter_pushdown() {
        ...
        assert!(should_process, "Should process vector with matching metadata");
        
        // Test selectivity estimation
        let selectivity = filter_pushdown.estimate_selectivity(&filter);
        assert!(selectivity > 0.0 && selectivity <= 1.0, "Selectivity should be between 0 and 1");
    }
    ...
    async fn test_progressive_search_pipeline() {
        ...
        assert!(stats.binary_candidates > 0, "Binary stage should process candidates");
        if config.enable_int8_stage {
            assert!(stats.int8_candidates > 0, "INT8 stage should process candidates");
        }
    }
    ...
    async fn test_integrated_search_optimization() {
        ...
        for i in 0..20 {
            assert_eq!(
                search_results1.results[i].id,
                search_results2.results[i].id,
