    ...
    fn check_bloom_filters(&self, record: &VectorRecord, filter: &FilterExpression) -> bool {
        match filter {
            ...
            FilterExpression::And(exprs) => {
                exprs.iter().all(|expr| self.check_bloom_filters(record, expr))
            }
            FilterExpression::Or(exprs) => {
                exprs.iter().any(|expr| self.check_bloom_filters(record, expr))
            }
            FilterExpression::Not(expr) => {
                !self.check_bloom_filters(record, expr)
            }
        }
    }
    ...
    fn estimate_selectivity(&self, filter: &FilterExpression) -> f64 {
        match filter {
            ...
            FilterExpression::Or(exprs) => {
                ...
                let selectivities: Vec<f64> = exprs.iter()
                    .map(|expr| self.estimate_selectivity(expr))
                    .collect();
                
                // P(A or B) = P(A) + P(B) - P(A and B)
                1.0 - selectivities.iter()
                    .map(|s| 1.0 - s)
                    .product::<f64>()
            }
            FilterExpression::Not(expr) => {
    ...
    fn estimate_comparison_selectivity(
        ...
        match operator {
            ...
            ComparisonOperator::NotEquals => {
                1.0 - self.estimate_comparison_selectivity(stats, &ComparisonOperator::Equals, value)
            }
            ...
            ComparisonOperator::In => {
                if let Value::Array(values) = value {
                    values.iter()
                        .map(|v| self.estimate_comparison_selectivity(stats, &ComparisonOperator::Equals, v))
                        .sum::<f64>()
                        .min(1.0)
                } else {
                    0.5
            ...
            match value_opt {
                Some(value) => {
                    ...
                    *value_histogram.entry(value.clone()).or_insert(0) += 1;
                    ...
                    if min_value.is_none() || self.compare_values(value, min_value.as_ref().unwrap()) < 0 {
                        min_value = Some(value.clone());
                    }
                    if max_value.is_none() || self.compare_values(value, max_value.as_ref().unwrap()) > 0 {
