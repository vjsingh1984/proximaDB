...
mod tests {
    ...
    fn test_sst_config_defaults() {
        ...
        // Test block size
        assert_eq!(config.block_size_kb, 4096); // 4MB blocks
        assert_eq!(config.block_size_bytes(), 4096 * 1024);
    ...
    fn test_write_buffer_config_values_propagate() {
        ...
        assert_eq!(toml_config.write_buffer_size_mb * 1024 * 1024, 8192 * 1024 * 1024);
        assert_eq!(toml_config.vector_count_threshold, 20000);
        assert_eq!(toml_config.enable_wal, true);
        assert_eq!(toml_config.write_buffer_directory, "./test_data/write_buffer");
    }
