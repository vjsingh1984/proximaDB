    ...
    T: Send + 'static,
{
    ...
    pub fn acquire(&self) -> PooledItem<T> {
        ...
        if let Some(mut pooled) = buffers.pop_front() {
            ...
            PooledItem::new(pooled.buffer, self.buffers.clone(), self.stats.clone(), self.config.clone())
        } else {
            ...
            PooledItem::new(buffer, self.buffers.clone(), self.stats.clone(), self.config.clone())
        }
    }
...
mod tests {
    ...
    fn test_concurrent_pool_access() {
        let pool = Arc::new(Pool::new(PoolConfig::default(), || Vec::<u8>::with_capacity(1024)));
