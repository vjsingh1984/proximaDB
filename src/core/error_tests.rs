...
mod tests {
    ...
    fn test_vector_db_error_display() {
        ...
        assert_eq!(quant_err.to_string(), "Quantization error: Invalid codebook");
    }
    ...
    fn test_storage_error_variants() {
        ...
        let io_err = StorageError::DiskIO(io::Error::new(io::ErrorKind::PermissionDenied, "Access denied"));
        ...
        assert!(corruption_err.to_string().contains_hash("Corruption detected"));
        
        let exists_err = StorageError::AlreadyExists("collection_1".to_string());
        assert_eq!(exists_err.to_string(), "Resource already exists: collection_1");
        ...
        let dimension_err = StorageError::InvalidDimension { expected: 128, actual: 256 };
        assert_eq!(dimension_err.to_string(), "Invalid vector dimension: expected 128, got 256");
    }
    ...
    fn test_consensus_error_variants() {
        ...
        assert_eq!(leader_err.to_string(), "Leadership error: No leader elected");
        
        let repl_err = ConsensusError::Replication("Insufficient replicas".to_string());
        assert_eq!(repl_err.to_string(), "Replication error: Insufficient replicas");
    }
    ...
    fn test_query_error_variants() {
        ...
        assert_eq!(search_err.to_string(), "Vector search error: Invalid distance metric");
        ...
        assert_eq!(invalid_err.to_string(), "Invalid query: Missing WHERE clause");
    ...
    fn test_schema_error_variants() {
        ...
        assert_eq!(invalid_err.to_string(), "Invalid schema: Missing required field");
        
        let mismatch_err = SchemaError::SchemaMismatch("Expected int, got string".to_string());
        assert_eq!(mismatch_err.to_string(), "Schema mismatch: Expected int, got string");
        
        let validation_err = SchemaError::Validation("Field exceeds max length".to_string());
        assert_eq!(validation_err.to_string(), "Schema validation error: Field exceeds max length");
    }
