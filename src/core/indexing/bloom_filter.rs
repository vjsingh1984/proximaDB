...
impl BloomFilterCollection {
    ...
    pub fn get_or_create(&mut self, key: &str) -> &mut BloomFilter {
        self.filters.entry(key.to_string()).or_insert_with(|| {
            BloomFilter::new(
                self.default_expected_elements,
                self.default_false_positive_rate,
            )
        })
    }
    ...
    pub fn any_might_contain<T: Hash>(&self, element: &T) -> bool {
        self.filters
            .values()
            .any(|filter| filter.might_contain(element))
    }
    ...
    pub fn collection_stats(&self) -> BloomFilterCollectionStats {
        ...
        let avg_false_positive_rate = if self.filters.is_empty() {
            ...
            self.filters
                .values()
                .map(|f| f.actual_false_positive_rate())
                .sum::<f64>()
...
mod tests {
    ...
    fn test_bloom_filter_collection() {
        ...
        assert!(collection
            .get("sstable1")
            .unwrap()
            .might_contain(&"vector_1"));
        assert!(collection
            .get("sstable2")
            .unwrap()
            .might_contain(&"vector_3"));
        assert!(!collection
            .get("sstable1")
            .unwrap()
            .might_contain(&"vector_3"));
