    ...
    pub fn load_with_defaults<P: AsRef<str>>(config_path: P) -> Result<Config, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            ...
            tokio::task::block_in_place(|| {
                handle.block_on(Self::load_config_async(config_url))
            })
        } else {
            ...
            let rt = Runtime::new()?;
            rt.block_on(async {
                Self::load_config_async(config_url).await
            })
        }
    }
    
    /// Async config loading using unified filesystem API
    async fn load_config_async(config_url: &str) -> Result<Config, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if filesystem_factory.exists(config_url).await? {
            ...
            info!("✅ Configuration loaded and merged with defaults from: {}", config_url);
            Ok(merged_config)
        } else {
