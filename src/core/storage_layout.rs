...
impl StoragePathResolver {
    ...
    pub async fn ensure_collection_directories(
        &mut self,
        collection_uuid: &str,
    ...
    fn get_or_assign_instance(&mut self, collection_uuid: &str) -> Result<u32, StorageLayoutError> {
        ...
        let instance_id = match &self.config.assignment_strategy {
            CollectionAssignmentStrategy::RoundRobin => {
                ...
                let next_instance =
                    (self.assignment_cache.len() % self.config.base_paths.len()) + 1;
        ...
        self.assignment_cache
            .insert(collection_uuid.to_string(), instance_id);
        Ok(instance_id)
    }
...
mod tests {
    ...
    fn test_default_paths() {
        ...
        assert_eq!(
            wal_path,
            PathBuf::from("/data/proximadb/1/wal/test-uuid-123")
        );
        assert_eq!(
            storage_path,
