...
impl SqlParser {
    ...
    fn parse_between_condition(&mut self, field: String) -> Result<Condition> {
        ...
        if !self.check_keyword("AND") {
            return Err(anyhow!(
                "Expected 'AND' after BETWEEN lower bound at position {}",
                self.position
            ));
        }
        self.consume_keyword("AND");
    ...
    fn parse_field_with_json_path(&mut self) -> Result<String> {
        ...
        if self.check_chars("->>") {
            ...
            if !self.check_char('\'') {
                return Err(anyhow!(
                    "Expected string literal after ->> at position {}",
                    self.position
                ));
            }
    ...
    fn parse_value_list(&mut self) -> Result<Value> {
        ...
        loop {
            ...
            if self.check_char(',') {
                ...
                return Err(anyhow!(
                    "Expected ',' or ')' in value list at position {}",
                    self.position
                ));
            }
        }
    ...
    fn check_char(&mut self, ch: char) -> bool {
        self.skip_whitespace();
        if self.position < self.query.len() && self.query.chars().nth(self.position) == Some(ch) {
