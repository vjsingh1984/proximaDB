...
mod tests {
    ...
    use crate::query::sql_engine::parser::{
        OrderType, OrderByClause, SortDirection, WhereClause, 
        Condition, ComparisonOp, Value as SqlValue,
    };
    ...
    fn create_sql_test_vectors() -> Vec<SqlTestVector> {
        ...
        for i in 0..120 {
            ...
            let vector = match i % 6 {
                ...
                5 => (0..128).map(|j| ((i + j) as f32).powi(2) / 10000.0).collect(), // Quadratic
                _ => unreachable!(),
            };
            
            // Create rich metadata for SQL filtering
            let mut metadata = HashMap::new();
            metadata.insert("category".to_string(), Value::String(format!("cat_{}", i % 15)));
            metadata.insert("priority".to_string(), Value::Number(serde_json::Number::from(i % 7)));
            metadata.insert("source".to_string(), Value::String(
                match i % 4 {
                    0 => "user",
                    1 => "system", 
                    2 => "api",
            ...
            metadata.insert("score".to_string(), Value::Number(serde_json::Number::from_f64((i as f64) / 10.0).unwrap()));
            metadata.insert("tags".to_string(), Value::Array(vec![
                Value::String(format!("tag_{}", i % 5)),
                Value::String(format!("tag_{}", (i + 1) % 5)),
            ]));
            metadata.insert("created_year".to_string(), Value::Number(serde_json::Number::from(2020 + (i % 5))));
            metadata.insert("region".to_string(), Value::String(
                match i % 3 {
                    0 => "north",
                    1 => "south", 
                    2 => "west",
    ...
    async fn test_sql_hardware_backend_selection() -> Result<()> {
        ...
        assert!(parsed_query.order_by.is_some(), "Should have ORDER BY clause");
        if let Some(order_by) = parsed_query.order_by {
            ...
            assert_eq!(order_by.direction, SortDirection::Desc, "Should be DESC order");
        }
        ...
        for test_case in test_cases.iter().take(12) { // Test subset for efficiency
            ...
            if let Some(order_by) = &parsed_query.order_by {
                if let OrderType::VectorSimilarity { metric, .. } = &order_by.order_type {
                    ...
                    assert_eq!(parsed_metric, test_case.expected_distance_metric, 
            ...
            if let Some(order_by) = &parsed_query.order_by {
                if let OrderType::VectorSimilarity { metric, .. } = &order_by.order_type {
                    ...
                    for wal_vec in wal_vectors.iter().take(5) {
                        ...
                        assert!(!distance_result.raw_value.is_nan(),
                            "WAL vector distance should not be NaN for {}", test_name);
                        assert!(distance_result.normalized_score >= 0.0 && distance_result.normalized_score <= 1.0,
                    ...
                    assert_eq!(batch_results.len(), mixed_vectors.len(),
                        "Batch should return all results for {}", test_name);
                    
                    for (i, result) in batch_results.iter().enumerate() {
                        assert!(!result.raw_value.is_nan(),
                            "Mixed batch result {} should not be NaN for {}", i, test_name);
                    ...
                    assert_eq!(metric.to_lowercase(), expected_metric_str,
                        "WAL search should use {} metric", expected_metric_str);
                }
            }
    ...
    async fn test_sql_storage_engine_integration() -> Result<()> {
        ...
        let test_engines = vec![
            ("LSM", SqlTestEngine::Lsm),
            ("VIPER", SqlTestEngine::Viper),
        ];
        ...
        for (engine_name, engine_type) in test_engines {
            for (metric_name, metric_enum) in &engine_test_metrics {
                ...
                assert!(parsed_query.order_by.is_some(), "Should have ORDER BY clause");
                assert!(parsed_query.where_conditions.is_some(), "Should have WHERE clause");
                ...
                assert!(!distance_result.raw_value.is_nan(), 
                    "Distance should not be NaN for {}", test_name);
                assert!(distance_result.normalized_score >= 0.0 && distance_result.normalized_score <= 1.0,
    ...
    async fn test_sql_performance_hardware_acceleration() -> Result<()> {
        ...
        for (test_name, sql_query, metric, expected_limit) in performance_tests {
            ...
            let distance_results = distance_compute.calculate_distance_batch(
                &query_vector,
                &vector_refs,
                &metric,
            );
            ...
            // Verify results
            assert_eq!(distance_results.len(), expected_limit, "Should compute all distances");
