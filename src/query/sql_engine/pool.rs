...
use crate::query::sql_engine::parser::{ParsedQuery, SqlParser};
use anyhow::Result;
...
impl PoolStats {
    ...
    pub fn update_pool_size(&self, size: usize) {
        ...
        loop {
            ...
            if self
                .peak_pool_size
                .compare_exchange_weak(current_peak, size, Ordering::Relaxed, Ordering::Relaxed)
                .is_ok()
            {
                break;
            }
...
mod tests {
    ...
    fn test_concurrent_parsing() {
        ...
        debug!(
            "Concurrent test - Created: {}, Reused: {}, CPUs: {}",
            created, reused, num_cpus
        );
    ...
    fn test_parser_pool_performance() {
        ...
        debug!(
            "  Pool stats - Created: {}, Reused: {}, Pool size: {}, Peak: {}",
        ...
        assert!(
            reuse_ratio > 0.85,
            "Reuse ratio should be >85% with limited pool, got {:.2}",
            reuse_ratio
        );
        ...
        assert!(
            created <= num_cpus * 2,
            "Created parsers ({}) should be <= 2x CPU count ({})",
            created,
            num_cpus * 2
        );
    }
    ...
    fn test_concurrent_stress() {
        ...
        debug!(
            "  Total queries: {}/{} successful in {:?}",
            total_success, expected_total, elapsed
        );
        debug!(
            "  Throughput: {:.0} queries/sec",
        ...
        debug!(
            "  Parser stats - Created: {}, Reused: {} ({:.1}% reuse)",
        ...
        assert!(
            reuse_ratio > 70.0,
            "Parser reuse ratio should be >70%, got {:.1}%",
            reuse_ratio
        );
    }
}
