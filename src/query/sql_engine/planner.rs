    ...
    fn convert_condition(
        ...
        match condition {
            ...
            Condition::And(left, right) => {
                ...
                Ok(crate::core::search::FilterExpression::And(vec![
            ...
            Condition::Or(left, right) => {
                let left_expr = self.convert_condition(left)?;
                let right_expr = self.convert_condition(right)?;
                Ok(crate::core::search::FilterExpression::Or(vec![
                    left_expr, right_expr,
                ]))
            }
            Condition::Not(inner) => {
                let inner_expr = self.convert_condition(inner)?;
                Ok(crate::core::search::FilterExpression::Not(Box::new(
                    inner_expr,
                )))
            }
    ...
    fn convert_sql_value(&self, value: &Value) -> Result<serde_json::Value> {
        match value {
            ...
            Value::Number(n) => Ok(serde_json::Value::Number(
                serde_json::Number::from_f64(*n).ok_or_else(|| anyhow!("Invalid number"))?,
            )),
