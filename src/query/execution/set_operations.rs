...
use anyhow::Result;
use tracing::debug;
use crate::query::ast::{Query, SetOp, Cte};
use crate::query::execution::{ExecutionPlan, ExecutionOperation, ExecutionStrategy, SeedingStrategy};
...
pub trait SetOperationPlanner {
    ...
    fn plan_set_operation(&self, left: &Query, op: &SetOp, all: bool, right: &Query) -> Result<ExecutionPlan>;
}

impl SetOperationPlanner for crate::query::execution::planner::ExecutionPlanner {
    /// Plan Common Table Expression (CTE) queries
    fn plan_cte(&self, ctes: &[Cte], query: &Query) -> Result<ExecutionPlan> {
        debug!("Planning CTE query with {} common table expressions", ctes.len());
        ...
        Ok(ExecutionPlan {
            ...
            seeding_strategy: SeedingStrategy::Sequential,
