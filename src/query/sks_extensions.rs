...
impl SksExecutor {
    ...
    pub async fn execute_similar(
        ...
        info!("Executing SIMILAR function on field: {}", similar.embedding_field);
    ...
    pub async fn execute_assemble(
        ...
        info!("Executing ASSEMBLE function with {} context items", context_items.len());
        ...
        let rows = assembled_context.into_iter()
...
impl SksFunction {
    ...
    pub fn to_execution_operations(&self) -> Result<Vec<ExecutionOperation>> {
        match self {
            ...
            SksFunction::Follow { start_node, edge_type, max_depth, direction: _ } => {
            ...
            SksFunction::Assemble { context_items, assembly_strategy: _, max_context_size: _ } => {
    ...
    fn create_threshold_filter(&self, threshold: Option<f32>) -> Result<Option<crate::core::search::FilterExpression>> {
...
mod sks_integration_tests {
    ...
    fn test_hashmap_metadata_performance_in_sks() {
        ...
        assert!(conversion_time.as_millis() < 10, 
                "HashMap metadata conversion should be very fast, took {:?}", conversion_time);
    }
