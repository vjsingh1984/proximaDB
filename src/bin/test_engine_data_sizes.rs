...
fn main() {
    ...
    println!(
        "  Vectors: {} x {} x {} bytes = {} bytes ({:.2} MB)",
        num_vectors,
        dimension,
        bytes_per_float,
        vector_data_size,
        vector_data_size as f64 / (1024.0 * 1024.0)
    );

    // ID strings (assuming ~20 bytes per ID)
    let id_data_size = num_vectors * 20;
    println!(
        "  IDs: {} x ~20 bytes = {} bytes ({:.2} KB)",
        num_vectors,
        id_data_size,
        id_data_size as f64 / 1024.0
    );

    // Metadata (2 fields per record, ~50 bytes total)
    let metadata_size = num_vectors * 50;
    println!(
        "  Metadata: {} x ~50 bytes = {} bytes ({:.2} KB)",
        num_vectors,
        metadata_size,
        metadata_size as f64 / 1024.0
    );

    // Timestamps and versions
    let timestamp_size = num_vectors * 8 * 2; // 2 i64 fields
    println!(
        "  Timestamps: {} x 16 bytes = {} bytes ({:.2} KB)",
        num_vectors,
        timestamp_size,
        timestamp_size as f64 / 1024.0
    );
    ...
    println!(
        "  SST (Proxima + compression): ~{:.1} MB",
        total_raw_size as f64 / (1024.0 * 1024.0) * 0.7
    );
    println!(
        "  SWIFT (Proxima blocks): ~{:.1} MB",
        total_raw_size as f64 / (1024.0 * 1024.0) * 0.7
    );
    println!(
        "  VIPER (Parquet + Zstd): ~{:.1} MB",
        total_raw_size as f64 / (1024.0 * 1024.0) * 0.5
    );
    println!(
        "  NOVA (Parquet + Zstd): ~{:.1} MB",
        total_raw_size as f64 / (1024.0 * 1024.0) * 0.5
    );
    ...
    let estimated_record_size = single_record.id.len() +                          // ID
        (single_record.vector.len() * 4) +                // Vector data
        50 +                                               // Metadata (estimated)
        16; // Timestamps

    println!(
        "\n📏 Single record estimated size: {} bytes",
