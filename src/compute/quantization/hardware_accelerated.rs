...
impl AcceleratedQuantization {
    ...
    pub fn new() -> Self {
            ...
            #[cfg(feature = "gpu")]
            {
                if caps.gpu.backend != crate::core::hardware_capabilities::GpuBackend::None {
                    match caps.gpu.backend {
                        crate::core::hardware_capabilities::GpuBackend::CUDA => HardwareBackend::CUDA,
                        crate::core::hardware_capabilities::GpuBackend::ROCm => HardwareBackend::ROCm,
                        crate::core::hardware_capabilities::GpuBackend::MPS => HardwareBackend::MPS,
                        crate::core::hardware_capabilities::GpuBackend::OpenCL => HardwareBackend::OpenCL,
        ...
        tracing::info!("AcceleratedQuantization initialized with backend: {:?}", backend);
        Self { backend }
    }
    
    fn select_cpu_backend(caps: &crate::core::hardware_capabilities::HardwareCapabilities) -> HardwareBackend {
    ...
    fn quantize_u8_scalar(&self, values: &[f32]) -> Result<(Vec<u8>, f32, f32)> {
        ...
        let quantized: Vec<u8> = values.iter().map(|&v| {
            let normalized = ((v - min) / range).clamp(0.0, 1.0);
    ...
    fn quantize_u16_scalar(&self, values: &[f32]) -> Result<(Vec<u16>, f32, f32)> {
        ...
        let quantized: Vec<u16> = values.iter().map(|&v| {
            let normalized = ((v - min) / range).clamp(0.0, 1.0);
