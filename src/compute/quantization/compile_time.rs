...
macro_rules! quantize_compile_time {
    ($vec:expr, Int8, $dim:expr) => {{
        use $crate::compute::quantization::compile_time::{Int8CompileTime, CompileTimeQuantization};
        Int8CompileTime::<$dim>::quantize_const($vec)
    }};
    ($vec:expr, PQ4, $dim:expr, $subvectors:expr) => {{
        use $crate::compute::quantization::compile_time::{PQ4CompileTime, CompileTimeQuantization};
