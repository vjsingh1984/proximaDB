...
impl QuantizationEngine {
    ...
    pub fn new(config: QuantizationConfig) -> Result<Self> {
        ...
        Ok(Self { config, quantizer })
    }

    /// Create the appropriate quantizer based on configuration
    fn create_quantizer(
        config: &QuantizationConfig,
    ) -> Result<Box<dyn VectorQuantizer + Send + Sync>> {
        match config.quantization_type {
            ...
            QuantizationType::None => Err(ProximaDBError::Config(
                "No quantization specified".to_string(),
            )),
        }
    }
