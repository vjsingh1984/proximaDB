...
impl HardwareCapabilities {
    ...
    pub fn get() -> &'static HardwareCapabilities {
        HARDWARE_CAPABILITIES
            .get()
            .expect("Hardware capabilities not initialized - call initialize() first")
    }
    ...
    fn log_capabilities(capabilities: &HardwareCapabilities) {
        ...
        info!(
            "  CPU: {} ({} cores, {} threads)",
            cpu.model_name, cpu.cores, cpu.threads
        );
        info!(
            "  SIMD: {:?} (SSE:{} AVX:{} AVX2:{} FMA:{})",
            paths.simd_level, cpu.has_sse, cpu.has_avx, cpu.has_avx2, cpu.has_fma
        );

        if gpu.has_cuda {
            info!(
                "  CUDA: {} devices available (version: {:?})",
                gpu.devices.len(),
                gpu.cuda_version
            );
            for device in &gpu.devices {
                if device.is_cuda {
                    info!(
                        ...
                        device.id,
                        device.name,
                        device.memory_total_mb as f64 / 1024.0
                    );
                }
            }
        }
    ...
    fn get_page_size() -> u32 {
        unsafe { libc::sysconf(libc::_SC_PAGESIZE) as u32 / 1024 }
    }
