...
impl HNSWIndex {
    pub fn new(
        ...
        let distance_computer =
            crate::compute::distance::create_distance_computer(distance_metric, use_simd);
                        ...
                        if let Some((vector, _)) = self.vectors.get(&neighbor) {
                            ...
                            if dynamic_candidates.len() < ef {
                                ...
                                if dist < worst_dist.0 {
                                    candidates
                                        .push(std::cmp::Reverse((OrderedFloat(dist), neighbor)));
                                    dynamic_candidates.push((OrderedFloat(dist), neighbor));
...
impl VectorSearchAlgorithm for HNSWIndex {
    fn add_vector(
        ...
        for layer in (level + 1..self.layers.len()).rev() {
            curr_nearest = self
                .search_layer(&vector, &curr_nearest, 1, layer)
                .into_iter()
                .map(|(node, _)| node)
                .collect();
        }
    ...
    fn search(&self, query: &[f32], k: usize) -> Result<Vec<SearchResult>, String> {
        tracing::debug!(
            "🔍 HNSWIndex::search: query_dim={}, k={}, vectors={}, entry_point={:?}",
            query.len(),
            k,
            self.vectors.len(),
            self.entry_point
    ...
    fn search_with_filter(
        ...
        tracing::debug!(
            "🔍 HNSWIndex::search_with_filter: query_dim={}, k={}, vectors={}",
            query.len(),
            k,
            self.vectors.len()
        );
        ...
        tracing::debug!(
            "🔍 HNSWIndex::search_with_filter: got {} raw results",
            all_results.len()
        );
        ...
        tracing::debug!(
            "🔍 HNSWIndex::search_with_filter: returning {} filtered results",
            filtered.len()
        );
        Ok(filtered)
    }
...
impl VectorSearchAlgorithm for BruteForceIndex {
    ...
    fn memory_usage(&self) -> MemoryUsage {
        ...
        let metadata_bytes = self
            .vectors
            .iter()
            .map(|(_, (_, metadata))| metadata.as_ref().map(|m| m.len() * 100).unwrap_or(0))
            .sum::<usize>();
...
pub fn create_search_algorithm(
    ...
    match algorithm {
        crate::compute::IndexAlgorithm::HNSW {
            m, ef_construction, ..
