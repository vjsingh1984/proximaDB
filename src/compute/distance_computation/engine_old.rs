...
fn get_cached_simd_calculator(metric: DistanceMetric) -> Box<dyn super::core::DistanceCompute> {
    // Direct core implementation to avoid the circular dependency and adapter overhead
        ...
        if vec_a.len() == vec_b.len() {
            ...
            let raw_value = match metric {
                ...
                DistanceMetric::Cosine | DistanceMetric::Euclidean => {
                    ...
                    let caps = get_hardware_capabilities();
                    if self.gpu_enabled
                        && self.gpu_accelerator_lazy.get().and_then(|g| g.as_ref()).is_some()
