...
///
/// Provides centralized creation of tenant and domain knowledge graphs
/// with proper RBAC policy integration.
...
impl KnowledgeGraphFactory {
    ...
    /// * `domain_name` - Domain name within tenant
    /// * `domain_config` - Domain-specific configuration
    /// * `rbac_policy` - RBAC policy for the domain
    ...
    /// # Arguments
    ///
    /// * `global_store` - Legacy global entity store
    /// * `migration_plan` - Migration plan with tenant/domain mappings
...
///
/// Provides centralized access to tenant knowledge graphs across the system.
/// Supports a default tenant for backward compatibility with legacy code.
pub struct TenantKnowledgeGraphRegistry {
    /// Map of tenant ID to knowledge graph
    tenant_graphs: Arc<dashmap::DashMap<String, Arc<TenantKnowledgeGraph>>>,
    /// Default tenant ID for legacy code compatibility
...
impl TenantKnowledgeGraphRegistry {
    ...
    /// # Returns
    ///
    /// Some(graph) if found, None otherwise
    ...
    /// # Arguments
    ///
    /// * `tenant_id` - Tenant ID to use as default
    ...
    /// # Returns
    ///
    /// Some(graph) if default tenant is set, None otherwise
