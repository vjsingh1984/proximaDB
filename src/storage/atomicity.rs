...
pub enum OperationType {
    ...
    /// Batch write operations (legacy)
    BatchWrite,

    /// Vector batch operations (modern)
    VectorBatch,
}
...
pub enum ResourceId {
    ...
    /// Memtable partition resource
    MemtablePartition(String),
...
pub enum OperationResult {
    ...
    Simple {
        ...
        /// 🚫 DEPRECATED: WAL entries generated by this operation
        /// Use BatchWrite results for batch-oriented operations instead
        #[deprecated(note = "Use BatchWrite results for batch-oriented operations instead")]
    ...
    /// Batch write operation result (modern)
    BatchWrite {
        /// Batch identifier
        batch_id: String,
        /// Number of sequences written
        sequences_written: u64,
        /// Total bytes written
        bytes_written: u64,
        /// Memtable write time in microseconds
        memtable_time_us: u128,
        /// Disk write time in microseconds
        disk_time_us: u128,
        /// Total operation time in microseconds
        total_time_us: u128,
    },

    /// Vector batch operation result (modern WalVectorBatch-based)
    VectorBatch {
        /// Batch ID from WalVectorBatch
        batch_id: crate::storage::persistence::wal::BatchId,
        /// Sequences assigned to vectors in the batch
        sequences: Vec<u64>,
...
/// 🚫 DEPRECATED: Vector insert operation implementation
/// Use batch-oriented operations with WalVectorBatch instead
#[deprecated(note = "Use batch-oriented operations with WalVectorBatch instead")]
#[derive(Debug)]
...
/// 🚫 DEPRECATED: Vector update operation implementation
/// Use batch-oriented operations with WalVectorBatch instead
#[deprecated(note = "Use batch-oriented operations with WalVectorBatch instead")]
#[derive(Debug)]
...
/// 🚫 DEPRECATED: Vector delete operation implementation
/// Use batch-oriented operations with WalVectorBatch instead
#[deprecated(note = "Use batch-oriented operations with WalVectorBatch instead")]
#[derive(Debug)]
...
/// 🎯 MODERN: Vector batch operation implementation using native WalVectorBatch
/// This replaces deprecated individual operations with efficient batch processing
#[derive(Debug)]
pub struct VectorBatchOperation {
    pub collection_id: CollectionId,
    pub vector_batch: crate::storage::memtable::specialized::wal_behavior::WalVectorBatch,
}

impl VectorBatchOperation {
    /// Create a new vector batch operation from a single vector (batch of size 1)
    pub fn from_single_vector(
        ...
        let vector_batch = WalVectorBatch {
            batch_id,
    ...
    /// Create a new vector batch operation from multiple vectors
    pub fn from_multiple_vectors(
        collection_id: CollectionId,
        ...
        let total_size_bytes = vector_records.iter()
            .map(|v| v.actual_size_bytes())
            .sum();
        
        let batch_id = BatchId::new(collection_id.clone(), 1, vector_count as u64);
    ...
    /// Create operation directly from an existing WalVectorBatch
    pub fn from_wal_batch(
        vector_batch: crate::storage::memtable::specialized::wal_behavior::WalVectorBatch,
    ) -> Self {
        let collection_id = vector_batch.batch_id.collection_id.clone();
        Self {
            collection_id,
            vector_batch,
        }
...
/// Bulk vector operations implementation (LEGACY - use VectorBatchOperation instead)
#[derive(Debug)]
...
impl AtomicOperation for VectorInsertOperation {
    async fn execute(&self, _context: &mut TransactionContext) -> Result<OperationResult> {
        ...
        // Create WAL entry using modern AvroPayload format
        let avro_data = self.vector_record.to_avro_bytes()
            .map_err(|e| anyhow::anyhow!("Failed to serialize vector record to Avro: {}", e))?;
        ...
        let wal_entry = WalEntry {
            ...
            expires_at: None,
            version: 1,
            batch_id: None,     // Will be assigned by WAL manager
        };
...
impl AtomicOperation for VectorUpdateOperation {
    async fn execute(&self, _context: &mut TransactionContext) -> Result<OperationResult> {
        ...
        // Create WAL entry for update using modern AvroPayload format
        let avro_data = self.vector_record.to_avro_bytes()
            .map_err(|e| anyhow::anyhow!("Failed to serialize vector record to Avro: {}", e))?;
        ...
        let wal_entry = WalEntry {
            ...
            collection_id: self.collection_id.clone(),
            operation: WalOperation {
                operation_type: "upsert".to_string(), // Update is also upsert in modern format
                payload_data: avro_data,
                payload_format: "avro".to_string(),
                vector_count: 1,
            },
...
impl AtomicOperation for VectorDeleteOperation {
    async fn execute(&self, _context: &mut TransactionContext) -> Result<OperationResult> {
        ...
        // Create delete vector record with expires_at for logical delete
        let delete_record = VectorRecord {
            id: self.vector_id.clone(),
            collection_id: self.collection_id.clone(),
            vector: vec![], // Empty vector for delete operation
            metadata: std::collections::HashMap::new(),
            timestamp: Utc::now().timestamp_micros(),
            created_at: Utc::now().timestamp_micros(),
            updated_at: Utc::now().timestamp_micros(),
            expires_at: Some(Utc::now().timestamp_micros()), // Logical delete - expires immediately
            version: 1,
            rank: None,
            score: None,
            distance: None,
        ...
        let avro_data = delete_record.to_avro_bytes()
            .map_err(|e| anyhow::anyhow!("Failed to serialize delete record to Avro: {}", e))?;
...
/// 🎯 MODERN: Vector batch operation implementation using native WalVectorBatch
#[async_trait::async_trait]
impl AtomicOperation for VectorBatchOperation {
    async fn execute(&self, _context: &mut TransactionContext) -> Result<OperationResult> {
        let start_time = Instant::now();

        tracing::debug!(
            "🎯 BATCH_ATOMIC: Executing vector batch operation for collection {} with {} vectors",
            self.collection_id,
            self.vector_batch.vector_records.len()
        );

        // TODO: Integrate with actual WAL system when available
        // For now, simulate the batch operation
        let vector_count = self.vector_batch.vector_records.len();
        let batch_size = self.vector_batch.total_size_bytes;
        
        // Simulate batch execution time (scales with vector count)
        tokio::time::sleep(Duration::from_millis(vector_count as u64)).await;

        // Generate sequential sequence numbers for the batch
        let sequences: Vec<u64> = (1..=vector_count as u64).collect();

        let execution_time = start_time.elapsed();

        tracing::info!(
            "✅ BATCH_ATOMIC: Successfully executed batch {} with {} vectors in {:?}",
            self.vector_batch.batch_id.batch_uuid,
            vector_count,
            execution_time
        );

        Ok(OperationResult::VectorBatch {
    ...
    async fn rollback(&self, _context: &TransactionContext) -> Result<()> {
        tracing::debug!(
            "🔄 BATCH_ATOMIC: Rolling back vector batch operation for collection {} (batch {})",
            self.collection_id,
            self.vector_batch.batch_id.batch_uuid
        );
        
        // TODO: Implement rollback logic
        // - Remove batch from memtable if already written
        // - Compensate any side effects
    ...
    async fn validate(&self, _context: &TransactionContext) -> Result<()> {
        // Validate the batch
        if self.vector_batch.vector_records.is_empty() {
            return Err(anyhow::anyhow!("Empty vector batch not allowed"));
        }

        // Validate collection ID consistency
        if self.collection_id != self.vector_batch.batch_id.collection_id {
            return Err(anyhow::anyhow!(
                "Collection ID mismatch: operation={}, batch={}",
                self.collection_id,
                self.vector_batch.batch_id.collection_id
            ));
        }

        // Validate individual vectors
        for (i, vector_record) in self.vector_batch.vector_records.iter().enumerate() {
            if vector_record.vector.is_empty() {
                return Err(anyhow::anyhow!(
                    "Vector {} in batch has empty vector data",
                    i
                ));
            }
            
            if vector_record.collection_id != self.collection_id {
                return Err(anyhow::anyhow!(
                    "Vector {} has mismatched collection ID: expected {}, got {}",
                    i,
                    self.collection_id,
                    vector_record.collection_id
                ));
            }
        }

        tracing::debug!(
            "✅ BATCH_ATOMIC: Validated vector batch with {} vectors for collection {}",
            self.vector_batch.vector_records.len(),
            self.collection_id
        );
    ...
    fn affected_resources(&self) -> Vec<ResourceId> {
        ...
        // Add individual vector resources
        for vector_record in &self.vector_batch.vector_records {
            resources.push(ResourceId::Vector(vector_record.id.clone()));
        }
    ...
    fn priority(&self) -> OperationPriority {
        // Batch operations get high priority due to their efficiency
        OperationPriority::High
    }

    fn estimated_duration(&self) -> Duration {
        // Estimate based on number of vectors in batch
        let vector_count = self.vector_batch.vector_records.len() as u64;
        Duration::from_millis(vector_count.min(100)) // Cap at 100ms for very large batches
    }
}
