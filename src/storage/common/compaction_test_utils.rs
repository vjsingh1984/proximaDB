...
impl StorageEngine for MockStorageEngine {
    ...
    async fn execute_compaction(
        ...
        Ok(MockCompactionResult {
            ...
            files_created: vec![format!(
                "L{}_{}_output.{}",
                task.target_level, operation_id, self.file_extension
            )],
...
impl CompactionTestEnv {
    ...
    pub async fn count_files(&self, extension: &str) -> Result<usize> {
        ...
        let files = registry
            .discover_files(&self.filesystem, &self.data_dir, extension)
            .await?;
        Ok(files.values().map(|v| v.len()).sum())
    }

    /// Get files by level
    pub async fn get_files_by_level(
        &self,
        extension: &str,
...
impl ConcurrencyTester {
    ...
    pub async fn test_concurrent_requests(
        ...
        collection_id: &str,
        data_dir: &str,
    ) -> Result<Vec<Result<Option<CompactionExecution<MockStorageEngine>>>>> {
        ...
        for engine in &self.engines {
            ...
            let handle = tokio::spawn(async move {
                orchestrator
                    .schedule_compaction(&engine, &collection_id, &data_dir)
                    .await
            });
...
impl PerformanceTester {
    ...
    pub async fn benchmark_file_discovery(
        &self,
        data_dir: &str,
        extension: &str,
        _file_count: usize,
        ...
        let start = Instant::now();
        let _files = registry
            .discover_files(&self.orchestrator.filesystem, data_dir, extension)
            .await?;
...
impl IntegrationTestSuite {
    ...
    pub async fn test_complete_workflow() -> Result<()> {
        ...
        let execution = env
            .orchestrator
            .schedule_compaction(&engine, "test_collection", &env.data_dir)
            .await?;
    ...
    pub async fn test_error_handling() -> Result<()> {
        ...
        // Create files
        env.create_test_files(&[(0, "test", 1024), (0, "test", 1024), (0, "test", 1024)])
            .await?;
    ...
    pub async fn test_concurrency_limits() -> Result<()> {
        ...
        let results = tester
            .test_concurrent_requests("test_collection", &data_dir)
            .await?;
        ...
        assert!(
            successes <= 1,
            "Expected at most 1 successful compaction due to collection limits"
        );
        assert!(
            no_compaction_needed + successes + failures == 5,
            "Expected all 5 operations to complete"
        );

        // Either we have successful compactions or no compaction was needed
        assert!(
            successes > 0 || no_compaction_needed > 0,
...
mod tests {
    ...
    async fn test_env_setup() {
        ...
        env.create_test_files(&[(0, "sst", 1024), (1, "sst", 2048)])
            .await
            .unwrap();
    ...
    async fn test_performance_benchmarks() {
        ...
        assert!(
            ...
            "Filename parsing too slow: {:?}",
            duration
        );

        // Test file discovery performance
        let env = CompactionTestEnv::new().await.unwrap();
        env.create_test_files(&[(0, "sst", 1024), (1, "sst", 1024), (2, "sst", 1024)])
            .await
            .unwrap();

        let tester = PerformanceTester::new(env.orchestrator);
        let duration = tester
            .benchmark_file_discovery(&env.data_dir, "sst", 3)
            .await
            .unwrap();
        assert!(
            duration < Duration::from_millis(500),
            "File discovery too slow: {:?}",
            duration
        );
    }
    ...
    async fn test_integration_workflow() {
        IntegrationTestSuite::test_complete_workflow()
            .await
            .unwrap();
    }
    ...
    async fn test_concurrency_limits() {
        IntegrationTestSuite::test_concurrency_limits()
            .await
            .unwrap();
    }
}
