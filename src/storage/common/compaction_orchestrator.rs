...
use crate::index::axis::eventlog::{EventLogService, EventLogQuery};
...
pub enum OperationType {
    Flush { level: u32 },
    Compaction { source_level: u32, target_level: u32 },
    Recovery,
    Maintenance,
}
...
impl OperationType {
    ...
    pub fn conflicts_with(&self, other: &OperationType) -> bool {
        match (self, other) {
            ...
            (OperationType::Flush { level: l1 }, OperationType::Compaction { source_level, .. }) |
            (OperationType::Compaction { source_level, .. }, OperationType::Flush { level: l1 }) => {
                *l1 == *source_level
            },
            // Compactions conflict if they share levels
            (OperationType::Compaction { source_level: s1, target_level: t1 }, 
             OperationType::Compaction { source_level: s2, target_level: t2 }) => {
    ...
    pub fn priority(&self) -> u8 {
        match self {
            ...
            OperationType::Compaction { source_level: 0, .. } => 2, // Level 0 compaction
...
impl Drop for OperationLock {
    fn drop(&mut self) {
        if let Err(e) = self.coordinator.release_operation(&self.operation_id, &self.collection_id) {
            error!("Failed to release operation lock {}: {}", self.operation_id, e);
        } else {
...
impl CompactionCoordinator {
    ...
    async fn request_operation_internal(
        ...
        if self.config.queue_aware_compaction {
            if let OperationType::Compaction { .. } = &operation_type {
                if let Some(decision) = self.evaluate_queue_aware_compaction(collection_id, &operation_type).await? {
        ...
        let mut collection_ops = self.active_operations.entry(collection_id.to_string())
    ...
    pub async fn request_operation(
        ...
        let operation_id = coordinator.request_operation_internal(
            collection_id,
            operation_type,
            estimated_duration,
        ).await?;
    ...
    fn release_operation(&self, operation_id: &str, collection_id: &str) -> Result<()> {
        ...
        if let Ok(mut global_state) = self.global_state.try_write() {
            global_state.active_operations_count = global_state.active_operations_count.saturating_sub(1);
        }
    ...
    async fn evaluate_queue_aware_compaction(
        ...
        let defer_info = self.deferred_compactions.get(collection_id);
    ...
    async fn defer_compaction(&self, collection_id: &str, operation_type: OperationType) {
        let mut entry = self.deferred_compactions.entry(collection_id.to_string())
...
impl FilenameCodec {
    ...
    pub fn parse_level(&self, filename: &str) -> u32 {
        let pattern = self.level_pattern.get_or_init(|| {
            Regex::new(r"^L(\d+)_").unwrap()
        });
    ...
    pub fn parse_timestamp(&self, filename: &str) -> u64 {
        let pattern = self.timestamp_pattern.get_or_init(|| {
            Regex::new(r"L\d+_(\d{8}T\d{6})_").unwrap()
        });
    ...
    pub fn is_tiered_filename(&self, filename: &str, extension: &str) -> bool {
        let pattern = self.full_pattern.get_or_init(|| {
            Regex::new(r"^L\d+_\d{8}T\d{6}_[a-f0-9]{8}\.\w+$").unwrap()
        });
...
impl TieredFileRegistry {
    ...
    pub async fn discover_files(
        ...
        debug!("📋 Scanning {} entries in: {}", entries.len(), data_directory);
        ...
        for entry in entries {
            ...
            if !entry.metadata.is_directory 
                ...
                && self.filename_codec.is_tiered_filename(&entry.name, extension) {
                
                let level = self.filename_codec.parse_level(&entry.name);
                let timestamp = self.filename_codec.parse_timestamp(&entry.name);
                
                debug!("✅ Discovered {} file: {} at level {}", extension, entry.name, level);
                ...
                files_by_level.entry(level).or_insert_with(Vec::new).push(metadata);
            } else {
...
impl<E: StorageEngine> CompactionExecution<E> {
    ...
    pub async fn execute(self, engine: &E) -> Result<E::CompactionResult> {
        ...
        info!("🚀 Starting compaction execution: {}", self.task.operation_id());
        
        let result = engine.execute_compaction(self.task).await
            .context("Compaction execution failed")?;
        
        let duration = start_time.elapsed();
        info!("✅ Compaction completed in {:?}: {} files created, {} files deleted", 
              duration, result.files_created().len(), result.files_deleted().len());
...
impl CompactionOrchestrator {
    ...
    pub async fn schedule_compaction<E: StorageEngine>(
        ...
        debug!("🔍 Checking compaction needs for collection: {}", collection_id);
        
        // Discover files using registry
        let files_by_level = self.registry.discover_files(
            &self.filesystem,
            data_directory,
    ...
    async fn create_compaction_execution<E: StorageEngine>(
        ...
        let operation_type = OperationType::Compaction { source_level, target_level };
        ...
        let converted_files: Vec<E::FileMetadata> = input_files.into_iter()
            .filter_map(|_f| {
...
mod tests {
    ...
    async fn test_operation_conflicts() {
        ...
        let compact_l0_l1 = OperationType::Compaction { source_level: 0, target_level: 1 };
        let compact_l1_l2 = OperationType::Compaction { source_level: 1, target_level: 2 };
    ...
    async fn test_coordinator_limits() {
        ...
        ).await.expect("First operation should succeed");
        ...
        let _lock2 = CompactionCoordinator::request_operation(
            ...
            OperationType::Flush { level: 0 },
            Some(Duration::from_secs(10)),
        ).await.expect("Different collection should succeed");
    ...
    fn test_filename_codec() {
        ...
        let invalid = "invalid_filename.sst";
    ...
    fn test_staging_detector() {
        ...
        assert!(!detector.is_staging("normal_file.sst"));
    }
}