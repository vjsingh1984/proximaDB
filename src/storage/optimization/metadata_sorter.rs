...
impl MetadataSorter {
    ...
    pub fn from_filterable_specs(filterable_columns: &[FilterableColumnSpec]) -> Self {
        ...
        columns_by_cardinality.sort_by_key(|col| {
            col.estimated_cardinality.unwrap_or(1000) // Default cardinality if not specified
        });
    ...
    pub fn sort_for_encoding(&self, mut records: Vec<VectorRecord>) -> Result<(Vec<VectorRecord>, SortingStats)> {
    ...
    fn analyze_metadata_distribution(&self, records: &[VectorRecord], stats: &mut SortingStats) {
        for sort_key in &self.config.primary_sort_keys {
            ...
            stats.distinct_values_per_key.insert(sort_key.clone(), cardinality);
    ...
    fn estimate_compression_improvement(&self, records: &[VectorRecord], stats: &SortingStats) -> f64 {
