...
impl InMemoryRelationsStore {
    ...
    fn relation_key(
        ...
        format!(
            "{}/rel/{}/{}/{}",
            collection_id, source_id, relation_type, target_id
        )
    }
...
impl RelationsStore for InMemoryRelationsStore {
    ...
    async fn traverse(
        ...
        queue.push_back((vec![start_entity_id.to_string()], vec![], 0.0, 0));
            ...
            if let Some(forward) = self.forward_edges.get(&key) {
                ...
                for (rel_type, edge) in edges_to_process {
                    ...
                    if depth + 1 == max_depth
                        || !self
                            .forward_edges
                            .contains_key(&Self::edge_key(collection_id, &edge.entity_id))
                    {
    ...
    async fn find_paths(
        ...
        let all_paths = self
            .traverse(collection_id, source_id, None, max_depth)
            .await?;
...
mod tests {
    ...
    fn test_relation_key_generation() {
        let key =
            InMemoryRelationsStore::relation_key("collection1", "entity1", "cites", "entity2");
