...
impl HdfsFileSystem {
    ...
    pub async fn new(config: HdfsConfig) -> FsResult<Self> {
        tracing::info!("🐘 Initializing HDFS filesystem for NameNode: {}", config.namenode_url);
...
impl HdfsClient {
    async fn new(config: HdfsConfig) -> FsResult<Self> {
        tracing::debug!("🌐 Creating HDFS WebHDFS client with timeout: {}s", config.timeout_seconds);
    ...
    async fn test_connection(&self) -> FsResult<()> {
        ...
        let url = format!("{}/?op=LISTSTATUS&user.name={}", 
        ...
        if response.status().is_success() {
            ...
            Err(FilesystemError::Network(
                format!("HDFS connectivity test failed: {}", response.status())
            ))
        }
    }
    ...
    async fn read_file(&self, path: &str) -> FsResult<Vec<u8>> {
        let url = format!("{}/{}?op=OPEN&user.name={}", 
                         self.base_url, path.trim_start_matches('/'), self.config.user);
        ...
        if response.status().is_success() {
            ...
            Err(FilesystemError::Network(
                format!("HDFS read error: {}", response.status())
            ))
        }
    }
    
    /// Write file to HDFS
    async fn write_file(&self, path: &str, data: &[u8], options: Option<FileOptions>) -> FsResult<()> {
        let options = options.unwrap_or_default();
        
        // Step 1: Create file (WebHDFS two-step process)
        let create_url = format!("{}/{}?op=CREATE&user.name={}&blocksize={}&replication={}&overwrite={}", 
                                self.base_url, 
        ...
        if create_response.status().as_u16() == 307 {
            ...
            if let Some(location) = create_response.headers().get("location") {
                let datanode_url = location.to_str()
                    .map_err(|e| FilesystemError::Network(format!("Invalid location header: {}", e)))?;
                
                tracing::debug!("📤 Writing data to DataNode: {}", datanode_url);
                
                let write_response = self.http_client
                    .put(datanode_url)
                    .body(data.to_vec())
                    .send()
                    .await
                ...
                if write_response.status().is_success() {
                    ...
                    Err(FilesystemError::Network(
                        format!("HDFS write error: {}", write_response.status())
                    ))
                }
            } else {
                Err(FilesystemError::Network("No location header in HDFS create response".to_string()))
            }
        } else {
            tracing::error!("❌ HDFS create error: {}", create_response.status());
            Err(FilesystemError::Network(
                format!("HDFS create error: {}", create_response.status())
            ))
        }
    }
    ...
    async fn append_file(&self, path: &str, data: &[u8]) -> FsResult<()> {
        ...
        let append_url = format!("{}/{}?op=APPEND&user.name={}", 
                                self.base_url, path.trim_start_matches('/'), self.config.user);
        
        tracing::debug!("➕ Appending to HDFS file: {} ({} bytes)", path, data.len());
        
        let append_response = self.http_client
            .post(&append_url)
            .send()
            .await
            .map_err(|e| {
        ...
        if append_response.status().as_u16() == 307 {
            ...
            if let Some(location) = append_response.headers().get("location") {
                ...
                if write_response.status().is_success() {
                    ...
                    Err(FilesystemError::Network(
                        format!("HDFS append write error: {}", write_response.status())
                    ))
                }
            } else {
                Err(FilesystemError::Network("No location header in HDFS append response".to_string()))
            }
        } else {
            tracing::error!("❌ HDFS append error: {}", append_response.status());
            Err(FilesystemError::Network(
                format!("HDFS append error: {}", append_response.status())
            ))
        }
    }
    ...
    async fn delete_path(&self, path: &str) -> FsResult<()> {
        let url = format!("{}/{}?op=DELETE&user.name={}&recursive=true", 
                         self.base_url, path.trim_start_matches('/'), self.config.user);
        ...
        if response.status().is_success() {
            ...
            Err(FilesystemError::Network(
                format!("HDFS delete error: {}", response.status())
            ))
        }
    }
    ...
    async fn get_file_status(&self, path: &str) -> FsResult<serde_json::Value> {
        let url = format!("{}/{}?op=GETFILESTATUS&user.name={}", 
                         self.base_url, path.trim_start_matches('/'), self.config.user);
        ...
        if response.status().is_success() {
            ...
            Err(FilesystemError::Network(
                format!("HDFS status error: {}", response.status())
            ))
        }
    }
    ...
    async fn list_directory(&self, path: &str) -> FsResult<Vec<serde_json::Value>> {
        ...
        if response.status().is_success() {
            let list_json: serde_json::Value = response.json().await
                .map_err(|e| FilesystemError::Network(e.to_string()))?;
            
            let file_statuses = list_json["FileStatuses"]["FileStatus"].as_array()
                .unwrap_or(&vec![])
                .clone();
            
            tracing::debug!("✅ Listed {} entries in HDFS directory", file_statuses.len());
            Ok(file_statuses)
        } else {
            tracing::error!("❌ HDFS list error: {}", response.status());
            Err(FilesystemError::Network(
                format!("HDFS list error: {}", response.status())
            ))
        }
    }
    ...
    async fn create_directory(&self, path: &str) -> FsResult<()> {
        let url = format!("{}/{}?op=MKDIRS&user.name={}", 
                         self.base_url, path.trim_start_matches('/'), self.config.user);
        ...
        if response.status().is_success() {
            ...
            Err(FilesystemError::Network(
                format!("HDFS mkdir error: {}", response.status())
            ))
        }
    }
}
...
impl FileSystem for HdfsFileSystem {
    ...
    async fn write(&self, path: &str, data: &[u8], options: Option<FileOptions>) -> FsResult<()> {
        ...
        let normalized_path = self.normalize_path(path);
        self.client.write_file(&normalized_path, data, options).await
    }
    ...
    async fn metadata(&self, path: &str) -> FsResult<FileMetadata> {
        ...
        let modification_time = file_status["modificationTime"].as_u64()
            .and_then(|ts| chrono::DateTime::from_timestamp_millis(ts as i64));
        let access_time = file_status["accessTime"].as_u64()
            .and_then(|ts| chrono::DateTime::from_timestamp_millis(ts as i64));
        
        let permissions = file_status["permission"].as_str().map(|s| s.to_string());
        
        tracing::debug!("✅ HDFS metadata retrieved: {} bytes, dir: {}", size, is_directory);
    ...
    async fn list(&self, path: &str) -> FsResult<Vec<DirEntry>> {
        ...
        for status in file_statuses {
            ...
            let modification_time = status["modificationTime"].as_u64()
                .and_then(|ts| chrono::DateTime::from_timestamp_millis(ts as i64));
