...
impl FileSystem for LocalFileSystem {
        ...
        match fs::read(&resolved_path).await {
            ...
            Err(e) => match e.kind() {
                std::io::ErrorKind::NotFound => {
                    Err(FilesystemError::NotFound(resolved_path.display().to_string()))
                },
                std::io::ErrorKind::PermissionDenied => {
                    Err(FilesystemError::PermissionDenied(resolved_path.display().to_string()))
                },
    ...
    async fn write(&self, path: &str, data: &[u8], options: Option<FileOptions>) -> FsResult<()> {
        ...
        if !options.overwrite && resolved_path.exists() {
            return Err(FilesystemError::AlreadyExists(resolved_path.display().to_string()));
        }
        ...
        if let Some(permissions) = self.config.default_permissions {
            ...
            fs::set_permissions(&resolved_path, perms).await
                .map_err(FilesystemError::Io)?;
        }
    ...
    async fn delete(&self, path: &str) -> FsResult<()> {
        ...
        if !resolved_path.exists() {
            return Err(FilesystemError::NotFound(resolved_path.display().to_string()));
        }
    ...
    async fn create_dir(&self, path: &str) -> FsResult<()> {
        ...
        fs::create_dir(&resolved_path).await
