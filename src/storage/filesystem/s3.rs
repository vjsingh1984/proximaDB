...
impl S3FileSystem {
    ...
    async fn create_credential_provider(
    ...
    ) -> FsResult<Box<dyn CredentialProvider>> {
        match &config.provider {
            CredentialProviderType::Static => {
                    ...
                    (&config.access_key_id, &config.secret_access_key)
                {
                    ...
                    Err(FilesystemError::Config(
                        "Static credentials require access_key_id and secret_access_key"
                            .to_string(),
                    ))
                }
            }
            ...
            CredentialProviderType::Profile(profile_name) => Ok(Box::new(
                super::auth::ProfileCredentialProvider::new(profile_name.clone()),
            )),
...
impl S3Client {
    ...
    async fn put_object(
        ...
        let mut request = self
            ...
            .header(
                "Authorization",
                self.create_auth_header(credentials, "PUT", bucket, key),
            )
            .body(data.to_vec());
    ...
    fn create_auth_header(
        ...
        format!(
            "AWS4-HMAC-SHA256 Credential={}/20241214/{}/s3/aws4_request",
...
impl FileSystem for S3FileSystem {
    ...
    async fn write(&self, path: &str, data: &[u8], options: Option<FileOptions>) -> FsResult<()> {
        ...
        if data.len() as u64 > self.config.multipart_threshold {
            ...
            self.client
                .put_object(&bucket, &key, data, &credentials, options)
                .await
        }
    }
    ...
    async fn delete(&self, path: &str) -> FsResult<()> {
        ...
        let response = self
            ...
            .header(
                "Authorization",
                self.client
                    .create_auth_header(&credentials, "DELETE", &bucket, &key),
            )
            .send()
            .await
            .map_err(|e| FilesystemError::Network(e.to_string()))?;
    ...
    async fn metadata(&self, path: &str) -> FsResult<FileMetadata> {
        ...
        let response = self
            ...
            .header(
                "Authorization",
                self.client
                    .create_auth_header(&credentials, "HEAD", &bucket, &key),
            )
            .send()
            .await
            .map_err(|e| FilesystemError::Network(e.to_string()))?;
