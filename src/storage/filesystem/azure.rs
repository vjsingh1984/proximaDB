...
use super::auth::{AzureCredentials, AzureCredentialProvider};
...
impl AzureFileSystem {
    ...
    async fn create_credential_provider(config: &AzureCredentialConfig) -> FsResult<Box<dyn AzureCredentialProvider>> {
        match &config.provider {
            ...
            AzureCredentialProviderType::ManagedIdentity => {
            ...
            AzureCredentialProviderType::ServicePrincipal => {
                if let (Some(client_id), Some(client_secret), Some(tenant_id)) = 
                    (&config.sp_client_id, &config.sp_client_secret, &config.tenant_id) {
    ...
    fn parse_azure_url(&self, path: &str) -> FsResult<(String, String)> {
        if path.starts_with('/') {
            ...
            Err(FilesystemError::Config(
                format!("Invalid Azure path format: {}", path)
            ))
        }
    }
}
...
impl AzureClient {
    ...
    /// Get Azure blob
    async fn get_blob(&self, container: &str, blob_path: &str, credentials: &AzureCredentials) -> FsResult<Vec<u8>> {
        let url = if self.config.hierarchical_namespace {
            ...
            // Blob Storage API
            format!("https://{}.blob.core.windows.net/{}/{}", 
                   self.config.account_name, container, blob_path)
        };
        ...
        if response.status().is_success() {
            ...
            Err(FilesystemError::NotFound(format!("adls://{}/{}", container, blob_path)))
        } else {
            Err(FilesystemError::Network(
                format!("Azure error: {}", response.status())
            ))
        }
    }
    
    /// Put Azure blob
    async fn put_blob(&self, container: &str, blob_path: &str, data: &[u8], credentials: &AzureCredentials, options: Option<FileOptions>) -> FsResult<()> {
        let url = if self.config.hierarchical_namespace {
            format!("https://{}.dfs.core.windows.net/{}/{}", 
                   self.config.account_name, container, blob_path)
        } else {
        ...
        if response.status().is_success() {
            ...
            Err(FilesystemError::Network(
                format!("Azure PUT error: {}", response.status())
            ))
        }
    }
}
...
impl FileSystem for AzureFileSystem {
    async fn read(&self, path: &str) -> FsResult<Vec<u8>> {
        ...
        self.client.get_blob(&container, &blob_path, &credentials).await
    }
    
    async fn write(&self, path: &str, data: &[u8], options: Option<FileOptions>) -> FsResult<()> {
        let (container, blob_path) = self.parse_azure_url(path)?;
        let credentials = self.credential_provider.get_credentials().await?;
        self.client.put_blob(&container, &blob_path, data, &credentials, options).await
    }
    ...
    async fn delete(&self, path: &str) -> FsResult<()> {
        ...
        if response.status().is_success() || response.status().as_u16() == 404 {
            ...
            Err(FilesystemError::Network(
                format!("Azure DELETE error: {}", response.status())
            ))
        }
    }
    ...
    async fn metadata(&self, path: &str) -> FsResult<FileMetadata> {
        ...
        if response.status().is_success() {
            ...
            let blob_tier = response.headers()
            ...
            Err(FilesystemError::Network(
                format!("Azure HEAD error: {}", response.status())
            ))
        }
    }
