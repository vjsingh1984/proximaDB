...
async fn test_proto_metadata_encoding() {
    ...
    assert_eq!(vector.metadata[0].value, "Hello 世界 🌍");
}
...
async fn test_proto_memory_only_mode() {
    ...
    config.performance.sync_mode = crate::storage::persistence::wal::config::SyncMode::MemoryOnly;
