...
impl WalBatchStrategy for AvroSerializationStrategy {
    ...
    async fn write_proto_batch(
        ...
        // Avro strategy doesn't handle proto format directly
        Err(anyhow::anyhow!(
            "Proto format not supported by AvroSerializationStrategy. Use ProtoSerializationStrategy instead."
        ))
    }
    ...
    async fn write_avro_batch(
        ...
        debug!(
            "📝 Writing Avro batch for collection {} with {} bytes",
            collection_id,
            avro_bytes.len()
        );
        
        // 1. Deserialize to get vectors
        let vectors = self.serializer.deserialize_batch(avro_bytes)?;
        ...
        let batch = WalVectorBatch {
            ...
            total_size_bytes: avro_bytes.len(),
            is_flushed: false,
        };
...
impl AvroSerializationStrategy {
    ...
    fn should_persist_to_disk(&self) -> bool {
        match self.config.performance.sync_mode {
            crate::storage::persistence::wal::config::SyncMode::Always => true,
            crate::storage::persistence::wal::config::SyncMode::PerBatch => true,
            crate::storage::persistence::wal::config::SyncMode::Periodic => true,
            crate::storage::persistence::wal::config::SyncMode::Never => false,
            crate::storage::persistence::wal::config::SyncMode::MemoryOnly => false,
        }
    }
    ...
    fn trigger_background_flush(&self, collection_id: &str) {
        ...
        tokio::spawn(async move {
            ...
            tracing::info!("Background flush would happen here for collection {}", collection_id);
        });
    }
