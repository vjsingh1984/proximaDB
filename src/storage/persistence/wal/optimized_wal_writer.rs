...
impl OptimizedWalWriter {
    ...
    async fn get_cached_assignment(
        ...
        let base_path = if assignment.wal_url.starts_with("file://") {
            assignment.wal_url.strip_prefix("file://").unwrap_or(&assignment.wal_url)
        } else {
            &assignment.wal_url
        };
        
        let cached = CachedAssignment {
            storage_url: assignment.wal_url.clone(),
            collection_wal_dir: base_path.to_string(),
            logs_dir: format!("{}/logs", base_path),
    ...
    async fn write_combined_batch(
        ...
        let full_url = if assignment.storage_url.ends_with('/') {
            format!("{}{}/logs/{}", assignment.storage_url, collection_id, wal_filename)
        } else {
            format!("{}/{}/logs/{}", assignment.storage_url, collection_id, wal_filename)
        };
