...
impl WalDiskManager {
    ...
    pub fn new(
        ...
        info!("🎯 Creating WalDiskManager with base dir: {:?}", wal_base_dir.as_ref());
    ...
    pub async fn write_batch(
        ...
        debug!(
            "📝 Writing WAL batch {} for collection {} to {} ({} bytes)",
            batch_id.to_base62(),
            collection_id,
            file_url,
            data.len()
        ...
        self.ensure_directory_exists(&dir_path).await?;
    ...
    async fn ensure_directory_exists(&self, dir_path: &Path) -> Result<()> {
...
mod tests {
    ...
    async fn create_test_manager() -> (WalDiskManager, TempDir) {
        ...
        let manager = WalDiskManager::new(
            filesystem_factory,
            temp_dir.path(),
        );
