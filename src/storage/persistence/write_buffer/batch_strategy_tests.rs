//! Comprehensive tests for WriteBufferBatchStrategy trait and implementations
//! 
//! These tests cover:
...
#[cfg(test)]
mod write_buffer_batch_strategy_tests {
    use super::super::*;
    ...
    use crate::storage::persistence::write_buffer::{WriteBufferConfig, WriteBufferStats, WriteBufferOperation};
    ...
    #[derive(Debug)]
    struct MockWriteBufferBatchStrategy {
        strategy_name: &'static str,
        filesystem: Option<Arc<FilesystemFactory>>,
        initialized: bool,
        write_buffer_behavior: Option<MockWriteBufferBehavior>,
        distance_compute: crate::compute::unified_distance::UnifiedDistanceCompute,
    }

    #[derive(Debug)]
    struct MockWriteBufferBehavior {
        collections: Arc<RwLock<HashMap<String, Vec<WriteBufferVectorBatch>>>>,
    }
    ...
    impl MockWriteBufferBehavior {
        ...
        async fn add_batch(&self, collection_id: &str, batch: WriteBufferVectorBatch) {
        ...
        async fn get_unflushed_batches(&self, collection_id: &str) -> Result<Vec<WriteBufferVectorBatch>> {
        ...
        async fn get_stats(&self) -> Result<HashMap<String, crate::storage::persistence::write_buffer::WriteBufferStats>> {
            let collections = self.collections.read().await;
            ...
            for (collection_id, batches) in collections.iter() {
                ...
                stats.insert(collection_id.clone(), crate::storage::persistence::write_buffer::WriteBufferStats {
    ...
    impl MockWriteBufferBatchStrategy {
        fn new(strategy_name: &'static str) -> Self {
            Self {
                strategy_name,
                filesystem: None,
                initialized: false,
                write_buffer_behavior: Some(MockWriteBufferBehavior::new()),
                distance_compute: crate::compute::unified_distance::UnifiedDistanceCompute::default(),
            }
        }
    }

    #[async_trait]
    impl crate::compute::unified_distance::DistanceComputeProvider for MockWriteBufferBatchStrategy {
        fn distance_compute(&self) -> &crate::compute::unified_distance::UnifiedDistanceCompute {
            &self.distance_compute
        }
    }

    #[async_trait]
    impl WriteBufferBatchStrategy for MockWriteBufferBatchStrategy {
        ...
        fn set_storage_engine(&self, _storage_engine: Arc<dyn crate::storage::traits::UnifiedStorageEngine>) {
            // Mock implementation - no-op
        }

        async fn write_native_batch(&self, batch: WriteBufferVectorBatch, collection_id: &str) -> Result<Vec<u64>> {
            if let Some(behavior) = &self.write_buffer_behavior {
        ...
        async fn flush_collection(&self, _collection_id: &str) -> Result<crate::storage::traits::FlushResult> {
    ...
    fn create_test_batch(collection_id: &str, vector_count: usize) -> WriteBufferVectorBatch {
        let vectors: Vec<VectorRecord> = (0..vector_count)
            .map(|i| create_test_vector_record(&format!("{}_{}", collection_id, i), vec![i as f32; 128]))
            .collect();
    ...
    async fn test_strategy_initialization() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_strategy");
    ...
    async fn test_write_native_batch() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_write");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        ...
        let result = strategy.write_native_batch(batch.clone(), collection_id).await;
        assert!(result.is_ok());
    ...
    async fn test_write_vector_batch_with_sync() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_sync");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        let result = strategy.write_vector_batch_with_sync(batch.clone(), collection_id, true).await;
        assert!(result.is_ok());
        
        // Test with sync disabled
        let result = strategy.write_vector_batch_with_sync(batch, collection_id, false).await;
        assert!(result.is_ok());
    }

    #[tokio::test]
    async fn test_read_all_batches() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_read");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        strategy.write_native_batch(batch, collection_id).await.unwrap();
    ...
    async fn test_collection_stats() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_stats");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        strategy.write_native_batch(batch1, collection_id).await.unwrap();
    ...
    async fn test_global_stats() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_global_stats");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        strategy.write_native_batch(batch1, "collection1").await.unwrap();
    ...
    fn test_vector_serialization_deserialization() {
        let strategy = MockWriteBufferBatchStrategy::new("test_serialization");
    ...
    fn test_serialization_error_handling() {
        let strategy = MockWriteBufferBatchStrategy::new("test_serialization_errors");
    ...
    async fn test_cloud_batch_operations() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_cloud");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        let result = strategy.write_batch_to_cloud(collection_id, &batch, cloud_url).await;
    ...
    async fn test_cloud_health_check() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_cloud_health");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
    ...
    async fn test_cloud_batch_listing() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_cloud_list");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        let result = strategy.list_cloud_batches(collection_id, cloud_base_url).await;
    ...
    async fn test_uninitialized_strategy() {
        let strategy = MockWriteBufferBatchStrategy::new("test_uninitialized");
    ...
    async fn test_empty_batch_handling() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_empty");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();

        // Create empty batch
        let empty_batch = WriteBufferVectorBatch {
            batch_id: BatchId::new(),
    ...
    async fn test_large_batch_handling() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_large");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        let result = strategy.write_native_batch(large_batch, "large_collection").await;
        assert!(result.is_ok());
    ...
    async fn test_concurrent_batch_operations() {
        let mut strategy = Arc::new(MockWriteBufferBatchStrategy::new("test_concurrent"));
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                ...
                strategy_clone.write_native_batch(batch, &format!("concurrent_{}", i)).await
            });
            handles.push(handle);
        }
    ...
    async fn test_write_vector_batch_unified_bincode() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_unified_bincode");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        // Test unified write
        let result = strategy.write_vector_batch_unified("unified_collection", &payload, "bincode").await;
        assert!(result.is_ok());
    ...
    async fn test_write_vector_batch_unified_unsupported_format() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_unified_unsupported");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        let result = strategy.write_vector_batch_unified("test_collection", &payload, "unsupported_format").await;
        assert!(result.is_err());
    ...
    async fn test_search_vectors_similarity() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_similarity");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        let result = strategy.search_vectors_similarity(collection_id, &query_vector, 5, Some(DistanceMetric::Cosine)).await;
    ...
    async fn test_search_vector_by_id() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_search_by_id");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        let result = strategy.search_vector_by_id(collection_id, &vector_id.to_string()).await;
    ...
    async fn test_strategy_recovery() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_recovery");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
    ...
    async fn test_force_sync() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_force_sync");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
    ...
    async fn test_strategy_close() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_close");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
    ...
    async fn test_flush_operations() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_flush");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
    ...
    async fn test_should_trigger_flush() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_flush_trigger");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        strategy.write_native_batch(small_batch, collection_id).await.unwrap();
    ...
    async fn test_delete_vector() {
        let mut strategy = MockWriteBufferBatchStrategy::new("test_delete");
        let filesystem = Arc::new(FilesystemFactory::new(Default::default()).await.unwrap());
        let config = WriteBufferConfig::default();
        strategy.initialize(&config, filesystem).await.unwrap();
        ...
        let result = strategy.delete_vector(collection_id, &vector_id.to_string()).await;
        assert!(result.is_ok());
    ...
    fn test_write_buffer_config_defaults() {
        ...
        // Note: Actual fields depend on WriteBufferConfig implementation
        // This test verifies the config can be created and used
        assert_eq!(std::mem::size_of_val(&config), std::mem::size_of::<WriteBufferConfig>());
    }
