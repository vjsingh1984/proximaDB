...
//!
//! This implements the WriteBufferBatchStrategy trait using the new clean architecture
...
impl DistanceComputeProvider for BincodeSerializationStrategy {
...
#[async_trait]
impl WriteBufferBatchStrategy for BincodeSerializationStrategy {
    ...
    async fn write_native_batch(&self, batch: WriteBufferVectorBatch, collection_id: &str) -> Result<Vec<u64>> {
        ...
        if self.should_persist_to_disk() {
            ...
            let should_sync = match self.config.performance.sync_mode {
                crate::storage::persistence::write_buffer::config::SyncMode::Always => true,
    ...
    async fn force_sync(&self, collection_id: Option<&String>) -> Result<()> {
        ...
        if let Some(collection_id) = collection_id {
            ...
            for batch in unflushed_batches {
                let file_info = crate::storage::persistence::write_buffer::WriteBufferFileInfo {
...
impl BincodeSerializationStrategy {
    ...
    fn should_persist_to_disk(&self) -> bool {
        match self.config.performance.sync_mode {
            crate::storage::persistence::write_buffer::config::SyncMode::Always => true,
            crate::storage::persistence::write_buffer::config::SyncMode::PerBatch => true,
            crate::storage::persistence::write_buffer::config::SyncMode::Periodic => true,
            crate::storage::persistence::write_buffer::config::SyncMode::Never => false,
            crate::storage::persistence::write_buffer::config::SyncMode::MemoryOnly => false,
        }
    }
