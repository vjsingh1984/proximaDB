...
use crate::storage::atomic::{
    ...
    TransactionLog,
    TransactionState,
    write_buffer::WriteBufferTransaction,
};
use crate::storage::persistence::write_buffer::{
    BatchId, WriteBufferOperation,
...
use crate::storage::persistence::write_buffer::optimized_path_resolver::{
...
impl AtomicWalSync {
    ...
    pub async fn sync_batch(
        ...
        let log_entry = TransactionLog {
            transaction_id: transaction_id.clone(),
            operation: AtomicOperation::WriteBufferSync {
                collection_id: collection_id.clone(),
                batch_id: batch_id.clone(),
    ...
    fn convert_to_proto(batch: &WriteBufferVectorBatch) -> Vec<u8> {
