...
async fn test_flush_coordinator_creation() {
    let coordinator = WriteBufferFlushCoordinator::new();
...
async fn test_register_storage_engine() {
    ...
    // Register engine
    coordinator.register_storage_engine("VIPER", mock_engine).await;
...
async fn test_initiate_flush_memory_mode() {
    let coordinator = WriteBufferFlushCoordinator::new();
    coordinator.initialize_flush_state("memory_test").await.unwrap();
    
    let sequences = vec![1, 2, 3, 4, 5];
    let data_source = coordinator.initiate_flush(
        "memory_test",
        sequences.clone(),
        &SyncMode::MemoryOnly,
    ).await.unwrap();
...
async fn test_initiate_flush_disk_mode() {
    let coordinator = WriteBufferFlushCoordinator::new();
    coordinator.initialize_flush_state("disk_test").await.unwrap();
...
async fn test_acknowledge_flush() {
    let coordinator = WriteBufferFlushCoordinator::new();
    coordinator.initialize_flush_state("ack_test").await.unwrap();
    
    // Initiate a flush
    let sequences = vec![1, 2, 3];
    let _data_source = coordinator.initiate_flush(
        "ack_test",
        sequences.clone(),
        &SyncMode::MemoryOnly,
    ).await.unwrap();
    ...
    let cleanup = coordinator.acknowledge_flush(
        "ack_test",
        flush_id,
        sequences.clone(),
    ).await.unwrap();
...
async fn test_execute_coordinated_flush_with_data() {
    ...
    coordinator.register_storage_engine("VIPER", mock_engine.clone()).await;
...
async fn test_cancel_flush() {
    let coordinator = WriteBufferFlushCoordinator::new();
    coordinator.initialize_flush_state("cancel_test").await.unwrap();
...
async fn test_cleanup_collection() {
    ...
    coordinator.initialize_flush_state("cleanup_test").await.unwrap();
    let _data_source = coordinator.initiate_flush(
        "cleanup_test",
        vec![1, 2, 3],
        &SyncMode::Always,
...
async fn test_drop_collection() {
    ...
    coordinator.initialize_flush_state("drop_test").await.unwrap();
...
async fn test_multiple_pending_flushes() {
    let coordinator = WriteBufferFlushCoordinator::new();
    coordinator.initialize_flush_state("multi_flush").await.unwrap();
