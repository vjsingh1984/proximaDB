...
use crate::storage::persistence::write_buffer::{
    ProtoSerializationStrategy, WriteBufferBatchStrategy, WriteBufferConfig, BatchId,
};
    ...
    VectorRecord {
        ...
        metadata: vec![
            MetadataItem {
                ...
                value: Some(crate::proto::proximadb::metadata_item::Value::StringValue("3".to_string())),
            },
            MetadataItem {
                key: "encoding".to_string(), 
                value: Some(crate::proto::proximadb::metadata_item::Value::StringValue("protobuf".to_string())),
            },
        ],
...
async fn test_proto_metadata_encoding() {
    ...
    vector.metadata = vec![
        MetadataItem {
            ...
            value: Some(crate::proto::proximadb::metadata_item::Value::StringValue("Hello 世界 🌍".to_string())),
        },
        MetadataItem {
            key: "special_chars".to_string(),
            value: Some(crate::proto::proximadb::metadata_item::Value::StringValue("!@#$%^&*()_+-={}[]|\\:\";<>?,./".to_string())),
        },
        MetadataItem {
            key: "empty".to_string(),
            value: Some(crate::proto::proximadb::metadata_item::Value::StringValue("".to_string())),
        },
    ];
    
    // Verify metadata is properly set
    assert_eq!(vector.metadata.len(), 3);
    assert!(matches!(&vector.metadata[0].value, Some(crate::proto::proximadb::metadata_item::Value::StringValue(s)) if s == "Hello 世界 🌍"));
}
...
async fn test_proto_memory_only_mode() {
    ...
    config.performance.sync_mode = crate::storage::persistence::write_buffer::config::SyncMode::MemoryOnly;
