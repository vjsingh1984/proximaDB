...
use crate::storage::persistence::write_buffer::{
    AvroSerializationStrategy, WriteBufferBatchStrategy, WriteBufferConfig, BatchId,
};
...
fn create_test_config() -> WriteBufferConfig {
    WriteBufferConfig {
        memtable: crate::storage::persistence::write_buffer::config::MemTableConfig {
            memtable_type: crate::storage::persistence::write_buffer::config::MemTableType::default(),
        ...
        performance: crate::storage::persistence::write_buffer::config::PerformanceConfig {
            memory_flush_size_bytes: 5 * 1024 * 1024, // 5MB
            sync_mode: crate::storage::persistence::write_buffer::config::SyncMode::Always,
            ..Default::default()
        },
        enable_mvcc: true,
        ..Default::default()
    ...
    VectorRecord {
        ...
        metadata: vec![
            ...
            MetadataItem {
                ...
                value: Some(crate::proto::proximadb::metadata_item::Value::StringValue("1".to_string())),
            },
        ],
...
/// Create test batch
fn create_test_batch(vectors: Vec<VectorRecord>) -> WriteBufferVectorBatch {
...
async fn test_avro_write_and_read_batch() {
    ...
    // Write batch
    let sequences = strategy.write_native_batch(batch.clone(), collection_id)
        .await
    ...
    let mut ids: Vec<String> = retrieved.iter()
...
async fn test_avro_search_by_id() {
    ...
    let found = strategy.search_vector_by_id(collection_id, &"search_test".to_string())
        .await
    ...
    let not_found = strategy.search_vector_by_id(collection_id, &"non_existent".to_string())
        .await
        .expect("Failed to search");
...
async fn test_avro_write_with_sync() {
    ...
    let sequences = strategy.write_vector_batch_with_sync(batch, collection_id, true)
        .await
...
async fn test_avro_read_all_batches() {
    ...
    // Read all batches
    let all_batches = strategy.read_all_batches(collection_id, None)
        .await
    ...
    // Read with limit
    let limited_batches = strategy.read_all_batches(collection_id, Some(2))
        .await
...
async fn test_avro_empty_collection_operations() {
    ...
    let search_results = strategy.search_vectors_similarity(
        collection_id,
        &vec![0.1; 64],
        10,
        None,
...
mod integration_tests {
    ...
    impl UnifiedStorageEngine for MockStorageEngine {
        ...
        async fn do_compact(&self, _params: &crate::storage::traits::CompactionParameters) -> Result<crate::storage::traits::CompactionResult> {
    ...
    async fn test_avro_flush_with_storage_engine() {
        ...
        // Flush collection
        let flush_result = strategy.flush_collection(collection_id)
            .await
