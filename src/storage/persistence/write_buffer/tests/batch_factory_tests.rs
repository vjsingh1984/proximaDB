...
mod tests {
    ...
    use crate::storage::persistence::write_buffer::{WriteBufferBatchFactory, WriteBufferStrategyType};
    use crate::storage::WriteBufferConfig;
    use std::sync::Arc;
    ...
    fn create_test_config(temp_dir: &TempDir) -> WriteBufferConfig {
    ...
    async fn test_create_avro_serialization_strategy() {
        ...
        let strategy = WriteBufferBatchFactory::create_batch_serialization_strategy(
    ...
    async fn test_create_from_config_avro() {
        ...
        let strategy = WriteBufferBatchFactory::create_from_config(&config, filesystem)
    ...
    fn test_available_strategies() {
        let strategies = WriteBufferBatchFactory::available_strategies();
    ...
    fn test_avro_strategy_info() {
        let info = WriteBufferBatchFactory::get_strategy_info(&WriteBufferStrategyType::AvroBatch);
    ...
    fn test_bincode_strategy_info() {
        let info = WriteBufferBatchFactory::get_strategy_info(&WriteBufferStrategyType::BincodeBatch);
    ...
    fn test_strategy_comparison() {
        let comparison = WriteBufferBatchFactory::compare_strategies();
    ...
    fn test_strategy_info_consistency() {
        let avro_info = WriteBufferBatchFactory::get_strategy_info(&WriteBufferStrategyType::AvroBatch);
        let bincode_info = WriteBufferBatchFactory::get_strategy_info(&WriteBufferStrategyType::BincodeBatch);
    ...
    async fn test_serialization_strategy_initialization() {
        ...
        for strategy_type in &[WriteBufferStrategyType::AvroBatch, WriteBufferStrategyType::BincodeBatch, WriteBufferStrategyType::ProtoBatch] {
            let strategy = WriteBufferBatchFactory::create_batch_serialization_strategy(
                strategy_type.clone(),
                &config,
                filesystem.clone()
            ...
            assert!(strategy.get_write_buffer_behavior().is_none());
    ...
    async fn test_concurrent_strategy_creation() {
        ...
        let tasks: Vec<_> = (0..5).map(|i| {
            ...
            let strategy_type = if i % 2 == 0 { WriteBufferStrategyType::AvroBatch } else { WriteBufferStrategyType::BincodeBatch };
            
            tokio::spawn(async move {
                WriteBufferBatchFactory::create_batch_serialization_strategy(strategy_type, &cfg, fs).await
            })
        }).collect();
    ...
    fn test_strategy_info_completeness() {
        for strategy_type in &[WriteBufferStrategyType::AvroBatch, WriteBufferStrategyType::BincodeBatch] {
            let info = WriteBufferBatchFactory::get_strategy_info(strategy_type);
