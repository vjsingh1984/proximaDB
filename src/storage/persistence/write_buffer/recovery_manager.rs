...
use crate::storage::persistence::write_buffer::{
    WriteBufferDiskManager, WriteBufferFileInfo, WriteBufferFlushCoordinator,
...
impl RecoveryManager {
    ...
    pub async fn register_storage_engine(
        ...
        self.flush_coordinator.register_storage_engine(engine_type, engine).await;
        
        info!("✅ Registered {} storage engine for collection {}", engine_type, collection_id);
        Ok(())
    }
    
    /// Recover all collections from disk to storage engines in parallel
    pub async fn recover_all(&self) -> Result<RecoveryStats> {
        info!("🔄 Starting parallel WAL recovery (mode: {:?})", self.recovery_mode);
        
        // Get the recovery thread pool
        let thread_pool = get_recovery_thread_pool();
        
        // Start recovery phase - this acquires all CPU resources
        let recovery_guard = thread_pool.start_recovery().await
        ...
        info!("Found {} collections to recover using {} threads", collections.len(), num_cpus::get());
        ...
        for collection_id in collections {
            ...
            let task = thread_pool.execute_recovery_task(
                "recover_collection",
                async move {
                    info!("🧵 Starting recovery for collection: {}", collection_id_clone);
        ...
        info!("⏳ Waiting for {} parallel recovery tasks to complete...", recovery_tasks.len());
        ...
        recovery_guard.complete(successful_collections, total_vectors).await;
        ...
        info!(
            ...
            successful_collections, total_vectors, recovery_errors,
            pool_stats.peak_concurrent_threads, pool_stats.total_recovery_time_ms
        );
        Ok(stats)
    }
    ...
    async fn recover_collection_internal(
        ...
        disk_manager: Arc<WriteBufferDiskManager>,
        storage_engines: Arc<tokio::sync::RwLock<HashMap<String, Arc<dyn UnifiedStorageEngine>>>>,
        _flush_coordinator: Arc<WriteBufferFlushCoordinator>,
        recovery_mode: RecoveryMode,
        progress_callback: Option<RecoveryProgressCallback>,
        ...
        if vectors_recovered > 0 {
            info!("🧹 Cleaning up {} WAL files after recovery", sorted_files.len());
...
impl ParallelRecoveryManager {
    ...
    pub fn new(
        ...
        info!("🎯 Creating ParallelRecoveryManager with {} workers", num_workers);
    ...
    pub async fn register_storage_engine(
        ...
        self.recovery_manager.register_storage_engine(collection_id, engine).await
    }
    
    /// Recover all collections in parallel
    pub async fn recover_all_parallel(&self) -> Result<RecoveryStats> {
        info!("🔄 Starting parallel WAL recovery with {} workers", self.num_workers);
    ...
    pub async fn recover_collection_parallel(
        &self,
        collection_id: &str,
    ) -> Result<u64> {
        info!(
        ...
        let wal_files = self.recovery_manager.disk_manager
...
mod tests {
    ...
    use crate::storage::persistence::write_buffer::{BatchId, SerializationFormat};
    use crate::storage::memtable::specialized::write_buffer_behavior::WriteBufferVectorBatch;
    use tempfile::TempDir;

    async fn create_test_managers() -> (Arc<WriteBufferDiskManager>, Arc<WriteBufferFlushCoordinator>, RecoveryManager, TempDir) {
    ...
    async fn test_recovery_manager_direct_to_storage() {
        let (disk_manager, flush_coordinator, mut recovery_manager, temp_dir) = create_test_managers().await;
        ...
        recovery_manager.register_storage_engine(collection_id, storage_engine.clone()).await
        ...
        let remaining_files = disk_manager.list_collection_files(collection_id).await
