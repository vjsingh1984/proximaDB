...
impl From<&crate::core::config::WalStorageConfig> for WriteBufferConfig {
    fn from(core_config: &crate::core::config::WalStorageConfig) -> Self {
        ...
        write_buffer_config.multi_disk.data_directories = core_config.write_buffer_urls.clone();
        write_buffer_config.multi_disk.distribution_strategy = match core_config.distribution_strategy {
        ...
        write_buffer_config.multi_disk.collection_affinity = core_config.collection_affinity;

        // Set performance thresholds
        write_buffer_config.performance.memory_flush_size_bytes = core_config.memory_flush_size_bytes;
        write_buffer_config.performance.global_flush_threshold = core_config.global_flush_threshold;

        // Apply optional configuration overrides from config.toml
        if let Some(strategy_type) = &core_config.strategy_type {
            write_buffer_config.strategy_type = match strategy_type.as_str() {
        ...
        if let Some(memtable_type) = &core_config.memtable_type {
            write_buffer_config.memtable.memtable_type = match memtable_type.as_str() {
        ...
        if let Some(sync_mode) = &core_config.sync_mode {
            write_buffer_config.performance.sync_mode = match sync_mode.as_str() {
        ...
        if let Some(batch_threshold) = core_config.batch_threshold {
            write_buffer_config.performance.batch_threshold = batch_threshold;
        }

        if let Some(write_buffer_mb) = core_config.write_buffer_size_mb {
            write_buffer_config.performance.write_buffer_size = write_buffer_mb * 1024 * 1024;
            // Convert MB to bytes
        }

        if let Some(concurrent_flushes) = core_config.concurrent_flushes {
            write_buffer_config.performance.concurrent_flushes = concurrent_flushes;
        }

        if let Some(global_shrink_factor) = core_config.global_shrink_factor {
            write_buffer_config.performance.global_shrink_factor = global_shrink_factor;
        }
