...
mod filesystem_api_tests {
    ...
    async fn test_local_filesystem_basic_operations() -> Result<()> {
        ...
        // Write to staging
        coordinator
            .write_to_staging(&operation.operation_id, "test_basic.txt", test_data)
            .await?;
    ...
    async fn test_local_filesystem_directory_operations() -> Result<()> {
        ...
        factory
            .create_dir_all(&format!("{}/nested/deep", test_dir))
            .await?;

        // Create files directly
        factory
            .write(&format!("{}/file1.txt", test_dir), b"content1", None)
            .await?;
        factory
            .write(&format!("{}/file2.txt", test_dir), b"content2", None)
            .await?;
    ...
    async fn test_error_handling() -> Result<()> {
        ...
        let result = fs
            .read("file:///tmp/proximadb-fs-tests/nonexistent.txt")
            .await;
        assert!(result.is_err());

        // Get metadata for non-existent file
        let result = fs
            .metadata("file:///tmp/proximadb-fs-tests/nonexistent.txt")
            .await;
        assert!(result.is_err());
        ...
        let result = fs
            .write(
                "file:///tmp/proximadb-fs-tests/nonexistent/dir/file.txt",
                b"test",
                None,
            )
            .await;
