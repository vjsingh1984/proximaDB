...
mod write_ahead_log_batch_strategy_tests {
    ...
    impl MockWriteBufferBehavior {
        ...
        async fn get_stats(&self) -> Result<HashMap<String, crate::storage::persistence::write_ahead_log::WALStats>> {
            let collections = self.collections.read().await;
            ...
            for (collection_id, batches) in collections.iter() {
                ...
                stats.insert(collection_id.clone(), crate::storage::persistence::write_ahead_log::WALStats {
    ...
    fn test_wal_config_defaults() {
        ...
        assert_eq!(std::mem::size_of_val(&config), std::mem::size_of::<WALConfig>());
    }
