...
impl RecoveryThreadPool {
    ...
    pub async fn start_recovery(&self) -> Result<RecoveryGuard<'_>> {
        ...
        info!(
            "🔒 Recovery phase started - acquiring {} threads",
            self.max_threads
        );
    ...
    pub async fn execute_recovery_task<F, T>(&self, task_name: &str, task: F) -> Result<T>
    where
        ...
        T: Send + 'static,
    {
        if !self.is_recovering.load(Ordering::Acquire) {
            return Err(anyhow::anyhow!(
                "Recovery not in progress - call start_recovery() first"
            ));
        }
        ...
        debug!(
            "🧵 Recovery task '{}' starting (active threads: {}/{})",
            task_name, active, self.max_threads
        );
        ...
        debug!(
            "✅ Recovery task '{}' completed (remaining threads: {})",
            task_name, remaining
        );
...
impl<'a> RecoveryGuard<'a> {
    ...
    pub async fn complete(self, collections_processed: usize, vectors_recovered: u64) {
        ...
        info!(
            ...
            duration_ms, self.pool.max_threads
        );

        let stats = self.pool.stats.read().await;
        info!(
            "📊 Recovery stats: {} collections, {} vectors, peak {} concurrent threads",
            stats.collections_processed, stats.vectors_recovered, stats.peak_concurrent_threads
        );
    }
}
...
pub fn initialize_recovery_thread_pool(max_threads: Option<usize>) -> &'static RecoveryThreadPool {
    RECOVERY_THREAD_POOL.get_or_init(|| match max_threads {
...
mod tests {
    ...
    async fn test_recovery_thread_pool() {
        ...
        for i in 0..10 {
            let pool_ref = &pool;
            let task = pool_ref.execute_recovery_task("test_task", async move {
