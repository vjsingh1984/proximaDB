...
use super::builder::{StorageSystemConfig, DataStorageConfig, StorageLayoutStrategy};
...
impl ConfigValidator {
    ...
    pub fn validate_data_storage(config: &DataStorageConfig) -> Result<()> {
        ...
        if config.segment_size < 1024 * 1024 {
            bail!("Segment size must be at least 1MB, got {}", config.segment_size);
        }
        
        if config.segment_size > 1024 * 1024 * 1024 {
            bail!("Segment size cannot exceed 1GB, got {}", config.segment_size);
        }
        ...
        if config.cache_size_mb > 1024 * 1024 {
            bail!("Cache size cannot exceed 1TB, got {}MB", config.cache_size_mb);
        }
    ...
    pub fn validate_filesystem(config: &FilesystemConfig) -> Result<()> {
        ...
        if let Some(ref default_fs) = config.default_fs {
            Self::validate_storage_url(default_fs)
                .context("Invalid default filesystem URL")?;
        }
    ...
    pub fn validate_environment() -> Result<()> {
        ...
        if sys_info < 1024 * 1024 * 1024 {  // Less than 1GB
        ...
        if cpu_count < 2 {
            eprintln!("⚠️  Warning: System has only {} CPU core, performance may be limited", cpu_count);
        }
    ...
    pub fn generate_recommendations(config: &StorageSystemConfig) -> Vec<String> {
        ...
        let total_cache = config.data_storage.cache_size_mb + config.storage_performance.memory_pool_mb;
        if total_cache > 8192 {
        ...
        if !config.data_storage.compression.compress_vectors && config.data_storage.data_urls.len() > 1 {
        ...
        if config.data_storage.tiering_config.is_none() && config.data_storage.data_urls.len() > 2 {
            recommendations.push(
                "Consider enabling tiered storage for better cost/performance optimization".to_string()
            );
        }
...
mod tests {
    use super::*;
    use crate::storage::builder::{StorageSystemConfig, DataStorageConfig};
    ...
    fn test_url_validation() {
        ...
        assert!(ConfigValidator::validate_storage_url("hdfs://namenode/path").is_err()); // Missing port
    }
