...
impl Memtable {
    ...
    pub async fn put(&self, record: VectorRecord) -> Result<u64> {
        ...
        let entry = MemtableEntry {
            sequence,
            operation: MemtableOperation::Put {
                record: record.clone(),
            },
            timestamp: Utc::now(),
        };
        ...
        let collection_table =
            data.entry(record.collection_id.clone())
        ...
        if let Some(&old_sequence) = collection_table.id_to_sequence.get(&record.id) {
            if let Some(old_entry) = collection_table.vectors.remove(&old_sequence) {
                ...
                collection_table.stats.size_bytes =
                    collection_table.stats.size_bytes.saturating_sub(old_size);
            }
        } else {
            // New vector ID
    ...
    pub async fn delete(&self, collection_id: CollectionId, vector_id: VectorId) -> Result<u64> {
        ...
        let entry = MemtableEntry {
            sequence,
            operation: MemtableOperation::Delete {
        ...
        if let Some(old_sequence) = collection_table.id_to_sequence.remove(&vector_id) {
            if let Some(old_entry) = collection_table.vectors.remove(&old_sequence) {
                ...
                collection_table.stats.size_bytes =
                    collection_table.stats.size_bytes.saturating_sub(old_size);
                collection_table.stats.active_count =
                    collection_table.stats.active_count.saturating_sub(1);
            }
        }
    ...
    pub async fn get_batch(
        ...
        collection_id: &CollectionId,
        vector_ids: &[VectorId],
    ) -> Result<Vec<Option<VectorRecord>>> {
        let data = self.data.read().await;
    ...
    pub async fn operation_count(&self) -> usize {
        let data = self.data.read().await;
        data.values().map(|table| table.stats.operation_count).sum()
    }
    ...
    pub async fn get_collection_stats(
        &self,
        collection_id: &CollectionId,
    ) -> Option<MemtableCollectionStats> {
        let data = self.data.read().await;
        data.get(collection_id)
            .map(|table| MemtableCollectionStats {
        ...
        match filter_value {
            ...
            serde_json::Value::Array(filter_array) => {
                if let serde_json::Value::Array(record_array) = record_value {
                    ...
                    filter_array
                        .iter()
                        .all(|filter_item| record_array.contains(filter_item))
                } else {
...
mod tests {
    ...
    async fn test_memtable_basic_operations() {
        ...
        // Test get
        let retrieved = memtable
            .get(&record.collection_id, &record.id)
            .await
            .unwrap();
    ...
    async fn test_memtable_size_tracking() {
        ...
        memtable
            .delete(record.collection_id, record.id)
            .await
            .unwrap();
