...
#[derive(Debug, Clone)]
pub struct MetadataWriteBufferConfig {
    /// Base write buffer configuration
    pub base_config: WriteBufferConfig,
...
impl Default for MetadataWriteBufferConfig {
    fn default() -> Self {
...
pub struct MetadataWriteBufferManager {
    /// Underlying write buffer manager with modern batch strategy (Avro)
    write_buffer_strategy: crate::storage::persistence::write_buffer::WriteBufferManager,
...
impl std::fmt::Debug for MetadataWriteBufferManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MetadataWriteBufferManager")
...
impl MetadataWriteBufferManager {
    /// Create new metadata write buffer manager
    pub async fn new(
        config: MetadataWriteBufferConfig,
        filesystem: Arc<FilesystemFactory>,
    ) -> Result<Self> {
        tracing::debug!("🚀 Creating MetadataWriteBufferManager with B+Tree memtable for sorted access");

        // Create write buffer manager using modern batch factory pattern
        let write_buffer_manager = crate::storage::persistence::write_buffer::WriteBufferManager::create_with_batch_factory(
    ...
    pub async fn upsert_collection(&self, metadata: VersionedCollectionMetadata) -> Result<()> {
        ...
        // Write to write buffer using modern batch architecture through WriteBufferBehaviorWrapper
        if let Some(behavior_wrapper) = self.write_buffer_strategy.get_write_buffer_behavior_wrapper() {
            // Create WriteBufferVectorBatch for the metadata
            let batch = crate::storage::memtable::specialized::write_buffer_behavior::WriteBufferVectorBatch {
    ...
    pub async fn get_collection(
        ...
        // Search in write buffer using modern get_vector_by_id through WriteBufferBehaviorWrapper
        let vector_id = format!("metadata_{}", collection_id);
        let vector_record = if let Some(behavior_wrapper) = self.write_buffer_strategy.get_write_buffer_behavior_wrapper() {
    ...
    async fn recover_metadata_from_write_buffer(&self) -> Result<()> {
        ...
        let recovered_entries = if let Some(behavior_wrapper) = self.write_buffer_strategy.get_write_buffer_behavior_wrapper() {
            if let Ok(stats_map) = behavior_wrapper.get_stats().await {
    ...
    pub async fn delete_collection(&self, collection_id: &str) -> Result<bool> {
        ...
        if exists {
            ...
            if let Some(behavior_wrapper) = self.write_buffer_strategy.get_write_buffer_behavior_wrapper() {
                // Create WriteBufferVectorBatch for the delete
                let vector_count = delete_batch_records.len() as u64;
                let _end_sequence = if vector_count > 0 { 2 + vector_count - 1 } else { 2 };
                let delete_batch = crate::storage::memtable::specialized::write_buffer_behavior::WriteBufferVectorBatch {
                    batch_id: crate::storage::persistence::write_buffer::BatchId::new(),
    ...
    pub async fn flush(&self) -> Result<()> {
        ...
        // Modern WriteBufferManager doesn't have flush method - data is automatically persisted
