...
pub struct LocalRocksDbBackend {
    /// Configuration
...
impl LocalRocksDbBackend {
    ...
    async fn update_stats(&self, op: StatOp) {
        if let Some(ref metrics) = self.metrics_collector {
            match op {
                StatOp::Read(bytes) => {
                    metrics.record(MetricsOperationType::Read, 0, true, Some(bytes as usize));
                }
                StatOp::Write(bytes) => {
                    metrics.record(MetricsOperationType::Write, 0, true, Some(bytes as usize));
                }
                StatOp::Delete => {
                    metrics.record(MetricsOperationType::Delete, 0, true, None);
                }
                StatOp::CacheHit => {
                    metrics.record(MetricsOperationType::CacheHit, 0, true, None);
                }
                StatOp::CacheMiss => {
                    metrics.record(MetricsOperationType::CacheMiss, 0, true, None);
                }
                StatOp::Compaction => {
                    // Compaction is a special RocksDB metric, just record it as a write
                    metrics.record(MetricsOperationType::Write, 0, true, None);
                }
            }
        }
    }
    ...
    pub async fn upsert_collection_proto(&self, proto_collection: &Collection) -> Result<()> {
        ...
        match db {
            DbHandle::Transactional(db) => {
                ...
                txn.put_cf(&cf_uuid_index, uuid.as_bytes(), uuid.as_bytes())?;
            ...
            DbHandle::Regular(db) => {
                ...
                let cf_uuid_index = db
                    ...
                    .ok_or_else(|| anyhow::anyhow!("Column family not found"))?;
                batch.put_cf(&cf_uuid_index, uuid.as_bytes(), uuid.as_bytes());
...
impl DbHandle {
    ...
    fn cf_handle(&self, cf_name: &str) -> Option<&rocksdb::ColumnFamily> {
        match self {
            DbHandle::Transactional(db) => db.cf_handle(cf_name),
            DbHandle::Regular(db) => db.cf_handle(cf_name),
        }
    }
}
...
#[async_trait]
impl MetadataProvider for LocalRocksDbBackend {
...
// Additional legacy methods - will be removed in future versions
impl LocalRocksDbBackend {
    /// Create or update collection record (legacy - prefer upsert_collection_proto)
    async fn upsert_collection_record(&self, record: Collection) -> Result<()> {
        let db = self.get_db().await?;
...
// MetadataMetrics removed - now using injected UnifiedMetricsCollector

// Implement InternalCollectionProvider to maintain backward compatibility
#[async_trait]
impl InternalCollectionProvider for LocalRocksDbBackend {
