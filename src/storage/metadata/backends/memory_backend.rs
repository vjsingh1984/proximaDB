...
impl MemoryMetadataBackend {
    pub fn new() -> Self {
        Self {
            ...
            system_metadata: Arc::new(RwLock::new(SystemMetadata::default_with_node_id(
                "memory-node-1".to_string(),
            ))),
            config: None,
    ...
    async fn apply_filter(&self, filter: &MetadataFilter) -> Result<Vec<CollectionId>> {
        ...
        tracing::debug!(
            "🔍 Applied memory filter, candidates: {:?}",
            candidates.as_ref().map(|c| c.len())
        );
    ...
    async fn record_operation(&self, start_time: std::time::Instant, success: bool) {
        ...
        stats.avg_operation_time_ns =
            (stats.avg_operation_time_ns * (stats.total_operations - 1) as f64 + elapsed_ns)
...
impl MetadataBackend for MemoryMetadataBackend {
    ...
    async fn update_stats(
        ...
        if success {
            tracing::debug!(
                "📊 Updated stats for collection {}: vectors={:+}, size={:+}",
                collection_id,
                vector_delta,
                size_delta
            );
    ...
    async fn batch_operations(&self, operations: Vec<MetadataOperation>) -> Result<()> {
        ...
        tracing::debug!(
            "🔄 Starting memory batch operation: {} items",
            operations.len()
        );
    ...
    async fn commit_transaction(&self, transaction_id: &str) -> Result<()> {
        ...
        tracing::debug!(
            "💾 Memory transaction commit placeholder: {}",
            transaction_id
        );
        Ok(())
    }
    ...
    async fn rollback_transaction(&self, transaction_id: &str) -> Result<()> {
        // No-op for memory backend
        tracing::debug!(
            "🔄 Memory transaction rollback placeholder: {}",
            transaction_id
        );
        Ok(())
    }
    ...
    async fn backup(&self, location: &str) -> Result<String> {
        ...
        tracing::debug!(
            "📦 Memory backup placeholder - would serialize to: {}, backup_id: {}",
            location,
            backup_id
        );
    ...
    async fn restore(&self, backup_id: &str, location: &str) -> Result<()> {
        ...
        tracing::debug!(
            "🔄 Memory restore placeholder - would deserialize from: {}, backup_id: {}",
            location,
            backup_id
        );
