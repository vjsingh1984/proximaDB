...
impl RocksDbMetadataBackend {
    ...
    async fn initialize(&self) -> Result<()> {
        info!("🗄️ Initializing RocksDB metadata backend at {:?}", self.config.db_path);
        ...
        if self.config.enable_transactions {
            ...
            let db = TransactionDB::open_cf_descriptors(&db_opts, &txn_db_opts, &self.config.db_path, cf_descriptors)
            ...
            *self.db.write().await = Some(Arc::new(db));
            ...
            let db = DBWithThreadMode::<MultiThreaded>::open_cf_descriptors(&db_opts, &self.config.db_path, cf_descriptors)
...
impl RocksDbMetadataBackend {
    ...
    pub async fn create_backup(&self) -> Result<()> {
        if let Some(ref backup_config) = self.config.backup_config {
            ...
            if backup_config.max_backups > 0 {
                self.cleanup_old_backups(backup_path, backup_config.max_backups).await?;
            }
    ...
    pub async fn health_check(&self) -> Result<()> {
        ...
        if self.config.enable_statistics {
            ...
            debug!("RocksDB Stats - Reads: {}, Writes: {}, Bytes read: {}, Bytes written: {}",
...
mod tests {
    ...
    async fn test_rocksdb_backend_basic_operations() {
        ...
        // Test delete
        let deleted = backend.delete_collection_by_uuid("test-uuid-123").await.unwrap();
        assert!(deleted);
