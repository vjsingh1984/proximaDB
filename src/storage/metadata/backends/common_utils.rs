...
pub mod serialization {
    ...
    pub fn deserialize_collection(data: &[u8]) -> Result<Collection> {
        Collection::decode(data)
            .context("Failed to decode collection from protobuf")
    }
}
...
pub mod paths {
    ...
    pub fn collection_path(base: &Path, collection_id: &str) -> PathBuf {
        base.join("collections").join(format!("{}.pb", collection_id))
    }
    ...
    pub fn snapshot_path(base: &Path, timestamp: u64) -> PathBuf {
        base.join("snapshots").join(format!("snapshot_{}.pb", timestamp))
    }
...
pub mod filesystem {
    ...
    pub async fn ensure_directory<F: FileSystem>(
        fs: &F,
        path: &str,
    ) -> Result<()> {
    ...
    pub async fn atomic_write<F: FileSystem>(
        fs: &F,
        path: &str,
        data: Vec<u8>,
    ) -> Result<()> {
        ...
        // Write to temp file
        fs.write(&temp_path, &data, None).await
            .with_context(|| format!("Failed to write temp file: {}", temp_path))?;
        
        // Atomic rename
        fs.move_file(&temp_path, path).await
...
mod tests {
    ...
    fn test_create_batches() {
        use batch::{create_batches, BatchConfig};
