...
impl FilestoreMetadataBackend {
    ...
    pub async fn new(
        ...
        info!(
            "📁 FilestoreMetadataBackend: Working in root directory specified by URL: {}",
            config.filestore_url
        );
    ...
    async fn replay_incremental_operations(
        &self,
                        ...
                        if !data.is_empty() {
                            ...
                            for value in reader {
                                if let Ok(avro_value) = value {
                                    ...
                                    if let Ok(avro_op) =
                                        apache_avro::from_value::<AvroOperationRead>(&avro_value)
                                    {
                                        let op_type = match avro_op.operation_type.as_str() {
                                            ...
                                            _ => {
                                                info!(
                                                    "⚠️ Unknown operation type: {}, skipping",
                                                    avro_op.operation_type
                                                );
                                                continue;
                                            }
                                        };
    ...
    async fn write_atomically(
        ...
        let strategy = WriteStrategyFactory::create_metadata_strategy(
            fs,
            self.config.temp_directory.as_deref(),
        )?;
        ...
        info!(
            "💾 Atomic write with optimized strategy: {}",
            final_path.display()
        );
        fs.write_atomic(&final_path.to_string_lossy(), data, Some(file_options))
            .await?;
    ...
    async fn write_incremental_operation(&self, operation: IncrementalOperation) -> Result<()> {
        info!(
            "💾 Writing incremental operation: seq={}, type={:?}, id={}",
        ...
        let filename = format!(
            "op_{:08}_{}.avro",
            operation.sequence_number, operation.timestamp
        );
    ...
    pub async fn upsert_collection_record(&self, record: CollectionRecord) -> Result<()> {
        info!(
            "🆕 Upserting collection record: {} (UUID: {})",
            record.name, record.uuid
        );
    ...
    pub async fn get_collection_record_by_name(
        &self,
        name: &str,
    ...
    pub async fn get_collection_record_by_name_or_uuid(
        &self,
        identifier: &str,
    ) -> Result<Option<CollectionRecord>> {
        let _guard = self.recovery_lock.read().await;
    ...
    pub async fn list_collections(
        &self,
        filter: Option<Box<dyn Fn(&CollectionRecord) -> bool + Send + Sync>>,
        ...
        let collections: Vec<_> = if let Some(filter_fn) = filter {
            ...
            self.single_index
                .filter_collections(|record| filter_fn(record))
                .into_iter()
                .map(|arc_record| (*arc_record).clone())
                .collect()
        } else {
