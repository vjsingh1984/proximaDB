...
mod tests {
    ...
    impl MockAtomicMetadataStore {
        ...
        async fn begin_transaction(&self, isolation_level: IsolationLevel) -> Result<TransactionId> {
            let id = Uuid::new_v4();
        ...
        async fn add_to_transaction(&self, transaction_id: &TransactionId, operation: MetadataOperation) -> Result<()> {
                    ...
                    for op in &tx.operations {
                        match op {
                            MetadataOperation::CreateCollection(metadata) => {
                                self.metadata.write().await.insert(metadata.id.clone(), metadata.clone());
                            }
                            MetadataOperation::UpdateCollection { collection_id, metadata } => {
                                self.metadata.write().await.insert(collection_id.clone(), metadata.clone());
                            }
                            ...
                            MetadataOperation::UpdateStats { collection_id, vector_delta, size_delta } => {
                                if let Some(metadata) = self.metadata.write().await.get_mut(collection_id) {
                                    ...
                                    metadata.total_size_bytes = (metadata.total_size_bytes as i64 + size_delta).max(0) as u64;
                                }
                            }
                            _ => {}
                        }
    ...
    impl MetadataStoreInterface for MockAtomicMetadataStore {
        async fn create_collection(&self, metadata: CollectionMetadata) -> Result<()> {
            self.metadata.write().await.insert(metadata.id.clone(), metadata);
            Ok(())
        }
        ...
        async fn update_collection(&self, collection_id: &str, metadata: CollectionMetadata) -> Result<()> {
            self.metadata.write().await.insert(collection_id.to_string(), metadata);
            Ok(())
        }
        ...
        async fn update_stats(&self, collection_id: &str, vector_delta: i64, size_delta: i64) -> Result<()> {
            if let Some(metadata) = self.metadata.write().await.get_mut(collection_id) {
                metadata.vector_count = (metadata.vector_count as i64 + vector_delta).max(0) as u64;
                metadata.total_size_bytes = (metadata.total_size_bytes as i64 + size_delta).max(0) as u64;
            }
    ...
    async fn test_begin_transaction() {
        ...
        let tx1 = store.begin_transaction(IsolationLevel::ReadCommitted).await
            .expect("Failed to begin ReadCommitted transaction");
        
        let tx2 = store.begin_transaction(IsolationLevel::RepeatableRead).await
            .expect("Failed to begin RepeatableRead transaction");
        
        let tx3 = store.begin_transaction(IsolationLevel::Serializable).await
    ...
    async fn test_add_to_transaction() {
        ...
        let transaction_id = store.begin_transaction(IsolationLevel::ReadCommitted).await
        ...
        store.add_to_transaction(&transaction_id, operation).await
    ...
    async fn test_commit_transaction() {
        ...
        store.commit_transaction(&transaction_id).await
    ...
    async fn test_abort_transaction() {
        ...
        store.abort_transaction(&transaction_id).await
    ...
    async fn test_abort_nonexistent_transaction() {
        ...
        store.abort_transaction(&fake_transaction_id).await
    ...
    async fn test_metadata_store_interface_create_collection() {
        ...
        store.create_collection(metadata).await
    ...
    async fn test_metadata_store_interface_get_collection() {
        ...
        store.create_collection(metadata.clone()).await
            .expect("Failed to create collection");
    ...
    async fn test_metadata_store_interface_update_collection() {
        ...
        store.update_collection("test_collection", metadata.clone()).await
    ...
    async fn test_metadata_store_interface_delete_collection() {
        ...
        let deleted = store.delete_collection("test_collection").await
            .expect("Failed to delete collection");
        assert!(deleted);
        
        // Should return false for already deleted collection
        let deleted_again = store.delete_collection("test_collection").await
    ...
    async fn test_metadata_store_interface_list_collections() {
        ...
        store.create_collection(metadata1).await
            .expect("Failed to create collection 1");
        store.create_collection(metadata2).await
            .expect("Failed to create collection 2");
        
        // List without filter
        let collections = store.list_collections(None).await
        ...
        let filtered_collections = store.list_collections(Some(filter)).await
    ...
    async fn test_metadata_store_interface_update_stats() {
        ...
        store.update_stats("test_collection", 100, 1024).await
            .expect("Failed to update stats");
        
        // Negative deltas should also work
        store.update_stats("test_collection", -10, -100).await
    ...
    async fn test_metadata_store_interface_batch_operations() {
        ...
        store.batch_operations(operations).await
