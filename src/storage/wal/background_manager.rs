...
impl BackgroundMaintenanceManager {
    ...
    pub async fn trigger_flush_if_needed(
            ...
            if let Some(status) = status_map.get(collection_id) {
                match status {
                    ...
                    BackgroundTaskStatus::Flushing => {
                        debug!("🔄 Flush already in progress for collection {}, skipping", collection_id);
                    ...
                    BackgroundTaskStatus::Compacting => {
                        ...
                        debug!("📈 Upgrading compaction to flush+compact for collection {}", collection_id);
                        drop(status_map);
                        let mut status_map = self.collection_status.write().await;
                        status_map.insert(collection_id.clone(), BackgroundTaskStatus::FlushAndCompact);
                        return Ok(false);
                    }
                    BackgroundTaskStatus::FlushAndCompact => {
                        debug!("⏳ Flush+compact already queued for collection {}, skipping", collection_id);
                        let mut stats = self.stats.lock().await;
                        stats.flush_operations_skipped += 1;
                        return Ok(false);
                    }
        ...
        tokio::spawn(async move {
            ...
            info!("🚿 Starting background flush for collection {} ({}MB)", 
                  collection_id_clone, current_memory_size / (1024 * 1024));
            ...
            if needs_compaction {
                info!("🔄 Triggering compaction after flush for collection {}", collection_id_clone);
                
                // Update status to compacting
                {
                    let mut status_map = status_map_clone.write().await;
                    status_map.insert(collection_id_clone.clone(), BackgroundTaskStatus::Compacting);
                }
                ...
                info!("✅ Background compaction completed for collection {} in {}ms", 
                      collection_id_clone, compaction_duration.as_millis());
            }
            ...
            info!("✅ Background flush completed for collection {} in {}ms", 
                  collection_id_clone, duration.as_millis());
        });
    ...
    async fn should_trigger_compaction_after_flush(collection_id: &CollectionId) -> bool {
    ...
    pub async fn get_collection_status(&self, collection_id: &CollectionId) -> BackgroundTaskStatus {
        let status_map = self.collection_status.read().await;
        status_map.get(collection_id)
            .cloned()
