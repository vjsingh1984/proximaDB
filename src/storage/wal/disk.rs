...
use super::{WalEntry, FlushResult};
use super::config::{WalConfig, DiskDistributionStrategy, SyncMode};
...
impl CollectionDiskLayout {
    ...
    fn next_segment_path(&mut self) -> PathBuf {
        ...
        self.base_directory.join(format!("segment_{:08}.wal", self.current_segment))
    }
...
impl WalDiskManager {
    ...
    */
    ...
    async fn get_or_create_layout(&self, collection_id: &CollectionId) -> Result<()> {
        ...
        if !layouts.contains_key(collection_id) {
            ...
            fs::create_dir_all(&base_dir).await
                .with_context(|| format!("Failed to create collection directory: {:?}", base_dir))?;
            
            let layout = CollectionDiskLayout::new(
                collection_id.clone(),
                disk_index,
                base_dir,
            );
    ...
    async fn write_segment(&self, path: &Path, data: &[u8]) -> Result<u64> {
        ...
        self.filesystem.write(&url, data, None).await
        ...
        match self.config.performance.sync_mode {
            SyncMode::Always | SyncMode::PerBatch => {
                self.filesystem.sync(&url).await
    ...
    */
    ...
    fn estimate_compression_ratio(&self, entries: &[WalEntry], compressed_size: u64) -> f64 {
        let uncompressed_size: usize = entries.iter()
            .map(|e| self.estimate_entry_size(e))
            .sum();
    ...
    async fn calculate_average_compression_ratio(&self, layouts: &HashMap<CollectionId, CollectionDiskLayout>) -> f64 {
