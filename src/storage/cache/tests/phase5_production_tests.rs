...
async fn test_monitoring_dashboard() {
    ...
    assert_eq!(
        vector_status.memory_allocated_mb,
        config.global.total_memory_mb * 40 / 100
    );
}
...
async fn test_cache_system_stress() {
    ...
    for i in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            for j in 0..100 {
                coord
                    .on_vector_access(&format!("vec_{}_{}", i, j))
                    .await
                    .ok();
            }
        }));
    ...
    for i in 0..5 {
        ...
        handles.push(tokio::spawn(async move {
            for j in 0..50 {
                coord
                    .on_query_execution(&format!("query_{}_{}", i, j))
                    .await
                    .ok();
            }
        }));
    ...
    for i in 0..3 {
        ...
        handles.push(tokio::spawn(async move {
            for j in 0..20 {
                coord
                    .orchestrate_cascade_invalidation(&format!("vec_{}_{}", i, j))
                    .await
                    .ok();
            }
        }));
...
use super::super::orchestrator::{
    CascadeInvalidator as CascadeUpdatePropagator, CrossCacheOrchestrator,
};
