...
async fn test_filter_bitmap_cache_roaring() {
    ...
    // Cache filters
    cache.put_with_hooks("filter1".to_string(), filter1.clone()).await;
    cache.put_with_hooks("filter2".to_string(), filter2.clone()).await;
    
    // Test filter combination
    let combined = cache.combine_filters(&["filter1", "filter2"], FilterOp::And).await;
...
async fn test_index_structure_cache_hot_paths() {
    ...
    // Cache nodes
    cache.put_with_hooks("root".to_string(), root_node.clone()).await;
    cache.put_with_hooks("child1".to_string(), child1.clone()).await;
    cache.put_with_hooks("child2".to_string(), child2.clone()).await;
