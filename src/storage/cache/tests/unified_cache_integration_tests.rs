...
use crate::storage::cache::specialized::{
    VectorStore, IndexNodeCache, BitmapFilterCache, MetadataStore,
    vector_store::{SstBlockKey, CompressedBlock, CompressionType},
    index_node_cache::{SstableIndex, SstIndexEntry, MetadataStats},
};
...
mod sst_cache_integration {
    ...
    async fn test_sst_vector_block_caching() {
        ...
        // Cache vectors
        vector_cache.cache_block_vectors(&block_key, vectors.clone()).await.unwrap();
...
mod unified_search_cache_tests {
    ...
    async fn test_cross_engine_cache_sharing() {
        ...
        metadata_cache.put("shared_schema", serde_json::json!({"format": "unified"}))
            .await.unwrap();
    ...
    async fn test_cache_memory_limits() {
        ...
        for i in 0..100 {
            ...
            let _ = small_cache.cache_compressed_block(
                &block_key,
                data,
                CompressionType::None,
                1024,
...
use tracing::{debug, error, info};
    ...
    async fn test_batch_caching_performance() {
        ...
        for i in 0..1000 {
            ...
            let _ = vector_cache.cache_compressed_block(
                &key,
                vec![i as u8; 100],
                CompressionType::Lz4,
                100,
