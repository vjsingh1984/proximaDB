...
use super::{TenantContext, TenantConfig, TenantStatus, TenantResourceTracker, ResourceLimits};
...
impl TenantManager {
    ...
    pub async fn create_tenant(
        ...
        info!("Created tenant: {} for organization: {}", tenant_id, config.organization_name);
    ...
    pub fn validate_user_tenant_access(
        ...
        if user_tenant_id != requested_tenant_id {
            warn!("User from tenant {} attempted to access tenant {}", 
                  user_tenant_id, requested_tenant_id);
            return Err(anyhow!("Access denied: user not authorized for tenant {}", 
                             requested_tenant_id));
        }
        Ok(())
    }
...
mod tests {
    ...
    async fn test_tenant_creation() {
        ...
        let result = manager.create_tenant("test_tenant".to_string(), config).await;
        assert!(result.is_ok());
    ...
    async fn test_duplicate_tenant_prevention() {
        ...
        let result1 = manager.create_tenant("duplicate_test".to_string(), config.clone()).await;
        assert!(result1.is_ok());
        
        // Second creation should fail
        let result2 = manager.create_tenant("duplicate_test".to_string(), config).await;
