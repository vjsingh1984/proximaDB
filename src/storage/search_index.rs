 ...
 * 🔄 MIGRATION: Use src/storage/vector/indexing/manager.rs instead.
 * 📅 DEPRECATION: Phase 2 - Index Consolidation (replaces SearchIndexManager)
 *
...
use crate::compute::algorithms::{create_search_algorithm, SearchResult, VectorSearchAlgorithm};
use crate::compute::{DistanceMetric, IndexAlgorithm};
use crate::core::{CollectionId, StorageError, VectorId, VectorRecord};
use crate::storage::{CollectionMetadata, Result};
...
impl SearchIndexManager {
    ...
    pub async fn add_vector(
        ...
        if let Some(index) = indexes.get_mut(collection_id) {
            ...
            let metadata = if record.metadata.is_empty() {
                ...
                let json_metadata: HashMap<String, serde_json::Value> = record
                    .metadata
                    .iter()
    ...
    pub async fn remove_vector(
        ...
        if let Some(index) = indexes.get_mut(collection_id) {
            let removed = index
                .remove_vector(&vector_id.to_string())
                .map_err(|e| StorageError::IndexError(e))?;
            Ok(removed)
        } else {
            Ok(false)
    ...
    pub async fn search(&self, request: SearchRequest) -> Result<Vec<SearchResult>> {
        ...
        if let Some(index) = indexes.get(&request.collection_id) {
            ...
            let results = if let Some(filter) = request.filter {
                ...
                index.search(&request.query, request.k).map_err(|e| {
            ...
            tracing::debug!(
                "🔍 SearchIndexManager::search results: {} matches",
                results.len()
            );
            Ok(results)
        } else {
            tracing::error!(
                "🔥 Index for collection {} not found",
                request.collection_id
            );
    ...
    pub async fn get_index_stats(
        ...
        if let Some(index) = indexes.get(collection_id) {
            ...
            stats.insert(
                ...
                serde_json::json!(memory_usage.index_size_bytes),
            );
            stats.insert(
                "vector_data_bytes".to_string(),
                serde_json::json!(memory_usage.vector_data_bytes),
            );
            stats.insert(
                "metadata_bytes".to_string(),
                serde_json::json!(memory_usage.metadata_bytes),
            );
            stats.insert(
                "total_bytes".to_string(),
                serde_json::json!(memory_usage.total_bytes),
            );
    ...
    pub async fn optimize_index(&self, collection_id: &CollectionId) -> Result<()> {
        ...
        if let Some(index) = indexes.get_mut(collection_id) {
            index.optimize().map_err(|e| StorageError::IndexError(e))?;
        }
    ...
    pub async fn rebuild_index(
        ...
        if let Some(_index) = indexes.get(collection_id) {
            let vector_data: Vec<(String, Vec<f32>, Option<HashMap<String, serde_json::Value>>)> =
                vectors
                    .into_iter()
                    .map(|record| {
                        let id = record.id.to_string();
...
mod tests {
    ...
    async fn test_search_index_basic() {
        ...
        // Create index
        index_manager
            .create_index("test_collection".to_string(), &metadata)
            .await
            .unwrap();
        ...
        for vector in &vectors {
            index_manager
                .add_vector(&"test_collection".to_string(), vector)
                .await
                .unwrap();
        }
        ...
        let stats = index_manager
            .get_index_stats(&"test_collection".to_string())
            .await
            .unwrap();
