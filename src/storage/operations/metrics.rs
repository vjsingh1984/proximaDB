...
use crate::storage::operations::{OperationType, OperationStatistics};
...
impl OperationMetrics {
    ...
    pub async fn record_operation(
        ...
        info!("📊 Recorded {} operation: duration={:?}, success={}", 
              operation_type_name(operation_type), duration, success);
    }

    /// Get average duration for operation type
    pub async fn get_average_duration(&self, operation_type: OperationType) -> Option<std::time::Duration> {
    ...
    pub async fn get_operation_statistics(&self) -> OperationStatistics {
        ...
        let successful_operations: u64 = success_rates.values().map(|r| r.successful_attempts).sum();
        let failed_operations: u64 = success_rates.values().map(|r| r.total_attempts - r.successful_attempts).sum();
        
        let average_duration_ms = if total_operations > 0 {
            let total_duration: std::time::Duration = duration_stats.values()
        ...
        let operations_by_type = duration_stats.iter()
    ...
    pub async fn generate_prometheus_metrics(&self) -> Vec<String> {
        ...
        for (op_type, stats) in duration_stats.iter() {
            ...
            if stats.count > 0 {
                let avg_duration_ms = stats.total_duration.as_secs_f64() * 1000.0 / stats.count as f64;
                metrics.push(format!(
...
mod metrics_tests {
    ...
    async fn test_prometheus_metrics_generation() {
        ...
        metrics.record_operation(OperationType::Flush, std::time::Duration::from_millis(100), true).await;
        metrics.record_operation(OperationType::Flush, std::time::Duration::from_millis(200), true).await;
        
        let prometheus_metrics = metrics.generate_prometheus_metrics().await;
        
        assert!(!prometheus_metrics.is_empty());
        assert!(prometheus_metrics.iter().any(|m| m.contains("proximadb_operations_total")));
        assert!(prometheus_metrics.iter().any(|m| m.contains("proximadb_operation_duration_ms")));
    }
}