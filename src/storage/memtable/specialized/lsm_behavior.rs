    ...
    T: MemtableCore<String, LsmEntry> + Send + Sync,
{
    ...
    pub async fn merge_with<U>(
        &self,
        other: &LsmBehaviorWrapper<U>,
...
mod tests {
    ...
    async fn test_lsm_behavior_wrapper() {
        ...
        let result = lsm_wrapper
            .insert_lsm_entry("test_key".to_string(), lsm_entry.clone())
            .await;
        assert!(result.is_ok());
    ...
    async fn test_lsm_compaction_logic() {
        ...
        for i in 0..10 {
            ...
            lsm_wrapper
                .insert_lsm_entry(format!("key_{}", i), entry)
                .await
                .unwrap();
        }
