    ...
    V: Clone + Send + Sync + Debug,
{
    ...
    fn estimate_entry_size(key: &K, value: &V) -> usize {
        ...
        // For the key, use std::mem::size_of since keys are typically simple types (u64, etc.)
        ...
        tracing::debug!(
            "📊 BTREE_SIZE_ESTIMATE: key_size={}, value_size={}, overhead={}, total={}",
            key_size,
            value_size,
            overhead,
            total
    ...
    V: Clone + Send + Sync + Debug + 'static,
{
    async fn insert(&self, key: K, value: V) -> Result<u64> {
        ...
        tracing::debug!(
            "📊 BTREE_INSERT: Estimated entry size: {} bytes",
            entry_size
        );
        ...
        tracing::info!(
            ...
            new_count,
            new_count as i32 - old_count as i32
        );
        drop(data);
        ...
        *size = size.saturating_sub(old_size).saturating_add(entry_size);
        ...
        tracing::info!(
            "📊 BTREE_INSERT: Size tracking - old_total={}B, new_total={}B, delta={}B, count={}",
            old_total_size,
            new_total_size,
            size_delta,
            new_count
    ...
    async fn size_bytes(&self) -> usize {
        ...
        tracing::debug!(
            "📊 BTREE_SIZE: Current memtable size: {} bytes ({} MB)",
            size,
            size / 1024 / 1024
        );
        size
    ...
    async fn clear_up_to(&self, threshold: K) -> Result<usize> {
        ...
        let keys_to_remove: Vec<K> = data.range(..=threshold).map(|(k, _)| k.clone()).collect();
    ...
    V: Clone + Send + Sync + Debug + 'static,
{
    ...
    async fn cleanup_versions(&self, logical_key: &str, keep_count: usize) -> Result<usize> {
        ...
        physical_keys.sort();
        let keys_to_remove = physical_keys
            .drain(0..physical_keys.len() - keep_count)
            .collect::<Vec<_>>();
...
mod tests {
    ...
    async fn test_btree_clear_operations() {
        ...
        assert_eq!(
            remaining,
            vec![(4u64, "value4".to_string()), (5u64, "value5".to_string()),]
        );
    }
}
