...
/// Test LsmSearchConfig functionality
#[cfg(test)]
mod config_tests {
    use super::*;
    ...
    fn test_lsm_search_config_default() {
        let config = LsmSearchConfig::default();
...
/// Test LsmUnifiedSearchEngine construction
#[cfg(test)]
mod construction_tests {
    ...
    async fn test_new_with_default_config() {
        ...
        let engine = LsmUnifiedSearchEngine::new(
            sstable_reader,
            distance_compute,
            quantization_engine,
        );
        
        assert_eq!(engine.engine_id(), "LsmUnifiedSearchEngine");
    ...
    async fn test_with_custom_config() {
        ...
        let custom_config = LsmSearchConfig {
            enable_bloom_filters: false,
            enable_block_cache: true,
            enable_mvcc_resolution: false,
            max_sstables: 25,
        ...
        let engine = LsmUnifiedSearchEngine::with_config(
            sstable_reader,
            distance_compute,
            quantization_engine,
            custom_config.clone(),
