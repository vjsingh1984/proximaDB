...
//! LSM Manifest for SSTable Tracking
//! 
//! The manifest provides a centralized record of all SSTable files in the LSM tree,
...
/// LSM Manifest for tracking SSTable files
...
impl LsmManifest {
    ...
    pub async fn load(&self) -> Result<()> {
        let path_str = self.manifest_path.to_string_lossy();
        
        // Detect filesystem scheme from path or use default
        let fs_url = if path_str.contains("://") {
            // Path already has scheme
            path_str.to_string()
        } else {
            // Assume local file
        ...
        match fs.read(&path_str).await {
            Ok(data) => {
                ...
                info!("Loaded LSM manifest version {} with {} files", 
            ...
            Err(e) => {
                debug!("No existing manifest found, starting fresh: {}", e);
                Ok(())
            }
        }
    }
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        if let Some(coordinator) = &self.atomic_coordinator {
            // Use atomic write
            let staging_config = StagingConfig {
                base_url: self.manifest_path.parent()
            ...
            let op = coordinator.begin_atomic_operation(&staging_config).await?;
            let filename = self.manifest_path.file_name()
                .ok_or_else(|| anyhow::anyhow!("Invalid manifest filename"))?
                .to_string_lossy();
            
            coordinator.write_to_staging(&op.operation_id, &filename, &data).await?;
            coordinator.finalize_atomic_operation(&op.operation_id).await?;
            
            debug!("Atomically saved manifest version {}", current.version);
        } else {
            // Direct write
            let path_str = self.manifest_path.to_string_lossy();
            ...
            let fs = self.filesystem.get_filesystem(&fs_url)?;
            // Allow overwrite for manifest updates
            let write_options = crate::storage::persistence::filesystem::FileOptions {
                overwrite: true,
                ..Default::default()
            };
            fs.write(&path_str, &data, Some(write_options)).await?;
            
            debug!("Saved manifest version {} directly", current.version);
        }
    ...
    pub async fn get_files_at_level(&self, level: u8) -> Vec<SstableFileInfo> {
        let current = self.current_version.read().await;
        current.files
            .values()
    ...
    pub async fn get_overlapping_files(
        &self, 
        min_key: &str, 
        max_key: &str
    ) -> Vec<SstableFileInfo> {
    ...
    pub async fn get_files_with_metadata(
        &self,
        column: &str,
    ...
    pub async fn get_compaction_candidates(&self, level: u8, threshold: usize) -> Vec<Vec<SstableFileInfo>> {
        let files = self.get_files_at_level(level).await;
    ...
    pub async fn get_stats(&self) -> ManifestStats {
        let current = self.current_version.read().await;
...
mod tests {
    ...
    async fn create_test_manifest() -> (LsmManifest, TempDir) {
        let temp_dir = TempDir::new().unwrap();
        let filesystem = Arc::new(
            FilesystemFactory::new(Default::default()).await.unwrap()
        );
        
        let manifest = LsmManifest::new(
