...
impl ThreeStageFilterPipeline {
    ...
    pub async fn filter_with_three_stages(
        ...
        info!("🔍 SST 3-Stage Filter: Processing file {} with {} blocks", 
              file_path, data_blocks.len());
        ...
        let qualifying_blocks = self.stage2_index_entry_filtering(
            filter_expr, 
            index_entries, 
            data_blocks, 
            &mut stats
        ...
        let qualifying_indices = self.stage3_row_filtering(
            filter_expr,
            &qualifying_blocks,
            &mut stats
        ).await?;
        
        info!("✅ SST 3-Stage Filter: {} total qualifying indices from {}", 
              qualifying_indices.len(), file_path);
            ...
            FilterExpression::Comparison { field, operator, value } => {
                match operator {
                    ...
                    _ => {
                        ...
                        debug!("🌸 Stage 1: Non-equality operator {:?} - assuming potential matches", operator);
                        true
                    }
                }
            }
    ...
    async fn stage2_index_entry_filtering(
        ...
        info!("📊 Stage 2: Evaluating {} index entries for block pruning", index_entries.len());
        ...
        for entry in index_entries {
            block_index_map.entry(entry.block_id)
        ...
        for block in data_blocks {
            if let Some(block_entries) = block_index_map.get(&block.block_id) {
                if self.block_might_contain_matches(filter_expr, block_entries)? {
                    ...
                    debug!("📊 Stage 2: Block {} skipped - index stats rule out matches", block.block_id);
                    stats.blocks_skipped_by_index += 1;
                }
            } else {
                warn!("📊 Stage 2: No index entries for block {} - including in processing", block.block_id);
                qualifying_blocks.push(block.clone());
            }
        }
        ...
        info!("📊 Stage 2: {} out of {} blocks qualify for row-level filtering", 
              qualifying_blocks.len(), data_blocks.len());
    ...
    fn block_might_contain_matches(
        ...
        match filter_expr {
            FilterExpression::Comparison { field, operator, value } => {
                ...
                let field_stats: Vec<_> = block_entries.iter()
                    .filter_map(|entry| {
                ...
                if field_stats.is_empty() {
                    ...
                    debug!("📊 Index Check: No stats for field '{}' - assuming matches", field);
                    return Ok(true);
                }
                ...
                let might_match = match operator {
                    ComparisonOperator::Equals => {
                        field_stats.iter().any(|(min_val, max_val, _null_count)| {
                            ...
                            let ge_min = crate::core::search::json_comparison::compare_json_values(value, min_val) 
                                != std::cmp::Ordering::Less;
                            let le_max = crate::core::search::json_comparison::compare_json_values(value, max_val) 
                    ...
                    ComparisonOperator::GreaterThan => {
                        field_stats.iter().any(|(min_val, _max_val, _null_count)| {
                            ...
                            crate::core::search::json_comparison::compare_json_values(min_val, value) 
                                == std::cmp::Ordering::Greater
                        })
                    }
                    ComparisonOperator::LessThan => {
                        field_stats.iter().any(|(_min_val, max_val, _null_count)| {
                            // Some value in block must be < filter value
                            crate::core::search::json_comparison::compare_json_values(max_val, value) 
                                == std::cmp::Ordering::Less
                        })
                    }
                    ComparisonOperator::GreaterThanOrEqual => {
                        field_stats.iter().any(|(_min_val, max_val, _null_count)| {
                            // Some value in block must be >= filter value
                            let max_ge_filter = crate::core::search::json_comparison::compare_json_values(max_val, value);
                            max_ge_filter == std::cmp::Ordering::Greater || max_ge_filter == std::cmp::Ordering::Equal
                        })
                    }
                    ComparisonOperator::LessThanOrEqual => {
                        field_stats.iter().any(|(min_val, _max_val, _null_count)| {
                            // Some value in block must be <= filter value  
                            let min_le_filter = crate::core::search::json_comparison::compare_json_values(min_val, value);
                            min_le_filter == std::cmp::Ordering::Less || min_le_filter == std::cmp::Ordering::Equal
                        })
                    }
                    _ => {
    ...
    async fn stage3_row_filtering(
        ...
        info!("🔍 Stage 3: Row-level filtering on {} qualifying blocks", qualifying_blocks.len());
        
        for block in qualifying_blocks {
            // Convert SstRecord to VectorRecord for filtering
            let vector_records: Vec<VectorRecord> = block.records.iter()
            ...
            let block_indices = match filter_expr {
                FilterExpression::And(_) | FilterExpression::Or(_) => {
                    self.batch_filter.as_ref().evaluate_parallel_filters_immutable(&vector_records, filter_expr).await?
                }
                _ => {
                    self.row_filter.as_ref().filter_vector_records_immutable(&vector_records, filter_expr)?
                }
            };
            ...
            debug!("🔍 Stage 3: Block {} contributed {} qualifying records", 
                   block.block_id, all_qualifying_indices.len());
        }
        
        stats.stage3_duration_us = stage_start.elapsed().as_micros() as u64;
        stats.final_qualifying_count = all_qualifying_indices.len();
        
        info!("🔍 Stage 3: Found {} total qualifying records", all_qualifying_indices.len());
        ...
        vec![
            IndexEntry {
                ...
                metadata_min_values: HashMap::from([
                    ("category".to_string(), serde_json::json!("books")),
                ]),
