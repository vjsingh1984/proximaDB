...
mod tests {
    ...
    use crate::storage::engines::sst::readers::unified_sstable_reader::{
        ModularBlockReader, SstDirectReader, BlockIterator, ReadMode, BlockReader,
    };
    ...
    async fn test_hierarchical_sst_write_read() {
        ...
        for i in 0..10 {  // Reduced to 10 for simpler debugging
        ...
        writer.write_sorted_vector_records(sorted_records_iter, record_count).await.expect("Should write");
        ...
        let block = reader.read_data_block(0, ReadMode::Buffered).await.expect("Should read block");
        ...
        for i in 0..200 {
            let record = SstRecord {
                ...
                metadata: vec![
                    ...
                    MetadataItem {
                        key: "type".to_string(),
                        value: Some(metadata_item::Value::StringValue(if i < 100 { "A" } else { "B" }.to_string())),
                    },
                ],
