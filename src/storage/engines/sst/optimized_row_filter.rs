...
impl SSTRowFilterEvaluator {
    ...
    pub async fn filter_blocks_parallel(
        ...
        info!("SST Row Filter: {} blocks have qualifying records, {} total matches", 
              block_results.len(), total_qualifying);
    ...
    fn get_or_convert_metadata(
        ...
        if !record_id.is_empty() && !record_id.starts_with("idx_") {
            self.metadata_cache.insert(record_id.to_string(), metadata_map.clone());
        }
    ...
    fn get_or_convert_vector_metadata(
        ...
        if !cache_key.starts_with("idx_") && cache_key.len() < 100 {
            self.metadata_cache.insert(cache_key.to_string(), metadata_map.clone());
        }
    ...
    pub fn cache_stats(&self) -> (usize, usize) {
        ...
        let memory_bytes = self.metadata_cache.iter()
    ...
    pub fn filter_vector_records_immutable(
        ...
        debug!("SST Row Filter (immutable): Evaluating {} VectorRecords", records.len());
        ...
        debug!("SST Row Filter (immutable): {} out of {} VectorRecords qualify", 
               qualifying_indices.len(), records.len());
...
impl SSTBatchFilterEvaluator {
    ...
    pub async fn evaluate_parallel_filters(
        ...
        match filter_expr {
            FilterExpression::And(sub_exprs) => {
                info!("SST Parallel Filter: Evaluating {} AND conditions", sub_exprs.len());
                
                // Evaluate each condition separately (can be parallelized)
                let mut condition_results = Vec::new();
                for expr in sub_exprs {
                    let indices = self.base_evaluator.filter_vector_records_fast(records, expr)?;
                    condition_results.push(indices);
                }
                
                // Intersection of all results
                let intersection = self.intersect_indices(condition_results);
                info!("SST Parallel Filter: AND result has {} qualifying indices", intersection.len());
                Ok(intersection)
            }
            
            FilterExpression::Or(sub_exprs) => {
                info!("SST Parallel Filter: Evaluating {} OR conditions", sub_exprs.len());
                ...
                info!("SST Parallel Filter: OR result has {} qualifying indices", union.len());
                Ok(union)
            }
            
            FilterExpression::Not(sub_expr) => {
                info!("SST Parallel Filter: Evaluating NOT condition");
                let positive_indices = self.base_evaluator.filter_vector_records_fast(records, sub_expr)?;
                let all_indices: std::collections::HashSet<usize> = (0..records.len()).collect();
                let positive_set: std::collections::HashSet<usize> = positive_indices.into_iter().collect();
            ...
            _ => {
                ...
                self.base_evaluator.filter_vector_records_fast(records, filter_expr)
            }
        }
    }
    ...
    pub async fn evaluate_parallel_filters_immutable(
        ...
        match filter_expr {
            FilterExpression::And(sub_exprs) => {
                debug!("SST Parallel Filter (immutable): Evaluating {} AND conditions", sub_exprs.len());
                
                let mut condition_results = Vec::new();
                for expr in sub_exprs {
                    let indices = self.base_evaluator.filter_vector_records_immutable(records, expr)?;
                    condition_results.push(indices);
                }
            ...
            FilterExpression::Or(sub_exprs) => {
                debug!("SST Parallel Filter (immutable): Evaluating {} OR conditions", sub_exprs.len());
            ...
            _ => {
                // Single condition - use base evaluator
                self.base_evaluator.filter_vector_records_immutable(records, filter_expr)
            }
        }
    }
}
        ...
        for i in 0..count {
            ...
            metadata.push(crate::proto::proximadb::MetadataItem {
                ...
                value: Some(crate::proto::proximadb::metadata_item::Value::StringValue(
                    if i % 3 == 0 { "electronics".to_string() } else { "books".to_string() }
                )),
            });
            ...
            metadata.push(crate::proto::proximadb::MetadataItem {
                key: "brand".to_string(),
                value: Some(crate::proto::proximadb::metadata_item::Value::StringValue(
                    if i % 7 == 0 { "Apple".to_string() } else { "Samsung".to_string() }
                )),
            });
