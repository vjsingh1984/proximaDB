...
use super::{
    MigrationConfig, MigrationStatus, MigrationEvent, MigrationEventType,
    ValidationConfig, PerformanceConfig, MigrationStrategy,
};
...
impl EngineMigrator {
    ...
    pub async fn create_plan(&self) -> Result<MigrationPlan> {
        info!("Creating migration plan from {:?} to {:?}", 
    ...
    async fn execute_copy_then_switch(
        ...
        for collection_plan in collections {
            self.log_event(MigrationEventType::CollectionStarted, 
                          Some(collection_plan.collection_id.clone()),
                          "Starting collection migration".to_string()).await;
            
            match self.migrate_collection_copy_then_switch(collection_plan).await {
                Ok(records) => {
                    migrated_collections.push(collection_plan.collection_id.clone());
                    total_records += records;
                    
                    self.log_event(MigrationEventType::CollectionCompleted,
                                  Some(collection_plan.collection_id.clone()),
                                  format!("Migrated {} records", records)).await;
                }
                Err(e) => {
                    errors.push(format!("Collection {}: {}", collection_plan.collection_id, e));
                    
                    self.log_event(MigrationEventType::Error,
                                  Some(collection_plan.collection_id.clone()),
                                  e.to_string()).await;
                }
            }
    ...
    fn assess_migration_risks(&self, collections: &[CollectionMigrationPlan]) -> RiskAssessment {
        ...
        let total_size_gb: f64 = collections.iter()
...
impl MigrationProgress {
    ...
    pub fn update_progress(&mut self, migrated_records: u64) {
        ...
        if self.throughput_records_per_second > 0.0 && self.total_records > self.migrated_records {
            ...
            self.estimated_completion = Some(chrono::Utc::now() + 
...
mod tests {
    ...
    fn test_resource_requirements_calculation() {
        ...
        let migrator_result = tokio::runtime::Runtime::new().unwrap().block_on(async {
            EngineMigrator::new(config).await
        });
