...
use crate::compute::distance_computation::{
    ...
    QuantizedDistanceResult, SelectedFormat,
};

use super::{
    quantized_calculator::UniversalQuantizedCalculator,
    hardware_manager::OptimizationStrategy,
    adapter::{CandidateVector, AdapterError, AdapterResult},
};
...
pub enum RefinementStrategy {
    ...
    EarlyTermination { target_count: usize, quality_threshold: f32 },
    /// Dynamic stage selection based on dataset characteristics
    Dynamic { dataset_size: usize, target_recall: f32 },
}
...
impl ProgressiveRefinementPipeline {
    ...
    pub async fn execute_progressive_search(
        ...
        trace!("Starting progressive refinement for {} candidates", candidates.len());
        ...
        for &stage in &self.stages {
            ...
            if current_candidates.len() <= target_count {
                debug!("Skipping stage {:?} - already have {} candidates (target: {})", 
                       stage, current_candidates.len(), target_count);
                continue;
            }
            
            trace!("Executing refinement stage {:?} with {} candidates", stage, current_candidates.len());
            ...
            debug!("Stage {:?} completed in {}μs, {} candidates remaining", 
        ...
        let final_stage = stages_used.last().copied().unwrap_or(RefinementStage::Binary);
        ...
        let vector_ids: Vec<Uuid> = current_candidates.iter()
            .take(max_results)
            .map(|c| c.id)
            .collect();
        ...
        let quality_metrics = QualityMetrics {
            ...
            average_confidence: similarity_results.iter()
                .map(|r| r.normalized_score)
                .sum::<f32>() / similarity_results.len().max(1) as f32,
        ...
        debug!("Progressive refinement completed in {}μs with {} final results", 
               total_time, similarity_results.len());
    ...
    async fn execute_stage(
        ...
        match stage {
            RefinementStage::Binary => {
                self.execute_binary_stage(query_vector, candidates, distance_metric, target_count).await
            },
            RefinementStage::INT8 => {
                self.execute_int8_stage(query_vector, candidates, distance_metric, target_count).await
            },
            RefinementStage::PQ => {
                self.execute_pq_stage(query_vector, candidates, distance_metric, target_count).await
            },
            RefinementStage::FP32 => {
                self.execute_fp32_stage(query_vector, candidates, distance_metric, target_count).await
            },
        }
    }
    ...
    async fn execute_binary_stage(
        ...
        for candidate in candidates {
            ...
            let hamming_distance = self.compute_hamming_distance(&query_binary, &candidate_binary)?;
    ...
    async fn execute_int8_stage(
        ...
        for candidate in candidates {
            ...
            let results = self.quantized_calculator.compute_distances(
                query_vector,
                &[quantized_data],
                distance_metric,
                &SelectedFormat::Int8,
    ...
    fn convert_to_quantized_int8(&self, data: &[u8]) -> AdapterResult<crate::compute::distance_computation::QuantizedVectorData> {
        use crate::compute::distance_computation::{QuantizedVectorData, Int8VectorData};
    ...
    fn convert_to_quantized_pq(&self, data: &[u8], segments: usize, _bits: usize) -> AdapterResult<crate::compute::distance_computation::QuantizedVectorData> {
        use crate::compute::distance_computation::{QuantizedVectorData, PQVectorData};
    ...
    async fn generate_final_similarities(
        ...
        for candidate in candidates.iter().take(max_results) {
            ...
            let similarity_result = SimilarityResult {
                raw_value: candidate.quality_score.unwrap_or(0.5),
                metric: *distance_metric,
                normalized_score: candidate.quality_score.unwrap_or(0.5),
                rank_value: candidate.quality_score.unwrap_or(0.5),
            };
    ...
    fn calculate_quality_improvement(&self, stage_qualities: &HashMap<RefinementStage, f32>) -> f32 {
        if stage_qualities.is_empty() {
            return 0.0;
        }
        
        let min_quality = stage_qualities.values().fold(f32::INFINITY, |a, &b| a.min(b));
        let max_quality = stage_qualities.values().fold(f32::NEG_INFINITY, |a, &b| a.max(b));
