...
impl HardwareAccelerationManager {
    ...
    pub async fn new(
        ...
        debug!("Selected optimization strategy: {:?}", current_strategy);
    ...
    pub async fn update_strategy(&mut self, workload_characteristics: &WorkloadCharacteristics) -> AdapterResult<()> {
        let new_strategy = self.select_strategy_for_workload(workload_characteristics)?;
        
        if new_strategy != self.current_strategy {
            debug!("Updating optimization strategy from {:?} to {:?}", self.current_strategy, new_strategy);
        ...
        T: AcceleratedComputation,
    {
        ...
        let result = match strategy {
            ...
            OptimizationStrategy::Parallel => {
                ...
                computation.execute_parallel(self.capabilities.cpu_cores).await
            },
            OptimizationStrategy::Hybrid => {
                self.stats.simd_operations += 1;
                self.stats.parallel_operations += 1;
                computation.execute_hybrid(&self.simd_capabilities, self.capabilities.cpu_cores).await
            },
            _ => {
                computation.execute_scalar().await
            },
        }.map_err(|e| AdapterError::HardwareAcceleration(format!("Accelerated computation failed: {}", e)))?;
        
        // Update performance statistics
        let operation_time = start_time.elapsed().as_micros() as u64;
        self.update_performance_stats(operation_time, strategy).await;
    ...
    pub async fn get_statistics(&self) -> HardwarePerformanceStats {
        ...
        if stats.total_operations > 0 {
            stats.acceleration_usage_rate = 
                (stats.simd_operations + stats.parallel_operations) as f32 / stats.total_operations as f32;
        }
    ...
    fn detect_simd_capabilities(capabilities: &HardwareCapabilities) -> AdapterResult<SIMDCapabilities> {
        Ok(SIMDCapabilities {
            ...
            sse2_supported: capabilities.sse2_supported,
            avx_supported: capabilities.avx_supported,
            avx2_supported: capabilities.avx2_supported,
            avx512_supported: capabilities.avx512_supported,
            neon_supported: capabilities.neon_supported,
            popcnt_supported: capabilities.popcnt_supported,
            bmi_supported: false, // Would need to detect BMI support
            vector_register_width: if capabilities.avx512_supported {
                512
            } else if capabilities.avx2_supported {
                256
            } else if capabilities.sse2_supported || capabilities.neon_supported {
                128
            } else {
                64
            },
            max_f32_lanes: if capabilities.avx512_supported {
                16
            } else if capabilities.avx2_supported {
                8
            } else if capabilities.sse2_supported || capabilities.neon_supported {
            ...
            max_i32_lanes: if capabilities.avx512_supported {
                16
            } else if capabilities.avx2_supported {
                8
            } else if capabilities.sse2_supported || capabilities.neon_supported {
            ...
            max_i8_lanes: if capabilities.avx512_supported {
                64
            } else if capabilities.avx2_supported {
                32
            } else if capabilities.sse2_supported || capabilities.neon_supported {
    ...
    fn select_strategy_for_workload(&self, workload: &WorkloadCharacteristics) -> AdapterResult<OptimizationStrategy> {
        match (workload.vector_size, workload.batch_size, workload.operation_type) {
    ...
    fn select_computation_strategy(&self, vector_size: usize, batch_size: usize) -> AdapterResult<OptimizationStrategy> {
    ...
    async fn update_performance_stats(&mut self, operation_time_us: u64, strategy: OptimizationStrategy) {
        self.stats.total_operations += 1;
        
        // Update average operation time (exponential moving average)
        let alpha = 0.1; // Smoothing factor
        self.stats.average_operation_time_us = 
            (alpha * operation_time_us as f32 + (1.0 - alpha) * self.stats.average_operation_time_us as f32) as u64;
        
        // Calculate speedup ratios (simplified)
        match strategy {
            OptimizationStrategy::SIMD => {
                self.stats.simd_speedup_ratio = 
                    (self.stats.simd_speedup_ratio * 0.9) + (2.5 * 0.1); // Assume ~2.5x speedup
            },
            OptimizationStrategy::Parallel => {
                let core_speedup = (self.capabilities.cpu_cores as f32 * 0.8).min(8.0); // 80% efficiency cap
...
pub trait AcceleratedComputation {
    ...
    async fn execute_simd(&self, capabilities: &SIMDCapabilities) -> Result<Self::Output, Self::Error>;
    ...
    async fn execute_hybrid(&self, capabilities: &SIMDCapabilities, num_cores: usize) -> Result<Self::Output, Self::Error>;
}