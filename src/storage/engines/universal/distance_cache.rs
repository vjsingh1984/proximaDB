...
impl DistanceTableCache {
    ...
    fn create_cache_key(&self, key: &DistanceTableKey) -> String {
        format!(
            "dist_{}_{}_{}_{:?}",
            key.query_hash, key.segments, key.bits, key.metric
        )
    }
    ...
    pub async fn warm_cache_batch<F>(
        &self,
        keys: Vec<DistanceTableKey>,
        compute_fn: F,
    ) -> Result<()>
    where
...
mod tests {
    ...
    async fn test_distance_cache_creation() {
        ...
        let cache_orchestrator =
            Arc::new(crate::storage::cache::orchestrator::CrossCacheOrchestrator::new(1024 * 1024));

        let cache = DistanceTableCache::new(&config, cache_orchestrator)
            .await
            .unwrap();
    ...
    async fn test_distance_cache_get_or_compute() {
        ...
        let compute_fn =
            || -> Result<Vec<Vec<f32>>> { Ok(vec![vec![1.0, 2.0, 3.0], vec![4.0, 5.0, 6.0]]) };
    ...
    async fn test_related_key_generation() {
        ...
        assert!(
            !related_keys
                .iter()
                .any(|k| k.bits == base_key.bits && k.segments == base_key.segments)
        );
    }
