...
mod tests {
    ...
    async fn test_universal_adapter_creation() {
        ...
        assert!(adapter.is_ok(), "Failed to create universal adapter: {:?}", adapter.err());
    }
    ...
    async fn test_progressive_refinement_pipeline() {
        ...
        assert!(result.is_ok(), "Progressive distance computation failed: {:?}", result.err());
    }
    ...
    async fn test_storage_engine_adapters() {
        ...
        assert!(matches!(optimal_format, StorageFormat::QuantizedINT8 { .. }));
    }
    ...
    async fn test_hardware_acceleration_manager() {
        ...
        assert!(matches!(strategy, OptimizationStrategy::SIMD | OptimizationStrategy::Scalar));
    }
    ...
    fn test_storage_format_properties() {
        ...
        let int8_format = StorageFormat::QuantizedINT8 { scale: 1.0, zero_point: 0 };
        assert_eq!(int8_format.data_size_per_vector(128), 128);
        assert!(int8_format.supports_hardware_acceleration());
        
        let pq_format = StorageFormat::QuantizedPQ { segments: 8, bits: 8 };
    ...
    async fn test_engine_adapter_vector_conversion() {
        ...
        let fp32_result = adapter.convert_vectors(&test_vectors, &StorageFormat::FP32).await;
    ...
    async fn test_memory_usage_estimation() {
        ...
        assert!(memory_usage >= expected_min && memory_usage <= expected_max,
                "Memory usage {} not in expected range [{}, {}]", memory_usage, expected_min, expected_max);
    }
}

// Re-export commonly used types for tests
pub use super::{
    adapter::*, config::*, conversion::*, hardware_manager::*, 
    progressive_refinement::*, quantized_calculator::*, storage_integration::*
};
