...
mod benchmarks {
    ...
    fn bench_pca_training(b: &mut Bencher) {
        ...
        b.iter(|| {
            let model = clustering::PCAModel::train(&vectors, 16)
                .expect("PCA model training should succeed with valid input");
            black_box(model);
        });
    }
    ...
    fn bench_pca_projection(b: &mut Bencher) {
        ...
        b.iter(|| {
            let projected = model
                .project(&test_vector)
                .expect("PCA projection should succeed with compatible dimensions");
            black_box(projected);
        });
    }
    ...
    async fn bench_flush_performance() {
        ...
        .await
        .expect("HelixEngine creation should succeed with valid config");
        ...
        for size in [100, 500, 1000, 5000] {
            ...
            let result = engine
                .do_flush(&params)
                .await
                .expect("Flush operation should succeed with valid parameters");
    ...
    async fn bench_search_with_pruning() {
        ...
        let results = engine
            ...
            .await
            .expect("Search operation should succeed with valid query context");
        let elapsed = start.elapsed();
    ...
    async fn bench_compaction_throughput() {
        ...
        let result = engine
            ...
            .await
            .expect("Compaction operation should succeed with valid parameters");
        let elapsed = start.elapsed();
    ...
    fn bench_hilbert_ordering_quality() {
        ...
        // Train PCA model
        let model = clustering::PCAModel::train(&vectors, 8)
            .expect("PCA model training should succeed with valid input");
        ...
        let mut keys_and_ids: Vec<(u64, String)> = vectors
            ...
            .map(|v| {
                let key = model
                    .project_and_compute_hilbert(&v.vector)
                    .expect("Hilbert key computation should succeed with compatible dimensions");
                (key, v.id.clone())
            })
            .collect();
        ...
        for window in keys_and_ids.windows(2) {
            ...
            let cluster1 = id1
                .split('_')
                .nth(1)
                .expect("Vector ID should have cluster_id component");
            let cluster2 = id2
                .split('_')
                .nth(1)
                .expect("Vector ID should have cluster_id component");
    ...
    async fn bench_liquid_clustering_effectiveness() {
        ...
        let all_ids: Vec<String> = hot_vectors
            .iter()
        ...
        let hot_avg_score: f32 = hot_vectors
            .iter()
            .map(|id| hints.get(*id).copied().unwrap_or(0.0))
            .sum::<f32>()
            / hot_vectors.len() as f32;

        let cold_avg_score: f32 = cold_vectors
            .iter()
            .map(|id| hints.get(*id).copied().unwrap_or(0.0))
            .sum::<f32>()
            / cold_vectors.len() as f32;
        ...
        assert!(
            hot_avg_score > cold_avg_score * 10.0,
            "Hot vectors should have much higher scores"
        );
    }
...
mod criterion_benchmarks {
    ...
    fn pca_benchmark(c: &mut Criterion) {
        ...
        for dims in [128, 256, 512, 768, 1024] {
            ...
            group.bench_with_input(BenchmarkId::new("train", dims), &vectors, |b, vectors| {
                b.iter(|| {
                    clustering::PCAModel::train(vectors, 16)
                        .expect("PCA model training should succeed with valid input")
                });
            });
        }
    ...
    fn hilbert_benchmark(c: &mut Criterion) {
        ...
        for dims in [2, 3, 8, 16, 32] {
            ...
            group.bench_with_input(
                ...
                &vector,
                |b, vector| {
                    b.iter(|| clustering::compute_hilbert_key(vector));
                },
            );
        }
