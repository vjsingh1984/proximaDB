...
impl ColumnarIdIndex {
    ...
    pub fn get_row_groups_for_range(&self, start: &str, end: &str) -> Vec<usize> {
    ...
    pub fn stats(&self) -> IndexStats {
        IndexStats {
            ...
            total_pages: self.row_group_index.iter()
                .map(|rg| rg.page_indexes.len())
                .sum(),
            bloom_filter_size: self.bloom_filters.iter()
                .map(|bf| bf.bits.len() * 8)
                .sum(),
            file_path: self.file_path.clone(),
        }
    }
    ...
    pub async fn optimize(&mut self) -> Result<()> {
        ...
        self.row_group_index.sort_by(|a, b| a.id_range.0.cmp(&b.id_range.0));
    ...
    pub async fn merge_with(&mut self, other: ColumnarIdIndex) -> Result<()> {
        info!("Merging ID indexes: {} + {}", self.file_path, other.file_path);
