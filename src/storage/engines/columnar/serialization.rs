...
use arrow_array::{
    Array, ArrayRef, BinaryArray, FixedSizeBinaryArray, Float32Array, 
    Int8Array, RecordBatch, UInt8Array,
};
use arrow_array::builder::{
    BinaryBuilder, FixedSizeBinaryBuilder, Float32Builder, 
    Int8Builder, UInt8Builder,
};
...
use crate::compute::distance_computation::{
    SelectedFormat, QuantizedVectorData, Int8VectorData, PQVectorData,
};
use crate::core::hardware_capabilities::get_hardware_capabilities;
use super::{QuantizationConfig, CompressionMetadata};
...
impl MemoryPools {
    ...
    fn get_fp32_vector(&self, size: usize) -> Vec<f32> {
    ...
    fn return_fp32_vector(&self, vec: Vec<f32>) {
        if vec.capacity() <= 4096 { // Don't pool very large vectors
            let mut pool = self.fp32_pool.lock().unwrap();
    ...
    fn return_binary_vector(&self, vec: Vec<u8>) {
        if vec.capacity() <= 2048 { // Binary vectors are smaller
            let mut pool = self.binary_pool.lock().unwrap();
            if pool.len() < 100 {
                pool.push(vec);
            }
...
impl ColumnarSerializer {
    ...
    pub async fn serialize_vectors(
        ...
        info!("Serializing {} vector records with transparent quantization", records.len());
        ...
        let (binary_array, int8_arrays, pq_array, quant_stats) = if let (Some(ref engine), Some(ref quant_config)) = 
            ...
            (None, None, None, QuantizationStats {
    ...
    pub async fn deserialize_vectors(
        ...
        info!("Deserializing vectors with format preference: {:?}", format_preference);
        ...
        let total_time = start_time.elapsed().as_secs_f64() * 1000.0;
        info!("Deserialization completed in {:.2}ms using format: {:?}", total_time, selected_format);
    ...
    fn serialize_fp32_zero_copy(&self, vectors: &[&[f32]], dimension: usize) -> Result<ArrayRef> {
        ...
        let mut buffer = self.memory_pools.get_fp32_vector(total_elements);
        
        // Copy vectors into contiguous buffer with SIMD alignment
        for vector in vectors {
            if vector.len() != dimension {
                return Err(anyhow::anyhow!("Vector dimension mismatch: expected {}, got {}", dimension, vector.len()));
            }
        ...
        for data in quantized_data {
            if let Some(ref filter_quant) = data.filter {
                ...
                if binary_data.len() != binary_size {
                    return Err(anyhow::anyhow!("Binary vector size mismatch: expected {}, got {}", binary_size, binary_data.len()));
                }
        ...
        for data in quantized_data {
            if let Some(ref fast_quant) = data.fast {
                ...
                if int8_data.len() != self.config.dimension {
                    return Err(anyhow::anyhow!("INT8 vector size mismatch: expected {}, got {}", self.config.dimension, int8_data.len()));
                }
        ...
        for data in quantized_data {
            if let Some(ref primary_quant) = data.primary {
                ...
                if pq_data.len() != pq_size {
                    return Err(anyhow::anyhow!("PQ vector size mismatch: expected {}, got {}", pq_size, pq_data.len()));
                }
    ...
    fn deserialize_fp32_vectors(&self, array: &ArrayRef) -> Result<Vec<Vec<f32>>> {
        match array.data_type() {
            DataType::FixedSizeBinary(size) => {
                let fixed_array = array.as_any().downcast_ref::<FixedSizeBinaryArray>()
            ...
            _ => Err(anyhow::anyhow!("Unsupported array type for FP32 vectors: {:?}", array.data_type())),
        }
    }
    ...
    async fn deserialize_binary_vectors(&self, array: &ArrayRef) -> Result<Vec<Vec<f32>>> {
        let binary_array = array.as_any().downcast_ref::<FixedSizeBinaryArray>()
            .ok_or_else(|| anyhow::anyhow!("Failed to downcast to FixedSizeBinaryArray for binary vectors"))?;
        ...
        warn!("Binary vector deserialization to FP32 not fully implemented - returning zero vectors");
    ...
    fn deserialize_int8_vectors(
        ...
        let int8_array = vector_array.as_any().downcast_ref::<FixedSizeBinaryArray>()
            .ok_or_else(|| anyhow::anyhow!("Failed to downcast INT8 vector array"))?;
        
        let scale_array = scale_array.as_any().downcast_ref::<Float32Array>()
            .ok_or_else(|| anyhow::anyhow!("Failed to downcast scale array"))?;
        
        let zero_point_array = zero_point_array.as_any().downcast_ref::<Int8Array>()
        ...
        for i in 0..int8_array.len() {
            if !int8_array.is_null(i) && !scale_array.is_null(i) && !zero_point_array.is_null(i) {
                ...
                return Err(anyhow::anyhow!("Missing data for INT8 vector at index {}", i));
            }
        }
    ...
    async fn deserialize_pq_vectors(&self, array: &ArrayRef) -> Result<Vec<Vec<f32>>> {
        let pq_array = array.as_any().downcast_ref::<FixedSizeBinaryArray>()
            .ok_or_else(|| anyhow::anyhow!("Failed to downcast to FixedSizeBinaryArray for PQ vectors"))?;
...
mod tests {
    ...
    fn test_format_selection() {
        ...
        arrays.insert("vector".to_string(), Arc::new(Float32Array::from(vec![1.0, 2.0, 3.0])) as ArrayRef);
        arrays.insert("vector_binary".to_string(), Arc::new(BinaryArray::from_opt_vec(vec![Some(b"test")])) as ArrayRef);
        
        // Test highest quality preference
        let format = serializer.select_optimal_format(&arrays, FormatPreference::HighestQuality).unwrap();
        assert!(matches!(format, SelectedFormat::FP32));
        
        // Test fastest read preference
        let format = serializer.select_optimal_format(&arrays, FormatPreference::FastestRead).unwrap();
    ...
    fn test_memory_pools() {
        ...
        // Test FP32 pool
        let vec1 = pools.get_fp32_vector(100);
        assert!(vec1.capacity() >= 100);
        
        pools.return_fp32_vector(vec1);
        
        let vec2 = pools.get_fp32_vector(50);
