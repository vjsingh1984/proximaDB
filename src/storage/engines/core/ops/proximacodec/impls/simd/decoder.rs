...
impl RawDecoder for SimdDecoder {
    ...
    fn decode_f32(&self, data: &[u8], scheme: &ProximaScheme, count: usize) -> Result<Vec<f32>> {
        match scheme {
            ProximaScheme::Delta { base } => {
                trace!("🚀 [SIMD] Decoding {} bytes with Delta (base={}, count={})", data.len(), base, count);
    ...
    fn decode_i64(&self, data: &[u8], scheme: &ProximaScheme, count: usize) -> Result<Vec<i64>> {
        match scheme {
            ProximaScheme::Delta { base } => {
                trace!("🚀 [SIMD] Decoding {} bytes as i64 with Delta (base={}, count={})", data.len(), base, count);
    ...
    fn decode_i32(&self, data: &[u8], scheme: &ProximaScheme, count: usize) -> Result<Vec<i32>> {
        match scheme {
            ProximaScheme::Delta { base } => {
                trace!("🚀 [SIMD] Decoding {} bytes as i32 with Delta (base={}, count={})", data.len(), base, count);
