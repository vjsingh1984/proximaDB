...
pub fn encode_f32(values: &[f32], bits: u8) -> Result<Vec<u8>> {
    ...
    // Convert to i32 and apply zigzag encoding
    let zigzag_values: Vec<u32> = values
        .iter()
        .map(|&v| {
            let v_i32 = v.to_bits() as i32;
            zigzag_encode_i32(v_i32)
        })
        .collect();
...
pub fn decode_f32(data: &[u8], count: usize) -> Result<Vec<f32>> {
    ...
    // Decode zigzag and convert back to f32
    let result = zigzag_values
        .iter()
        .map(|&zz| {
            let v_i32 = zigzag_decode_u32(zz);
            f32::from_bits(v_i32 as u32)
        })
        .collect();
