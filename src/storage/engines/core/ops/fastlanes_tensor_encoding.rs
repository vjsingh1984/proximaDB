...
use super::fastlanes_encoding::{FastLanesEncoder, FastLanesDecoder, FastLanesScheme};
...
pub fn decode_sparse_tensor(
    ...
    if let Some(expected) = expected_dimension {
        if dimension != expected {
            return Err(anyhow::anyhow!("Dimension mismatch: expected {}, got {}", expected, dimension));
        }
    }
...
pub fn encode_quantized_tensor(
    ...
    match quant_type {
        ...
        QuantizationType::ProductQuantization { subvectors, codebook_size } => {
            ...
            for subvec_idx in 0..subvectors {
                ...
                for code_idx in 0..codebook_size {
                    for dim_idx in 0..subvector_dim {
                        ...
                        let value = vectors[vec_idx * dimension + subvec_idx * subvector_dim + dim_idx];
                        codebook.push(value);
            ...
            for vec_idx in 0..num_vectors {
                for subvec_idx in 0..subvectors {
                    ...
                    for code_idx in 0..codebook_size {
                        let mut dist = 0.0f32;
                        for dim_idx in 0..subvector_dim {
                            let vec_val = vectors[vec_idx * dimension + subvec_idx * subvector_dim + dim_idx];
                            let code_val = codebooks[subvec_idx][code_idx * subvector_dim + dim_idx];
...
pub fn decode_quantized_tensor(data: &[u8]) -> Result<(Vec<f32>, usize, usize, QuantizationType)> {
    ...
    match quant_type[0] {
        ...
        1 => {
            ...
            Ok((dense, num_vectors, dimension, QuantizationType::ProductQuantization {
...
pub fn transpose_to_columnar(vectors: &[f32], num_vectors: usize, dimension: usize) -> Vec<Vec<f32>> {
...
pub fn transpose_to_row_major(columns: &[Vec<f32>], num_vectors: usize, dimension: usize) -> Vec<f32> {
...
pub fn choose_optimal_tensor_encoding(
    ...
    if sparsity > 0.9 {
        ...
        FastLanesScheme::Delta { base: min_val as i64 }
...
mod tests {
    ...
    fn test_sparse_tensor_roundtrip() {
        ...
        // Test COO format
        let encoded = encode_sparse_tensor(&dense, num_vectors, dimension, SparseFormat::COO, 0.01).unwrap();
        ...
        // Test CSR format
        let encoded = encode_sparse_tensor(&dense, num_vectors, dimension, SparseFormat::CSR, 0.01).unwrap();
        let (decoded, n, d) = decode_sparse_tensor(&encoded, Some(dimension)).unwrap();
    ...
    fn test_quantized_tensor_roundtrip() {
        ...
        // Test INT8
        let encoded = encode_quantized_tensor(&vectors, num_vectors, dimension, QuantizationType::INT8).unwrap();
        ...
        // Test Binary
        let encoded = encode_quantized_tensor(&vectors, num_vectors, dimension, QuantizationType::Binary).unwrap();
        let (decoded, n, d, qt) = decode_quantized_tensor(&encoded).unwrap();
