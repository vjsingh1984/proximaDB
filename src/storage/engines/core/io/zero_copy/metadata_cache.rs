...
impl MmappedMetadata {
    ...
    pub fn can_skip_file(&self, query_context: &QueryContext) -> Result<bool, ProximaDBError> {
        ...
        Ok(self.serializer.can_skip_file(metadata.as_ref().as_ref(), query_context))
    }

    /// Get required data ranges for selective reading
    pub fn get_required_ranges(&self, query_context: &QueryContext) -> Result<Option<Vec<DataRange>>, ProximaDBError> {
        let metadata = self.get_metadata()?;
        Ok(self.serializer.get_required_ranges(metadata.as_ref().as_ref(), query_context))
    }
...
impl CacheStatistics {
    ...
    pub fn avg_serialization_time_ms(&self) -> f64 {
        if self.hits + self.misses == 0 { 0.0 } else {
...
impl ZeroCopyMetadataCache {
    ...
    pub fn register_serializer(&self, serializer: Arc<dyn MetadataSerializer>) {
        ...
        debug!(engine_id = serializer.engine_id(), "Registered metadata serializer");
    }
    ...
    pub async fn get_metadata(
        ...
        if let Ok(mmap_metadata) = self.load_from_disk(&cache_file_path, Arc::clone(&serializer)).await {
        ...
        let metadata = self.create_cache_entry(
            file_path,
            collection_id,
            engine_type,
            &cache_key,
    ...
    pub async fn can_skip_file(
        ...
        let metadata = self.get_metadata(file_path, collection_id, engine_type).await?;
    ...
    fn create_cache_key(&self, file_path: &str, collection_id: &str, engine_type: &str) -> String {
        format!("{}:{}:{}", collection_id, engine_type, self.hash_string(file_path))
    }
    ...
    async fn enforce_cache_limits(&self) {
        ...
        if removed > 0 {
            ...
            info!(
                ...
                current_memory,
                current_entries,
                "Evicted cache entries due to limits"
            );
        }
    }
}
