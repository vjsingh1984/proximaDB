...
pub async fn viper_optimization_example() -> Result<()> {
    ...
    println!("  - ID-less storage: {}", recommendations.use_id_less_storage);
    println!("  - Progressive search: {}", recommendations.enable_progressive_search);
    ...
    // Read data with optimized reader
    {
        ...
        let bloom_filters = reader.load_bloom_filters(file_path.to_str().unwrap()).await?;
        println!("  - Loaded bloom filters: {} bytes", bloom_filters.total_size_bytes);
        
        // Test progressive search
        let query_vector: Vec<f32> = (0..768).map(|i| (i as f32) * 0.001).collect();
        
        let search_results = reader.progressive_search(
        ...
        println!("  - Progressive search found {} results", search_results.len());
        
        // Test optimized batch lookup
        let test_ids = if recommendations.use_id_less_storage {
            // Generate implicit IDs
            (0..5).map(|i| IdLessLookup::generate_implicit_id(0, i)).collect()
        } else {
        ...
        let lookup_results = if recommendations.use_id_less_storage {
            ...
            reader.optimized_batch_id_lookup(
                &[file_path.to_str().unwrap().to_string()],
                &test_ids,
            ).await?
        };
...
pub async fn nova_optimization_example() -> Result<()> {
    ...
    println!("  - Bloom filters: {} (for 50M vectors)", recommendations.use_bloom_filters);
    println!("  - ID-less storage: {} (saves ~400MB for 50M IDs)", recommendations.use_id_less_storage);
    println!("  - Progressive search: {}", recommendations.enable_progressive_search);
    println!("  - Row group size: {} (larger for analytics)", recommendations.row_group_size);
    ...
    // Write optimized for NOVA's analytical patterns
    {
        ...
        for i in 0..25_000 {
            let vector: Vec<f32> = (0..1024).map(|j| {
        ...
        println!("  - File size: {} bytes ({:.1} MB)", stats.file_size, stats.file_size as f64 / 1_000_000.0);
        println!("  - Row groups: {}", stats.total_row_groups);
        println!("  - Compression ratio: {:.2}x", stats.compression_ratio);
        println!("  - Storage savings vs uncompressed: {:.1}%", (1.0 - 1.0/stats.compression_ratio) * 100.0);
    }
    ...
    // NOVA's analytical read patterns
    {
        ...
        let mut iterator = reader.create_streaming_iterator(
        ...
        while let Some(batch) = iterator.next().await? {
            ...
            if row_groups_processed <= 3 {
                println!("  - Processed row group {}: {} vectors", row_groups_processed, batch.num_rows());
            }
        }
        
        println!("  - Total: {} row groups, {} vectors", row_groups_processed, total_vectors);
        ...
        if recommendations.use_bloom_filters {
            let sample_ids: Vec<String> = if recommendations.use_id_less_storage {
                (0..1000).map(|i| IdLessLookup::generate_implicit_id(i / 100, i % 100)).collect()
            } else {
                (0..1000).map(|i| format!("nova_analytics_{:08}", i)).collect()
            };
            
            let (efficiency, fp_rate) = reader.test_bloom_filter_efficiency(
                file_path.to_str().unwrap(),
                &sample_ids,
            ).await?;
...
mod tests {
    ...
    async fn test_performance_comparison() {
        ...
        assert!(result.is_ok(), "Performance comparison should complete successfully");
    }
}