...
pub enum ReadingStrategy {
    ...
    MetadataFiltered { use_reconstruction: bool },
    /// Two-stage quantized search
    QuantizedTwoStage { candidate_count: usize },
...
pub struct UnifiedParquetReader {
    ...
    bandwidth_optimizer: Option<
        Arc<crate::storage::engines::core::io::zero_copy::bandwidth_optimizer::BandwidthOptimizer>,
    >,
    ...
    id_index: Arc<
        RwLock<Option<crate::storage::engines::core::formats::columnar::id_index::ColumnarIdIndex>>,
    >,
...
impl UnifiedParquetReader {
    ...
    pub async fn with_id_less_mode(
        filesystem: Arc<FilesystemFactory>,
        config: ColumnarConfig,
    ) -> Result<Self> {
    ...
    pub async fn read_metadata_with_fs(
        &self,
        file_path: &str,
        intelligent_fs: &Arc<dyn FileSystem>,
    ) -> Result<Arc<ParquetMetaData>> {
        debug!(
            "Reading Parquet metadata from: {} (with footer cache)",
            file_path
        );
    ...
    pub async fn read_row_groups_with_page_pruning(
        ...
        let should_use_ranges = self
            .should_use_range_reading(file_path, &metadata, row_group_indices)
            .await;

        if should_use_ranges {
            debug!(
                "Using range-based reading for {} row groups",
                row_group_indices.len()
            );
            return self
                .read_row_groups_with_ranges(
                    file_path,
                    &metadata,
                    row_group_indices,
                    column_projection,
        ...
        let pruned_pages = self
            .prune_pages_with_indexes(&metadata, row_group_indices, filter)
            .await?;
        ...
        let mut reader = {
            ...
            match (needs_projection, !row_group_indices.is_empty()) {
                (Some(projection), true) => reader_builder
                    .with_projection(projection)
                    .with_row_groups(row_group_indices.to_vec())
                    .build()?,
                (Some(projection), false) => reader_builder.with_projection(projection).build()?,
                (None, true) => reader_builder
                    .with_row_groups(row_group_indices.to_vec())
                    .build()?,
                (None, false) => reader_builder.build()?,
            }
        };
        ...
        debug!(
            "Read {} batches from {} row groups with page pruning",
            batches.len(),
            row_group_indices.len()
        );
        Ok(batches)
    ...
    pub async fn batch_id_lookup(
        ...
        info!(
            "Batch ID lookup for {} IDs across {} files",
            ids.len(),
            file_paths.len()
        );
        ...
        for file_path in file_paths {
            ...
            for (rg_idx, _row_group) in metadata.row_groups().iter().enumerate() {
                ...
                let batches = self
                    .read_row_groups_projected(file_path, &[rg_idx], Some(&["id".to_string()]))
                    .await?;
                        ...
                        for row_idx in 0..batch.num_rows() {
                            ...
                            if ids.contains(&record_id.to_string()) {
                                ...
                                if let Some(record) = self
                                    .load_record_at_position(file_path, rg_idx, row_idx as u32)
                                    .await?
                                {
                                    results.push(record);
                                }
    ...
    pub async fn search_columnar(
        ...
        info!(
            "Columnar search across {} files for top-{}",
            file_paths.len(),
            top_k
        );
        ...
        for file_path in file_paths {
            ...
            let batches = self
                .read_row_groups_projected(file_path, &relevant_groups, projection.as_deref())
                .await?;
    ...
    pub async fn row_group_statistics(&self, file_path: &str) -> Result<Vec<RowGroupStats>> {
        ...
        for (idx, row_group) in metadata.row_groups().iter().enumerate() {
            stats.push(RowGroupStats {
                ...
                has_quantized_columns: self
                    .has_quantized_columns(&metadata.file_metadata().schema_descr()),
    ...
    pub async fn create_streaming_iterator(
        ...
        self.optimizer
            .create_streaming_iterator(file_path, filter, column_projection)
            .await
    }
    ...
    pub async fn optimized_batch_id_lookup(
        ...
        info!(
            "Optimized batch ID lookup for {} IDs across {} files",
            ids.len(),
            file_paths.len()
        );
        let mut results = Vec::new();
    ...
    async fn id_lookup_with_index(
        ...
        if let Some(ref index) = *index_guard {
            ...
            warn!(
                "ID index not available for {}, falling back to scan",
                file_path
            );
    ...
    async fn ensure_id_index_built(&self, file_path: &str) -> Result<()> {
        ...
        if index_guard.is_none() {
            ...
            let mut index =
                crate::storage::engines::core::formats::columnar::id_index::ColumnarIdIndex::new(
                    file_path.to_string(),
                );
            // Build index from row groups (assuming ID is column 0)
            let metadata = self.read_metadata(file_path).await?;
            index
                .build_from_row_groups(metadata.row_groups(), 0)
                .await?;
    ...
    async fn load_vector_at_location(
        ...
        let batches = self
            .read_row_groups_projected(&location.file_path, &[location.row_group_id], None)
            .await?;
    ...
    async fn sequential_id_lookup(
        ...
        debug!(
            "Performing sequential ID lookup for {} IDs in {}",
            ids.len(),
            file_path
        );
        ...
        let mut iterator = self
            .create_streaming_iterator(file_path, None, None)
            .await?;
                ...
                .and_then(|col| col.as_any().downcast_ref::<StringArray>())
            {
                for row_idx in 0..batch.num_rows() {
                    ...
                    if id_set.contains(&row_id.to_string()) {
                        if let Some(record) =
                            self.extract_vector_record_from_batch(&batch, row_idx)?
                        {
                            results.push(record);
                        }
                    }
    ...
    pub async fn lookup_by_implicit_ids(
        &self,
        implicit_ids: &[String],
    ) -> Result<Vec<VectorRecord>> {
        ...
        for implicit_id in implicit_ids {
            let (row_group, row_index) =
                super::parquet_writer::IdLessLookup::parse_implicit_id(implicit_id)?;
            // For now, assume single file (would need file routing in production)
            location_map
                .entry("default.parquet".to_string())
        ...
        for (file_path, locations) in location_map {
            for (row_group, row_index) in locations {
                if let Some(record) = self
                    .load_record_at_position(&file_path, row_group as usize, row_index)
                    .await?
                {
                    results.push(record);
                }
    ...
    async fn prune_pages_with_indexes(
        ...
        for &rg_idx in row_group_indices {
            ...
            let page_info = self
                .prune_pages_in_row_group(row_group, rg_idx, filter)
                .await?;
        ...
        debug!(
            "Page pruning result: {} pages selected from {} row groups",
            total_pages,
            row_group_indices.len()
        );
        Ok(pruned_pages)
    ...
    async fn prune_pages_in_row_group(
        ...
        debug!(
            "Row group {} page pruning: {:.1}% pages pruned",
            row_group_idx,
            pruning_ratio * 100.0
        );
        Ok(PagePruningInfo {
    ...
    pub async fn test_bloom_filter_efficiency(
        ...
        for id in sample_ids {
            ...
            if bloom_says_present {
                ...
                let actual_results = self
                    .batch_id_lookup(&[file_path.to_string()], &[id.clone()])
                    .await?;
        ...
        info!(
            "Bloom filter efficiency: {:.3}, FP rate: {:.3}",
            efficiency, false_positive_rate
        );
            ...
            if top_k < 100 && total_rows > 100000 {
                ...
                return Ok(ReadingStrategy::QuantizedTwoStage {
                    candidate_count: top_k
                        * self.strategy_selector.quantization_candidate_multiplier,
                });
            }
        }
    ...
    pub async fn search_vectors(
        ...
        debug!(
            ...
            collection_context.file_paths.len(),
            collection_context.filterable_columns
        );
    ...
    pub async fn read_for_similarity_search(
        ...
        for file_path in file_paths {
            ...
            let strategy = self
                .select_reading_strategy(file_path, filter, top_k)
                .await?;

            // Read using selected strategy
            let batches = self
                .read_with_strategy(file_path, &strategy, filter)
                .await?;
    ...
    {
        Box::pin(async move {
            self.read_with_strategy_impl(file_path, strategy, filter)
                .await
        })
    }
    ...
    async fn read_with_strategy_impl(
        ...
        match strategy {
            ReadingStrategy::DirectArrow {
                use_column_projection,
                ..
            } => {
                let projection = if *use_column_projection {
                    self.get_schema_mapping(file_path)
                        .await
                ...
                self.read_row_groups_projected(file_path, &all_row_groups, projection.as_deref())
                    .await
            }
            ReadingStrategy::MetadataFiltered { .. } => {
                ...
                self.read_row_groups_projected(file_path, &selected, None)
                    .await
            }
            ReadingStrategy::QuantizedTwoStage { candidate_count } => {
                // Stage 1: Read quantized columns for fast filtering
                let schema_mapping = self
                    .get_schema_mapping(file_path)
                    .await
            ...
            ReadingStrategy::Hybrid {
                ...
                match self
                    .read_with_strategy(file_path, primary_strategy, filter)
                    .await
                {
                    Ok(results) if !results.is_empty() => Ok(results),
                    _ => {
                        // Fallback to secondary strategy
                        self.read_with_strategy(file_path, fallback_strategy, filter)
                            .await
                    }
                }
            }
    ...
    pub async fn read_single_file_for_similarity_search(
        ...
        debug!(
            "Selected reading strategy: {:?} for top_k={}",
            strategy, top_k
        );
        ...
        for batch in batches {
            ...
            if let (Some(vector_col), Some(id_col)) =
                (batch.column_by_name("vector"), batch.column_by_name("id"))
            {
                let vectors = vector_col
                    .as_any()
            ...
            for result in fetched_data {
                if let Ok((idx, data)) = result {
                    ...
                    if cache_size + batch_size < self.config.max_cache_size_bytes {
                        ...
                        debug!(
                            "Cached row group {}, cache size: {}MB",
                            idx,
