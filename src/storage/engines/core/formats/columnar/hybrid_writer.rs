...
use crate::storage::engines::core::formats::columnar::{
    StreamingParquetWriter, BatchParquetWriter, ParquetWriterConfig,
...
impl HybridParquetWriter {
    ...
    pub async fn new<P: AsRef<Path>>(
        ...
        info!("Creating hybrid Parquet writer: {:?}, mode: {:?}", 
    ...
    pub async fn write(&self, records: Vec<VectorRecord>) -> Result<()> {
        ...
        // Update statistics
        self.stats.total_records.fetch_add(batch_size as u64, Ordering::Relaxed);
    ...
    async fn write_streaming(&self, records: Vec<VectorRecord>) -> Result<()> {
        ...
        self.stats.streaming_writes.fetch_add(records.len() as u64, Ordering::Relaxed);
        Ok(())
    }
    ...
    async fn detect_and_switch_mode(&self) -> Result<()> {
        ...
        if current_mode != recommended_mode {
            info!("Switching mode from {:?} to {:?} based on pattern: {:?}",
    ...
    async fn detect_pattern(&self) -> InsertionPattern {
        ...
        let avg_inter_batch_time = if !inter_batch_times.is_empty() {
            let total_nanos: u128 = inter_batch_times.iter()
                .map(|d| d.as_nanos())
                .sum();
    ...
    async fn start_background_flush_task(&mut self) {
        ...
        let flush_fn = {
            ...
            move || {
                ...
                async move {
                    self_clone.flush_buffer(false).await
                }
            }
        };
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if Instant::now().duration_since(last_flush) > buffer_time_limit {
                    ...
                    if buffer_size > 0 {
                        debug!("Time-based buffer flush triggered ({} records)", buffer_size);
