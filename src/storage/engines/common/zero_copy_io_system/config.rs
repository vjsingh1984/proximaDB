...
impl ZeroCopyIOSystemBuilder {
    ...
    pub fn with_access_prediction(mut self, enabled: bool) -> Self {
        self.config.download_optimizer.access_prediction.enable_learning = enabled;
        self
    }
    ...
    fn register_default_serializers(&mut self, filesystem: Arc<FilesystemFactory>) -> Result<(), ProximaDBError> {
        // Register SST serializer
        let sst_serializer = crate::storage::engines::row_based::sst_metadata_serializer::SstMetadataSerializer::new(filesystem.clone());
        self.serializers.push(Box::new(sst_serializer));
        
        // Register Parquet serializer
        let parquet_serializer = crate::storage::engines::columnar::parquet_metadata_serializer::ParquetMetadataSerializer::new(filesystem.clone());
