...
use super::common::{RaptorFileMetadata, CentroidStats, VectorCentroidCompressionMetadata};
...
impl RaptorMetadataSerializer {
    ...
    async fn extract_metadata(&self, file_path: &str, _collection_id: &str) -> Result<RaptorCachedMetadata> {
        ...
        let footer: super::common::RaptorFooter = bincode::deserialize(&footer_data)
            .map_err(|e| ProximaDBError::Storage(StorageError::Serialization(format!("Failed to deserialize footer: {}", e))))?;
        
        // Extract bloom filter data if present
        let bloom_filter_data = if let Some(bloom_meta) = &footer.file_metadata.bloom_filter_metadata {
            fs.read_range(
                file_path,
                bloom_meta.offset,
                bloom_meta.size
...
impl MetadataSerializer for RaptorMetadataSerializer {
    ...
    fn serialize_metadata(&self, file_path: &str, collection_id: &str) -> Result<Vec<u8>, ProximaDBError> {
        ...
        let metadata = runtime.block_on(self.extract_metadata(file_path, collection_id))
            .map_err(|e| ProximaDBError::Storage(StorageError::SstStorage(format!("Failed to extract metadata: {}", e))))?;
        
        // Serialize using bincode for efficiency
        bincode::serialize(&metadata)
            .map_err(|e| ProximaDBError::Storage(StorageError::Serialization(format!("Failed to serialize metadata: {}", e))))
    }
    
    fn deserialize_metadata(&self, data: &[u8]) -> Result<Box<dyn EngineMetadata>, ProximaDBError> {
        let metadata: RaptorCachedMetadata = bincode::deserialize(data)
            .map_err(|e| ProximaDBError::Storage(StorageError::Serialization(format!("Failed to deserialize metadata: {}", e))))?;
        Ok(Box::new(metadata))
    }
    
    fn can_skip_file(&self, metadata: &dyn EngineMetadata, query_context: &QueryContext) -> bool {
        let raptor_meta = metadata.as_any()
    ...
    fn get_required_ranges(&self, metadata: &dyn EngineMetadata, query_context: &QueryContext) -> Option<Vec<DataRange>> {
        ...
        if query_context.query_vector.is_some() {
            ...
            if !raptor_meta.rowgroup_offsets.is_empty() {
                let ranges: Vec<DataRange> = raptor_meta.rowgroup_offsets.windows(2)
                    .map(|w| DataRange {
                        offset: w[0],
                        length: w[1] - w[0],
