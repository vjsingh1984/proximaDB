...
mod tests {
    ...
    use crate::storage::engines::raptor::common::{
    ...
    fn test_phase1_boundary_detection_basic() {
        ...
        let mut centroid_distances: Vec<(usize, f32)> = centroids
            ...
            .enumerate()
            .map(|(i, c)| {
                let dist = distance_compute.calculate(
                    &query,
                    c,
                    DistanceMetric::Euclidean,
                ).unwrap();
        ...
        let primary: Vec<usize> = centroid_distances.iter()
            .take(3)
            .map(|(i, _)| *i)
            .collect();
    ...
    fn test_phase2_spillover_detection() {
        ...
        let pxk_matrix = builder.build_pxk_matrix(
            &rowgroup_vectors,
            &centroids,
            16,
            0,
        ...
        println!("P×K matrix storage strategy: {:?}", pxk_matrix.storage_strategy);
        ...
        println!("Spillover: {} of {} vectors ({:.1}%)", 
        ...
        if spillover_percentage > 15.0 {
            ...
            assert!(spillover_percentage > 0.0, "Should detect some spillover with noise");
        }
    }
    ...
    fn test_adaptive_pxk_coverage_formula() {
        ...
        for (k, d, min_expected, max_expected) in test_cases {
            ...
            assert!(
                ...
                coverage, min_expected, max_expected, k, d
            );
        }
    }
    ...
    fn test_5_component_boosting() {
        ...
        println!("  d₁(runtime): {:.3} × {:.1} = {:.3}", 
                 components.d1_runtime, alpha[0], components.d1_runtime * alpha[0]);
        println!("  d₂(mean):    {:.3} × {:.1} = {:.3}", 
                 components.d2_stored_mean, alpha[1], components.d2_stored_mean * alpha[1]);
        println!("  d₃(density): {:.3} × {:.1} = {:.3}", 
                 components.d3_stored_density, alpha[2], components.d3_stored_density * alpha[2]);
        println!("  d₄(min):     {:.3} × {:.1} = {:.3}", 
                 components.d4_stored_min, alpha[3], components.d4_stored_min * alpha[3]);
        println!("  d₅(max):     {:.3} × {:.1} = {:.3}", 
                 components.d5_stored_max, alpha[4], components.d5_stored_max * alpha[4]);
        
        // Verify the boosted distance is reasonable
        assert!(boosted_distance > 0.0 && boosted_distance < 1.0,
                "Boosted distance {} should be in valid range", boosted_distance);
