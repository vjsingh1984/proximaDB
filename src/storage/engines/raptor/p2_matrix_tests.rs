#[cfg(test)]
mod tests {
    use super::super::common::P2Matrix;
    ...
    fn test_p2_matrix_upper_triangle_indexing() {
        ...
        for ((i, j), expected_idx) in test_cases {
            ...
            assert_eq!(idx, expected_idx, "Index for ({},{}) should be {}", i, j, expected_idx);
        }
    }
    ...
    async fn test_build_p2_matrix() -> Result<()> {
        ...
        for dist in distances {
            assert!((dist - 1.0).abs() < 0.01, "Orthogonal vectors should have cosine distance ~1.0");
        }
    ...
    async fn test_p2_matrix_fastlanes_encoding() -> Result<()> {
        ...
        println!("FastLanes compression: {} -> {} bytes ({:.2}x)", 
                 quantized.len(), encoded.len(),
    ...
    fn test_p2_matrix_memory_scaling() {
        ...
        for (p, expected_bytes) in test_cases {
            ...
            assert_eq!(upper_triangle_size, expected_bytes,
                      "P={} should require {} bytes", p, expected_bytes);
    ...
    async fn test_p2_matrix_search_integration() -> Result<()> {
        ...
        use super::super::consolidated_reader::{RaptorReader, IntraRowgroupMatrix};
