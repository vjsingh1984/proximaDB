...
/// Filename generator for row-based engines
pub struct FilenameGenerator;

impl FilenameGenerator {
    /// Generate SST filename
    pub fn generate_sst_filename(level: u8) -> String {
        let codec = FilenameCodec::new();
        codec.generate(level as u32, "sst")
    }
    
    /// Generate SWIFT filename
    pub fn generate_swift_filename(level: u8) -> String {
        let codec = FilenameCodec::new();
        codec.generate(level as u32, "swift")
    }
    
    /// Parse level from filename
    pub fn parse_level_from_filename(filename: &str) -> Option<u8> {
        let codec = FilenameCodec::new();
        codec.parse_level(filename).map(|level| level as u8)
    }
    
    /// Generate index filename
    pub fn generate_index_filename(base_filename: &str, index_type: &str) -> String {
        format!("{}.{}.idx", base_filename, index_type)
    }
    
    /// Generate bloom filter filename
    pub fn generate_bloom_filename(base_filename: &str) -> String {
        format!("{}.bloom", base_filename)
    }
}
