...
impl SstMetadataSerializer {
    pub fn new(filesystem: Arc<crate::storage::persistence::filesystem::FilesystemFactory>) -> Self {
        Self { filesystem }
    }
    ...
    async fn extract_metadata(&self, file_path: &str) -> Result<SstMetadata, ProximaDBError> {
        ...
        for (i, &(offset, size)) in block_locations.iter().enumerate() {
            ...
            let (record_count, min_hash, max_hash) = self.parse_block_statistics(&block_header_data)?;
...
impl MetadataSerializer for SstMetadataSerializer {
    ...
    fn can_skip_file(&self, metadata: &dyn EngineMetadata, query_context: &QueryContext) -> bool {
        ...
        if !query_context.id_lookups.is_empty() {
            ...
            let global_bloom_data = &sst_metadata.variable_data[
                sst_metadata.global.bloom_filter_offset as usize..
                (sst_metadata.global.bloom_filter_offset + sst_metadata.global.bloom_filter_size) as usize
            ];
