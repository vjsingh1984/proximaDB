...
use crate::storage::engines::sst::bloom_filter::{SstableBloomFilter, BloomFilterConfig as SstBloomConfig};
...
impl RowBasedIdIndex {
    ...
    pub fn new(index_type: IndexType, config: IndexConfiguration) -> Self {
        let mut index = Self {
        ...
        match index_type {
            IndexType::Dense(dense_config) => {
                index.dense_index = Some(DenseIndex::new(dense_config));
            }
            IndexType::Hierarchical(hierarchical_config) => {
    ...
    pub async fn lookup(&self, key: &str) -> Option<BlockLocation> {
        ...
        match &self.index_type {
            IndexType::BTree => self.btree_index.get(key).cloned(),
            IndexType::HashMap => self.hash_index.get(key).cloned(),
            IndexType::Hybrid => {
    ...
    pub async fn range_lookup(
        ...
        match &self.index_type {
            IndexType::BTree | IndexType::Hybrid => {
                self.btree_index
    ...
    fn initialize_hierarchical_index(&mut self, config: HierarchicalConfig) {
        for level in 0..config.levels {
            ...
            let bloom_filter = if config.bloom_per_level {
                Some(SstableBloomFilter::new_default())
            } else {
                None
            };
...
mod tests {
    ...
    async fn test_btree_index_operations() {
        ...
        let mut index = RowBasedIdIndex::new(IndexType::BTree, config);
    ...
    async fn test_dense_index_operations() {
        ...
        let config = IndexConfiguration {
            index_type: IndexType::Dense(dense_config),
            ..Default::default()
        };
    ...
    async fn test_hybrid_index_operations() {
        let config = IndexConfiguration::default();
        let mut index = RowBasedIdIndex::new(IndexType::Hybrid, config);
