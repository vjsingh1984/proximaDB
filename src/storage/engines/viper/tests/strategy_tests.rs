...
use super::super::test_data_generator::{
    ParquetTestDataGenerator, QuantizationType, TestDataConfig,
};
use super::super::{
    FilterValue, MetadataFilter, QuantizationMethod, QueryAnalysis, ReaderConfig, ReadingStrategy,
    ReadingStrategySelector, UnifiedParquetReader, UnifiedQuery,
};
...
async fn test_metadata_filtered_strategy() -> Result<()> {
    ...
    filters.insert(
        "category".to_string(),
        FilterValue::Equals("technology".to_string()),
    );
...
async fn test_quantized_two_stage_strategy() -> Result<()> {
    ...
    assert!(
        result.strategy_used.contains_hash("QuantizedTwoStage")
            || result.strategy_used.contains_hash("DirectArrow")
    );
    assert!(result.total_candidates > 0);
    assert!(result.processing_time_ms > 0);
...
async fn test_hybrid_strategy() -> Result<()> {
    ...
    filters.insert(
        "category".to_string(),
        FilterValue::In(vec!["technology".to_string(), "science".to_string()]),
    );
...
async fn test_strategy_selector_logic() -> Result<()> {
    ...
    let strategy_quant = selector
        .select_strategy(&query_with_quant, &analysis_quant)
        .await?;
    ...
    let strategy_filtered = selector
        .select_strategy(&query_with_filters, &analysis_filtered)
        .await?;
    ...
    let strategy_large = selector
        .select_strategy(&query_large, &analysis_large)
        .await?;
