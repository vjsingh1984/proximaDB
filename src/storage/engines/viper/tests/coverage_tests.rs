...
use super::super::{
    FileMetadata, FilterValue, MetadataFilter, OptimizationStats, QuantizationMethod,
    QueryAnalysis, ReaderCache, ReaderConfig, ReadingStrategy, ReadingStrategySelector,
    SchemaMapping, SeekRange, Stage2Strategy, UnifiedParquetReader, UnifiedQuery,
    UnifiedReadResult, VectorPosition,
};
...
async fn test_optimization_statistics() -> Result<()> {
    ...
    for query in queries {
        ...
        let total_activity =
            stats.cache_hits + stats.cache_misses + stats.seek_operations + stats.range_requests;
...
async fn test_return_vectors_flag() -> Result<()> {
    ...
    assert_eq!(
        result_with_vectors.vectors.len(),
        result_without_vectors.vectors.len()
    );
