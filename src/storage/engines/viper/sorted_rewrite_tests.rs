...
#[cfg(test)]
mod tests {
    use super::super::ml_clustering::{KMeansConfig, MLClusteringEngine};
    use super::super::pipeline::*;
    use super::super::quantization::{QuantizationConfig, VectorQuantizationEngine};
    ...
    async fn test_sorting_strategy_by_id() {
        ...
        let processor = VectorRecordProcessor {
            ...
            schema_adapter,
            ml_clustering: std::sync::Arc::new(tokio::sync::Mutex::new(MLClusteringEngine::new(
                KMeansConfig::default(),
            ))),
            quantization: std::sync::Arc::new(tokio::sync::Mutex::new(
                VectorQuantizationEngine::new(QuantizationConfig::default()),
            )),
        ...
        println!(
            "✅ ID sorting test passed: {} records correctly sorted",
            records.len()
        );
    }
    ...
    async fn test_sorting_strategy_composite_optimal() {
        ...
        println!(
            "✅ Composite optimal sorting test passed: {} records correctly sorted",
            records.len()
        );
    }
    ...
    async fn test_sorting_strategy_cluster_then_sort() {
        ...
        println!(
            "✅ Cluster-then-sort test passed: {} records processed",
            records.len()
        );
    }
    ...
    async fn test_custom_sorting_strategies() {
        ...
        for (name, comparison_type) in test_cases {
            ...
            let processor = VectorRecordProcessor {
                ...
                schema_adapter,
                stats: std::sync::Arc::new(tokio::sync::RwLock::new(ProcessingStats::default())),
                ml_clustering: std::sync::Arc::new(tokio::sync::Mutex::new(
                    MLClusteringEngine::new(KMeansConfig::default()),
                )),
                quantization: std::sync::Arc::new(tokio::sync::Mutex::new(
                    VectorQuantizationEngine::new(QuantizationConfig::default()),
                )),
            ...
            assert_eq!(
                records.len(),
                50,
                "All records should be preserved for strategy {}",
                name
            );
    ...
    async fn test_postprocessing_column_pruning() {
        ...
        assert!(
            processor.config.enable_postprocessing,
            "Postprocessing should be enabled"
        );
        assert!(
            processor.config.enable_compression,
    ...
    async fn test_sorted_rewrite_compaction() {
        ...
        assert!(
            execution_result.entries_processed > 0,
        ...
        assert!(
            execution_result.compression_improvement >= 0.0,
            "Should have non-negative compression improvement"
        );

        println!(
            "✅ Sorted rewrite compaction test passed: {} entries, {:.1}% compression",
    ...
    async fn test_hybrid_compaction_sequential() {
        ...
        assert!(
            result.is_ok(),
            "Hybrid sequential compaction should succeed"
        );

        let execution_result = result.unwrap();
        assert!(
            execution_result.entries_processed > 0,
            "Should process entries from both strategies"
        );

        println!(
            "✅ Hybrid sequential compaction test passed: {} entries processed",
            execution_result.entries_processed
        );
    }
    ...
    async fn test_hybrid_compaction_conditional() {
        ...
        let secondary_strategy = Box::new(CompactionType::SortedRewrite {
            ...
            reorganization_strategy: ReorganizationStrategy::BySimilarityClusters {
                cluster_count: 3,
            },
        ...
        let compaction_type = CompactionType::HybridCompaction {
            primary_strategy,
            secondary_strategy,
            coordination_mode: CompactionCoordinationMode::Conditional {
        ...
        assert!(
            result.is_ok(),
            "Hybrid conditional compaction should succeed"
        );
    ...
    async fn test_reorganization_strategies() {
        ...
        let strategies = vec![
            ...
            (
                "BySimilarityClusters",
                ReorganizationStrategy::BySimilarityClusters { cluster_count: 5 },
            ),
            (
                "ByTemporalPattern",
                ReorganizationStrategy::ByTemporalPattern {
        ...
        for (name, strategy) in strategies {
            let result =
                CompactionEngine::apply_reorganization_strategy(&test_records, &strategy).await;

            assert!(
                result.is_ok(),
                "Reorganization strategy '{}' should succeed",
                name
            );
            ...
            assert!(
                !batches.is_empty(),
                "Should produce at least one batch for strategy '{}'",
                name
            );
            ...
            assert_eq!(
                total_records,
                test_records.len(),
                "Should preserve all records for strategy '{}'",
                name
            );

            println!(
                "✅ Reorganization strategy '{}' test passed: {} batches",
                name,
                batches.len()
            );
        }
    ...
    async fn test_multi_stage_reorganization() {
        ...
        let result =
            CompactionEngine::apply_reorganization_strategy(&test_records, &multi_stage_strategy)
                .await;
        ...
        assert_eq!(
            total_records,
            test_records.len(),
            "Should preserve all records through multi-stage"
        );

        println!(
            "✅ Multi-stage reorganization test passed: {} → {} batches",
            test_records.len(),
            batches.len()
        );
    }
    ...
    async fn test_compression_calculation() {
        ...
        for target_ratio in target_ratios {
            let result =
                CompactionEngine::calculate_achieved_compression(&test_records, target_ratio).await;

            assert!(result.is_ok(), "Compression calculation should succeed");

            let achieved_compression = result.unwrap();
            assert!(
                achieved_compression >= 0.0 && achieved_compression <= 1.0,
                "Compression ratio should be between 0 and 1"
            );
            assert!(
                achieved_compression <= target_ratio,
                "Achieved compression should not exceed target"
            );

            println!(
                "✅ Compression calculation test: target {:.1}% → achieved {:.1}%",
                target_ratio * 100.0,
                achieved_compression * 100.0
            );
        }
    }
    ...
    async fn test_performance_benefits_verification() {
        ...
        let sorted_result = CompactionEngine::execute_compaction_by_type(&sorted_rewrite_task)
            .await
            .unwrap();
        let merging_result = CompactionEngine::execute_compaction_by_type(&file_merging_task)
            .await
            .unwrap();

        // Verify both compaction types complete successfully
        assert!(
            sorted_result.entries_processed > 0,
            "Sorted rewrite should process entries"
        );
        assert!(
            merging_result.entries_processed > 0,
            "File merging should process entries"
        );

        // Sorted rewrite typically provides better compression, but not always guaranteed
        // Just verify both achieve some compression improvement
        assert!(
            sorted_result.compression_improvement >= 0.0,
            "Sorted rewrite should have non-negative compression"
        );
        assert!(
            merging_result.compression_improvement >= 0.0,
            "File merging should have non-negative compression"
        );

        println!(
            "✅ Performance benefits verification: sorted rewrite {:.1}% vs file merging {:.1}%",
    ...
    async fn test_sorted_rewrite_end_to_end() {
        ...
        for (i, strategy) in sorting_strategies.iter().enumerate() {
            ...
            assert!(
                result.is_ok(),
                "Sorted rewrite should succeed for strategy {:?}",
                strategy
            );

            let execution_result = result.unwrap();
            println!(
                "   ✅ Strategy {} completed: {} entries, {:.1}% compression, {}ms",
                i + 1,
                execution_result.entries_processed,
                execution_result.compression_improvement * 100.0,
    ...
    async fn test_large_dataset_performance() {
        ...
        let execution_result = result.unwrap();
        println!(
            "✅ Large dataset test passed: {} records in {}ms (total wall clock: {}ms)",
            execution_result.entries_processed,
            execution_result.execution_time_ms,
            duration.as_millis()
        );
    ...
    async fn test_error_handling_and_recovery() {
        ...
        // Should handle gracefully by returning an error
        assert!(result.is_err(), "Should return error for invalid cluster count");
        println!("✅ Error handling test completed: properly rejected invalid cluster count");
    }
}
