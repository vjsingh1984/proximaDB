...
impl VIPERColumnFilterEvaluator {
    ...
    pub async fn evaluate_predicate_pushdown(
        ...
        info!("🎯 VIPER Predicate Pushdown: Evaluating filter on {} rows", total_rows);
        ...
        self.load_metadata_columns_only(parquet_file, &required_columns).await?;
        ...
        let qualifying_indices = match filter_expr {
            FilterExpression::And(_) | FilterExpression::Or(_) => {
                self.evaluate_parallel_column_filters(filter_expr, total_rows).await?
            }
            _ => {
        ...
        info!("🎯 VIPER Predicate Pushdown: {} out of {} rows qualify ({}% selectivity)", 
              qualifying_indices.len(), total_rows, 
    ...
    async fn evaluate_parallel_column_filters(
        ...
        match filter_expr {
            FilterExpression::And(sub_exprs) => {
                info!("🔀 VIPER Parallel AND: Processing {} conditions", sub_exprs.len());
                ...
                for (i, expr) in sub_exprs.iter().enumerate() {
                    ...
                    debug!("🔀 AND Condition {}: {} qualifying indices", i, indices.len());
                ...
                info!("🔀 VIPER Parallel AND: Final result has {} indices", intersection.len());
                Ok(intersection)
            }
            
            FilterExpression::Or(sub_exprs) => {
                info!("🔀 VIPER Parallel OR: Processing {} conditions", sub_exprs.len());
                
                // Evaluate each condition on its respective column(s)
                let mut condition_results = Vec::new();
                for (i, expr) in sub_exprs.iter().enumerate() {
                    let indices = self.evaluate_single_column_filter(expr, total_rows)?;
                    debug!("🔀 OR Condition {}: {} qualifying indices", i, indices.len());
                    condition_results.push(indices);
                }
                
                // Union of all results (OR logic)
                let union = self.union_indices(condition_results);
                info!("🔀 VIPER Parallel OR: Final result has {} indices", union.len());
                Ok(union)
            }
        ...
        match filter_expr {
            FilterExpression::Comparison { field, operator, value } => {
                ...
                if let Some(column_data) = self.column_cache.get(field) {
                    ...
                    for (row_idx, column_value) in column_data.iter().enumerate() {
                        ...
                        let matches = match operator {
                            ComparisonOperator::Equals => {
                                if let (serde_json::Value::Number(n1), serde_json::Value::Number(n2)) = (column_value, value) {
                                    crate::core::search::json_comparison::compare_json_numbers(n1, n2)
                                } else {
                                    column_value == value
                                }
                            }
                            ComparisonOperator::GreaterThan => {
                                crate::core::search::json_comparison::compare_json_values(column_value, value) 
                            ...
                            ComparisonOperator::GreaterThanOrEqual => {
                                let ord = crate::core::search::json_comparison::compare_json_values(column_value, value);
                            ...
                            _ => {
                                ...
                                let metadata_map = [(field.clone(), column_value.clone())].into_iter().collect();
                    ...
                    debug!("🎯 Column '{}' filter: {} matches out of {} rows", 
                           field, qualifying_indices.len(), column_data.len());
                    Ok(qualifying_indices)
                } else {
    ...
    async fn load_metadata_columns_only(
        ...
        for name in required_columns {
            ...
            let all_records = reader.read_all_vectors(parquet_file, &[]).await?;
...
impl VIPERSelectiveReader {
    ...
    pub async fn read_vectors_by_indices(
        ...
        info!("🎯 VIPER Selective Read: Reading {} vectors from {} (total I/O savings)", 
              qualifying_indices.len(), parquet_file);
        
        // For now, read all and filter - TODO: implement true selective parquet reading
        let all_records = self.reader.read_all_vectors(parquet_file, &[]).await?;
        ...
        info!("🎯 VIPER Selective Read: Retrieved {} vectors ({:.1}% I/O savings)", 
              selected_records.len(), io_savings);
