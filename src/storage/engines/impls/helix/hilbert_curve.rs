...
impl HilbertCurve {
    ...
    pub fn new(dimensions: usize, bits_per_dim: usize) -> Self {
        assert!(dimensions > 0 && dimensions <= 16, "Dimensions must be 1-16");
        assert!(bits_per_dim > 0 && bits_per_dim <= 21, "Bits per dimension must be 1-21");
...
impl HilbertStats {
    pub fn update_encoding_time(&mut self, time_us: u64) {
        ...
        self.avg_encoding_time_us = 
            (self.avg_encoding_time_us * n + time_us as f64) / (n + 1.0);
