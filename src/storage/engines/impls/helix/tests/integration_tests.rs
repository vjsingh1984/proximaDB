...
async fn test_liquid_clustering() {
    ...
    for pos in hot_positions {
        assert!(pos < 100, "Hot vector at position {} should be near beginning", pos);
    }
}
...
async fn test_progressive_search() {
    ...
    let pruned_count = coordinator.prune_by_hilbert_range(&sstables, query_hilbert).len();
...
async fn test_zone_maps() {
    use crate::storage::engines::impls::helix::zone_maps::{ZoneMap, ZoneMapIndex, ZoneMapBuilder};
...
async fn test_end_to_end_search() {
    ...
    // Execute search
    let results = engine.search_vectors_unified(&search_context).await.unwrap();
