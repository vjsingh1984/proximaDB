...
impl EnhancedPCAModel {
    ...
    pub fn optimal_components_for_variance(
        &self, 
        target_variance_ratio: f32
    ) -> usize {
...
fn calculate_cumulative_variance(
    eigenvalues: &DVector<f32>,
    total_variance: f32,
) -> Vec<f32> {
...
impl EnhancedPCAModel {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        let (nrows, ncols, components_data, mean_data, eigenvalues_data, cumulative_variance, n_components, model_version): 
            (usize, usize, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, usize, u32) = 
            bincode::deserialize(data).map_err(|e| anyhow::anyhow!("Failed to deserialize PCA model: {}", e))?;
