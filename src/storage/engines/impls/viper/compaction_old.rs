    ...
    async fn compact_parquet_files_with_config(
        ...
        for (file_idx, input_file) in input_files.iter().enumerate() {
            ...
            for batch_result in reader {
                ...
                use crate::storage::engines::core::formats::columnar::{FIELD_ID, FIELD_VERSION};
                let id_array = batch
                ...
                let version_array = batch.column_by_name(FIELD_VERSION).unwrap();
                ...
                for i in 0..batch.num_rows() {
                    ...
                    if should_keep {
                        ...
                        for (col_idx, field) in batch.schema().fields().iter().enumerate() {
                            ...
                            use crate::storage::engines::core::formats::columnar::FIELD_VECTOR_FP32;
                            if field.name() == FIELD_VECTOR_FP32 || field.name() == "vector" {
...

            {
                ...
                let vector_records: Vec<VectorRecord> = file_records
                    ...
                    .filter_map(|(idx, record)| {
                        ...
                        // VIPER stores vectors as FIELD_VECTOR_FP32 in parquet files
                        use crate::storage::engines::core::formats::columnar::FIELD_VECTOR_FP32;

                        // Debug: Log the fields we're looking for
                        if idx < 5 {
                            info!("🔍 Looking for vector field '{}' in record {:?}", FIELD_VECTOR_FP32, record.id);
                            info!("   Available fields: {:?}", record.row_data.keys().collect::<Vec<_>>());
                        }

                        // Try "vector" first (actual schema field name), then "vector_fp32" for compatibility
                        let vector_field = if record.row_data.contains_key("vector") {
                            if idx < 5 {
                                debug!("   Using 'vector' field for vector data");
                            }
                            "vector"
                        } else if record.row_data.contains_key(FIELD_VECTOR_FP32) {
                            if idx < 5 {
                                info!("   Using '{}' field for vector data", FIELD_VECTOR_FP32);
                            }
                            FIELD_VECTOR_FP32
                        } else {
                            warn!("    ⚠️ Record {:?} has no 'vector' or '{}' field! Available fields: {:?}",
                                record.id, FIELD_VECTOR_FP32, record.row_data.keys().collect::<Vec<_>>());
                            ""
                        };
                        ...
                        let vector = if !vector_field.is_empty() {
                            ...
                            .and_then(|v| {
                                ...
                                if idx < 5 {
                                    info!("   Vector field value type: is_array={}, is_null={}, is_object={}",
                                        v.is_array(), v.is_null(), v.is_object());
                                }
                                if let Some(arr) = v.as_array() {
                                    let vec_data = arr.iter()
                                        .filter_map(|v| {
                                            // Try both as_f64 and as_i64 for compatibility
                                            v.as_f64()
                                                .map(|f| f as f32)
                                    ...
                                    if vec_data.is_empty() {
                                        warn!("    ⚠️ Vector array is empty for record {:?}", record.id);
                                        None
                                    } else {
                                        if idx < 5 {
                                            info!("      ✅ Extracted vector with {} dimensions for record {:?}", vec_data.len(), record.id);
                                        }
                                        Some(vec_data)
                                    }
                                } else {
                        ...
                        if vector.is_empty() {
                            error!("❌ Skipping record with empty vector: {:?} (fields: {:?})",
                        ...
                        // 1. Filterable columns (stored as separate columns)
                        // 2. Non-filterable metadata (stored in extra_meta Map)
                        ...
                        // First, extract filterable columns from row_data
                        // These are stored as separate columns in the parquet file
                        if let Some(filterable_cols) = filterable_columns.as_ref() {
                            for col in filterable_cols.iter() {
                                if let Some(value) = record.row_data.get(&col.name) {
                                    // Convert JSON value to SqlValue based on column type
                                    let sql_value = if let Some(s) = value.as_str() {
                                        crate::proto::proximadb_v1::SqlValue {
                                        ...
                                        // Non-null value that we can't directly convert - convert to string
                                        crate::proto::proximadb_v1::SqlValue {
                                    ...
                                    metadata.insert(col.name.clone(), sql_value);
                                }
                            }
                        }

                        // Then, extract non-filterable metadata from extra_meta Map
                        if let Some(metadata_map) = record.row_data.get(crate::storage::engines::core::formats::columnar::FIELD_EXTRA_META) {
                            ...
                            if let Some(map_obj) = metadata_map.as_object() {
                                ...
                                for (k, v) in map_obj.iter() {
                                    ...
                                    metadata.insert(k.clone(), sql_value);
                                }
                            }
                        }
                ...
                info!("  📊 VIPER COMPACTION: Writing {} vector records (from {} RecordData) to output file",
                    vector_records.len(), file_records.len());
                if vector_records.len() != file_records.len() {
                    error!("  ⚠️ DATA LOSS WARNING: {} records lost during conversion!",
                        file_records.len() - vector_records.len());
                    error!("     This means some records didn't have valid vectors!");
                }
    ...
    fn extract_column_value(
        ...
        match column_data_type {
            ...
            DataType::FixedSizeBinary(_size) => {
                ...
                // Safety: row_idx is within bounds as checked by the caller
                let bytes = unsafe { array.value_unchecked(row_idx) };
