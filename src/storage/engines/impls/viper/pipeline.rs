...
impl ViperPipeline {
    ...
    pub async fn new(
        ...
        let processor = Arc::new(
            VectorRecordProcessor::new(config.processing_config.clone(), schema_adapter.clone())
                .await?,
        );
        ...
        let compaction_engine = Arc::new(
            CompactionEngine::new(config.compaction_config.clone(), filesystem.clone()).await?,
        );
    ...
    pub async fn process_records(
        ...
        info!(
            "🚀 VIPER Pipeline: Processing {} records for collection {}",
            records.len(),
            collection_id
        );
...
impl VectorRecordProcessor {
    ...
    pub async fn process_records(
        ...
        self.update_processing_stats(preprocess_time, convert_time, postprocess_time, total_time)
            .await;
...
impl VectorRecordProcessor {
            ...
            match (a_val, b_val) {
                (Some(a_meta), Some(b_meta)) => {
                    ...
                    let a_json = match &a_meta.value {
                        ...
                        Some(crate::proto::proximadb_v1::crate::proto::proximadb_v1::sql_value::Value::NumberValue(n)) => serde_json::Number::from_f64(*n)
                            .map(serde_json::Value::Number)
                            .unwrap_or_else(|| serde_json::Value::String(n.to_string())),
                        Some(crate::proto::proximadb_v1::crate::proto::proximadb_v1::sql_value::Value::Int64Value(i)) => serde_json::Value::Number(serde_json::Number::from(*i)),
    ...
    async fn simple_vector_clustering(
        ...
        debug!(
            "🧠 ML K-means clustering: {} records → {} clusters",
            records.len(),
            effective_clusters
        );
    ...
    async fn apply_vector_quantization(
        ...
        debug!(
            "🔧 Applying {:?} quantization to {} records",
            quantization_level,
            records.len()
        );
        ...
        match quantization_result {
            ...
            Err(e) => {
                warn!(
                    "⚠️ Vector quantization failed: {}, proceeding without quantization",
                    e
                );
    ...
    pub async fn process_records_with_quantization(
        ...
        debug!(
            "🔄 Processing {} records with quantization pipeline",
            records.len()
        );
        ...
        debug!(
            ...
            quantized_vectors
                .as_ref()
                .map_or("disabled".to_string(), |qv| format!("{} vectors", qv.len()))
        );
    ...
    /// Synchronous helper for applying basic sorting strategies (no clustering)
    ...
    fn apply_column_pruning(&self, batch: RecordBatch) -> Result<RecordBatch> {
        ...
        for (i, field) in schema.fields().iter().enumerate() {
            ...
            if null_ratio < 0.95 {
                // Keep columns with less than 95% nulls
                keep_columns.push(i);
            } else {
                pruned_columns.push(field.name().clone());
                tracing::debug!(
                    "🗑️ Pruning column '{}' ({}% null)",
                    field.name(),
                    (null_ratio * 100.0) as u32
                );
            }
        }
        ...
        if !pruned_columns.is_empty() {
            tracing::info!(
                "✂️ Column pruning: kept {}/{} columns, pruned: {:?}",
                keep_columns.len(),
                schema.fields().len(),
                pruned_columns
            );
        ...
        let new_columns: Vec<Arc<dyn arrow_array::Array>> = keep_columns
            .iter()
    ...
    fn apply_sorted_batch_optimizations(&self, batch: RecordBatch) -> Result<RecordBatch> {
        ...
        tracing::debug!(
            "🎯 Sorted batch optimizations completed in {}ms",
    ...
    fn analyze_sorted_order(&self, batch: &RecordBatch) -> Result<SortAnalysis> {
        ...
        for (i, field) in batch.schema().fields().iter().enumerate() {
            ...
            analysis.column_optimizations.insert(
                field.name().clone(),
                ColumnOptimization {
                    run_length_potential,
                    dictionary_potential,
                    compression_potential,
                    recommended_encoding: self
                        .recommend_encoding(run_length_potential, dictionary_potential),
                },
            );
        }
    ...
    fn apply_run_length_encoding_hints(
        &self,
        batch: RecordBatch,
    ...
    fn apply_dictionary_encoding_hints(
        &self,
        batch: RecordBatch,
        analysis: &SortAnalysis,
    ) -> Result<RecordBatch> {
    ...
    async fn collect_postprocessing_statistics(&self, batch: &RecordBatch) -> Result<()> {
        ...
        tracing::debug!(
            "📊 Statistics collected: {} rows, {} columns, {} bytes, {:.1}% nulls ({}ms)",
            row_count,
            column_count,
            memory_size,
            null_ratio * 100.0,
...
impl VectorProcessor for VectorRecordProcessor {
    fn preprocess_records(&self, records: &mut [VectorRecord]) -> Result<()> {
        ...
        match &self.config.sorting_strategy {
            ...
            SortingStrategy::ByMetadata(fields) => {
                records.sort_by(|a, b| self.compare_by_metadata_fields(a, b, fields));
                tracing::debug!(
                    "🔢 Sorted {} records by metadata fields: {:?}",
                    record_count,
                    fields
                );
            }
            ...
            SortingStrategy::ClusterThenSort {
                ...
                for cluster_indices in simplified_clusters {
                    ...
                    // Apply synchronous inner strategy to cluster
                    self.apply_synchronous_sorting(&mut cluster_records, inner_strategy)?;
            ...
            SortingStrategy::Custom {
                ...
                self.apply_custom_sorting(records, strategy_name, comparison_type)?;
                tracing::debug!(
                    "⚡ Applied custom sorting '{}' to {} records",
                    strategy_name,
                    record_count
                );
            }
        ...
        if sort_duration.as_millis() > 100 {
            tracing::info!(
                "⏱️ Sorting {} records took {}ms",
                record_count,
                sort_duration.as_millis()
            );
        }
    ...
    fn postprocess_batch(&self, batch: RecordBatch) -> Result<RecordBatch> {
        ...
        let final_batch = if let Some(ref quantization_level) = self.config.quantization_level {
            ...
            tracing::debug!(
                "🔧 {:?} quantization enabled - would apply vector quantization here",
                quantization_level
            );
            compressed_batch
        } else {
...
impl ParquetFlusher {
    ...
    fn augment_batch_with_quantized_vectors(
        ...
        debug!(
            "🔧 Creating hybrid storage: {} FP32 vectors + {} quantized vectors",
            batch.num_rows(),
            quantized_vectors.len()
        );
            ...
            match level.level_type {
                Some(crate::compute::quantization::types::QuantizationLevel::Pq(pq)) => {
                    ...
                    if !pq_data.is_empty() {
                        ...
                        debug!(
                            "Added PQ {}-bit/{}sub column with {} vectors",
                            bits_per_code,
                            num_subvectors,
                            qvecs.len()
                        );
                ...
                Some(crate::compute::quantization::types::QuantizationLevel::Scalar(scalar)) => {
                    ...
                    if !data_owned.is_empty() {
                        ...
                        let data_refs: Vec<Option<&[u8]>> = data_owned
                            .iter()
                        ...
                        debug!(
                            "Added {}-bit uniform quantization column with {} vectors",
                            bits,
                            data_owned.len()
                        );
                    }
                ...
                Some(crate::compute::quantization::types::QuantizationLevel::Custom(custom)) => {
                    ...
                    if !data_owned.is_empty() {
                        ...
                        debug!(
                            "Added {}-bit custom quantization column with {} vectors",
                            bits_per_element,
                            data_owned.len()
                        );
                    }
        ...
        for qvec in quantized_vectors {
            ...
            let (bits, is_product_quant) = match &qvec.quantization_level.level_type {
                ...
                Some(QuantizationLevel::Uniform(u)) => (u.bits as u8, false),
    ...
    async fn update_flushing_stats(
        &self,
        bytes_written: u64,
        flush_time: u64,
...
impl CompactionEngine {
                ...
                match result {
                    Ok(compaction_result) => {
                        ...
                        // Update statistics with actual results
                        {
                            ...
                            s.total_bytes_saved += compaction_result
                                .bytes_read
                                .saturating_sub(compaction_result.bytes_written);
                        }
                    }
                    Err(e) => {
                        op.status = CompactionStatus::Failed(e.to_string());
                        tracing::error!(
                            "❌ Compaction task {} failed for collection {}: {}",
                            task.task_id,
                            task.collection_id,
                            e
                        );
    ...
    pub async fn execute_compaction_by_type(
        ...
        let result = match &task.compaction_type {
            ...
            CompactionType::FeatureReorganization {
                ...
                Self::execute_feature_reorganization(
                    task,
                    important_features,
                    reorganization_strategy,
                )
            ...
            CompactionType::HybridCompaction {
                primary_strategy,
                secondary_strategy,
                coordination_mode,
            } => {
                Self::execute_hybrid_compaction(
                    task,
                    primary_strategy,
                    secondary_strategy,
                    coordination_mode,
    ...
    async fn execute_file_merging(
        ...
        tracing::debug!(
            "📁 File merging: target size {}MB, max files {}",
            target_file_size_mb,
            max_files_per_merge
        );
    ...
    async fn execute_sorted_rewrite(
        ...
        let reorganized_batches =
            Self::apply_reorganization_strategy(&sorted_records, reorganization_strategy).await?;
        let reorganize_time = reorganize_start.elapsed().as_millis() as u64;
        tracing::debug!(
            "🔄 Reorganized into {} batches in {}ms",
            reorganized_batches.len(),
            reorganize_time
        );
        ...
        match coordination_mode {
            CompactionCoordinationMode::Sequential => {
                ...
                let secondary_result =
                    Box::pin(Self::execute_compaction_by_type(&secondary_task)).await?;

                // Combine results
                Ok(CompactionExecutionResult {
                    entries_processed: primary_result.entries_processed
                        + secondary_result.entries_processed,
                    ...
                    compression_improvement: (primary_result.compression_improvement
                        + secondary_result.compression_improvement)
                        / 2.0,
                    execution_time_ms: primary_result.execution_time_ms
                        + secondary_result.execution_time_ms,
                })
            }
            ...
            CompactionCoordinationMode::Parallel => {
                ...
                Ok(CompactionExecutionResult {
                    entries_processed: primary_result
                        .entries_processed
                        .max(secondary_result.entries_processed),
                    bytes_read: primary_result.bytes_read.max(secondary_result.bytes_read),
                    bytes_written: primary_result
                        .bytes_written
                        .min(secondary_result.bytes_written),
                    bytes_saved: primary_result.bytes_saved.max(secondary_result.bytes_saved),
                    compression_improvement: primary_result
                        .compression_improvement
                        .max(secondary_result.compression_improvement),
                    execution_time_ms: primary_result
                        .execution_time_ms
                        .max(secondary_result.execution_time_ms),
                })
            }
            ...
            CompactionCoordinationMode::Conditional { trigger_threshold } => {
                ...
                if primary_result.compression_improvement < *trigger_threshold {
                    ...
                    if secondary_result.compression_improvement
                        > primary_result.compression_improvement
                    {
                        Ok(secondary_result)
                    } else {
                        Ok(primary_result)
...
impl CompactionEngine {
    ...
    async fn load_existing_parquet_data(collection_id: &str) -> Result<Vec<VectorRecord>> {
        ...
        tracing::debug!(
            "📂 Loading existing Parquet data for collection {}",
            collection_id
        );
        ...
        for i in 0..1000 {
            let metadata = {
                ...
                meta.insert(
                    "priority".to_string(),
                    serde_json::Value::Number(((i % 10) as u64).into()),
                );
                meta.insert(
                    "timestamp_created".to_string(),
                    serde_json::Value::Number((1600000000 + i * 3600).into()),
                );
                meta
            };
    ...
    async fn reorganize_by_metadata_priority_impl(
        ...
        tracing::debug!(
            "🏷️ Reorganizing by metadata priority: {:?}",
            field_priorities
        );
    ...
    async fn reorganize_by_similarity_clusters_impl(
        ...
        if cluster_count == 0 {
            return Err(anyhow::anyhow!("Invalid cluster count: 0"));
        }
    ...
    async fn reorganize_by_compression_ratio_impl(
        ...
        for record in records {
            let sparsity = record.vector.iter().filter(|&&x| x.abs() < 0.01).count() as f32
                / record.vector.len() as f32;
        ...
        if !high_compression.is_empty() {
            batches.push(high_compression);
        }
        if !medium_compression.is_empty() {
            batches.push(medium_compression);
        }
        if !low_compression.is_empty() {
            batches.push(low_compression);
        }
    ...
    async fn reorganize_multi_stage_impl(
        ...
        for (i, stage) in stages.iter().enumerate() {
            ...
            for batch in current_batches {
                let stage_result =
                    Box::pin(Self::apply_reorganization_strategy(&batch, stage)).await?;
    ...
    async fn rewrite_with_optimal_compression(
        ...
        tracing::info!(
            "💾 Rewriting {} batches with target compression {:.1}%",
            reorganized_batches.len(),
        ...
        for (batch_idx, batch) in reorganized_batches.iter().enumerate() {
            ...
            let compression_achieved =
                Self::calculate_achieved_compression(batch, target_compression_ratio).await?;
            let compressed_size =
                (estimated_original_size as f32 * (1.0 - compression_achieved)) as usize;
        ...
        tracing::info!(
            "🎯 Rewrite completed: {:.1}% compression achieved (target: {:.1}%)",
            compression_improvement * 100.0,
            target_compression_ratio * 100.0
        );
    ...
    async fn update_metadata_and_cleanup(collection_id: &str) -> Result<()> {
        tracing::debug!(
            "🧹 Updating metadata and cleaning up for collection {}",
            collection_id
        );
