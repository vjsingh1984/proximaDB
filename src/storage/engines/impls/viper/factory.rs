...
impl ViperFactory {
    ...
    pub fn create_for_collection(
        ...
        let schema_strategy =
            self.create_schema_strategy(&strategy_name, &config, collection_id)?;
        ...
        debug!(
            "✅ VIPER Factory: Created components with strategy '{}' and processor '{}'",
    ...
    pub fn register_schema_strategy(
        &mut self,
        name: String,
        factory: Box<dyn SchemaStrategyFactory>,
    ) {
    ...
    fn register_default_processors(&mut self) {
        self.processor_registry
            .insert("standard".to_string(), Box::new(StandardProcessorFactory));
        self.processor_registry.insert(
            "timeseries".to_string(),
            Box::new(TimeSeriesProcessorFactory),
        );
        self.processor_registry.insert(
            "similarity".to_string(),
            Box::new(SimilarityProcessorFactory),
        );
    }
...
impl ViperConfigurationBuilder {
    ...
    pub fn with_compression_optimization(mut self, enabled: bool) -> Self {
        self.config
            .optimization_config
            .enable_compression_optimization = enabled;
        self
    }
