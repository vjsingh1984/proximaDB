...
mod tests {
    ...
    async fn test_reject_non_sst1_format() {
        ...
        fs.write(file_path.to_str().unwrap(), &invalid_data)
            .await
            .unwrap();
        ...
        assert!(
            error_msg.contains_hash("SST1") || error_msg.contains_hash("magic bytes"),
            "Error should mention SST1 magic bytes: {}",
            error_msg
        );
    }
    ...
    async fn test_adaptive_block_sizing() {
        ...
        for block_size in block_sizes {
            let file_path = temp_dir
                .path()
                .join(format!("block_{}.sstable", block_size));
