...
mod tests {
    ...
    use crate::storage::engines::impls::sst::{
        DataBlock, SstRecord, DataBlockMetadata, SstableWriter, 
        CompressionAlgorithmSst, SstableHeader,
    };
    use crate::storage::engines::impls::sst::readers::sst_query_engine::{
        UnifiedSstableReader, ReadStrategy, ReadMode, SstableIndex,
    };
        ...
        (0..count)
            .map(|i| SstRecord {
                ...
                metadata: vec![
                    ...
                    MetadataItem {
                        key: "score".to_string(),
                        value: Some(metadata_item::Value::StringValue((i as f32 * 1.5).to_string())),
                    },
                ],
    ...
    async fn create_hierarchical_sst_file(path: &str) -> Result<()> {
        ...
        let fs = Arc::new(crate::storage::persistence::filesystem::FilesystemFactory::new(config).await.unwrap());
    ...
    async fn test_full_scan_strategy_validation() {
        ...
        let block0_records: Vec<_> = records.iter().filter(|r| r.id.starts_with("block0")).collect();
        let block1_records: Vec<_> = records.iter().filter(|r| r.id.starts_with("block1")).collect();
        let block2_records: Vec<_> = records.iter().filter(|r| r.id.starts_with("block2")).collect();
        
        assert_eq!(block0_records.len(), 100, "Should have 100 records from block 0");
        assert_eq!(block1_records.len(), 100, "Should have 100 records from block 1");
        assert_eq!(block2_records.len(), 100, "Should have 100 records from block 2");
    ...
    async fn test_range_scan_strategy_validation() {
        ...
        let block1_count = records.iter().filter(|r| r.id.starts_with("block1")).count();
        let block2_count = records.iter().filter(|r| r.id.starts_with("block2")).count();
    ...
    async fn test_block_level_filtering_validation() {
        ...
        let index = reader.read_index_block_async(&ReadStrategy::FullScan).await.unwrap();
        ...
        for (i, entry) in index.entries.iter().enumerate() {
            ...
            assert!(entry.offset > 0 || i == 0, "Block {} should have valid offset", i);
    ...
    async fn test_metadata_statistics_validation() {
        ...
        let target = records.iter().find(|r| {
            r.metadata.iter().any(|(k, v)| k == "score" && v == "25")
        });
    ...
    async fn test_random_block_access_validation() {
        ...
        let block2 = reader.read_data_block_async(2, ReadMode::Buffered).await.unwrap();
    ...
    async fn test_streaming_iterator_validation() {
        ...
        let fs = Arc::new(crate::storage::persistence::filesystem::FilesystemFactory::new(sst_path.clone()));
        let reader = UnifiedSstableReader::open(fs.clone(), &sst_path).await.unwrap();
        
        // Test streaming with BlockIterator
        let mut iterator = reader.create_block_iterator(ReadMode::Streaming).await.unwrap();
    ...
    async fn test_compression_handling_validation() {
        ...
        assert_eq!(records.len(), 1000, "Should decompress and read all records");
