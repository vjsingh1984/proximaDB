...
use crate::storage::engines::impls::sst::readers::sst_query_engine::{UnifiedSstableReader, CollectionContext};
...
async fn test_valid_sst1_magic_marker() {
    ...
    assert!(result.is_ok(), "Valid SST1 file should pass validation: {:?}", result);
}
...
async fn test_invalid_magic_marker_rejection() {
    ...
    let invalid_markers = vec![
        ...
        (b"ABCD\x08\x00\x00\x00test_data", "Random magic should be rejected"),
        (b"    \x08\x00\x00\x00test_data", "Spaces should be rejected"),
        (b"\x00\x00\x00\x00\x08\x00\x00\x00test_data", "Null bytes should be rejected"),
        (b"sst1\x08\x00\x00\x00test_data", "Lowercase sst1 should be rejected"),
    ];
...
async fn test_edge_case_magic_markers() {
    ...
    let edge_cases: Vec<(&[u8], &str)> = vec![
        ...
        (b" SST1\x08\x00\x00\x00", "Magic with leading space should be invalid"),
        (b"SST1\x00\x00\x00\x00", "Magic with null padding should be valid"),
    ];
