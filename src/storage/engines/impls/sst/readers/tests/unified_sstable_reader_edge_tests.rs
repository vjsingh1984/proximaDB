...
mod edge_tests {
    ...
    async fn test_high_dimensional_vectors() {
        ...
        assert_eq!(extreme_params.query_vectors.as_ref().unwrap()[0].len(), 65536);
    }
    ...
    async fn test_null_and_missing_values() {
        ...
        for filter in vec![filter_null, filter_is_null, filter_is_not_null, filter_empty, filter_empty_array, filter_empty_object] {
            let params = SearchParams {
                query_vectors: Some(vec![vec![0.1; 128]]),
                filter_expression: Some(filter),
                ..Default::default()
    ...
    async fn test_numeric_boundary_values() {
        ...
        for filter in vec![filter_max, filter_min, filter_epsilon, filter_inf, filter_neg_inf, filter_nan] {
            let params = SearchParams {
                query_vectors: Some(vec![vec![0.1; 128]]),
                filter_expression: Some(filter),
                ..Default::default()
    ...
    async fn test_batch_query_edge_cases() {
        ...
        // Very large batch
        let large_batch: Vec<Vec<f32>> = (0..10000)
            .map(|i| vec![i as f32 / 10000.0; 128])
            .collect();
        let params_large = SearchParams {
    ...
    async fn test_metadata_stats_edge_cases() {
        ...
        stats.insert("numeric_field".to_string(), (f64::MIN, f64::MAX, 0usize, usize::MAX));
        
        // String field with Unicode (represented as numeric values for simplicity)
        stats.insert("string_field".to_string(), (0.0f64, 1.0f64, 100usize, 0usize));
