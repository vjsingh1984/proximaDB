...
impl Compaction {
    ...
    pub async fn with_quantization_sorting(mut self) -> Result<Self> {
        ...
        if let Some(ref mut compactor) = self.sst_compactor {
            ...
            info!("🎯 Compaction configured with PQ-based similarity sorting for better compression");
        } else {
    ...
    async fn worker_loop(
        ...
        loop {
            ...
            if let Some(task) = task {
                ...
                let temp_manager = match Compaction::with_atomic_coordinator(config.clone(), atomic_coordinator.clone()).await {
                    Ok(manager) => manager,
                    Err(e) => {
