...
pub struct EnhancedRowGroupStats {
    ...
    /// Native Parquet metadata (not serialized, reconstructed on read)
    #[serde(skip)]
...
impl SuperBlock {
    ...
    pub fn new(
        ...
        if enhanced_stats.is_empty() {
            return Err(anyhow::anyhow!(
                "Cannot create SuperBlock from empty row group stats"
            ));
        }
    ...
    pub fn estimate_search_cost(&self, query_selectivity: f32) -> f32 {
        ...
        let selectivity_adjustment =
            query_selectivity * self.selectivity_hints.binary_reduction_factor;
        base_cost * selectivity_adjustment
    }

    /// Get ordered row groups by estimated search cost
    pub fn ordered_row_groups(&self, enhanced_stats: &[EnhancedRowGroupStats]) -> Vec<u32> {
        let mut row_group_costs: Vec<(u32, f32)> = enhanced_stats
            .iter()
            .filter(|stats| self.row_groups.contains(&stats.row_group_id))
            .map(|stats| {
                (
                    stats.row_group_id,
                    stats.search_cost_estimate.estimated_latency_ms,
                )
            })
            .collect();
    ...
    fn calculate_selectivity_hints(stats: &[EnhancedRowGroupStats]) -> SelectivityHints {
        ...
        for stat in stats {
            hints.binary_reduction_factor +=
                stat.quantized_selectivity.binary_effectiveness / count;
