...
use crate::compute::quantization::unified::{
    UnifiedQuantizationEngine, Codebook, CodebookData, TrainingConfig
};
...
impl QuantizedColumns {
    ...
    pub async fn build_pq_column(
        ...
        self.quantization_engine.train_pq_codebook(
            vectors,
            num_segments,
            bits_per_code,
            &codebook_id,
    ...
    pub fn to_record_batch(&self) -> Result<RecordBatch> {
        ...
        if let Some(ref binary_data) = self.binary_data {
            ...
            let binary_array: BinaryArray = binary_data.iter()
        ...
        if let Some(ref int8_data) = self.int8_data {
            ...
            let int8_array: BinaryArray = int8_data.vectors.iter()
                .map(|v| Some(v.as_slice()))
                .collect();
        ...
        if let Some(ref pq_data) = self.pq_data {
            ...
            let pq_array: BinaryArray = pq_data.codes.iter()
                .map(|v| Some(v.as_slice()))
                .collect();
