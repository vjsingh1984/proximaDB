...
mod tests {
    ...
    mod benchmark_tests {
        ...
        fn test_zone_map_performance() {
            ...
            for _ in 0..1000 {
                let _intersects = zone_map.intersects_query(&query, DistanceMetric::Euclidean, 10.0);
            }
        ...
        fn test_binary_sketch_performance() {
            ...
            let start = Instant::now();
            let sketches: Vec<BinarySketch> = vectors.iter().map(|v| BinarySketch::from_vector(v, 0.0)).collect();
            let creation_time = start.elapsed();
            ...
            let distances: Vec<u32> = sketches.iter().map(|sketch| query_sketch.hamming_distance(sketch)).collect();
