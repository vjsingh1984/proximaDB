//! ISP-Compliant Progressive Search Stage Adapters for NOVA
//!
//! This module provides adapter implementations of `ProgressiveSearchStage` for NOVA.
//! NOVA uses hybrid columnar format with zone maps for progressive search.
//! The progressive pipeline integrates with NOVA's zone map pruning.
//!
//! The adapters enable:
...
///
/// Wraps NOVA's binary quantization filtering with zone map awareness.
pub struct NovaBinaryStage {
    hamming_threshold: f32,
    quantization_engine: Arc<UnifiedQuantizationEngine>,
}

impl NovaBinaryStage {
...
#[async_trait]
impl ProgressiveSearchStage for NovaBinaryStage {
...
/// NOVA-specific INT8 stage adapter
///
/// Uses quantized columnar INT8 vectors for efficient zone-based processing.
pub struct NovaInt8Stage {
...
#[async_trait]
impl ProgressiveSearchStage for NovaInt8Stage {
    fn name(&self) -> &'static str {
        "NOVA-INT8"
    }
...
/// NOVA-specific FP32 stage adapter (final reranking)
pub struct NovaFp32Stage {
    distance_compute: Arc<UnifiedDistanceCompute>,
}
...
#[async_trait]
impl ProgressiveSearchStage for NovaFp32Stage {
    fn name(&self) -> &'static str {
        "NOVA-FP32"
    }
    ...
    async fn compute_distances(
        ...
        for candidate in &mut candidates {
            if let Some(ref vector) = candidate.vector {
                ...
                tracing::warn!("NOVA-FP32Stage: No vector for candidate {}", candidate.id);
                candidate.score = f32::MAX;
            }
        }
...
/// Create a standard NOVA progressive search pipeline using ISP-compliant stages
///
/// Note: NOVA uses zone maps for pre-filtering before the progressive pipeline.
pub fn create_nova_pipeline(
    quantization_engine: Arc<UnifiedQuantizationEngine>,
    distance_compute: Arc<UnifiedDistanceCompute>,
    hamming_threshold: f32,
    ...
    ProgressiveSearchCoordinator::new()
        .add_stage(Box::new(NovaBinaryStage::new(hamming_threshold, quantization_engine)))
        .add_stage(Box::new(NovaInt8Stage::new(distance_compute.clone())))
        .add_stage(Box::new(NovaFp32Stage::new(distance_compute)))
}
...
mod tests {
    ...
    #[test]
    fn test_nova_stage_names() {
        let (quant_engine, dist_compute) = create_test_engines();

        let binary_stage = NovaBinaryStage::new(0.7, quant_engine);
        assert_eq!(binary_stage.name(), "NOVA-Binary");

        let int8_stage = NovaInt8Stage::new(dist_compute.clone());
        assert_eq!(int8_stage.name(), "NOVA-INT8");

        let fp32_stage = NovaFp32Stage::new(dist_compute);
        assert_eq!(fp32_stage.name(), "NOVA-FP32");
    }

    #[test]
    fn test_nova_pipeline_creation() {
        let (quant_engine, dist_compute) = create_test_engines();

        let pipeline = create_nova_pipeline(quant_engine, dist_compute, 0.7);
