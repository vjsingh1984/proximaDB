...
use super::schema::{SchemaGenerationStrategy, ViperSchemaStrategy, LegacySchemaStrategy};
use super::processor::{VectorProcessor, VectorRecordProcessor, TimeSeriesVectorProcessor, SimilarityVectorProcessor};
use super::adapter::{VectorRecordSchemaAdapter, TimeSeriesAdapter, CompressedVectorAdapter, TimePrecision, VectorCompressionType};
...
impl ViperSchemaFactory {
    ...
    pub fn create_adaptive_strategy(collection_config: &CollectionConfig) -> Box<dyn SchemaGenerationStrategy> {
        ...
        if has_filterable_fields {
            ...
            Box::new(LegacySchemaStrategy::new(collection_config.collection_id.clone()))
        }
    }
}
...
impl VectorProcessorFactory {
    ...
    pub fn create_processor<'a>(strategy: &'a dyn SchemaGenerationStrategy) -> Box<dyn VectorProcessor + 'a> {
    ...
    pub fn create_time_series_processor<'a>(
        ...
        Box::new(TimeSeriesVectorProcessor::new(strategy, time_window_seconds))
    }
    ...
    pub fn create_adaptive_processor<'a>(
        ...
        } else if collection_config.filterable_metadata_fields.iter().any(|f| f.contains("timestamp") || f.contains("time")) {
...
impl AdapterFactory {
    ...
    pub fn create_adapter<'a>(strategy: &'a dyn SchemaGenerationStrategy) -> VectorRecordSchemaAdapter<'a> {
