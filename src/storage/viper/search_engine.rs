...
//! 🔄 MIGRATION: Use src/storage/vector/search/engine.rs instead.
//! 📅 DEPRECATION: Phase 1.2 - Search Consolidation (consolidates 3→1 search engines)
//!
//! This implementation will be deprecated in favor of the unified search engine
//! that consolidates all search functionality into a single, optimized system.
//! 
...
use super::index::{IndexStrategy, IndexVector, ViperIndexManager};
use super::types::*;
...
impl ViperProgressiveSearchEngine {
    ...
    pub async fn search(
        ...
        self.update_feature_cache(&context.collection_id, &important_features)
            .await?;
        ...
        let results = match context.search_strategy.clone() {
            ...
            SearchStrategy::Exhaustive => {
                self.exhaustive_search(context, cluster_hints, important_features)
                    .await?
            }
    ...
    async fn progressive_tier_search(
        ...
        for (storage_idx, storage_url) in storage_order.iter().enumerate() {
            ...
            let partitions = self
                .get_storage_partitions(&context.collection_id, storage_url, &cluster_hints)
                .await?;
            ...
            let storage_results = self
                .search_single_storage(&context, storage_url, partitions, &important_features)
                .await?;

            // Merge results with TTL filtering
            for result in storage_results {
                if !searched_vectors.contains(&result.vector_id)
                    && self.is_vector_valid(&result).await
                {
            ...
            if storage_idx < tier_thresholds.len()
                && result_heap.len() >= tier_thresholds[storage_idx]
            {
    ...
    async fn cluster_pruned_search(
        ...
        let clusters = if let Some(hints) = cluster_hints {
            ...
            self.predict_search_clusters(&context, confidence_threshold)
                .await?
        };

        // Limit to max clusters
        let search_clusters: Vec<ClusterId> = clusters.into_iter().take(max_clusters).collect();
        ...
        for (tier_level, searcher) in tier_searchers.iter() {
            let partitions = self
                .get_cluster_partitions(&context.collection_id, tier_level, &search_clusters)
                .await?;

            if !partitions.is_empty() {
                let tier_results = searcher
                    .search_tier(&context, partitions, &important_features)
                    .await?;
    ...
    async fn exhaustive_search(
        ...
        for (tier_level, searcher) in tier_searchers.iter() {
            let partitions = self
                .get_all_partitions(&context.collection_id, tier_level)
                .await?;
    ...
    async fn is_vector_valid(&self, result: &ViperSearchResult) -> bool {
        ...
        if let Some(expires_at) = self
            .extract_expires_at_from_metadata(&result.metadata)
            .await
        {
            let now = Utc::now();
            if now > expires_at {
                tracing::debug!(
                    "🕒 Vector {} expired at {}, skipping",
                    result.vector_id,
                    expires_at
                );
                return false;
    ...
    async fn extract_expires_at_from_metadata(
        &self,
    ...
    pub async fn search_two_phase(
        ...
        tracing::debug!(
            "🔍 Starting two-phase search for collection: {}",
        ...
        tracing::debug!(
            "📝 WAL search completed: {} results found",
            wal_results.len()
        );
        ...
        tracing::debug!(
            "💾 Storage search completed: {} results found",
            storage_results.len()
        );

        // Phase 3: Merge and deduplicate results with recency bias
        let merged_results = self
            .merge_two_phase_results(wal_results, storage_results, &context)
            .await?;

        let elapsed = start_time.elapsed();
        tracing::debug!(
            "✅ Two-phase search completed: {} total results in {:?}",
            merged_results.len(),
            elapsed
        );
    ...
    async fn merge_two_phase_results(
        ...
        storage_results: Vec<ViperSearchResult>,
        context: &ViperSearchContext,
    ) -> Result<Vec<ViperSearchResult>> {
        ...
        for (result, _from_wal) in all_results {
        ...
        tracing::debug!(
            "🔀 Merged {} unique results (from {} total candidates)",
            merged_results.len(),
            total_candidates
        );

        // Log merge statistics
        let wal_count = merged_results
            .iter()
            .filter(|r| r.partition_id.starts_with("wal:"))
            .count();
        let storage_count = merged_results.len() - wal_count;

        tracing::debug!(
            "📊 Final result composition: {} from WAL, {} from storage",
            wal_count,
            storage_count
        );
    ...
    pub async fn build_hnsw_index(
        ...
        tracing::info!(
            "🏗️ Building HNSW index for collection {} with {} vectors",
            collection_id,
            vectors.len()
        );

        // Convert to IndexVector format
        let index_vectors: Vec<IndexVector> = vectors
            .into_iter()
        ...
        // Build the index
        index_manager
            .build_indexes(index_vectors, distance_metric)
            .await?;
    ...
    pub async fn search_with_hnsw(
        ...
        if let Some(index_manager) = managers.get(&context.collection_id) {
            ...
            let index_results = index_manager
                .search(
            ...
            tracing::warn!(
                "⚠️ No HNSW index found for collection {}, falling back to scan",
                context.collection_id
            );
    ...
    pub async fn search_filtered_with_hnsw(
        ...
        tracing::info!(
            "🔍 Starting filtered HNSW search for collection: {}",
            context.collection_id
        );
        ...
        let filtered_candidates = if let Some(filters) = &context.filters {
            ...
            let candidates = self
                .apply_metadata_filters(&context.collection_id, filters)
                .await?;
            tracing::debug!(
                "✅ Metadata filtering reduced to {} candidates",
                candidates.len()
            );
            Some(candidates)
        } else {
        ...
        let hnsw_results = self
            .search_with_hnsw(context.clone(), filtered_candidates)
            .await?;
        ...
        let elapsed = start_time.elapsed();
        tracing::info!(
            "✅ Filtered HNSW search completed: {} results in {:?}",
            final_results.len(),
            elapsed
        );
    ...
    async fn load_vector_data(&self, _vector_id: &VectorId) -> Result<Option<Vec<f32>>> {
...
impl FeatureSelectionCache {
    ...
    fn update(&mut self, collection_id: CollectionId, features: Vec<usize>) {
        ...
        // Evict if necessary
        if self.feature_sets.len() >= self.max_cache_size
            && !self.feature_sets.contains_key(&collection_id)
        {
