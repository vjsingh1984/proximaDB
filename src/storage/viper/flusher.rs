...
impl ViperParquetFlusher {
    ...
    pub async fn flush_to_parquet(
        ...
        tracing::info!(
            "🔥 Starting VIPER Parquet flush for collection {} with {} entries",
            collection_id,
            entries.len()
        );
        ...
        tracing::info!(
            "✅ VIPER Parquet flush completed: {} entries, {} bytes, {}ms",
            clustered_entries.len(),
            bytes_written,
            flush_duration
        );
    ...
    async fn apply_viper_clustering(&self, entries: &[WalEntry]) -> Result<Vec<WalEntry>> {
        ...
        let mut cluster_groups: std::collections::HashMap<usize, Vec<WalEntry>> =
        ...
        tracing::debug!(
            "✅ VIPER clustering complete: {} clusters, {} entries",
            cluster_count,
            clustered_entries.len()
        );
    ...
    fn convert_to_arrow_batch(&self, entries: &[WalEntry]) -> Result<RecordBatch> {
        ...
        for entry in entries {
            ...
            match &entry.operation {
                WalOperation::Insert {
                    ...
                    let serialized =
                        bincode::serialize(record).context("Failed to serialize vector record")?;
                ...
                WalOperation::AvroPayload {
                    ...
                    operation_types.push(format!("AVRO_{}", operation_type.to_uppercase()));
                    vector_ids.push(None);
                    vector_data.push(Some(avro_data.clone()));
                }
            }
        }
        ...
        let vector_data_refs: Vec<Option<&[u8]>> =
            vector_data.iter().map(|v| v.as_deref()).collect();
