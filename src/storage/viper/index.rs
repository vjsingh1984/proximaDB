...
//! 🔄  MIGRATED TO: `src/storage/vector/indexing/manager.rs` (UnifiedIndexManager)
//! 📅  DEPRECATION DATE: Phase 2 consolidation
//!
...
impl HNSWIndex {
    ...
    fn get_m_nearest(&self, candidates: Vec<(VectorId, f32)>, m: usize) -> Vec<VectorId> {
        ...
        while let Some(current) = candidates_queue.pop() {
            ...
            for selected_id in &pruned {
                if let Some(selected_vector) = vectors.get(selected_id) {
                    let dist_to_selected = distance_computer
                        .distance(vectors.get(&current.id).unwrap(), selected_vector);
...
impl VectorIndex for HNSWIndex {
    ...
    async fn search(
        ...
        for layer in (1..self.layers.len()).rev() {
            current_nearest = self
                .search_layer(query, current_nearest.clone(), 1, layer)
                .await?
                .into_iter()
                .map(|(id, _)| id)
                .collect();
        ...
        let candidates = self
            .search_layer(query, current_nearest, ef.max(k), 0)
            .await?;
    ...
    async fn add_vector(&mut self, vector: IndexVector) -> Result<()> {
        ...
        for lc in (layer + 1..self.layers.len()).rev() {
            current_nearest = self
                .search_layer(&vector.vector, current_nearest.clone(), 1, lc)
                .await?
                .into_iter()
                .map(|(id, _)| id)
                .collect();
        ...
        for lc in (0..=layer).rev() {
            ...
            let neighbors = if self.config.enable_pruning {
                self.prune_connections(candidates.clone(), m, &vector.vector)
                    .await?
            } else {
            ...
            self.layers[lc]
                .graph
                .insert(vector.id.clone(), neighbors.clone());
                ...
                if let Some(neighbor_links) = self.layers[lc].graph.get_mut(neighbor_id) {
                    ...
                    if neighbor_links.len() > m {
                        ...
                        if let Some(neighbor_vector) = vectors_read.get(neighbor_id) {
                            ...
                            let neighbor_candidates: Vec<_> = neighbor_links_vec
                                .iter()
                                .filter_map(|id| {
                            ...
                            pruned_candidates.sort_by(|a, b| {
                                a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal)
                            });
                            pruned_candidates.truncate(m);

                            *neighbor_links =
                                pruned_candidates.into_iter().map(|(id, _)| id).collect();
                        }
                    }
                }
            }

            current_nearest = candidates.into_iter().map(|(id, _)| id).collect();
        }
