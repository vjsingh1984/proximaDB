...
impl CollectionLockManager {
    ...
    pub async fn acquire_read_lock(&self, collection_id: &CollectionId) -> Result<ReadLockGuard> {
        ...
        lock.reader_count += 1;
        tracing::debug!(
            "🔓 Acquired read lock for collection {}, readers: {}",
            collection_id,
            lock.reader_count
        );
    ...
    pub async fn acquire_write_lock(
        ...
        if lock.reader_count > 0 || lock.has_writer {
            return Err(anyhow::anyhow!(
                "Collection {} has active readers ({}) or writer",
                collection_id,
                lock.reader_count
            ));
        }
        ...
        tracing::info!(
            "🔒 Acquired write lock for collection {} ({:?})",
            collection_id,
            operation
        );
        ...
        tokio::spawn(async move {
            if let Some(manager) = lock_manager.upgrade() {
                ...
                if let Some(lock) = locks.get_mut(&collection_id) {
                    ...
                    tracing::debug!(
                        "🔓 Released read lock for collection {}, readers: {}",
                        collection_id,
                        lock.reader_count
                    );
        ...
        tokio::spawn(async move {
            if let Some(manager) = lock_manager.upgrade() {
                ...
                if let Some(lock) = locks.get_mut(&collection_id) {
                    ...
                    tracing::info!(
                        "🔒 Released write lock for collection {} ({:?})",
                        collection_id,
                        operation
                    );
...
impl AtomicFlusher {
    ...
    pub async fn atomic_flush(
        ...
        tracing::info!(
            "🚀 Starting optimized atomic flush for collection {} with {} records",
            collection_id,
            records.len()
        );
        ...
        let staging_url = format!(
            "viper/{}/collections/{}/__flush_{}",
            collection_id,
            collection_id,
            chrono::Utc::now().timestamp_millis()
        );

        let flushed_file_url = format!(
            "{}/flushed_{}.parquet",
            staging_url,
            chrono::Utc::now().timestamp_millis()
        );
        ...
        self.staging_coordinator
            .write_records_to_staging(&flushed_file_url, records, StagingOperationType::Flush)
            .await
            .context("Failed to write records to staging directory")?;
        ...
        let target_file_url = format!(
            "viper/{}/collections/{}/flushed_{}.parquet",
            collection_id,
            collection_id,
            chrono::Utc::now().timestamp_millis()
        );
        ...
        self.staging_coordinator
            .filesystem()
            .copy(&flushed_file_url, &target_file_url)
            .await
        ...
        tracing::info!(
            "✅ Optimized atomic flush completed for collection {}: {}",
            collection_id,
            target_file_url
        );
...
impl AtomicCompactor {
    ...
    pub async fn atomic_compact(
        ...
        tracing::info!(
            "🔄 Starting optimized atomic compaction for collection {} with {} files",
            collection_id,
            source_files.len()
        );
        ...
        let staging_url = format!(
            "viper/{}/collections/{}/__compaction_{}",
            collection_id,
            collection_id,
            chrono::Utc::now().timestamp_millis()
        );

        let compacted_file_url = format!(
            "{}/compacted_{}.parquet",
            staging_url,
            chrono::Utc::now().timestamp_millis()
        );
        ...
        let merged_records = self
            .staging_coordinator
            .read_and_merge_source_files(&source_files)
            .await
            .context("Failed to read and merge source files")?;

        if !merged_records.is_empty() {
            self.staging_coordinator
                .write_records_to_staging(
                    &compacted_file_url,
                    merged_records,
                    StagingOperationType::Compaction,
                )
                .await
                .context("Failed to write compacted records to staging")?;
        }
        ...
        let target_file_url = format!(
            "viper/{}/collections/{}/compacted_{}.parquet",
            collection_id,
            collection_id,
            chrono::Utc::now().timestamp_millis()
        );
        ...
        self.staging_coordinator
            .filesystem()
            .copy(&compacted_file_url, &target_file_url)
            .await
        ...
        tracing::info!(
            "✅ Optimized atomic compaction completed for collection {}: {} -> {}",
            collection_id,
            source_files.len(),
            target_file_url
        );
