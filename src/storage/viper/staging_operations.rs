...
impl StagingOperationsCoordinator {
    ...
    pub async fn write_records_to_staging(
        ...
        tracing::debug!(
            "📝 Writing {} records to staging with Parquet optimization ({:?}): {}",
            records.len(),
            operation_type,
            staging_url
        );
        ...
        self.sort_records_for_parquet_optimization(&mut records)
            .await?;

        // Step 2: Calculate optimal file sizing based on operation type
        let parquet_config = self
            .calculate_optimal_parquet_config(records.len(), &operation_type)
            .await?;

        tracing::debug!(
            "🎯 Parquet config ({:?}): {} rowgroups of {} rows, {} chunks",
            operation_type,
            parquet_config.num_rowgroups,
            parquet_config.rows_per_rowgroup,
            parquet_config.num_chunks
        );

        // Step 3: Write to staging with optimized Parquet format
        self.write_optimized_parquet(staging_url, records, parquet_config)
            .await
    ...
    async fn sort_records_for_parquet_optimization(
        &self,
        records: &mut [VectorRecord],
    ) -> Result<()> {
        tracing::debug!(
            "🔤 Sorting {} records for Parquet pushdown optimization",
            records.len()
        );
    ...
    async fn calculate_optimal_parquet_config(
        ...
        let (target_file_size_mb, min_rowgroup_size, max_rowgroup_size, compression_level) =
        ...
        let (rows_per_rowgroup, num_rowgroups) = if estimated_file_size_mb <= target_file_size_mb {
            ...
            let optimal_rowgroup_size = (record_count / divisor)
                .max(min_rowgroup_size)
                .min(max_rowgroup_size);
        ...
        if config.num_chunks > 1 {
            ...
            for chunk_idx in 0..config.num_chunks {
                ...
                let chunk_url = format!(
                    "{}_chunk_{}.parquet",
                    staging_url.trim_end_matches(".parquet"),
                    chunk_idx
                );

                self.write_single_parquet_chunk(&chunk_url, chunk_records, &config)
                    .await?;
            }
        } else {
            // Single file
            self.write_single_parquet_chunk(staging_url, records, &config)
                .await?;
        }
    ...
    async fn write_single_parquet_chunk(
        ...
        tracing::debug!(
            "📄 Writing Parquet chunk: {} records to {}",
            records.len(),
            file_url
        );
        ...
        let optimized_data = self
            .serialize_records_with_column_optimization(records, config)
            .await?;

        self.filesystem
            .write(file_url, &optimized_data, None)
            .await
    ...
    async fn serialize_records_with_column_optimization(
        ...
        tracing::debug!(
            "🔧 Serializing {} records with column optimization",
            records.len()
        );
        ...
        bincode::serialize(&optimized_records).context("Failed to serialize optimized records")
    }

    /// Read and merge multiple source files for compaction
    pub async fn read_and_merge_source_files(
        &self,
        source_files: &[String],
        ...
        for source_file in source_files {
            ...
            let records: Vec<VectorRecord> =
                bincode::deserialize(&data).context("Failed to deserialize source file")?;

            all_records.extend(records);
        }

        tracing::debug!(
            "✅ Merged {} total records from {} source files",
            all_records.len(),
            source_files.len()
        );
        Ok(all_records)
