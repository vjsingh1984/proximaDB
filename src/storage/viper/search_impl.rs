...
use crate::storage::search::{
    ExecutionStrategy, FilterCondition, HybridSearchStrategy, IndexInfo, IndexSpec, IndexUsage,
    PushdownOperation, SearchCost, SearchEngine, SearchOperation, SearchPlan, SearchResult,
    SearchStats,
};
...
impl ViperSearchEngine {
    ...
    async fn analyze_filter_pushdown(
        ...
        tracing::debug!(
            "🔍 VIPER SEARCH: Analyzing filter pushdown for collection {}",
            collection_id
        );
        ...
        for (field_name, condition) in filters {
            if filterable_fields.contains(&field_name.as_str()) {
                tracing::debug!(
                    "📊 VIPER SEARCH: Pushing down filter for field: {}",
                    field_name
                );
        ...
        tracing::info!(
            "🎯 VIPER SEARCH: Filter analysis complete - {} pushdown, {} remaining",
            pushdown_ops.len(),
            remaining_filters.len()
        );
    ...
    fn estimate_viper_cost<'a>(
        ...
        Box::pin(async move {
            tracing::debug!(
                "💰 VIPER SEARCH: Estimating cost for operation: {:?}",
                operation
            );
            ...
            let cost = match operation {
                ...
                SearchOperation::MetadataFilter { filters, limit } => {
                    ...
                    let result_count =
                        limit.unwrap_or((scan_cost as usize).min(estimated_vector_count));
                ...
                SearchOperation::HybridSearch {
                    ...
                    let filter_cost = self
                        .estimate_viper_cost(
                            collection_id,
                            &SearchOperation::MetadataFilter {
                    ...
                    let search_cost = self
                        .estimate_viper_cost(
                            collection_id,
                            &SearchOperation::SimilaritySearch {
                    ...
                    match strategy {
                        HybridSearchStrategy::FilterFirst => SearchCost {
                            ...
                            network_cost: 0.0,
                            total_estimated_ms: filter_cost.total_estimated_ms
                                + search_cost.total_estimated_ms * 0.1,
                            cardinality_estimate: *k,
                        },
                        _ => {
                            ...
                            SearchCost {
                                ...
                                network_cost: 0.0,
                                total_estimated_ms: filter_cost.total_estimated_ms
                                    + search_cost.total_estimated_ms,
                                cardinality_estimate: *k,
                            }
                        }
                    }
                }
                SearchOperation::RangeQuery { limit, .. } => SearchCost {
...
impl SearchEngine for ViperSearchEngine {
    ...
    async fn create_plan(
        ...
        tracing::info!(
            "📋 VIPER SEARCH: Creating execution plan for collection: {}",
            collection_id
        );
        ...
        let (execution_strategy, index_usage, pushdown_operations) = match &operation {
            SearchOperation::ExactLookup { vector_id } => {
                tracing::info!(
                    "🎯 VIPER SEARCH: Planning exact lookup for vector: {}",
                    vector_id
                );
                (
            ...
            SearchOperation::SimilaritySearch {
                ...
                tracing::info!(
                    "🔍 VIPER SEARCH: Planning similarity search with {} algorithm, k={}",
                    algorithm,
                    k
                );
                (
            ...
            SearchOperation::MetadataFilter { filters, limit } => {
                tracing::info!(
                    "📊 VIPER SEARCH: Planning metadata filter with {} conditions",
                    filters.len()
                );

                let (pushdown_ops, _remaining_filters) =
                    self.analyze_filter_pushdown(collection_id, filters).await?;
            ...
            SearchOperation::HybridSearch {
                ...
                tracing::info!(
                    "🔀 VIPER SEARCH: Planning hybrid search with strategy: {:?}",
                    strategy
                );

                let (filter_pushdown, _) =
                    self.analyze_filter_pushdown(collection_id, filters).await?;
            ...
            SearchOperation::RangeQuery { limit, .. } => {
                tracing::info!(
                    "📄 VIPER SEARCH: Planning range query with limit: {}",
                    limit
                );
                (
                    ExecutionStrategy::IndexScan {
        ...
        tracing::info!(
            "⚡ VIPER SEARCH: Plan created in {:?} - Strategy: {:?}, Pushdown ops: {}",
            planning_time,
            execution_strategy,
            pushdown_operations.len()
        );
    ...
    async fn execute_search(
        ...
        tracing::info!(
            "🚀 VIPER SEARCH: Executing search plan for collection: {}",
            collection_id
        );
        tracing::debug!(
            "📋 VIPER SEARCH: Plan strategy: {:?}",
        ...
        let results = match &plan.execution_strategy {
            ExecutionStrategy::IndexScan { index_name } => {
                tracing::info!(
                    "📇 VIPER SEARCH: Executing index scan using: {}",
                    index_name
                );
            ...
            ExecutionStrategy::VectorIndexScan { algorithm } => {
                tracing::info!(
                    "🔍 VIPER SEARCH: Executing vector index scan with algorithm: {}",
                    algorithm
                );
            ...
            ExecutionStrategy::IndexIntersection { indexes } => {
                tracing::info!(
                    "🔗 VIPER SEARCH: Executing index intersection with indexes: {:?}",
                    indexes
                );
            ...
            ExecutionStrategy::Hybrid { strategies } => {
                tracing::info!(
                    "🔀 VIPER SEARCH: Executing hybrid strategy with {} sub-strategies",
                    strategies.len()
                );
        ...
        tracing::info!(
            "✅ VIPER SEARCH: Search completed in {:?} - {} results, {} rows scanned",
            execution_time,
            results.len(),
            stats.rows_scanned
        );
        ...
        if stats.execution_time_ms > 100.0 {
            tracing::warn!(
                "🐌 VIPER SEARCH: Slow query detected - {}ms execution time",
        ...
        if stats.rows_scanned > stats.rows_returned * 10 {
            tracing::warn!(
                "📊 VIPER SEARCH: Inefficient scan ratio - {:.1}x rows scanned vs returned",
    ...
    async fn get_search_stats(
        ...
        tracing::debug!(
            "📊 VIPER SEARCH: Getting search stats for collection: {}",
            collection_id
        );
    ...
    async fn update_search_stats(
        ...
        tracing::debug!(
            "📈 VIPER SEARCH: Updating search stats for collection: {}",
            collection_id
        );
        tracing::info!(
            "📊 VIPER SEARCH: Operation {} took {:.2}ms, {}/{} rows",
    ...
    async fn list_indexes(&self, collection_id: &CollectionId) -> Result<Vec<IndexInfo>> {
        tracing::debug!(
            "📇 VIPER SEARCH: Listing indexes for collection: {}",
            collection_id
        );
    ...
    async fn create_index(
        ...
        tracing::info!(
            "🔧 VIPER SEARCH: Creating index '{}' of type '{}' for collection: {}",
            index_spec.name,
    ...
    async fn drop_index(&self, collection_id: &CollectionId, index_name: &str) -> Result<()> {
        tracing::info!(
            "🗑️ VIPER SEARCH: Dropping index '{}' for collection: {}",
            index_name,
            collection_id
        );
    ...
    async fn explain_plan(
        ...
        tracing::info!(
            "📝 VIPER SEARCH: Explaining plan for collection: {}",
            collection_id
        );
        ...
        tracing::info!(
            "📋 VIPER SEARCH: Plan explanation generated ({} chars)",
            explanation.len()
        );
        Ok(explanation)
    }
