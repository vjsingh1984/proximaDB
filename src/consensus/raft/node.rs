...
impl RaftNode {
    ...
    pub fn record_proposal_success(&mut self, latency_ms: f64) {
        ...
        self.metrics.avg_proposal_latency_ms = 
            (self.metrics.avg_proposal_latency_ms * (total_proposals - 1) as f64 + latency_ms) / total_proposals as f64;
    }
    ...
    fn get_health_message(&self) -> String {
        match &self.state {
            NodeState::Leader { followers, .. } => {
                ...
                format!("Leader with {}/{} active followers", active_followers, total_followers)
            }
            NodeState::Follower { leader_id, last_heartbeat } => {
            ...
            NodeState::Candidate { votes_received, votes_needed, election_start } => {
                let election_duration = election_start.elapsed();
                format!("Candidate with {}/{} votes, election running for {:?}", 
