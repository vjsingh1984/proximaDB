...
impl LogReplication {
    ...
    pub async fn initialize_replication(&self) -> Result<()> {
        ...
        if !node.is_leader() {
            return Err(ProximaDBError::Consensus(
                crate::core::ConsensusError::Raft("Cannot initialize replication - not a leader".to_string())
            ));
        }
        ...
        for &peer_id in peers.keys() {
            follower_states.insert(peer_id, FollowerReplicationState {
    ...
    pub async fn replicate_entries(&self, entries: &[LogEntry]) -> Result<()> {
        ...
        for (&peer_id, address) in peers.iter() {
            self.send_entries_to_follower(peer_id, address, entries).await?;
        }
    ...
    pub async fn handle_append_response(
        ...
        if let Some(state) = follower_states.get_mut(&follower_id) {
            ...
            if success {
                ...
                tracing::debug!("Replication to follower {} succeeded, match_index: {:?}", 
                ...
                tracing::warn!("Replication to follower {} failed, backing off to index {}", 
                          follower_id, state.next_index);
                
                // If too many failures, mark as unreachable
                if state.retry_count > 5 {
                    state.is_reachable = false;
                    tracing::error!("Follower {} marked as unreachable after {} failures", 
                               follower_id, state.retry_count);
                }
            }
        }
    ...
    pub async fn get_replication_status(&self) -> HashMap<u64, ReplicationStatus> {
        ...
        for (&follower_id, state) in follower_states.iter() {
            status.insert(follower_id, ReplicationStatus {
                follower_id,
    ...
    async fn send_entries_to_follower(
        ...
        if let Some(state) = follower_states.get_mut(&follower_id) {
            ...
            tracing::debug!("Sending {} entries to follower {} at {}", 
                       batch.len(), follower_id, address);
    ...
    async fn send_heartbeat_to_follower(&self, follower_id: u64, address: &str) -> Result<()> {
        ...
        self.send_entries_to_follower(follower_id, address, &[]).await
    }
