...
use crate::core::{ConsensusConfig, VectorDBError};
...
pub use message::{RaftCommand, RaftMessage, RaftResponse};
...
impl RaftConsensusEngine {
    ...
    pub async fn new(consensus_config: ConsensusConfig, node_id: u64) -> Result<Self> {
        ...
        let raw_node = RawNode::new(&raft_config, storage.clone(), &logger).map_err(|e| {
            ProximaDBError::Consensus(crate::core::ConsensusError::Raft(format!(
    ...
    pub async fn deactivate(&mut self) -> Result<()> {
        ...
        *is_active = false;
        ...
        tracing::info!(
            "Raft consensus engine deactivated for node {}",
            self.node_id
        );
        Ok(())
    }
    ...
    pub async fn propose(&self, command: RaftCommand) -> Result<RaftResponse> {
        if !*self.is_active.read().await {
            return Err(ProximaDBError::Consensus(
                crate::core::ConsensusError::Raft("Consensus engine not active".to_string()),
            ));
        }
        ...
        let data = serde_json::to_vec(&command).map_err(|e| {
            ProximaDBError::Storage(crate::core::StorageError::Serialization(format!(
        ...
        match tokio::time::timeout(Duration::from_secs(30), response_receiver).await {
            ...
            Ok(Err(_)) => Err(ProximaDBError::Consensus(
                crate::core::ConsensusError::Raft("Proposal response channel closed".to_string()),
            )),
            Err(_) => Err(ProximaDBError::Consensus(
                crate::core::ConsensusError::Raft("Proposal timeout".to_string()),
            )),
        }
    }
    ...
    pub async fn add_peer(&self, node_id: u64, address: String) -> Result<()> {
        ...
        if *self.is_active.read().await {
            ...
            raw_node.propose_conf_change(vec![], change).map_err(|e| {
                ProximaDBError::Consensus(crate::core::ConsensusError::Raft(format!(
                    "Failed to propose peer addition: {}",
                    e
                )))
            })?;
        }
        ...
        tracing::info!(
            "Added peer {} with address {}",
            node_id,
            self.peers.read().await.get(&node_id).unwrap()
        );
        Ok(())
    }
    ...
    pub async fn remove_peer(&self, node_id: u64) -> Result<()> {
        ...
        if *self.is_active.read().await {
            ...
            raw_node.propose_conf_change(vec![], change).map_err(|e| {
                ProximaDBError::Consensus(crate::core::ConsensusError::Raft(format!(
                    "Failed to propose peer removal: {}",
                    e
                )))
            })?;
        }
    ...
    fn create_raft_config(consensus_config: &ConsensusConfig, node_id: u64) -> Result<RaftConfig> {
        ...
        config
            .validate()
            .map_err(|e| ProximaDBError::Config(format!("Invalid Raft configuration: {}", e)))?;
