...
use raft::eraftpb::{SnapshotMetadata, ConfState};
...
impl SnapshotManager {
    ...
    pub async fn new<P: AsRef<Path>>(
        ...
        fs::create_dir_all(&snapshot_dir).await.map_err(|e| {
            ProximaDBError::Storage(crate::core::StorageError::DiskIO(e))
        })?;
        ...
        // Check if snapshot creation is already in progress
        {
            ...
            if *in_progress {
                return Err(ProximaDBError::Consensus(
                    crate::core::ConsensusError::Raft("Snapshot creation already in progress".to_string())
                ));
            }
            *in_progress = true;
        }
    ...
    async fn create_snapshot_internal(
        ...
        // Write to file
        fs::write(&file_path, &serialized_data).await.map_err(|e| {
            ProximaDBError::Storage(crate::core::StorageError::DiskIO(e))
        })?;
        ...
        tracing::info!("Created snapshot at index {} in file {:?}", 
                  applied_index, file_path);
    ...
    pub async fn load_snapshot(&self, snapshot_path: &Path) -> Result<SnapshotData> {
        ...
        tracing::info!("Loaded snapshot from {:?}, index: {}", 
                  snapshot_path, snapshot_data.applied_index);
    ...
    pub async fn install_snapshot(
        ...
        let filename = format!("installed_snapshot_{}_{}.db", 
                              metadata.index, metadata.term);
        let file_path = self.snapshot_dir.join(&filename);
        
        // Write snapshot data to file
        fs::write(&file_path, &snapshot_data).await.map_err(|e| {
            ProximaDBError::Storage(crate::core::StorageError::DiskIO(e))
        })?;
    ...
    async fn cleanup_old_snapshots(&self) -> Result<()> {
        ...
        tracing::info!("Cleaned up {} old snapshots", 
