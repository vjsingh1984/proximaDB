...
use super::{
    Alert, IndexMetrics, MetricsConfig, MetricsRateLimiter, QueryMetrics, ServerMetrics,
...
impl MetricsCollector {
    ...
    async fn collect_all_metrics(&self) -> Result<()> {
        ...
        // Add to history
        {
            ...
            let max_samples =
                (self.config.retention_hours * 3600) / self.config.collection_interval_seconds;
        ...
        let _ = self
            .event_sender
            .send(MetricsEvent::SystemMetricsUpdated(metrics));
    ...
    async fn process_alerts(&self, new_alerts: Vec<Alert>) -> Result<()> {
        ...
        for alert in new_alerts {
            ...
            match existing {
                ...
                None => {
                    ...
                    let _ = self
                        .event_sender
                        .send(MetricsEvent::AlertGenerated(alert.clone()));
                ...
                "memory_usage_percent" => {
                    let memory_percent = if current_metrics.server.memory_available_bytes > 0 {
                        (current_metrics.server.memory_usage_bytes as f64
                            / current_metrics.server.memory_available_bytes as f64)
                            * 100.0
                    } else {
                        0.0
                    };
        ...
        for alert_id in resolved_alerts {
            let _ = self
                .event_sender
                .send(MetricsEvent::AlertResolved(alert_id));
        }
    ...
    async fn cleanup_old_metrics(&self) {
        ...
        println!(
            "Cleaned up metrics history, {} samples remaining",
            history.len()
        );
    }
    ...
    pub async fn get_metrics_summary(&self) -> MetricsSummary {
        ...
        MetricsSummary {
            ...
            memory_usage_percent: if metrics.server.memory_available_bytes > 0 {
                (metrics.server.memory_usage_bytes as f64
                    / metrics.server.memory_available_bytes as f64)
                    * 100.0
            } else {
                0.0
            },
            disk_usage_percent: if metrics.server.disk_available_bytes > 0 {
                (metrics.server.disk_usage_bytes as f64
                    / (metrics.server.disk_usage_bytes + metrics.server.disk_available_bytes)
                        as f64)
                    * 100.0
            } else {
                0.0
