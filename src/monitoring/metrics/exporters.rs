...
use super::{Histogram, Metric, MetricType, MetricValue, Summary, SystemMetrics};
...
impl PrometheusExporter {
    ...
    fn format_metric_value(&self, metric: &Metric, value: &MetricValue) -> String {
        ...
        let labels = if value.labels.is_empty() {
            ...
            let label_str: Vec<String> = value
                .labels
                .iter()
                .map(|(k, v)| format!("{}=\"{}\"", k, v))
                .collect();
    ...
    fn format_histogram(
        ...
        for bucket in &histogram.buckets {
            let bucket_labels = if labels.is_empty() {
                ...
                let label_parts: Vec<String> = all_labels
                    .iter()
                    .map(|(k, v)| format!("{}=\"{}\"", k, v))
                    .collect();
                format!("{{{}}}", label_parts.join(","))
...
impl MetricsExporter for PrometheusExporter {
    ...
    fn export_system_metrics(&self, system_metrics: &SystemMetrics) -> Result<String> {
        ...
        writeln!(
            ...
            system_metrics.server.uptime_seconds
        )?;
        ...
        writeln!(
            ...
            system_metrics.server.cpu_usage_percent
        )?;

        writeln!(
            output,
            "# HELP proximadb_memory_usage_bytes Memory usage in bytes"
        )?;
        writeln!(output, "# TYPE proximadb_memory_usage_bytes gauge")?;
        writeln!(
            output,
            "proximadb_memory_usage_bytes {}",
            system_metrics.server.memory_usage_bytes
        )?;

        writeln!(
            output,
            "# HELP proximadb_memory_available_bytes Available memory in bytes"
        )?;
        writeln!(output, "# TYPE proximadb_memory_available_bytes gauge")?;
        writeln!(
            output,
            "proximadb_memory_available_bytes {}",
            system_metrics.server.memory_available_bytes
        )?;

        // Storage metrics
        writeln!(
            output,
            "# HELP proximadb_total_vectors Total number of vectors stored"
        )?;
        writeln!(output, "# TYPE proximadb_total_vectors gauge")?;
        writeln!(
            output,
            "proximadb_total_vectors {}",
            system_metrics.storage.total_vectors
        )?;

        writeln!(
            output,
            "# HELP proximadb_total_collections Total number of collections"
        )?;
        writeln!(output, "# TYPE proximadb_total_collections gauge")?;
        writeln!(
            output,
            "proximadb_total_collections {}",
            system_metrics.storage.total_collections
        )?;

        writeln!(
            output,
            "# HELP proximadb_storage_size_bytes Total storage size in bytes"
        )?;
        writeln!(output, "# TYPE proximadb_storage_size_bytes gauge")?;
        writeln!(
            output,
            "proximadb_storage_size_bytes {}",
            system_metrics.storage.storage_size_bytes
        )?;

        writeln!(
            output,
            "# HELP proximadb_cache_hit_rate Cache hit rate (0.0 to 1.0)"
        )?;
        writeln!(output, "# TYPE proximadb_cache_hit_rate gauge")?;
        writeln!(
            output,
            "proximadb_cache_hit_rate {}",
            system_metrics.storage.cache_hit_rate
        )?;

        // Query metrics
        writeln!(
            output,
            "# HELP proximadb_total_queries Total number of queries processed"
        )?;
        writeln!(output, "# TYPE proximadb_total_queries counter")?;
        writeln!(
            output,
            "proximadb_total_queries {}",
            system_metrics.query.total_queries
        )?;

        writeln!(
            output,
            "# HELP proximadb_queries_per_second Current queries per second"
        )?;
        writeln!(output, "# TYPE proximadb_queries_per_second gauge")?;
        writeln!(
            output,
            "proximadb_queries_per_second {}",
            system_metrics.query.queries_per_second
        )?;

        writeln!(
            output,
            "# HELP proximadb_query_latency_ms Query latency in milliseconds"
        )?;
        writeln!(output, "# TYPE proximadb_query_latency_ms gauge")?;
        writeln!(
            output,
            "proximadb_query_latency_ms{{quantile=\"0.5\"}} {}",
            system_metrics.query.p50_query_latency_ms
        )?;
        writeln!(
            output,
            "proximadb_query_latency_ms{{quantile=\"0.9\"}} {}",
            system_metrics.query.p90_query_latency_ms
        )?;
        writeln!(
            output,
            "proximadb_query_latency_ms{{quantile=\"0.99\"}} {}",
            system_metrics.query.p99_query_latency_ms
        )?;

        // Index metrics
        writeln!(
            output,
            "# HELP proximadb_total_indexes Total number of indexes"
        )?;
        writeln!(output, "# TYPE proximadb_total_indexes gauge")?;
        writeln!(
            output,
            "proximadb_total_indexes {}",
            system_metrics.index.total_indexes
        )?;

        writeln!(
            output,
            "# HELP proximadb_index_memory_usage_bytes Index memory usage in bytes"
        )?;
        writeln!(output, "# TYPE proximadb_index_memory_usage_bytes gauge")?;
        writeln!(
            output,
            "proximadb_index_memory_usage_bytes {}",
            system_metrics.index.index_memory_usage_bytes
        )?;
...
impl MetricsExporter for OpenTelemetryExporter {
    ...
    fn export_system_metrics(&self, system_metrics: &SystemMetrics) -> Result<String> {
        ...
        let metrics_data = vec![
            (
                "proximadb.uptime.seconds",
                system_metrics.server.uptime_seconds,
            ),
            (
                "proximadb.cpu.usage.percent",
                system_metrics.server.cpu_usage_percent,
            ),
            (
                "proximadb.memory.usage.bytes",
                system_metrics.server.memory_usage_bytes as f64,
            ),
            (
                "proximadb.storage.vectors.total",
                system_metrics.storage.total_vectors as f64,
            ),
            (
                "proximadb.storage.collections.total",
                system_metrics.storage.total_collections as f64,
            ),
            (
                "proximadb.queries.total",
                system_metrics.query.total_queries as f64,
            ),
            (
                "proximadb.queries.per_second",
                system_metrics.query.queries_per_second,
            ),
            (
                "proximadb.query.latency.p99.ms",
                system_metrics.query.p99_query_latency_ms,
            ),
        ];
...
impl MetricsFormatter {
    ...
    pub fn format_for_display(system_metrics: &SystemMetrics) -> String {
        ...
        writeln!(
            output,
            "ProximaDB System Metrics - {}",
            system_metrics.timestamp.format("%Y-%m-%d %H:%M:%S UTC")
        )
        .unwrap();
        ...
        writeln!(
            output,
            "  Memory Usage: {:.1} MB",
            system_metrics.server.memory_usage_bytes as f64 / 1024.0 / 1024.0
        )
        .unwrap();
        writeln!(
            output,
            "  Memory Available: {:.1} MB",
            system_metrics.server.memory_available_bytes as f64 / 1024.0 / 1024.0
        )
        .unwrap();
        ...
        writeln!(
            output,
            "  Storage Size: {:.1} MB",
            system_metrics.storage.storage_size_bytes as f64 / 1024.0 / 1024.0
        )
        .unwrap();
        writeln!(
            output,
            "  Cache Hit Rate: {:.1}%",
            system_metrics.storage.cache_hit_rate * 100.0
        )
        .unwrap();
        ...
        writeln!(
            output,
            "  Index Memory: {:.1} MB",
            system_metrics.index.index_memory_usage_bytes as f64 / 1024.0 / 1024.0
        )
        .unwrap();
