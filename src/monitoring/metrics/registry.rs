...
use super::{Histogram, HistogramBucket, Metric, MetricType, MetricValue, Summary};
...
impl MetricsRegistry {
    ...
    pub async fn register_histogram(
        ...
        let bucket_counters: Vec<Arc<RwLock<u64>>> =
            buckets.iter().map(|_| Arc::new(RwLock::new(0))).collect();
...
impl HistogramMetric {
    ...
    pub async fn get(&self) -> Histogram {
        ...
        let buckets = self.buckets.read().await;
        let histogram_buckets: Vec<HistogramBucket> = self
            .config
            .buckets
            .iter()
            .enumerate()
            .map(|(i, &upper_bound)| {
                let bucket_count = if let Some(bucket) = buckets.get(i) {
                    futures::executor::block_on(async { *bucket.read().await })
                } else {
                    0
                };
