...
impl ServerMetricsCollector {
    ...
    pub async fn new(registry: Arc<MetricsRegistry>) -> Result<Self> {
        ...
        let uptime_counter = registry.register_counter(
            "proximadb_uptime_seconds",
            "Server uptime in seconds",
            std::collections::HashMap::new(),
        ).await?;
        
        let cpu_usage_gauge = registry.register_gauge(
            "proximadb_cpu_usage_percent",
            "CPU usage percentage",
            std::collections::HashMap::new(),
        ).await?;
        
        let memory_usage_gauge = registry.register_gauge(
            "proximadb_memory_usage_bytes",
            "Memory usage in bytes",
            std::collections::HashMap::new(),
        ).await?;
        
        let memory_available_gauge = registry.register_gauge(
            "proximadb_memory_available_bytes",
            "Available memory in bytes",
            std::collections::HashMap::new(),
        ).await?;
        
        let disk_usage_gauge = registry.register_gauge(
            "proximadb_disk_usage_bytes",
            "Disk usage in bytes",
            std::collections::HashMap::new(),
        ).await?;
        
        let disk_available_gauge = registry.register_gauge(
            "proximadb_disk_available_bytes",
            "Available disk space in bytes",
            std::collections::HashMap::new(),
        ).await?;
        
        let network_bytes_in_counter = registry.register_counter(
            "proximadb_network_bytes_in_total",
            "Total bytes received from network",
            std::collections::HashMap::new(),
        ).await?;
        
        let network_bytes_out_counter = registry.register_counter(
            "proximadb_network_bytes_out_total",
            "Total bytes sent to network",
            std::collections::HashMap::new(),
        ).await?;
        
        let open_connections_gauge = registry.register_gauge(
            "proximadb_open_connections",
            "Number of open connections",
            std::collections::HashMap::new(),
        ).await?;
        
        let active_requests_gauge = registry.register_gauge(
    ...
    pub async fn update_system_metrics(
        ...
        self.memory_available_gauge.set(memory_available as f64).await;
        ...
        if network_bytes_in > current_in as u64 {
            self.network_bytes_in_counter.add((network_bytes_in - current_in as u64) as f64).await;
        }
        if network_bytes_out > current_out as u64 {
            self.network_bytes_out_counter.add((network_bytes_out - current_out as u64) as f64).await;
        }
