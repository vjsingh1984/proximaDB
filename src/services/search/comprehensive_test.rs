...
mod tests {
    ...
    use crate::compute::distance_computation::engine::{DistanceMode, UnifiedDistanceCompute};
    ...
    fn create_test_vectors() -> Vec<TestVector> {
        ...
        for i in 0..100 {
            ...
            metadata.insert(
                "source".to_string(),
                if i % 3 == 0 { "system" } else { "user" }.to_string(),
            );
    ...
    async fn test_hardware_backend_selection() -> Result<()> {
        ...
        assert!(
            backend_priority.is_some(),
            "Selected backend should be in preference list"
        );

        // Verify the backend is actually available
        assert!(
            available.contains_hash(&backend),
    ...
    async fn test_distance_computation_consistency() -> Result<()> {
        ...
        for metric in metrics {
            ...
            for mode in [
                ...
                debug!(
                    "  Mode {:?}: raw={:.4}, normalized={:.4}, rank={:.4}",
                ...
                assert!(
                    !result.normalized_score.is_nan(),
                    "Normalized score should not be NaN"
                );
                assert!(
            ...
            let batch_vectors: Vec<&[f32]> = test_vectors[1..6]
                .iter()
                .map(|v| v.vector.as_slice())
                .collect();

            let batch_results =
                distance_compute.calculate_distance_batch(query, &batch_vectors, &metric);
            assert_eq!(batch_results.len(), 5, "Batch should return 5 results");

            for (i, result) in batch_results.iter().enumerate() {
                assert!(
                    !result.raw_value.is_nan(),
                    "Batch result {} should not be NaN",
                    i
                );
                assert!(
                    result.normalized_score >= 0.0 && result.normalized_score <= 1.0,
    ...
    async fn test_viper_comprehensive_search() -> Result<()> {
        ...
        for test_case in test_cases.iter().take(6) {
            ...
            assert!(
                !sample_distance.raw_value.is_nan(),
                "Distance should not be NaN"
            );
            assert!(
                sample_distance.normalized_score >= 0.0 && sample_distance.normalized_score <= 1.0,
                "Normalized score should be in [0, 1]"
            );

            debug!(
                "  ✅ Case {} passed with backend {}",
                test_case.name, backend
            );
        }
    ...
    async fn test_wal_unflushed_vector_search() -> Result<()> {
        ...
        for metric in metrics {
            ...
            for wal_vec in wal_vectors.iter().take(5) {
                let distance_result =
                    distance_compute.calculate_distance(query_vector, &wal_vec.vector, &metric);

                // Verify results are valid
                assert!(
                    !distance_result.raw_value.is_nan(),
                    "WAL distance should not be NaN"
                );
                assert!(
            ...
            let batch_results =
                distance_compute.calculate_distance_batch(query_vector, &mixed_vectors, &metric);
    ...
    async fn test_wal_comprehensive_operators_and_metrics() -> Result<()> {
        debug!(
            "🚀 Testing WAL unflushed vectors with ALL operators, metrics, and accelerated platforms..."
        );
            ...
            for (op_name, operator) in &all_operators {
                ...
                let filtered_wal_vectors = match operator {
                    ...
                    QueryOperator::Not => {
                        ...
                        wal_vectors
                            .iter()
                            .filter(|v| v.metadata.get(key) != Some(&"system".to_string()))
                            .cloned()
                            .collect()
                    }
                };
                ...
                debug!(
                    "  📊 Filtered to {} WAL vectors",
                    filtered_wal_vectors.len()
                );
                ...
                for wal_vec in filtered_wal_vectors.iter().take(5) {
                    ...
                    assert!(
                        !distance_result.raw_value.is_nan(),
                        "WAL distance should not be NaN for {} with {}",
                        test_name,
                        wal_vec.id
                    );
                    assert!(
                        distance_result.normalized_score >= 0.0
                            && distance_result.normalized_score <= 1.0,
                        "WAL normalized score should be in [0, 1] for {} with {}",
                        test_name,
                        wal_vec.id
                    );
                    assert_eq!(distance_result.metric, *metric, "Metric should match");
                ...
                if filtered_wal_vectors.len() > 1 {
                    let batch_vectors: Vec<&[f32]> = filtered_wal_vectors
                        .iter()
                        .take(8)
                        .map(|v| v.vector.as_slice())
                        .collect();
                    ...
                    for (i, result) in batch_results.iter().enumerate() {
                        assert!(
                            !result.raw_value.is_nan(),
                            "Batch result {} should not be NaN for {}",
                            i,
                            test_name
                        );
                        assert!(
                            result.normalized_score >= 0.0 && result.normalized_score <= 1.0,
                            "Batch normalized score {} should be in [0, 1] for {}",
                            i,
                            test_name
                        );
                    ...
                    debug!(
                        "    📈 Batch performance: {:.0} vectors/sec",
                        vectors_per_sec
                    );
                    ...
                    assert!(
                        ...
                        "Hardware acceleration should achieve at least 50 vectors/sec for {}",
                        test_name
                    );
                }
                ...
                for mode in [
                    ...
                    if let Some((test_vec, _, _)) = individual_results.first() {
                        ...
                        assert!(
                            !mode_result.raw_value.is_nan(),
                            "Mode {:?} result should not be NaN for {}",
                            mode,
                            test_name
                        );
                        assert!(
                            mode_result.normalized_score >= 0.0
                                && mode_result.normalized_score <= 1.0,
                            "Mode {:?} normalized score should be in [0, 1] for {}",
                            mode,
                            test_name
                        );
                    }
                ...
                let avg_individual_time = individual_results
                    .iter()
                    .map(|(_, _, time)| time.as_nanos())
                    .sum::<u128>() as f64
                    / individual_results.len() as f64;

                let avg_score = individual_results
                    .iter()
                    .map(|(_, result, _)| result.normalized_score)
                    .sum::<f32>()
                    / individual_results.len() as f32;
                ...
                debug!(
                    "  ✅ {} passed: {} vectors, avg_time={:.1}ns, avg_score={:.3}",
                    test_name,
                    individual_results.len(),
                    avg_individual_time,
                    avg_score
        ...
        assert!(
            test_results.len() >= expected_combinations / 2,
    ...
    async fn test_wal_hardware_backend_consistency() -> Result<()> {
        ...
        let gpu_modes = vec![("GPU_ENABLED", true), ("GPU_DISABLED", false)];
        ...
        for (mode_name, gpu_enabled) in gpu_modes {
            ...
            for wal_vec in &wal_vectors {
                ...
                assert!(
                    !distance_result.raw_value.is_nan(),
                    "Distance should not be NaN in {} mode",
                    mode_name
                );
                assert!(
                    distance_result.normalized_score >= 0.0
                        && distance_result.normalized_score <= 1.0,
                    "Normalized score should be in [0, 1] in {} mode",
                    mode_name
                );
            ...
            let batch_vectors: Vec<&[f32]> =
                wal_vectors.iter().map(|v| v.vector.as_slice()).collect();
            let batch_results = distance_compute.calculate_distance_batch(
                query_vector,
                &batch_vectors,
                &test_metric,
            ...
            assert_eq!(
                batch_results.len(),
                wal_vectors.len(),
                "Batch should return all results in {} mode",
                mode_name
            );
        ...
        if results_by_mode.len() == 2 {
            ...
            for ((gpu_id, gpu_result), (cpu_id, cpu_result)) in
                gpu_results.iter().zip(cpu_results.iter())
            {
                assert_eq!(gpu_id, cpu_id, "Vector IDs should match");

                // Skip comparison if either result is infinity or NaN
                if gpu_result.raw_value.is_infinite()
                    || cpu_result.raw_value.is_infinite()
                    || gpu_result.raw_value.is_nan()
                    || cpu_result.raw_value.is_nan()
                {
                    debug!(
                        "  ⚠️ Vector {}: Skipping comparison due to infinity/NaN values",
                        gpu_id
                    );
                    continue;
                }
                ...
                assert!(
                    ...
                    distance_diff,
                    gpu_result.raw_value,
                    cpu_result.raw_value
                );
                assert!(
                    score_diff < 0.01,
                    "Score difference should be small: {} (GPU: {:.4}, CPU: {:.4})",
                    score_diff,
                    gpu_result.normalized_score,
                    cpu_result.normalized_score
                );

                debug!(
                    "  ✅ Vector {}: distance_diff={:.6}, score_diff={:.6}",
            ...
            for (i, (gpu_result, cpu_result)) in gpu_batch.iter().zip(cpu_batch.iter()).enumerate()
            {
                ...
                {
                    debug!(
                        "  ⚠️ Batch result {}: Skipping comparison due to infinity/NaN values",
                        i
                    );
                    continue;
                }
                ...
                let batch_score_diff =
                    (gpu_result.normalized_score - cpu_result.normalized_score).abs();

                assert!(
                    batch_distance_diff < 0.01,
                    "Batch distance difference should be small"
                );
                assert!(
                    batch_score_diff < 0.01,
                    "Batch score difference should be small"
                );
            }
        }
        ...
        for (op_name, operator) in operator_tests {
            for metric in &distance_metrics {
                ...
                for test_vec in filtered_vectors.iter().take(10) {
                    let distance_result =
                        distance_compute.calculate_distance(query_vector, &test_vec.vector, metric);
                ...
                if filtered_vectors.len() > 1 {
                    let batch_vectors: Vec<&[f32]> = filtered_vectors
                        .iter()
                        .take(5)
                        .map(|v| v.vector.as_slice())
                        .collect();
        ...
        for &dim in &vector_dimensions {
            for metric in &metrics {
                ...
                for &batch_size in &batch_sizes {
                    ...
                    let results =
                        distance_compute.calculate_distance_batch(&query, &vector_refs, metric);
                    ...
                    for (i, result) in results.iter().enumerate() {
                        ...
                        assert!(
                            result.normalized_score >= -0.0001 && result.normalized_score <= 1.0001,
                            "Normalized score at index {} = {} should be in [0, 1] (with tolerance)",
                            i,
                            result.normalized_score
                        );
                    }
                    ...
                    assert!(
                        ...
                        "Should process at least 100 vectors/sec"
                    );
                }
            }
        }
    ...
    async fn test_full_integration() -> Result<()> {
        ...
        let operators = vec![QueryOperator::Simple, QueryOperator::And, QueryOperator::Or];
        ...
        debug!(
            "📊 Integration test results: {}/{} passed ({:.1}%)",
        ...
        assert!(
            success_rate >= 90.0,
            "Integration test success rate should be at least 90%"
        );
