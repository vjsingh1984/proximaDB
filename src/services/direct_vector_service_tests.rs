//! Comprehensive targeted tests for DirectVectorService to improve coverage from 43.9% to 60%
//!
//! These tests focus on uncovered code paths and edge cases in DirectVectorService,
...
mod tests {
    ...
    use crate::services::direct_vector_service::{DirectVectorService, OptimizedFormat, WorkloadType};
    ...
    /// Create test environment for DirectVectorService
    async fn create_test_service() -> (DirectVectorService, TempDir) {
        ...
        let service = DirectVectorService::new(write_buffer_config, viper_engine, sst_engine)
            .await
            .expect("Failed to create DirectVectorService");

        (service, temp_dir)
    }

    /// Create test service with specific format
    async fn create_test_service_with_format(format: OptimizedFormat) -> (DirectVectorService, TempDir) {
        let temp_dir = TempDir::new().expect("Failed to create temp directory");
        ...
        let service = DirectVectorService::with_format(write_buffer_config, viper_engine, sst_engine, format)
            .await
            .expect("Failed to create DirectVectorService with format");
    ...
    async fn test_service_creation_with_workload_hints() {
        ...
        let service_write = DirectVectorService::with_workload_hint(
            write_buffer_config.clone(),
            viper_engine.clone(),
            sst_engine.clone(),
        ...
        let service_read = DirectVectorService::with_workload_hint(
            write_buffer_config.clone(),
            viper_engine.clone(),
            sst_engine.clone(),
        ...
        let service_schema = DirectVectorService::with_workload_hint(
            write_buffer_config.clone(),
            viper_engine.clone(),
            sst_engine.clone(),
        ...
        let service_balanced = DirectVectorService::with_workload_hint(
            write_buffer_config.clone(),
            viper_engine.clone(),
            sst_engine,
    ...
    async fn test_service_creation_with_format_override() {
        ...
        let service = DirectVectorService::with_workload_hint(
            write_buffer_config,
            viper_engine,
            sst_engine,
    ...
    async fn test_service_component_access() {
        ...
        let write_buffer = service.get_write_buffer_behavior_wrapper();
