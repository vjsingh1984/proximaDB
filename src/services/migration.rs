...
impl MigrationService {
    ...
    pub async fn migrate_collection_strategy(
        &self,
        request: StrategyMigrationRequest,
    ) -> Result<StrategyMigrationResponse> {
        tracing::info!(
            "🔄 Starting strategy migration for collection: {}",
        ...
        self.metadata_store
            .update_collection(&request.collection_id, metadata)
            .await
            .context("Failed to update collection metadata")?;

        tracing::info!(
            "✅ Strategy migration completed for collection: {} (change_id: {})",
            request.collection_id,
            change_id
        );
...
impl StrategyTemplates {
    ...
    pub fn viper_optimized() -> CollectionStrategyConfig {
        let mut config = CollectionStrategyConfig::default();
        config.indexing_config.algorithm = IndexingAlgorithm::PQ { m: 16, nbits: 8 };
