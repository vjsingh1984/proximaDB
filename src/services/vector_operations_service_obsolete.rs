...
//! ⚠️ OBSOLETE: Direct Vector Service - OLD VERSION (DEPRECATED) ⚠️
//!
//! THIS FILE IS DEPRECATED AND WILL BE REMOVED IN THE NEXT RELEASE
//! 
//! This version uses SEPARATE search and filter optimizers resulting in:
//! - 650+ lines of duplicate optimization code
//! - No cross-system optimization capabilities
//! - Manual coordination overhead between systems
//! - 15-25% worse performance for combined queries
//!
//! The NEW vector_operations_service.rs provides:
//! ✅ Single UnifiedQueryOptimizer for all queries
//! ✅ Cross-system optimization with filter pushdown
//! ✅ Combined execution steps (CombinedFilterSearch)
//! ✅ 39% less code with enhanced functionality
//!
//! See: docs/unified_query_optimizer_migration.md for migration guide
//!
//! Original: Eliminates WAL Manager Registry overhead by providing direct access to the global memtable.
...
// Query planner removed - optimization happens in search_vectors method
use crate::query::unified_search_optimizer::{UnifiedSearchOptimizer, OptimizerConfig, OptimizationGoal, SearchContext};
...
pub struct VectorOperationsService {
    ...
    search_optimizer: Arc<UnifiedSearchOptimizer>,
    
    /// Collection cache - shared by optimizer and AXIS (read-only access)
    /// This is THE single cache for collection metadata in this node
...
impl VectorOperationsService {
    ...
    pub async fn with_collection_service_and_workload_hint(
        ...
        // Initialize unified search optimizer - single source of truth for all optimization
        let search_optimizer = Arc::new(UnifiedSearchOptimizer::new(OptimizerConfig::default()));
        info!("🎯 Unified Search Optimizer initialized for all query optimization");
                    ...
                    if let Some(quant_config) = &config.quantization {
                        if quant_config.enabled {
                            ...
                            for (i, vector) in vectors.iter().enumerate() {
                                if vector.id.is_empty() || vector.id.as_ref().map_or(true, |id| id.is_empty()) {
                            ...
                            debug!("✅ ID validation passed for quantized collection {}", collection_id);
                        }
                    }
                }
            }
    ...
    pub async fn search_vectors(
        ...
        // Get collection from cache or fetch and cache it
        let collection_proto = self.get_cached_collection_or_none(collection_id).await;
        ...
        // Get collection stats for optimization
        let (collection_size, _) = self.global_memtable.inner()
            .get_collection_stats(collection_id)
            .await;
        
        // Get available files for this collection
        let available_files = self.get_collection_files(collection_id).await.unwrap_or_default();
        
        // Determine optimization goal from search params or use default
        let optimization_goal = search_params
            .and_then(|p| p.runtime_hints.as_ref())
            .map(|h| match h.goal {
                crate::query::unified_search_optimizer::OptimizationGoal::MaximizeRecall => OptimizationGoal::MaximizeRecall,
                crate::query::unified_search_optimizer::OptimizationGoal::MaximizeSpeed => OptimizationGoal::MaximizeSpeed,
                crate::query::unified_search_optimizer::OptimizationGoal::MinimizeMemory => OptimizationGoal::MinimizeMemory,
                crate::query::unified_search_optimizer::OptimizationGoal::MinimizeLatency => OptimizationGoal::MinimizeLatency,
                crate::query::unified_search_optimizer::OptimizationGoal::MaximizeThroughput => OptimizationGoal::MaximizeThroughput,
                crate::query::unified_search_optimizer::OptimizationGoal::Balanced => OptimizationGoal::Balanced,
            })
            ;
        
        // Create search context for optimizer (using cached collection)
        let search_context = SearchContext {
            collection: collection_proto.as_ref()
                .cloned()
                .unwrap_or_else(|| Arc::new(Default::default())),
            search_params: &search_params.cloned().unwrap_or_default(),
            optimization_goal,
            available_files,
            total_vectors: collection_size,
            query_vectors: Some(&[query_vector.to_vec()]),
        };
        
        // Ensure quantization engine exists for this collection
        if let Some(collection) = &collection_proto {
            self.search_optimizer.ensure_quantization_engine(collection_id, collection);
        }
        
        // Get optimized strategy from unified optimizer
        let optimization_strategy = self.search_optimizer
        ...
        let enable_two_stage = optimization_strategy.quantization
            .as_ref()
            .map(|q| q.use_two_stage)
            ;
        
        if enable_two_stage {
            debug!("🎯 Optimizer enabled two-stage search with quantization");
        }
    ...
    /// Get cached collection or fetch from collection service
    ...
    async fn get_cached_collection_or_none(&self, collection_id: &str) -> Option<Arc<crate::proto::proximadb::Collection>> {
        ...
        // Fetch from collection service if not cached
        if let Some(collection_service) = &self.collection_service {
            if let Ok(Some(collection)) = collection_service.get_proto_collection(collection_id).await {
                let arc_collection = Arc::new(collection);
                self.collection_cache.insert(collection_id.to_string(), arc_collection.clone());
                debug!("📋 Cached collection {} in VectorOperationsService", collection_id);
                return Some(arc_collection);
            }
        }
    ...
    /// Get cached collection or error
    pub async fn get_cached_collection(&self, collection_id: &str) -> Result<Arc<crate::proto::proximadb::Collection>> {
        self.get_cached_collection_or_none(collection_id)
            .await
            .ok_or_else(|| anyhow::anyhow!("Collection {} not found", collection_id))
    }
    
    /// Invalidate collection cache entry (e.g., after collection update)
    pub fn invalidate_collection_cache(&self, collection_id: &str) {
        self.collection_cache.remove(collection_id);
        debug!("🗑️ Invalidated cache for collection {}", collection_id);
    }
    
    /// Get collection cache for read-only access by AXIS or other components
    pub fn get_collection_cache_handle(&self) -> Arc<dashmap::DashMap<String, Arc<crate::proto::proximadb::Collection>>> {
        self.collection_cache.clone()
    }
    
    /// Get collection files for optimization decisions
    async fn get_collection_files(&self, collection_id: &str) -> Result<Vec<String>> {
        // Get storage engine for collection
        let engine_name = self.get_storage_engine(collection_id).await?;
        
        // Get files based on engine
        let files = match engine_name {
            "VIPER" => {
                // Get VIPER files for collection
                vec![format!("data/{}/parquet", collection_id)]  // Simplified for now
            }
            "SST" | "LSM" => {
                // Get SST files for collection  
                vec![format!("data/{}/sst", collection_id)]  // Simplified for now
            }
            _ => vec![],
        };
    ...
    */
    ...
    fn estimate_batch_size(&self, vectors: &[VectorRecord]) -> usize {
        vectors.len() * (
            ...
            vectors.first().map(|v| v.vector.len() * 4) + // f32 = 4 bytes
    ...
    /// Execute SQL query by converting to vector operations
    /// 
    /// SQL queries are translated to vector operations and then optimized
    /// using the same unified optimizer as REST/gRPC paths for consistency
    pub async fn execute_sql_with_planner(
        &self,
        parsed_query: &crate::query::sql_engine::parser::ParsedQuery,
        collection_id: &str,
    ) -> Result<Vec<SearchResult>> {
        info!("🔍 Executing SQL query for collection: {}", collection_id);
        
        // Use SQL engine's planner to convert to execution plan
        let sql_planner = crate::query::sql_engine::planner::QueryPlanner::new();
        let execution_plan = sql_planner.create_plan(parsed_query.clone())
            .context("Failed to create SQL execution plan")?;
        
        // Convert SQL execution plan to search parameters
        if let Some(vector_search) = execution_plan.vector_search {
            // Parse distance metric from string
            let distance_metric = self.parse_distance_metric(&vector_search.metric)
                ;
            
            let search_params = SearchParams {
                query_vectors: Some(vec![vector_search.query_vector.clone()]),
                top_k: Some(vector_search.top_k),
                distance_metric: Some(distance_metric),
                filter_expression: execution_plan.metadata_filter.map(|f| f.expression),
                // SQL doesn't specify runtime hints, optimizer will use defaults
                runtime_hints: None,
                ..Default::default()
            };
            
            // Execute through unified search path - same optimizer as REST/gRPC
            self.search_vectors(
                collection_id,
        ...
        } else if let Some(metadata_filter) = execution_plan.metadata_filter {
            // Pure metadata query without vector search
            self.execute_metadata_only_query(
                collection_id,
                Some(&metadata_filter.expression),
                execution_plan.limit,
            ).await
        } else {
            // No filters or vector search - return all
            warn!("SQL query has no filters or vector search, returning empty results");
            Ok(vec![])
        }
    }
    
    /// Parse distance metric string from SQL - supports all 13 proto-defined metrics
    fn parse_distance_metric(&self, metric_str: &str) -> Option<DistanceMetric> {
        match metric_str.to_lowercase().replace("_", "").replace("-", "").as_str() {
            "cosine" | "cosinesimilarity" | "cosinedistance" => Some(DistanceMetric::Cosine),
            "euclidean" | "l2" | "l2norm" => Some(DistanceMetric::Euclidean),
            "dot" | "dotproduct" | "innerproduct" => Some(DistanceMetric::DotProduct),
            "manhattan" | "l1" | "l1norm" | "cityblock" => Some(DistanceMetric::Manhattan),
            "hamming" => Some(DistanceMetric::Hamming),
            "jaccard" | "jaccardindex" => Some(DistanceMetric::Jaccard),
            "chebyshev" | "linf" | "linfinity" | "chessboard" => Some(DistanceMetric::Chebyshev),
            "canberra" => Some(DistanceMetric::Canberra),
            "minkowski" => Some(DistanceMetric::Minkowski),
            "angular" | "angulardistance" => Some(DistanceMetric::Angular),
            "braycurtis" | "bray" => Some(DistanceMetric::BrayCurtis),
            "hellinger" => Some(DistanceMetric::Hellinger),
            "custom" => {
                warn!("Custom distance metric not supported in SQL queries");
                None
            }
            _ => {
                warn!("Unknown distance metric '{}', defaulting to cosine", metric_str);
                None
            }
        }
    }
...
impl VectorOperationsService {
    ...
    fn apply_multi_tier_deduplication(
        ...
        let requires_ordering = search_params
            ...
            ; // Default to requiring ordering for safety
        ...
        let final_results: Vec<SearchResult> = deduplicated.into_iter().enumerate().map(|(idx, candidate)| {
            SearchResult {
                ...
                similarity: candidate.vector_record.similarity.map(|d| d as f32),
