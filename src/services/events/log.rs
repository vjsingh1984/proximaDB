...
use crate::index::axis::eventlog::{
    EventLogService as EventLogServiceTrait,
...
impl EventLogService {
    ...
    pub async fn new(
        ...
        ).await.context("Failed to create EventLog service")?;
        ...
        match inner.service_mode() {
            ...
            ServiceMode::Standalone { ref bind_address, port } => {
                info!("EventLog service running in standalone mode on {}:{}", bind_address, port);
            }
            ServiceMode::Distributed { ref node_id, .. } => {
                info!("EventLog service running in distributed mode as node {}", node_id);
            }
        }
    ...
    pub async fn can_compact(&self, collection_id: &str, file_path: &str) -> bool {
        match self.inner.can_compact(collection_id, file_path).await {
            ...
            Err(e) => {
                ...
                debug!("Error checking compaction status: {}, allowing compaction_info", e);
                true
            }
        }
    }
    ...
    pub async fn notify_flush(
        ...
        debug!("Flush event successfully recorded for collection {}", collection_id);
        Ok(())
    }
    ...
    pub async fn cleanup_compacted_files(
        ...
        self.inner.cleanup_compacted(collection_id, deleted_files).await
    }
