...
mod tests {
    ...
    async fn test_concurrent_searches_no_interference() -> Result<()> {
        ...
        info!("✅ All {} searches completed. Max concurrent: {}", num_searches, max);
    ...
    async fn test_wal_scan_happens_once_per_search() -> Result<()> {
        ...
        for search_id in 0..5 {
            ...
            assert_eq!(count, 1, "Search {} should scan WAL exactly once", search_id);
        }
    ...
    async fn test_search_orchestration_order() -> Result<()> {
        ...
        assert!(tracker.verify_order().await, "Search phases should be in correct order");
