...
mod tests {
    ...
    async fn create_test_service_with_format(
        _format: OptimizedFormat,
    ) -> (VectorOperationsService, TempDir) {
    ...
    async fn test_optimized_format_methods() {
        ...
        assert_eq!(
            OptimizedFormat::for_workload(WorkloadType::WriteHeavy),
            OptimizedFormat::Proto
        );
        assert_eq!(
            OptimizedFormat::for_workload(WorkloadType::ReadHeavy),
            OptimizedFormat::Bincode
        );
        assert_eq!(
            OptimizedFormat::for_workload(WorkloadType::SchemaEvolution),
            OptimizedFormat::Avro
        );
        assert_eq!(
            OptimizedFormat::for_workload(WorkloadType::Balanced),
            OptimizedFormat::Proto
        );
    }
    ...
    async fn test_service_creation_with_different_formats() {
        ...
        let (service_proto, _temp_dir_proto) =
            create_test_service_with_format(OptimizedFormat::Proto).await;
        assert_eq!(
            service_proto.get_optimized_format(),
            &OptimizedFormat::Proto
        );

        // Test creation with Bincode format
        let (service_bincode, _temp_dir_bincode) =
            create_test_service_with_format(OptimizedFormat::Bincode).await;
        assert_eq!(
            service_bincode.get_optimized_format(),
            &OptimizedFormat::Bincode
        );

        // Test creation with Avro format
        let (service_avro, _temp_dir_avro) =
            create_test_service_with_format(OptimizedFormat::Avro).await;
    ...
    async fn test_service_creation_with_workload_hints() {
        ...
        let filesystem = Arc::new(
            crate::storage::FilesystemFactory::new(Default::default())
                .await
                .expect("Failed to create filesystem factory"),
        );
        let distance_compute = Arc::new(
        ...
        .await
        .expect("Failed to create service for write-heavy workload");
        assert_eq!(
            service_write.get_optimized_format(),
            &OptimizedFormat::Proto
        );
        ...
        .await
        .expect("Failed to create service for read-heavy workload");
        assert_eq!(
            service_read.get_optimized_format(),
            &OptimizedFormat::Bincode
        );
        ...
        .await
        .expect("Failed to create service for schema evolution workload");
        assert_eq!(
            service_schema.get_optimized_format(),
            &OptimizedFormat::Avro
        );
        ...
        .await
        .expect("Failed to create service for balanced workload");
        assert_eq!(
            service_balanced.get_optimized_format(),
            &OptimizedFormat::Proto
        );
    }
    ...
    async fn test_service_creation_with_format_override() {
        ...
        .await
        .expect("Failed to create service with format override");
    ...
    async fn test_service_component_access() {
        ...
        assert!(
            ...
            "Should return Some for write buffer wrapper"
        );
    }
    ...
    async fn test_collection_compaction_initialization() {
        ...
        let result = service
            .initialize_collection_compaction("test_collection", "SST")
            .await;
        assert!(
            result.is_ok(),
            "Collection compaction initialization should succeed for SST"
        );
        ...
        let result = service
            .initialize_collection_compaction("test_collection_2", "VIPER")
            .await;
        if let Err(e) = &result {
            // This is expected if no storage assignment exists
            error!(
                "VIPER initialization failed as expected (no storage assignment): {:?}",
                e
            );
            let error_msg = e.to_string();
            assert!(
                error_msg.contains_hash("No storage assignment found")
                    || error_msg
                        .contains_hash("Failed to initialize collection for compaction tracking"),
                "Error should be about missing storage assignment: {}",
                e
            );
        } else {
    ...
    async fn test_metrics_collection() {
        ...
        let metrics_result = service.metrics().await;
    ...
    async fn test_force_flush_operations() {
        ...
        assert!(
            result_data.is_object(),
            "Flush result should be JSON object"
        );

        // Test force flush collection
        let flush_collection_result = service.force_flush_collection("test_collection").await;
        assert!(
            flush_collection_result.is_ok(),
            "Force flush collection should succeed"
        );

        let collection_result = flush_collection_result.unwrap();
        assert!(
            collection_result.is_object(),
            "Collection flush result should be JSON object"
        );
    }
    ...
    async fn test_vector_operations_edge_cases() {
        ...
        let get_result = service
            .vector("non_existent_collection", "test_id", true, true)
            .await;
        // Result depends on implementation - should handle gracefully
        assert!(
            get_result.is_ok() || get_result.is_err(),
            "Get vector should return a result"
        );

        // Test get vector by ID with empty ID
        let get_by_id_result = service
            .vector_by_id("test_collection", "", true, true)
            .await;
        assert!(
            get_by_id_result.is_ok() || get_by_id_result.is_err(),
            "Get vector by ID should handle empty ID"
        );
    }
    ...
    async fn test_search_vectors_edge_cases() {
        ...
        let search_result = service
            .unified_search("test_collection", empty_query, 10, None, None)
            .await;
        // Should handle empty query gracefully
        assert!(
            search_result.is_ok() || search_result.is_err(),
            "Search should handle empty query vector"
        );

        // Test search with zero k
        let query = vec![1.0, 2.0, 3.0];
        let search_result = service
            .unified_search("test_collection", query, 0, None, None)
            .await;
        // Should handle zero k gracefully
        assert!(
            search_result.is_ok() || search_result.is_err(),
            "Search should handle zero k"
        );
    }
    ...
    async fn test_debug_operations() {
        ...
        let debug_result = service
            .debug_list_all_unflushed_vectors("test_collection")
            .await;
        assert!(
            debug_result.is_ok(),
            "Debug list unflushed vectors should succeed"
        );

        let vectors = debug_result.unwrap();
        // Should return empty list for new collection
        assert!(
            vectors.is_none() || !vectors.is_empty(),
            "Debug should return vector list"
        );
    }
    ...
    async fn test_concurrent_format_switching() {
        ...
        for handle in handles {
            ...
            assert!(matches!(
                result,
                OptimizedFormat::Proto | OptimizedFormat::Bincode | OptimizedFormat::Avro
            ));
        }
    }
    ...
    async fn test_insert_vectors_direct_edge_cases() {
        ...
        let result = service
            .insert_vectors_direct("test_collection", empty_vectors)
            .await;
        assert!(
            result.is_ok() || result.is_err(),
            "Insert should handle empty vector list"
        );

        // Test insert with single vector
        let single_vector = Arc::new(vec![create_core_test_vector(
            "test_id",
            vec![1.0, 2.0, 3.0],
        )]);
        let result = service
            .insert_vectors_direct("test_collection", single_vector)
            .await;
        assert!(
            result.is_ok() || result.is_err(),
            "Insert should handle single vector"
        );
    }
    ...
    async fn test_handle_vector_batch_proto_vec() {
        ...
        let empty_batch = vec![];
        let result = service
            .handle_vector_batch_proto_vec("test_collection", empty_batch)
            .await;
        assert!(
            result.is_ok() || result.is_err(),
        ...
        let result = service
            .handle_vector_batch_proto_vec("test_collection", single_batch)
            .await;
        assert!(
            result.is_ok() || result.is_err(),
            "Should handle single vector batch"
        );
    }
}
