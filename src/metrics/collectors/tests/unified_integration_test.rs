...
mod tests {
    ...
    async fn test_access_pattern_metrics_integration() {
        ...
        assert!(sample.values.contains_key("access_pattern.total_file_accesses"));
        assert_eq!(sample.values["access_pattern.total_file_accesses"], 10.0);
        
        // Check correlation metrics
        assert!(sample.values.contains_key("access_pattern.correlation_hit_rate"));
    ...
    async fn test_filesystem_metrics_integration() {
        ...
        general_metrics.read_operations.fetch_add(50, std::sync::atomic::Ordering::Relaxed);
        general_metrics.write_operations.fetch_add(30, std::sync::atomic::Ordering::Relaxed);
        general_metrics.bytes_read.fetch_add(1024 * 1024 * 5, std::sync::atomic::Ordering::Relaxed);
        general_metrics.bytes_written.fetch_add(1024 * 1024 * 2, std::sync::atomic::Ordering::Relaxed);
        ...
        let total_cache_ops = sample.values["fs.memory_cache.hits"] + sample.values["fs.memory_cache.misses"];
    ...
    async fn test_unified_collector_aggregation() {
        ...
        for i in 0..5 {
            access_collector.record_access(
                format!("test_file_{}", i),
                "test_collection".to_string(),
                1024,
                1.0,
            ...
            filesystem_collector.zerocopy_metrics().record_cache_hit(100);
        }
        ...
        let collector_names: Vec<String> = samples.iter()
    ...
    async fn test_cache_metrics_with_latency() {
        ...
        let expected_hit_avg = hit_latencies.iter().sum::<u64>() as f64 / hit_latencies.len() as f64 / 1000.0;
        let expected_miss_avg = miss_latencies.iter().sum::<u64>() as f64 / miss_latencies.len() as f64 / 1000.0;
    ...
    async fn test_cloud_storage_metrics() {
        ...
        general.s3_operations.fetch_add(100, std::sync::atomic::Ordering::Relaxed);
        general.gcs_operations.fetch_add(50, std::sync::atomic::Ordering::Relaxed);
        general.azure_operations.fetch_add(25, std::sync::atomic::Ordering::Relaxed);
        general.local_operations.fetch_add(200, std::sync::atomic::Ordering::Relaxed);
        
        // Simulate multipart uploads
        general.multipart_uploads.fetch_add(10, std::sync::atomic::Ordering::Relaxed);
        general.parallel_downloads.fetch_add(15, std::sync::atomic::Ordering::Relaxed);
    ...
    async fn test_download_optimization_metrics() {
        ...
        for _ in 0..selective_count {
            zerocopy.selective_downloads.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            zerocopy.total_bytes_saved.fetch_add(1024 * 100, std::sync::atomic::Ordering::Relaxed); // 100KB saved per selective
        }
        
        for _ in 0..full_count {
            zerocopy.full_downloads.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            zerocopy.total_bytes_downloaded.fetch_add(1024 * 1024, std::sync::atomic::Ordering::Relaxed); // 1MB per full
        }
        
        let sample = collector.collect().await.unwrap();
        
        // Verify download metrics
        assert_eq!(sample.values["fs.downloads.selective"], selective_count as f64);
        ...
        assert!(selective_ratio > 0.5, "Should prefer selective downloads for bandwidth efficiency");
    }
    ...
    async fn test_error_tracking() {
        ...
        general.read_errors.fetch_add(5, std::sync::atomic::Ordering::Relaxed);
        general.write_errors.fetch_add(2, std::sync::atomic::Ordering::Relaxed);
        general.permission_errors.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        general.not_found_errors.fetch_add(3, std::sync::atomic::Ordering::Relaxed);
        ...
        let total_ops = general.read_operations.load(std::sync::atomic::Ordering::Relaxed) +
                       general.write_operations.load(std::sync::atomic::Ordering::Relaxed);
    ...
    async fn test_metric_collection_intervals() {
        ...
        assert_eq!(access_collector.recommended_interval(), Duration::from_secs(30));
        assert_eq!(filesystem_collector.recommended_interval(), Duration::from_secs(10));
        
        // Filesystem metrics should be collected more frequently than access patterns
        assert!(filesystem_collector.recommended_interval() < access_collector.recommended_interval());
    }
}