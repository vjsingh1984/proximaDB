...
impl MetricsCollector for SystemMetricsCollector {
    async fn collect(&self) -> Result<MetricsSample> {
        ...
        values.insert("memory_used_bytes".to_string(), memory_stats.used_bytes as f64);
        values.insert("memory_total_bytes".to_string(), memory_stats.total_bytes as f64);
        values.insert("memory_usage_percent".to_string(), memory_stats.usage_percent);
        values.insert("disk_used_bytes".to_string(), disk_stats.used_bytes as f64);
        values.insert("disk_total_bytes".to_string(), disk_stats.total_bytes as f64);
                    ...
                    if let Some(user_part) = line.split("CPU usage: ").nth(1) {
                        if let Some(user_str) = user_part.split('%').next() {
                            if let Ok(user_cpu) = user_str.trim().parse::<f64>() {
                                ...
                                let sys_cpu = user_part.split(',').nth(1)
