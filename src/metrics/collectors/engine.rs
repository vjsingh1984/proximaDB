...
impl EngineMetricsCollector {
    ...
    fn determine_winner(&self, stats: &HashMap<String, EngineStatistics>) -> Option<String> {
        ...
        for (name, stat) in stats {
            ...
            let composite_score = (latency_score * 0.4) + (error_score * 0.4) + (throughput_score * 0.2);
...
impl MetricsCollector for EngineMetricsCollector {
    async fn collect(&self) -> Result<MetricsSample> {
        ...
        for (engine_name, _weak_ref) in engines.iter() {
            ...
            values.insert(
                format!("{}_error_rate", engine_name),
                stats.error_rate,
            );
            values.insert(
...
mod tests {
    ...
    async fn test_engine_metrics_collection() {
        ...
        collector.record_operation("DSST", "search", 25.0, false, 1024).await;
        collector.record_operation("DSST", "search", 30.0, false, 2048).await;
        collector.record_operation("DVIPER", "search", 15.0, false, 4096).await;
