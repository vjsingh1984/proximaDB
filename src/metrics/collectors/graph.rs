...
impl GraphMetricsCollector {
    ...
    async fn collect_graph_metrics(&self) -> Result<GraphMetricsSample> {
        ...
        let (total_nodes, total_edges, active_engines) = 
            self.collect_basic_stats().await?;
    ...
    fn calculate_derived_metrics(&self, current: &GraphMetricsSample, previous: Option<&GraphMetricsSample>) -> HashMap<String, f64> {
        ...
        metrics.insert("graph_engines_active".to_string(), current.active_engines as f64);
        
        // Query metrics
        metrics.insert("graph_queries_total".to_string(), current.total_queries as f64);
        metrics.insert("graph_queries_successful".to_string(), current.successful_queries as f64);
        metrics.insert("graph_queries_failed".to_string(), current.failed_queries as f64);
        metrics.insert("graph_query_duration_avg_us".to_string(), current.avg_query_time_us);
        metrics.insert("graph_queries_slow_total".to_string(), current.slow_queries as f64);
        
        // Success rate
        if current.total_queries > 0 {
            let success_rate = (current.successful_queries as f64 / current.total_queries as f64) * 100.0;
            metrics.insert("graph_query_success_rate_percent".to_string(), success_rate);
        }
        
        // Traversal metrics
        metrics.insert("graph_traversal_bfs_total".to_string(), current.bfs_operations as f64);
        metrics.insert("graph_traversal_dfs_total".to_string(), current.dfs_operations as f64);
        metrics.insert("graph_traversal_duration_avg_us".to_string(), current.avg_traversal_time_us);
        metrics.insert("graph_traversal_nodes_avg".to_string(), current.avg_nodes_per_traversal);
        
        // Engine-specific metrics
        metrics.insert("graph_pulsar_shards".to_string(), current.pulsar_shards as f64);
        metrics.insert("graph_quasar_hot_ratio".to_string(), current.quasar_hot_ratio);
        metrics.insert("graph_orion_cache_hit_rate".to_string(), current.orion_cache_hit_rate);
        
        // Resource metrics
        metrics.insert("graph_memory_used_bytes".to_string(), current.memory_used_bytes as f64);
        metrics.insert("graph_cpu_usage_percent".to_string(), current.cpu_usage_percent);
        
        // Calculate rates if we have previous sample
        if let Some(prev) = previous {
            let time_diff = current.timestamp.duration_since(prev.timestamp).as_secs_f64();
            if time_diff > 0.0 {
                // Query rate
                ...
                let traversal_rate = ((current.bfs_operations + current.dfs_operations) - 
                                    (prev.bfs_operations + prev.dfs_operations)) as f64 / time_diff;
                ...
                metrics.insert("graph_nodes_growth_per_second".to_string(), node_growth_rate);
                metrics.insert("graph_edges_growth_per_second".to_string(), edge_growth_rate);
            }
        }
...
impl QuasarMetricsCollector {
    ...
    async fn collect_quasar_specific_metrics(&self) -> Result<HashMap<String, f64>> {
        ...
        metrics.insert("quasar_hot_tier_size_bytes".to_string(), 100.0 * 1024.0 * 1024.0);
        metrics.insert("quasar_cold_tier_size_bytes".to_string(), 500.0 * 1024.0 * 1024.0);
