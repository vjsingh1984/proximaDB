...
use crate::metrics::{AggregationWindow, InternalMetricsUpdater, MetricsAggregationEngine};
    ...
    pub async fn start(&self, interval: Duration) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) =
                    Self::report_to_metrics_system(updater.as_ref(), &aggregator, &metrics).await
                {
    ...
    async fn collect_metrics(base_metrics: &BaseCacheMetrics) -> CacheMetricsSnapshot {
        ...
        CacheMetricsSnapshot {
            ...
            l3_metrics: TierMetrics {
                hits: l3_hits,
                misses: l3_misses,
                hit_rate: if l3_total > 0 {
                    l3_hits as f64 / l3_total as f64
                } else {
                    0.0
                },
    ...
    pub async fn get_optimization_hints(&self) -> CacheOptimizationHints {
        ...
        CacheOptimizationHints {
            should_increase_l1_size: metrics.l1_metrics.hit_rate < 0.7
                && metrics.memory_usage.used_bytes
                    < metrics.memory_usage.total_allocated_bytes * 8 / 10,
