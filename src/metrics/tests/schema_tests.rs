...
mod tests {
    use super::super::super::schema::{
        CollectionMetrics, GlobalMetrics, FilterableColumnStats, IndexInfo, IndexBuildStatus,
        QueryOptimizationHints, OptimizationHint, HintType, HintPriority, ImprovementEstimate,
    };
    ...
    fn test_collection_metrics_full_initialization() {
        ...
        filterable_stats.insert("price".to_string(), FilterableColumnStats {
            ...
            min_value: Some(serde_json::Value::Number(serde_json::Number::from_f64(9.99).unwrap())),
            max_value: Some(serde_json::Value::Number(serde_json::Number::from_f64(999.99).unwrap())),
            most_common_values: vec![
                (serde_json::Value::Number(serde_json::Number::from_f64(19.99).unwrap()), 500),
                (serde_json::Value::Number(serde_json::Number::from_f64(29.99).unwrap()), 450),
            ],
        ...
        metrics.available_indexes = vec![
            IndexInfo {
                ...
                parameters: {
                    ...
                    params.insert("M".to_string(), serde_json::Value::Number(serde_json::Number::from(16)));
                    params.insert("efConstruction".to_string(), serde_json::Value::Number(serde_json::Number::from(200)));
                    params
                },
            },
            IndexInfo {
                ...
                build_status: IndexBuildStatus::Building { progress_percent: 75.5 },
                size_bytes: 20 * 1024 * 1024,
                vector_count: 37500,
                last_updated: 1640991600000, // 1 hour ago
                parameters: {
                    let mut params = HashMap::new();
                    params.insert("nlist".to_string(), serde_json::Value::Number(serde_json::Number::from(1000)));
    ...
    fn test_latency_percentiles_calculation() {
        ...
        let latencies = vec![100.0, 150.0, 200.0, 250.0, 300.0, 350.0, 400.0, 500.0, 800.0, 1000.0];
    ...
    fn test_metrics_serialization_deserialization() {
        ...
        assert!(deserialized_result.is_ok(), "Deserialization should succeed");
        
        let deserialized_metrics = deserialized_result.unwrap();
        
        // Verify all fields match
        assert_eq!(deserialized_metrics.collection_id, original_metrics.collection_id);
        assert_eq!(deserialized_metrics.vector_count, original_metrics.vector_count);
        assert_eq!(deserialized_metrics.dimension, original_metrics.dimension);
        assert_eq!(deserialized_metrics.total_inserts, original_metrics.total_inserts);
        assert_eq!(deserialized_metrics.total_searches, original_metrics.total_searches);
        assert_eq!(deserialized_metrics.avg_search_latency_us, original_metrics.avg_search_latency_us);
        assert_eq!(deserialized_metrics.sparsity_ratio, original_metrics.sparsity_ratio);
        assert_eq!(deserialized_metrics.cache_hit_ratio, original_metrics.cache_hit_ratio);
    ...
    fn test_index_build_status_variants() {
        ...
        for status in status_variants {
            ...
            assert!(serialized.is_ok(), "IndexBuildStatus serialization should succeed");
            
            // Test deserialization
            let json = serialized.unwrap();
            let deserialized: Result<IndexBuildStatus, _> = serde_json::from_str(&json);
            assert!(deserialized.is_ok(), "IndexBuildStatus deserialization should succeed");
            
            // Verify variant-specific behavior
            match (&status, deserialized.unwrap()) {
                (IndexBuildStatus::NotStarted, IndexBuildStatus::NotStarted) => {},
                (IndexBuildStatus::Building { progress_percent: p1 }, 
                 IndexBuildStatus::Building { progress_percent: p2 }) => {
                    assert_eq!(p1, &p2);
                },
                ...
                _ => panic!("IndexBuildStatus variant mismatch after serialization/deserialization"),
            }
        }
    ...
    fn test_query_optimization_hints_structure() {
        ...
        assert!(serialized.is_ok(), "QueryOptimizationHints serialization should succeed");
        
        let deserialized: Result<QueryOptimizationHints, _> = serde_json::from_str(&serialized.unwrap());
        assert!(deserialized.is_ok(), "QueryOptimizationHints deserialization should succeed");
    ...
    fn test_filterable_column_stats_edge_cases() {
        ...
        let string_stats = FilterableColumnStats {
            ...
            most_common_values: vec![
                (serde_json::Value::String("common_value".to_string()), 100),
            ],
            histogram_bounds: None,
        };
        ...
        for stats in stats_variants {
            ...
            assert!(serialized.is_ok(), "FilterableColumnStats serialization should succeed for {}", stats.data_type);
            
            // Test deserialization
            let deserialized: Result<FilterableColumnStats, _> = serde_json::from_str(&serialized.unwrap());
            assert!(deserialized.is_ok(), "FilterableColumnStats deserialization should succeed for {}", stats.data_type);
