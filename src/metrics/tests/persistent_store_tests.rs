...
mod tests {
    ...
    async fn test_collection_partitioning() {
        ...
        for collection_id in &test_collections {
            ...
            assert!(result.is_ok(), "Failed to store metrics for {}: {:?}", collection_id, result);
        }
    ...
    async fn test_metrics_cleanup_functionality() {
        ...
        let cleanup_result = store.cleanup_collection_metrics("cleanup_test_collection").await;
    ...
    async fn test_filesystem_factory_integration() {
        ...
        assert!(filesystem_factory.is_ok(), "Failed to get filesystem factory: {:?}", filesystem_factory);
