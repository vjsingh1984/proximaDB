...
impl MetricsQueryService {
    ...
    pub async fn get_collection_metrics(
        ...
        if let Some(cached) = cache.collections.get(collection_id) {
            if self.is_cache_valid(cached.cached_at, cached.ttl_seconds) {
                ...
                return self.format_metrics_response(
                    &cached.metrics,
                    if options.include_hints { Some(&cached.hints) } else { None },
                );
            }
        }
        drop(cache);
        ...
        // Update cache
        self.update_cache(collection_id, metrics.clone(), hints.clone()).await?;
        
        // Format response
        self.format_metrics_response(
            &metrics,
            if options.include_hints { Some(&hints) } else { None },
        )
    }
    ...
    pub async fn get_query_hints(
        ...
        if let Some(qtype) = query_type {
            hints.retain(|hint| {
                hint.applicable_queries.iter().any(|q| 
                    q == &qtype || q == "all"
                )
            });
        }
    ...
    async fn update_cache(
        ...
        if cache.current_size_bytes + estimated_size > cache.max_size_bytes {
            ...
            if let Some(oldest_key) = cache.collections.iter()
        ...
        // Add new entry
        cache.collections.insert(collection_id.to_string(), CachedMetrics {
            metrics,
            hints,
            cached_at: chrono::Utc::now().timestamp(),
