...
impl MetricsExporter for PrometheusExporter {
    fn export(&self, metrics: &MetricsSnapshot) -> Result<String> {
        ...
        for (collection_id, col_metrics) in &metrics.collections {
            writeln!(
                output,
                "# HELP vector_count_{{collection=\"{}\"}} Vector count",
                collection_id
            )?;
            ...
            writeln!(
                output,
                "# HELP search_qps_{{collection=\"{}\"}} Search QPS",
                collection_id
            )?;
            writeln!(output, "# TYPE search_qps gauge")?;
            writeln!(
                output,
                "search_qps{{collection=\"{}\"}} {}",
                collection_id, col_metrics.search_qps
            )?;

            writeln!(
                output,
                "# HELP cache_hit_rate_{{collection=\"{}\"}} Cache hit rate",
                collection_id
            )?;
            writeln!(output, "# TYPE cache_hit_rate gauge")?;
            writeln!(
                output,
                "cache_hit_rate{{collection=\"{}\"}} {}",
        ...
        writeln!(
            output,
            "# HELP cache_overall_hit_rate Overall cache hit rate"
        )?;
        ...
        writeln!(
            output,
            "# HELP cache_evictions_per_second Cache evictions per second"
        )?;
