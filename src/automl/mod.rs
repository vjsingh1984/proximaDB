...
//!
//! Extracted subsystems (optimization, prediction, tuning, workload) live in
//! `proximadb-automl` (TD-DECOMP-15); `service.rs` and `AutoMLCoordinator`
//! stay root-side (they depend on control-tier metrics types).

pub use proximadb_automl::{HyperparameterTuner, OptimizationGoal, OptimizationPipeline};
pub use proximadb_automl::{PerformancePredictor, PredictionModel, TuningConfig};
pub use proximadb_automl::{WorkloadAnalyzer, WorkloadPattern};
