...
impl TierEnforcement {
    ...
    pub fn new() -> Self {
        ...
        tier_definitions.insert(
            ...
            TierDefinition {
                ...
                feature_limits: [
                    (
                        "ai_queries".to_string(),
                        FeatureLimit {
                            limit_type: LimitType::Rate,
                            limit_value: LimitValue::Rate {
                                requests: 50,
    ...
    pub async fn enforce_tier_limits(
        ...
        debug!(
            "🔍 Enforcing tier limits: {} for feature {} (usage: {})",
            tier, feature, current_usage
        );
            ...
            if !allowed {
                return Ok(EnforcementResult {
                    ...
                    reason: format!(
                        ...
                        feature, current_usage, limit.limit_value
                    ),
                    usage_percentage,
                    upgrade_recommendation: Some(self.recommend_upgrade_for_feature(feature)),
                });
    ...
    fn check_limit_compliance(
        ...
        match &limit.limit_value {
            ...
            LimitValue::Rate {
                ...
                Ok((
                    current_usage <= *requests as u64,
                    current_usage as f64 / *requests as f64,
                ))
            }
            LimitValue::Boolean(allowed) => Ok((*allowed, if *allowed { 0.0 } else { 1.0 })),
            LimitValue::Percentage(max_percentage) => Ok((
                current_usage as f64 <= *max_percentage,
                current_usage as f64 / max_percentage,
            )),
        }
    }
...
mod tests {
    ...
    async fn test_free_tier_limits() {
        ...
        let result = enforcement
            .enforce_tier_limits("Free", "ai_queries", 60)
            .await
            .unwrap(); // Over 50/day limit
        assert!(!result.allowed);
        assert!(matches!(result.action, EnforcementAction::Block));

        // Test within limits
        let result = enforcement
            .enforce_tier_limits("Free", "ai_queries", 30)
            .await
            .unwrap(); // Under 50/day limit
        assert!(result.allowed);
    }
    ...
    async fn test_enterprise_tier_unlimited() {
        ...
        let result = enforcement
            .enforce_tier_limits("Enterprise", "ai_queries", 1000000)
            .await
            .unwrap();
