...
impl JwtService {
    ...
    pub async fn generate_token_pair(
        ...
        let access_token = encode(&self.header, &access_claims, &self.encoding_key)
            .map_err(|e| AuthError::InvalidToken(format!("Failed to encode access token: {}", e)))?;
            
        let refresh_token = encode(&self.header, &refresh_claims, &self.encoding_key)
            .map_err(|e| AuthError::InvalidToken(format!("Failed to encode refresh token: {}", e)))?;
    ...
    pub async fn refresh_token(
        ...
        self.generate_token_pair(&claims.sub, claims.tenant_id, roles).await
    }
