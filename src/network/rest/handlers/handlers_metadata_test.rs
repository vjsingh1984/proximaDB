...
mod tests {
    ...
    async fn test_metadata_conversion_helpers() {
        ...
        assert_eq!(json_metadata.get("category"), Some(&serde_json::Value::String("electronics".to_string())));
        assert_eq!(json_metadata.get("price"), Some(&serde_json::Value::Number(serde_json::Number::from_f64(99.99).unwrap())));
