...
use proximadb::server::{DistanceMetric, HardwareAcceleration, IndexingAlgorithm, ServerBuilder};
...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "   - Endpoint: {}:{}",
        basic_server.endpoint().0,
        basic_server.endpoint().1
    );
    println!(
        "   - Hardware: {:?}",
        basic_server.compute_config().acceleration
    );
    println!(
        "   - Indexing: {:?}",
        basic_server.indexing_config().default_algorithm
    );
    println!(
        "   - Distance: {:?}",
        basic_server.indexing_config().default_distance_metric
    );
    ...
    println!(
        "   - Endpoint: {}:{}",
        high_perf_server.endpoint().0,
        high_perf_server.endpoint().1
    );
    println!(
        "   - Hardware: {:?}",
        high_perf_server.compute_config().acceleration
    );
    println!(
        "   - CUDA devices: {:?}",
        high_perf_server.compute_config().cuda_devices
    );
    println!(
        "   - Storage layout: {:?}",
        high_perf_server.storage_system().storage_layout()
    );
    println!(
        "   - Data storage URLs: {}",
        high_perf_server
    ...
    println!(
        "   - Hardware: {:?}",
        cloud_server.compute_config().acceleration
    );
    println!(
        "   - Storage layout: {:?}",
        cloud_server.storage_system().storage_layout()
    );
    println!(
        "   - Auto algorithm selection: {}",
        cloud_server.indexing_config().enable_auto_selection
    );
    println!(
        "   - Auto distance selection: {}",
        cloud_server
            .indexing_config()
            .enable_auto_distance_selection
    );
    ...
    println!(
        "   - Endpoint: {}:{}",
        dev_server.endpoint().0,
        dev_server.endpoint().1
    );
    println!(
        "   - Indexing: {:?}",
        dev_server.indexing_config().default_algorithm
    );
    println!(
        "   - Zero-copy enabled: {}",
        dev_server
    ...
    println!(
        "   ✅ Performance: Strategy classes loaded once at startup for zero runtime overhead"
    );
