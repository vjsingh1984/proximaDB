...
use proximadb::storage::engines::common::{
    ...
    compression_common::{
        AdaptiveCompressionSettings, AdaptiveStrategy, CompressionDataType,
        CompressionHardwareConfig, ContextAwareCompressionConfig,
    },
    quantization_common::{
        BinaryThresholdStrategy, CodebookStrategy, HardwareQuantizationConfig,
        QuantizationQualityConfig, ScaleStrategy, ZeroPointStrategy,
    },
};

use proximadb::core::{
    compression::CompressionAlgorithm, hardware_capabilities::HardwareCapabilities,
};
...
async fn demonstrate_compression_synergy() -> Result<()> {
    ...
    for (name, test_data) in test_datasets {
        ...
        let compressed =
            compression_adapter.compress_with_universal_config(&test_data, &universal_config)?;

        println!(
            "   Algorithm selected: {:?} (adaptive: {})",
...
async fn demonstrate_quantization_synergy() -> Result<()> {
    ...
    let quantization_result =
        quantization_adapter.quantize_progressive(&test_vectors, &universal_config)?;

    println!(
        "✅ Progressive quantization completed in {} ms",
        quantization_result.total_time_ms
    );
    println!(
        "💾 Memory savings: {:.1}%",
        quantization_result.memory_savings * 100.0
    );
    println!(
        "🎯 Overall quality score: {:.2}",
        quantization_result.quality_score
    );
    ...
    let search_results =
        quantization_adapter.search_progressive(&query_vector, &quantization_result, 10)?;

    for (i, result) in search_results.iter().enumerate() {
        println!(
            "   Search stage {}: {} candidates in {} ms (precision: {:.2})",
            i + 1,
    ...
    println!(
        "   Average quantization time: {:.2} ms",
        stats.average_quantization_time_ms()
    );
    println!(
        "   Throughput: {:.2} vectors/second",
        stats.quantization_throughput_vectors_per_second()
    );
...
async fn demonstrate_combined_synergy() -> Result<()> {
    ...
    println!(
        "🔧 Processing {} vectors with {} dimensions",
        vectors.len(),
        vectors[0].len()
    );
    ...
    let quantization = UniversalQuantizationConfig {
        enabled: true,
        stages: vec![ProgressiveQuantizationStage {
            level: UniversalQuantizationLevel::Int8 {
    ...
    let compressed_result =
        compression_adapter.compress_with_universal_config(&quantized_data, &compression_config)?;
    ...
    let recovered_quantized: Vec<
        proximadb::storage::engines::common::quantization_adapter::StageQuantizationResult,
    > = bincode::deserialize(&decompressed_data)?;

    println!(
        "   ✅ Successfully recovered {} quantization stages",
        recovered_quantized.len()
    );
...
async fn main() -> Result<()> {
    ...
    println!(
        "💻 Detected hardware: {} cores, {:.1} GB RAM",
        hardware.cpu_cores(),
    ...
    println!(
        "   • Universal Compression ↔ Unified Compression: Configuration layer + Implementation"
    );
