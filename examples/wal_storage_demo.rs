...
use proximadb::storage::filesystem::{
    AuthConfig, 
    AwsAuthMethod, 
    AzureAuthMethod, 
    GcsAuthMethod
};
use proximadb::storage::wal::{
    WalSystemBuilder, 
    WalFormat, 
    CompressionLevel
};
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("   - Data URLs: {} disks", local_storage.config().data_storage.data_urls.len());
    println!("   - WAL format: {:?}", local_storage.config().wal_system.primary_format);
    println!("   - Zero-copy enabled: {}", local_storage.storage_performance().enable_zero_copy);
    ...
    use proximadb::storage::builder::{DataTieringConfig, TierConfig, AutoTierPolicies, AccessPattern};
    ...
    let dev_storage = StorageSystemBuilder::new()
        .with_multi_disk_data_storage(vec![
            "/tmp/proximadb/dev/data".to_string(),
        ])
    ...
    let wal_system = WalSystemBuilder::new()
        ...
        .with_s3_tier("s3_backup".to_string(), vec![
            "proximadb-wal-backup-us-east-1".to_string()
        ])
        .with_azure_tier("azure_backup".to_string(), vec![
            "proximadbwal/backup".to_string()
        ])
...
fn inspect_storage_config(storage: &proximadb::storage::StorageSystem) {
    ...
    println!("   💾 Data URLs: {}", storage.config().data_storage.data_urls.len());
    println!("   📝 WAL Format: {:?}", storage.config().wal_system.primary_format);
    println!("   🗜️ Compression: {}", 
             if storage.config().data_storage.compression.compress_vectors { "Enabled" } else { "Disabled" });
    println!("   ⚡ Zero-copy: {}", storage.storage_performance().enable_zero_copy);
    println!("   🏠 Cache: {}MB", storage.config().data_storage.cache_size_mb);
    println!();
}