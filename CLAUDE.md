...
- Sticky design pillars ALL new code must respect:
  ...
  - **Canonical internal model first**: new foundation, modality, storage, query, and catalog contracts use `ProximaRecord` plus `ProximaType`/`ProximaValue` internally. v2 protobuf is the protocol/wire alignment for this model. Legacy v1 `SqlValue`/`SqlObject` belongs only in compatibility adapters at protocol/service edges.
