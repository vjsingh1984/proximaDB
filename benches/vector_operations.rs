...
pub fn benchmark_vector_insertion(c: &mut Criterion) {
    ...
    for &count in &config.vector_counts {
        for &dimension in &config.dimensions {
            if count <= 10_000 || dimension <= 512 { // Skip very large combinations for CI
...
pub fn benchmark_vector_retrieval(c: &mut Criterion) {
    ...
    for &count in &[1_000, 10_000] { // Smaller datasets for retrieval
...
pub fn benchmark_vector_batch_operations(c: &mut Criterion) {
    ...
    for &batch_size in &batch_sizes {
        for &dimension in &[128, 256] {
            ...
            group.bench_with_input(
                BenchmarkId::new("batch_insert", format!("batch_{}x{}", batch_size, dimension)),
                &(batch_size, dimension),
                |b, &(batch_size, dimension)| {
                    let vectors = generator.generate_vectors(batch_size, dimension, 0.1, "bench_collection");
...
pub fn benchmark_vector_sparsity_impact(c: &mut Criterion) {
    ...
    for &sparsity in &config.sparsity_levels {
        ...
        group.bench_with_input(
            ...
            &sparsity,
            |b, &sparsity| {
                let vectors = generator.generate_vectors(count, dimension, sparsity, "bench_collection");
    ...
    group.bench_function("soft_delete", |b| {
        ...
        b.to_async(&runner.runtime).iter(|| async {
            ...
            // Then delete them
            {
                let mut storage_guard = storage.write().await;
                for vector in &vectors {
                    let _ = storage_guard.delete_vector(
                        black_box(&vector.collection_id),
                        black_box(&vector.id)
                    ).await;
                }
            }
