...
fn bench_flush_operation(c: &mut Criterion) {
    ...
    for count in [100, 1000, 10000].iter() {
        ...
        // Benchmark VIPER
        group.bench_with_input(
            BenchmarkId::new("VIPER", count),
            count,
            |b, _| {
                let engine = rt.block_on(async {
                    ViperEngine::new().unwrap()
                });
        ...
        // Benchmark DSST
        group.bench_with_input(
            BenchmarkId::new("DSST", count),
            count,
            |b, _| {
                let engine = rt.block_on(async {
                    DsstEngine::new().unwrap()
                });
        ...
        // Benchmark DVIPER
        group.bench_with_input(
            BenchmarkId::new("DVIPER", count),
            count,
            |b, _| {
                let engine = rt.block_on(async {
                    DviperEngine::new().unwrap()
                });
    ...
    for id in ids.iter() {
        ...
        group.bench_with_input(
            ...
            |b, id| {
                ...
                b.to_async(&rt).iter(|| async {
                    engine.get_vector_by_id("bench_collection", id).await.unwrap()
                });
            },
        );
...
fn bench_compaction(c: &mut Criterion) {
    ...
    group.bench_function("SST", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            engine.do_compact(&params).await.unwrap()
        });
    });
