...
pub fn benchmark_similarity_search(c: &mut Criterion) {
    ...
    for &dataset_size in &dataset_sizes {
        for &query_count in &query_counts {
            for &k in &k_values {
                if dataset_size <= 10_000 || k <= 50 { // Skip large combinations for CI
                    let dimension = 256;
                    group.throughput(Throughput::Elements(query_count as u64));
                    
                    group.bench_with_input(
                        BenchmarkId::new("ann_search", format!("{}d_{}q_k{}", dataset_size, query_count, k)),
                        &(dataset_size, query_count, k, dimension),
                        |b, &(dataset_size, query_count, k, dimension)| {
                            let vectors = generator.generate_vectors(dataset_size, dimension, 0.1, "search_bench");
                            ...
                            b.to_async(&runner.runtime).iter(|| async {
                                ...
                                // Perform similarity searches
                                {
                                    ...
                                    for query in &queries {
                                        let _ = storage_guard.search_similar_vectors(
...
pub fn benchmark_range_search(c: &mut Criterion) {
    ...
    for &threshold in &similarity_thresholds {
        ...
        group.bench_with_input(
            ...
            &threshold,
            |b, &threshold| {
                let vectors = generator.generate_vectors(dataset_size, dimension, 0.1, "range_bench");
                let queries = generator.generate_query_vectors(query_count, dimension, 0.1);
...
pub fn benchmark_sparse_vector_search(c: &mut Criterion) {
    ...
    for &sparsity in &config.sparsity_levels {
        if sparsity >= 0.5 { // Focus on sparse vectors
            group.throughput(Throughput::Elements(query_count as u64));
            ...
            group.bench_with_input(
                ...
                &sparsity,
                |b, &sparsity| {
                    let vectors = generator.generate_vectors(dataset_size, dimension, sparsity, "sparse_bench");
                    let queries = generator.generate_query_vectors(query_count, dimension, sparsity);
...
pub fn benchmark_hybrid_search(c: &mut Criterion) {
    ...
    for (name, selectivity) in filter_selectivities {
        ...
        group.bench_with_input(
            ...
            &selectivity,
            |b, &selectivity| {
                let vectors = generator.generate_vectors(dataset_size, dimension, 0.1, "hybrid_bench");
                let queries = generator.generate_query_vectors(query_count, dimension, 0.1);
                ...
                b.to_async(&runner.runtime).iter(|| async {
                    ...
                    // Perform hybrid searches
                    {
                        ...
                        for query in &queries {
                            ...
                            filter.insert("category".to_string(), serde_json::json!(format!("cat_{}", category_count)));
                            
                            let _ = storage_guard.search_with_metadata_filters(
...
pub fn benchmark_index_building(c: &mut Criterion) {
    ...
    for &dataset_size in &dataset_sizes {
        ...
        group.bench_with_input(
            ...
            &dataset_size,
            |b, &dataset_size| {
                let vectors = generator.generate_vectors(dataset_size, dimension, 0.1, "index_bench");
...
pub fn benchmark_concurrent_search(c: &mut Criterion) {
    ...
    for &tasks in &concurrent_tasks {
        ...
        group.bench_with_input(
            ...
            &tasks,
            |b, &tasks| {
                let vectors = generator.generate_vectors(dataset_size, dimension, 0.1, "concurrent_search");
                ...
                b.to_async(&runner.runtime).iter(|| async {
                    ...
                    for task_id in 0..tasks {
                        let queries = generator.generate_query_vectors(queries_per_task, dimension, 0.1);
