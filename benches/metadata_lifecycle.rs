...
fn create_test_metadata(index: usize) -> HashMap<String, serde_json::Value> {
    ...
    meta.insert(
        ...
        serde_json::Value::Number(
            serde_json::Number::from_f64(0.5 + (index % 50) as f64 / 100.0).unwrap(),
        ),
    );
    meta.insert(
...
fn benchmark_metadata_lifecycle() {
    ...
    for count in record_counts {
        ...
        let metadata_records: Vec<_> = (0..count).map(|i| create_test_metadata(i)).collect();
        ...
        println!(
            "   Memtable search: {:.1} records/sec",
            memtable_ops_per_sec
        );
