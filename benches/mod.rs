...
// TODO: Add additional benchmark modules
...
use proximadb::core::{CollectionId, Config, VectorId, VectorRecord};
...
impl BenchmarkDataGenerator {
    ...
    fn generate_metadata(
        ...
        metadata.insert(
            "category".to_string(),
            serde_json::json!(format!("cat_{}", index % 10)),
        );
        ...
        metadata.insert(
            "source".to_string(),
            serde_json::json!(format!("source_{}", index % 3)),
        );
...
impl BenchmarkRunner {
    ...
    pub async fn create_test_storage(&self) -> Arc<tokio::sync::RwLock<StorageEngine>> {
        ...
        let storage_engine = StorageEngine::new(config.storage)
            .await
    ...
    pub fn generate_report(results: &[BenchmarkResults]) -> String {
        ...
        if !results.is_empty() {
            let avg_throughput: f64 =
                results.iter().map(|r| r.throughput_per_second).sum::<f64>() / results.len() as f64;
            let avg_latency: f64 = results
                .iter()
                .map(|r| r.mean_duration.as_millis() as f64)
                .sum::<f64>()
                / results.len() as f64;
