...
use proximadb::core::serialization::{
    CompressionAlgorithm, VectorSerializationConfig,
    fixed_length::{Dim512, Dim1024, FixedLengthSerializer, FixedVector},
...
fn bench_compression_algorithms(c: &mut Criterion) {
    ...
    for (name, algorithm) in algorithms {
        ...
        group.bench_with_input(BenchmarkId::new("compress", name), &config, |b, config| {
            b.iter(|| {
                for vector in &vectors {
...
fn bench_fixed_vs_dynamic(c: &mut Criterion) {
    ...
    let fixed_vectors: Vec<FixedVector<Dim512>> = dynamic_vectors
        .iter()
...
fn bench_sst_optimizations(c: &mut Criterion) {
    ...
    let sst_records: Vec<proximadb::storage::engines::sst::SstRecord> = records
        .iter()
