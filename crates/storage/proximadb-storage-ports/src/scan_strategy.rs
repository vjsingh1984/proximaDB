...
//!
//! Hoisted from the root crate's `src/storage/scan_strategy.rs` as a root-crate
//! decomposition slice: the scan-strategy surface (`ScanStrategy`, `ScanIterator`,
//! `UnifiedScanEngine`, `ScanStatistics`, …) is a pure foundation-typed contract
//! every engine implements, so it belongs with the other engine-port traits. The
//! root re-exports it as `crate::storage::scan_strategy` for source compatibility.
...
/// Scan capabilities (re-export of the engine-capability descriptor that already
/// lives in this crate's `capabilities` module). Kept here so the historical
/// `scan_strategy::ScanCapabilities` path continues to resolve.
pub use crate::ScanCapabilities;
