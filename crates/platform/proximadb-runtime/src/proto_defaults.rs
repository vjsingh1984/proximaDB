//! Runtime defaults for protobuf DTOs.
//!
//! The protobuf crate owns generated contracts only. Defaults that inspect host
//! capacity or emit runtime diagnostics belong in this platform layer.
...
pub fn apply_collection_config_defaults(config: &mut CollectionConfig) {
    tracing::info!(
        "defaults BEFORE: name={}, distance_metric={:?}, storage_engine={:?}",
        config.name,
    ...
    if config.distance_metric.is_none() {
        ...
        tracing::info!("distance_metric already set: {:?}", config.distance_metric);
    }
    ...
    tracing::info!(
        "defaults AFTER: name={}, distance_metric={:?}, storage_engine={:?}",
        config.name,
        config.distance_metric,
        config.storage_engine
    );
...
mod tests {
    ...
    #[test]
    fn collection_defaults_fill_optional_runtime_policy() {
        let mut config = CollectionConfig {
            name: "test".to_string(),
            ..Default::default()
        };

        apply_collection_config_defaults(&mut config);
        ...
        assert_eq!(config.primary_index, Some(String::new()));
        assert_eq!(config.auto_index_selection, Some(true));
    }

    #[test]
    fn index_defaults_use_host_capacity_without_overwriting_existing_values() {
        ...
        apply_index_config_defaults(&mut config);

        assert_eq!(config.enabled, Some(false));
        assert_eq!(config.update_mode, Some(0));
        assert_eq!(config.enable_background_optimization, Some(true));
        assert_eq!(config.checkpoint_interval_ms, Some(60000));
        assert_eq!(config.is_primary, Some(false));
        assert_eq!(config.selectivity_threshold, Some(0.1));
        assert_eq!(config.use_quantization, Some(false));
        assert_eq!(config.queue_representation, Some(String::new()));
        assert!(config.build_concurrency.unwrap_or_default() > 0);
        assert!(config.memory_limit_mb.unwrap_or_default() >= 512);
    }

    #[test]
    fn storage_and_index_algorithm_defaults_are_stable() {
        let mut storage = StorageConfig::default();
        let mut hnsw = HnswConfig::default();
        let mut ivf = IvfConfig::default();
        let mut lsh = LshConfig::default();

        apply_storage_config_defaults(&mut storage);
        apply_hnsw_config_defaults(&mut hnsw);
        apply_ivf_config_defaults(&mut ivf, Some(10_000));
        apply_lsh_config_defaults(&mut lsh);

        assert_eq!(
            storage.compression,
            Some(CompressionAlgorithm::CompressionZstd as i32)
        );
        assert_eq!(storage.max_file_size_mb, Some(128));
        assert_eq!(storage.enable_caching, Some(true));
        assert_eq!(hnsw.m, Some(16));
        assert_eq!(hnsw.ef_construction, Some(200));
        assert_eq!(ivf.n_lists, Some(100));
        assert_eq!(ivf.n_probe, Some(10));
        assert_eq!(lsh.n_hash_tables, Some(10));
        assert_eq!(lsh.max_candidates, Some(1000));
    }
}
