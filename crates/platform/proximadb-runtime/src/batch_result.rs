//! Typed result structures for batch record/vector operations.
//!
//! TD-104 S3: relocated from the root crate (`src/services/operations/batch_result.rs`,
//! now a re-export shim) so the `RecordOpsPort` contract — which `proximadb-runtime`
//! owns — can return it without a foundation→platform dependency inversion. Pure
//! data (serde + primitives); no behavior change.
