...
/// PostgreSQL wire protocol server
///
/// Placeholder boundary in the API crate; the real implementation migrates
/// from `src/network/postgres/` (TD-104 S3-e: the dead `_handlers` field was
/// removed).
...
mod tests {
    ...
    async fn server_preserves_bind_address_and_reports_migration_status() {
        ...
        let server = PostgresServer::new(bind_addr);
