...
// pre-existing style in the moved tests
/*
...
impl HardwareBackend {
    ...
    pub fn detect() -> Self {
        ...
        #[cfg(feature = "gpu")]
        {
            use crate::get_hardware_capabilities;
...
/// A GPU-capability detector, registered by the (feature-gated) `compute::gpu`
/// subsystem so this foundation layer does not depend on `compute` (issue #162).
/// Returns the detected backend plus the available devices.
pub type GpuDetectorFn = fn() -> anyhow::Result<(GpuBackend, Vec<GpuDevice>)>;

static GPU_DETECTOR: OnceLock<GpuDetectorFn> = OnceLock::new();

/// Register the GPU-capability detector. Call once at startup, *before*
/// [`initialize_hardware_capabilities`], from the layer that owns GPU detection
/// (`compute::gpu`, under `#[cfg(feature = "gpu")]`). No-op if already set, so a
/// late or duplicate call cannot clobber an existing registration.
pub fn register_gpu_detector(detector: GpuDetectorFn) {
    let _ = GPU_DETECTOR.set(detector);
}
...
impl HardwareCapabilities {
    ...
    ///
    /// GPU detection lives in the (feature-gated) `compute::gpu` subsystem; this
    /// foundation layer must NOT depend on it — that direct call created the
    /// `hardware_capabilities` <-> `compute` circular dependency (issue #162) that
    /// blocked extracting this module into a foundation crate. The compute layer
    /// now registers its detector via [`register_gpu_detector`] at startup
    /// (`src/bin/server.rs`), *before* `initialize_hardware_capabilities` runs, so
    /// there is no init race. When no detector is registered (default / non-`gpu`
    /// builds) GPU capabilities are empty — the previous `not(feature = "gpu")`
    /// behaviour.
    fn detect_gpu() -> Result<GpuCapabilities> {
        let empty = || GpuCapabilities {
            backend: GpuBackend::None,
            devices: vec![],
            primary_device: None,
            total_memory: 0,
        ...
        let Some(detector) = GPU_DETECTOR.get() else {
            return Ok(empty());
        };
        match detector() {
            Ok((backend, devices)) => {
                ...
                if backend != GpuBackend::None {
                    ...
                    info!("GPU detector registered but no GPU devices detected");
                }
                Ok(GpuCapabilities {
                    backend,
                    devices,
