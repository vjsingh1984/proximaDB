...
// The engine was extracted from the root crate (which carries these crate-level
// clippy allows); mirror them so the moved code lints identically.
...
pub mod storage;
pub mod traversal;
pub use memory_pool::GraphMemoryPool;
pub use memory_pool::property_value_to_string;
...
pub struct OrionGraphEngine {
    ...
    pub memory_pool: Arc<GraphMemoryPool>,
...
impl OrionGraphEngine {
    ...
    pub async fn recover(&self) -> Result<()> {
        if let Some(ref persistence) = self.persistence {
            ...
            // prior behavior).
            let snapshot_lsn = if std::env::var("PROXIMADB_GRAPH_CANONICAL_REPLAY_SCOPE").as_deref()
                == Ok("1")
            {
...
mod tests {
    use super::*;
    use proximadb_graph_model::{PropertyValue, property_value};
