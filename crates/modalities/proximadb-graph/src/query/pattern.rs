...
use super::storage::GraphQueryStorage;
...
impl PatternMatcher {
    ...
    /// * `storage` - The graph read-side storage contract for node/edge access
    ...
    /// use proximadb_graph::query::{PatternMatcher, QueryContext};
    /// use std::sync::Arc;
    ///
    /// let matcher = PatternMatcher::new()?;
    /// let context = QueryContext::new();
    /// let storage: Arc<dyn GraphQueryStorage> = todo!();
    ...
    pub fn execute_query(
        ...
        self.execute_pattern(&compiled, storage, context)
    }
    ...
    pub fn execute_pattern(
        ...
        let initial_candidates = self.find_initial_candidates(pattern, storage)?;

        // For each initial candidate, try to extend the match
        for candidate in initial_candidates {
            if let Ok(matches) = self.extend_match(pattern, candidate, storage, context) {
                results.extend(matches);
            }
        }

        // Apply WHERE clauses
        let filtered_results = self.apply_where_clauses(pattern, results, storage)?;
    ...
    fn find_initial_candidates(
        ...
        let starting_pattern = pattern
            .nodes
            .iter()
            .min_by_key(|node| self.estimate_node_selectivity(node, storage))
            .ok_or_else(|| {
                ProximaDBError::InvalidInput("Pattern must contain at least one node".to_string())
            })?;

        // Find matching nodes
        let matching_nodes = self.find_matching_nodes(starting_pattern, storage)?;
    ...
    fn extend_match(
        ...
        for edge_pattern in &pattern.edges {
            ...
            for result in results {
                if let Ok(extended) = self.extend_with_edge_pattern(edge_pattern, result, storage) {
        ...
        for path_pattern in &pattern.paths {
            ...
            for result in results {
                if let Ok(extended) = self.extend_with_path_pattern(path_pattern, result, storage) {
                    new_results.extend(extended);
                }
            }
        ...
        for node_pattern in &pattern.nodes {
            ...
            for result in results {
                if let Ok(extended) = self.extend_with_node_pattern(node_pattern, result, storage) {
                    new_results.extend(extended);
                }
            }
    ...
    fn extend_with_edge_pattern(
        ...
        let matching_edges =
            self.find_matching_edges_from_node(&source_node.id, edge_pattern, storage)?;
    ...
    fn extend_with_path_pattern(
        ...
        let paths = self.find_variable_length_paths(&source_node.id, path_pattern, storage)?;
    ...
    fn extend_with_node_pattern(
        ...
        let matching_nodes = self.find_matching_nodes(node_pattern, storage)?;
    ...
    fn find_matching_nodes(
        ...
        let candidate_nodes = if node_pattern.labels.is_empty() {
            storage.get_all_nodes().map_err(|e| {
                ProximaDBError::Internal(format!("Failed to fetch graph nodes: {e}"))
            })?
        } else {
            ...
            for label in &node_pattern.labels {
                let label_nodes = storage.get_nodes_by_label(label).map_err(|e| {
                    ProximaDBError::Internal(format!(
                        "Failed to fetch graph nodes for label '{label}': {e}"
                    ))
                })?;
    ...
    fn find_matching_edges_from_node(
        ...
        let outgoing = match edge_pattern.direction {
            EdgeDirection::Outgoing | EdgeDirection::Bidirectional => storage
                .get_outgoing_edges(from_node_id, None)
                .map_err(|e| {
                    ProximaDBError::Internal(format!(
                        "Failed to fetch outgoing edges for node '{from_node_id}': {e}"
                    ))
                })?,
            EdgeDirection::Incoming => Vec::new(),
        };
        let incoming = match edge_pattern.direction {
            EdgeDirection::Incoming | EdgeDirection::Bidirectional => storage
                .get_incoming_edges(from_node_id, None)
                .map_err(|e| {
                    ProximaDBError::Internal(format!(
                        "Failed to fetch incoming edges for node '{from_node_id}': {e}"
                    ))
                })?,
            EdgeDirection::Outgoing => Vec::new(),
        };
        ...
        for edge in outgoing.into_iter().chain(incoming.into_iter()) {
            ...
            if let Some(target_node) = storage.get_node(target_node_id).map_err(|e| {
                ProximaDBError::Internal(format!(
                    "Failed to fetch target node '{target_node_id}': {e}"
                ))
            })? {
                matching_edges.push((edge, target_node));
            }
        }
    ...
    fn find_variable_length_paths(
        ...
        if let Some(start_node) = storage.get_node(from_node_id).map_err(|e| {
            ProximaDBError::Internal(format!(
                "Failed to fetch starting node '{from_node_id}': {e}"
            ))
        })? {
        ...
        while let Some(current_path) = queue.pop_front() {
            ...
            for edge in storage
                .get_outgoing_edges(&current_node.id, None)
                .map_err(|e| {
                    ProximaDBError::Internal(format!(
                        "Failed to fetch outgoing edges for path expansion from '{}': {e}",
                        current_node.id
                    ))
                })?
            {
                ...
                if let Some(target_node) = storage.get_node(&edge.to_node_id).map_err(|e| {
                    ProximaDBError::Internal(format!(
                        "Failed to fetch target node '{}' for path expansion: {e}",
                        edge.to_node_id
                    ))
                })? {
                    let mut new_path = current_path.clone();
                    new_path.elements.push(PathElement::Edge(edge));
    ...
    fn property_value_to_json(&self, value: &PropertyValue) -> serde_json::Value {
        match &value.value {
            ...
            Some(Value::IntValue(i)) => serde_json::Value::Number(serde_json::Number::from(*i)),
    ...
    fn apply_where_clauses(
        ...
        _storage: &Arc<dyn GraphQueryStorage>,
    ) -> QueryResult<Vec<MatchResult>> {
                ...
                match projection {
                    ...
                    PropertyProjection::Property { variable, property } => {
                            ...
                            && let Some(value) = self.get_binding_property(binding, property)
                        {
                            ...
                            new_bindings.insert(
                                alias.clone(),
                                VariableBinding::Node(std::sync::Arc::new(Node {
    ...
    fn estimate_node_selectivity(
        ...
        if !node_pattern.labels.is_empty() {
            ...
            storage
                .get_nodes_by_label(&node_pattern.labels[0])
                .map(|nodes| nodes.len())
                .unwrap_or_else(|_| {
                    storage
                        .get_all_nodes()
...
impl PatternCompiler {
    ...
    fn convert_match_clause(
        ...
        use super::cypher_ast::{Direction, PatternElement};
    ...
    fn convert_where_clause(
        &self,
        wc: &super::cypher_ast::WhereClause,
    ) -> QueryResult<WhereClause> {
        use super::ast::WhereClause as AstWhereClause;

        // Convert Expression to simple WhereClause representation
        match &wc.expression {
            super::cypher_ast::Expression::Variable(var) => {
    ...
    fn convert_return_clause(&self, rc: &ReturnClause) -> QueryResult<ReturnSpec> {
        use super::ast::PropertyProjection as AstProjection;
    ...
    fn convert_cypher_value_to_json(
        &self,
        val: &super::cypher_ast::CypherValue,
    ) -> QueryResult<serde_json::Value> {
        use super::cypher_ast::CypherValue;
...
mod tests {
    ...
    fn test_order_by_with_property_access() {
        ...
        let node1 = std::sync::Arc::new(Node {
            id: "1".to_string(),
        ...
        let node2 = std::sync::Arc::new(Node {
            id: "2".to_string(),
            labels: vec!["Person".to_string()],
            properties: {
                let mut map = std::collections::HashMap::new();
        ...
        let node3 = std::sync::Arc::new(Node {
            id: "3".to_string(),
            labels: vec!["Person".to_string()],
            properties: {
                let mut map = std::collections::HashMap::new();
        ...
        let results = vec![
            MatchResult {
                bindings: {
                    let mut map = std::collections::HashMap::new();
                    map.insert("n".to_string(), VariableBinding::Node(node1));
                    map
                },
                score: 0.5,
            },
            MatchResult {
                bindings: {
                    let mut map = std::collections::HashMap::new();
                    map.insert("n".to_string(), VariableBinding::Node(node2));
                    map
                },
                score: 0.9,
            },
            MatchResult {
                bindings: {
                    let mut map = std::collections::HashMap::new();
                    map.insert("n".to_string(), VariableBinding::Node(node3));
                    map
                },
                score: 0.2,
            },
    ...
    fn test_order_by_numeric_properties() {
        ...
        let results = vec![
            MatchResult {
                bindings: {
                    let mut map = std::collections::HashMap::new();
                    map.insert("p".to_string(), VariableBinding::Node(node1));
                    map
                },
                score: 0.5,
            },
            MatchResult {
                bindings: {
                    let mut map = std::collections::HashMap::new();
                    map.insert("p".to_string(), VariableBinding::Node(node2));
                    map
                },
                score: 0.9,
            },
            MatchResult {
                bindings: {
                    let mut map = std::collections::HashMap::new();
                    map.insert("p".to_string(), VariableBinding::Node(node3));
                    map
                },
                score: 0.2,
            },
