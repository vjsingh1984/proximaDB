...
use proximadb_proto::proximadb_v1::{
    Edge, EdgeQuery, Node, NodeQuery, PropertyValue, property_value::Value,
};
...
pub struct QueryExecutor {
    /// Narrow query backend used by the executor runtime.
    backend: Arc<dyn GraphQueryExecutorBackend>,
}
...
impl QueryExecutor {
    ...
    pub async fn execute(
        ...
        let mut current_results: Vec<QueryRow> = Vec::new();
    ...
    async fn execute_node_scan(
        ...
        let nodes = if let Some(labels) = labels {
            if let Some(label) = labels.first() {
                ...
                self.backend.query_nodes(&context.graph_id, query).await?
            } else {
                Vec::new()
            }
        } else {
    ...
    async fn execute_index_seek(
        ...
        let nodes = self.backend.query_nodes(&context.graph_id, query).await?;
    ...
    fn property_value_to_json(&self, value: &PropertyValue) -> serde_json::Value {
        match &value.value {
            ...
            Some(Value::BytesValue(_)) | None => serde_json::Value::Null,
            Some(Value::ArrayValue(values)) => serde_json::Value::Array(
                values
                    .values
                    .iter()
            ...
            Some(Value::ObjectValue(obj)) => serde_json::Value::Object(
                obj.fields
                    .iter()
                    .map(|(key, value)| (key.clone(), self.property_value_to_json(value)))
                    .collect(),
            ),
            Some(Value::VectorValue(vector)) => serde_json::Value::Array(
                vector
                    .values
                    .iter()
    ...
    fn evaluate_condition(&self, row: &QueryRow, condition: &FilterCondition) -> bool {
        match condition {
        ...
        for node in nodes {
            ...
            let property_value = node.properties.get(index_name).and_then(|pv| {
                pv.value.as_ref().map(|v| match v {
                    Value::DoubleValue(n) => serde_json::Value::from(*n),
                    Value::StringValue(s) => serde_json::Value::String(s.clone()),
                    Value::IntValue(n) => serde_json::Value::from(*n),
    ...
    fn execute_join(
        ...
        let mut right_index: HashMap<String, Vec<&QueryRow>> = HashMap::new();
        for row in &results {
...
#[cfg(test)]
mod tests {
    use super::super::planner::{CostEstimate, JoinType, PlanStep, PlanStepType, QueryPlan};
    use super::*;
    ...
    #[async_trait]
    impl GraphQueryExecutorBackend for MockQueryBackend {
        async fn query_nodes(
            ...
            Ok(self
                .nodes
                .iter()
                .filter(|node| query.labels.iter().any(|label| node.labels.contains(label)))
                .cloned()
                .collect())
        }
            ...
            Ok(self
                ...
                .filter(|edge| {
                    query
                        .from_node_id
                        .as_ref()
                        .is_none_or(|from_id| edge.from_node_id == *from_id)
                        && query
                            .to_node_id
                            .as_ref()
                            .is_none_or(|to_id| edge.to_node_id == *to_id)
                })
                .cloned()
                .collect())
        }
    ...
    fn test_edge(id: &str, from_node_id: &str, to_node_id: &str, edge_type: &str) -> Arc<Edge> {
        Arc::new(Edge {
            id: id.to_string(),
    ...
    async fn test_executor_node_scan() {
        let executor = QueryExecutor::new(Arc::new(MockQueryBackend {
            nodes: vec![test_node("test_node_1", "TestLabel")],
            edges: vec![],
        }));
    ...
    #[tokio::test]
    async fn test_executor_traverse_bfs() {
        let executor = QueryExecutor::new(Arc::new(MockQueryBackend {
            nodes: vec![test_node("n1", "Start"), test_node("n2", "Person")],
            edges: vec![test_edge("e1", "n1", "n2", "KNOWS")],
        }));
        ...
        let plan = QueryPlan {
            ...
            steps: vec![
                PlanStep {
                    step_type: PlanStepType::NodeScan {
                        labels: Some(vec!["Start".to_string()]),
                        property_filters: Vec::new(),
                    },
                    parameters: HashMap::new(),
                    cost: CostEstimate::zero(),
        ...
        let results = executor.execute(&plan, &QueryContext::new()).await.unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].get("source"), Some(&serde_json::json!("n1")));
    }
}
