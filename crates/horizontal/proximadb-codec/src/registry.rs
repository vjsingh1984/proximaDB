...
use crate::traits::{RawDecoder, RawEncoder};
...
impl ImplementationRegistry {
    ...
    pub fn new(hw_caps: HardwareCapabilities) -> Self {
        let gpu_info = match hw_caps.gpu.backend {
            proximadb_hardware_caps::GpuBackend::CUDA => "CUDA",
            proximadb_hardware_caps::GpuBackend::ROCm => "ROCm",
            proximadb_hardware_caps::GpuBackend::MPS => "MPS",
            proximadb_hardware_caps::GpuBackend::OpenCL => "OpenCL",
            proximadb_hardware_caps::GpuBackend::None => "None",
        };
