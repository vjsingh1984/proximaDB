...
mod tests {
    ...
    fn test_translate_pgwire_jsonb_vector_and_cypher_extensions() {
        ...
        assert!(result.contains("embedding <-> '[0.1, 0.2, 0.3]'::vector"));
        assert!(!result.contains("::jsonb"));
        assert!(!result.contains("VECTOR_DISTANCE"));
